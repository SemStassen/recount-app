    ...
    for (let i = 0, len = snapshot.length; i < len; i++) {
      ...
      switch (state.type) {
        case "Counter": {
          ...
          const startTime = this.startTimeFor(
            descriptor.name,
            intervalStartTime
          );
        ...
        case "Frequency": {
          ...
          for (const [freqKey, value] of state.state.occurrences) {
            ...
            if (isDelta) {
              const previousOccurrences =
                this.previousFrequencyState.get(metricKey);
          ...
          if (metricDataByName.has(state.id)) {
            ...
            metricDataByName
              .get(state.id)!
              .dataPoints.push(...(dataPoints as any));
          } else {
            const descriptor = descriptorFromState(state, attributes);
            addMetricData({
              dataPointType: DataPointType.SUM,
        ...
        case "Summary": {
          ...
          if (metricDataByName.has(`${state.id}_quantiles`)) {
            ...
            metricDataByName
              .get(`${state.id}_count`)!
              .dataPoints.push(countDataPoint as any);
            metricDataByName
              .get(`${state.id}_sum`)!
              .dataPoints.push(sumDataPoint as any);
          } else {
            const descriptor = descriptorFromState(state, attributes);
            addMetricData({
              dataPointType: DataPointType.SUM,
...
const instrumentTypeFromSnapshot = (
  state: Metric.Metric.Snapshot
): InstrumentType => {
  switch (state.type) {
    case "Histogram":
    ...
    case "Counter":
      return state.state.incremental
...
const determineValueType = (state: Metric.Metric.Snapshot): ValueType => {
  if (state.type === "Counter") {
    return typeof state.state.count === "bigint"
      ? ValueType.INT
      : ValueType.DOUBLE;
  } else if (state.type === "Gauge") {
    return typeof state.state.value === "bigint"
      ? ValueType.INT
      : ValueType.DOUBLE;
  }
