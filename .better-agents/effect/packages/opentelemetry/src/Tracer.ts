 ...
 * @since 4.0.0
 */
export class OtelTracer extends Context.Service<OtelTracer, Otel.Tracer>()(
...
export const makeExternalSpan = (options: {
    ...
    if (typeof options.traceState === "string") {
      try {
        next = Context.add(
          next,
          OtelTraceState,
          Otel.createTraceState(options.traceState)
        );
      } catch {
        //
      }
  ...
  return {
    ...
    sampled: Predicate.isNotUndefined(options.traceFlags)
      ? isSampled(options.traceFlags)
      : true,
    annotations,
  };
};
 ...
 * @since 4.0.0
 */
export const layerGlobalTracer: Layer.Layer<OtelTracer, never, Resource> =
  layerTracer.pipe(Layer.provide(layerGlobalProvider));
 ...
 * @since 4.0.0
 */
export const layerGlobal: Layer.Layer<OtelTracer, never, Resource> =
  Layer.effect(Tracer.Tracer, make).pipe(Layer.provideMerge(layerGlobalTracer));
 ...
 * @since 4.0.0
 */
export const layerWithoutOtelTracer: Layer.Layer<never, never, OtelTracer> =
  Layer.effect(Tracer.Tracer, make);
...
export const layer: Layer.Layer<
  ...
  never,
  OtelTracerProvider | Resource
> = layerWithoutOtelTracer.pipe(Layer.provideMerge(layerTracer));
 ...
 * @since 4.0.0
 */
export const currentOtelSpan: Effect.Effect<
  Otel.Span,
...
const convertOtelTimeInput = (
  input: Otel.TimeInput | undefined,
  clock: Clock.Clock
): bigint => {
  if (input === undefined) {
...
export const withSpanContext: {
  ...
  2,
  <A, E, R>(self: Effect.Effect<A, E, R>, spanContext: Otel.SpanContext) =>
