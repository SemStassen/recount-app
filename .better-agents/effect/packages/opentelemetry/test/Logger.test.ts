...
describe("Logger", () => {
  describe("provided", () => {
    ...
    it.effect("maps Effect LogLevel to OTel SeverityNumber spec values", () => {
      ...
      const SeverityLayer = NodeSdk.layer(
        Effect.sync(() => ({
      ...
      return Effect.gen(function* () {
        ...
        const byText = Object.fromEntries(
          records.map((r) => [r.severityText, r.severityNumber])
        );
    ...
    it.effect(
      "uses monotonic clock timestamps and keeps them aligned with spans",
      () => {
        ...
        return Effect.gen(function* () {
          yield* Effect.log("test").pipe(Effect.withSpan("parent"));
