...
describe("atom-solid", () => {
  ...
  describe("AtomRef", () => {
    ...
    it("updates when AtomRef prop value changes", () => {
      const ref = AtomRef.make({ count: 0, label: "a" });
      let observed: number | undefined;
      const dispose = renderAccessor(
        () => useAtomRefPropValue(() => ref, "count"),
        (value) => {
          observed = value;
        }
      );
...
const renderAtomRef = function <A>(
...
const renderAccessor = function <A>(
  makeAccessor: () => Accessor<A>,
  onValue: (_: A) => void
) {
  return createRoot((dispose) => {
...
const renderAtomValue = function <A, B = A>(
  ...
  return createRoot((dispose) => {
    const run = () => {
      const accessor = options?.map
        ? useAtomValue(() => atom, options.map)
        : useAtomValue(() => atom);
      createEffect(() => {
...
const renderAtomResource = function <A, E>(
  ...
  let resource: Resource<A> | undefined;
  const dispose = createRoot((dispose) => {
