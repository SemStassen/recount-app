...
describe("atom-react", () => {
  ...
  describe("runtime", () => {
    test("can inject test layers", () => {
      ...
      const numberAtom = runtime.atom(
        TheNumber.use((_) => Effect.succeed(_.n))
      );
  ...
  describe("ScopedAtom", () => {
    test("throws when used outside Provider", () => {
      ...
      expect(() => render(<TestComponent />)).toThrow(
        "ScopedAtom used outside of its Provider"
      );
    });
  ...
  describe("hydration", () => {
    test("basic hydration with number atom and result atoms", () => {
      ...
      function Result1() {
        ...
        return AsyncResult.match(value, {
          onSuccess: (result) => (
            <div data-testid="value-1">{result.value}</div>
          ),
      ...
      function Result2() {
        ...
        return AsyncResult.match(value, {
          onSuccess: (result) => (
            <div data-testid="value-2">{result.value}</div>
          ),
      ...
      function Result3() {
        ...
        return AsyncResult.match(value, {
          onSuccess: (result) => (
            <div data-testid="value-3">{result.value}</div>
          ),
      ...
      expect(screen.getByTestId("hydration-basic-value")).toHaveTextContent(
        "1"
      );
    ...
    test("empty state is a no-op", () => {
      ...
      expect(screen.getByTestId("hydration-empty-state")).toHaveTextContent(
        "OK"
      );
    });
