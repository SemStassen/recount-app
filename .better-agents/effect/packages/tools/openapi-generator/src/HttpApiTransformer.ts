...
export const toImplementation = (
  ...
  const groupSources = groups.map((group) =>
    renderGroup(group, security.endpointMiddlewares)
  );
...
const groupOperations = (
  parsed: ParsedOpenApi
): ReadonlyArray<GroupRenderModel> => {
...
const renderEndpoint = (
  ...
  const endpoint =
    options.length === 0
      ? `HttpApiEndpoint.${operation.method}(${JSON.stringify(endpointName)}, ${JSON.stringify(
          toHttpApiPath(operation.path)
        )})`
      : `HttpApiEndpoint.${operation.method}(${JSON.stringify(endpointName)}, ${JSON.stringify(
          toHttpApiPath(operation.path)
        )}, { ${options.join(", ")} })`;
  ...
  if (operation.operationId !== undefined) {
    annotations.push(
      `annotate(OpenApi.Identifier, ${JSON.stringify(operation.operationId)})`
    );
  }
  if (operation.metadata.summary !== undefined) {
    annotations.push(
      `annotate(OpenApi.Summary, ${JSON.stringify(operation.metadata.summary)})`
    );
  }
  if (operation.metadata.description !== undefined) {
    annotations.push(
      `annotate(OpenApi.Description, ${JSON.stringify(operation.metadata.description)})`
    );
  }
  ...
  if (operation.metadata.externalDocs !== undefined) {
    annotations.push(
      `annotate(OpenApi.ExternalDocs, ${JSON.stringify(operation.metadata.externalDocs)})`
    );
  }
...
const renderPayload = (operation: ParsedOperation): string | undefined => {
  ...
  const payloads = operation.requestBodyRepresentable.map((schema) =>
    renderMediaSchema(schema)
  );
...
const renderMediaSchema = (media: ParsedOperationMediaTypeSchema): string => {
  switch (media.encoding) {
    ...
    case "form-url-encoded": {
      ...
      return `${media.schema}.pipe(HttpApiSchema.asFormUrlEncoded({ contentType: ${JSON.stringify(
        media.contentType
      )} }))`;
    }
    case "text": {
...
const renderApiAnnotations = (parsed: ParsedOpenApi): ReadonlyArray<string> => {
  ...
  if (parsed.metadata.summary !== undefined) {
    annotations.push(
      `annotate(OpenApi.Summary, ${JSON.stringify(parsed.metadata.summary)})`
    );
  }
  if (parsed.metadata.description !== undefined) {
    annotations.push(
      `annotate(OpenApi.Description, ${JSON.stringify(parsed.metadata.description)})`
    );
  }
  if (parsed.metadata.license !== undefined) {
    annotations.push(
      `annotate(OpenApi.License, ${JSON.stringify(parsed.metadata.license)})`
    );
  }
  if (parsed.metadata.servers !== undefined) {
    annotations.push(
      `annotate(OpenApi.Servers, ${JSON.stringify(parsed.metadata.servers)})`
    );
  }
...
const buildSecurityRenderModel = (
  parsed: ParsedOpenApi
): SecurityRenderModel => {
  const allocateName = makeNameAllocator();
  ...
  for (const securityScheme of parsed.securitySchemes) {
    ...
    securityDeclarations.push(
      `export const ${declarationName} = ${renderSecurityScheme(securityScheme)}`
    );
  }
  ...
  for (const operation of parsed.operations) {
    ...
    for (const requirement of operation.effectiveSecurity) {
      ...
      if (schemes.length === 1) {
        ...
        if (
          schemeDeclarations.has(schemeName) &&
          !seenOrSchemes.has(schemeName)
        ) {
    ...
    if (operationMiddlewareNames.length > 0) {
      endpointMiddlewares.set(
        toOperationKey(operation),
        operationMiddlewareNames
      );
    }
  }
  ...
  function getOrSecurityMiddlewareName(schemes: ReadonlyArray<string>): string {
    ...
    const className = allocateName(
      `${getSecurityMiddlewareBaseName(schemes, "Or")}SecurityMiddleware`
    );
    const securityEntries = schemes
      .map(
        (name) => `${JSON.stringify(name)}: ${schemeDeclarations.get(name)!}`
      )
      .join(", ");
    middlewareDeclarations.push(
      `export class ${className} extends HttpApiMiddleware.Service<${className}>()(${JSON.stringify(
        `${schemes.join(" | ")} security`
  ...
  function getAndSecurityMiddlewareName(
    ...
    const className = allocateName(
      `${getSecurityMiddlewareBaseName(schemes, "And")}SecurityMiddleware`
    );
    middlewareDeclarations.push(
      `class ${className} extends HttpApiMiddleware.Service<${className}>()(${JSON.stringify(
        `${schemes.join(" & ")} security`
      )}) {}`
    );
    middlewareNames.set(key, className);
...
const getSecurityMiddlewareBaseName = (
  ...
  const [head, ...tail] = schemes.map((scheme) =>
    ensureIdentifier(scheme, "Security")
  );
  return tail.length === 0
    ? head
    : [head, ...tail.map((scheme) => `${joiner}${scheme}`)].join("");
};

const renderSecurityScheme = (
  securityScheme: ParsedOpenApiSecurityScheme
): string => {
  let source: string;
  switch (securityScheme.type) {
    ...
    case "apiKey": {
      source = `HttpApiSecurity.apiKey({ key: ${JSON.stringify(securityScheme.key!)}, in: ${JSON.stringify(
        securityScheme.in!
      )} })`;
      break;
    }
  ...
  if (
    (securityScheme.type === "bearer" || securityScheme.type === "http") &&
...
const toOperationKey = (operation: ParsedOperation): string =>
  `${operation.method}:${operation.path}`;

const toHttpApiPath = (path: string): string =>
  path.replace(/{([^}]+)}/g, ":$1");
