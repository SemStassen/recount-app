...
const format = Flag.choice("format", [
  "httpclient",
...
const root = Command.make("openapigen", { spec, format, name, patch }).pipe(
  Command.withHandler(
    Effect.fnUntraced(function* ({ name, spec, format, patch }) {
      ...
      if (patch.length > 0) {
        ...
        patchedSpec = yield* OpenApiPatch.applyPatches(
          parsedPatches,
          patchedSpec
        ).pipe(
          Effect.mapError((error) => new CliError.UserError({ cause: error }))
      ...
      const source = yield* generator.generate(
        patchedSpec as unknown as OpenAPISpec,
        {
          name,
          format,
          onWarning: (warning) => {
 ...
 * @since 4.0.0
 */
export const run: Effect.Effect<void, CliError.CliError, Command.Environment> =
  Command.run(root, {
    version: "0.0.0",
  });
