    ...
    describe("file paths", () => {
      ...
      it.effect("fails on file with unsupported operation", () =>
        Effect.gen(function* () {
          ...
          const exit = yield* Effect.exit(
            OpenApiPatch.parsePatchInput(filePath)
          );
          assert.isTrue(Exit.isFailure(exit));
        }).pipe(Effect.provide(testLayer))
      );
        ...
        const patches = [
          {
            source: "test",
            patch: [
              { op: "add" as const, path: "/info/version", value: "1.0.0" },
            ],
          },
        ];
        ...
        const patches = [
          {
            source: "patch1",
            patch: [
              { op: "replace" as const, path: "/info/title", value: "Step 1" },
            ],
          },
          {
            source: "patch2",
            patch: [
              { op: "replace" as const, path: "/info/version", value: "1.0.0" },
            ],
          },
          {
            source: "patch3",
            patch: [
              { op: "add" as const, path: "/info/x-patched", value: true },
            ],
          },
        ];
        const result = yield* OpenApiPatch.applyPatches(patches, document);
        ...
        const patches = [
          {
            source: "test",
            patch: [
              { op: "replace" as const, path: "/info/nonexistent", value: "x" },
            ],
          },
        ];
        const exit = yield* Effect.exit(
          OpenApiPatch.applyPatches(patches, document)
        );
        assert.isTrue(Exit.isFailure(exit));
      })
    );
        ...
        const patches = [
          {
            ...
            patch: [
              {
                op: "replace" as const,
                path: "/info/nonexistent1",
                value: "x",
              },
              { op: "remove" as const, path: "/info/nonexistent2" },
              {
                op: "replace" as const,
                path: "/info/nonexistent3",
                value: "y",
              },
            ],
          },
        ...
        const patches = [
          {
            source: "test",
            patch: [
              { op: "replace" as const, path: "/info/title", value: "Changed" },
            ],
          },
        ];
        const result = yield* OpenApiPatch.applyPatches(patches, document);
        assert.strictEqual(
          (result as { info: { description: string } }).info.description,
          "Unchanged"
        );
        assert.deepStrictEqual((result as { paths: object }).paths, {
          "/users": { get: {} },
        });
        assert.deepStrictEqual((result as { components: object }).components, {
          schemas: {},
        });
      })
    );
  ...
  describe("error messages", () => {
    ...
    it.effect("JsonPatchAggregateError has descriptive message", () =>
      Effect.gen(function* () {
        ...
        assert.include(
          error.message,
          "1. [./fix.json] op 0 (replace at /info/x)"
        );
        assert.include(
          error.message,
          "2. [./other.json] op 1 (remove at /paths/~1users)"
        );
      })
    );
  });
