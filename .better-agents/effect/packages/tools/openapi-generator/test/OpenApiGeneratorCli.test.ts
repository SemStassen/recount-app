...
const fixturePath = (fileName: string) =>
  `${import.meta.dirname}/fixtures/${fileName}`;
...
const runCli = Effect.fnUntraced(function* (args: ReadonlyArray<string>) {
  ...
  return yield* Effect.gen(function* () {
    ...
    const stdout =
      stdoutLines.length > 0 ? String(stdoutLines[stdoutLines.length - 1]) : "";
...
describe("openapigen CLI", () => {
  ...
  it.effect("routes --format values and defaults to httpclient", () =>
    Effect.gen(function* () {
      ...
      const defaultResult = yield* runCli([
        "--spec",
        spec,
        "--name",
        "CliClient",
      ...
      assert.include(
        httpapiResult.stdout,
        'export class CliClient extends HttpApi.make("CliClient")'
      );
    })
  );
  ...
  it.effect(
    "writes warnings to stderr and keeps stdout as generated source",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "separates generated source and warnings when spawned as a child process",
    () =>
      Effect.gen(function* () {
        const spec = fixturePath("cli-warning-spec.json");
