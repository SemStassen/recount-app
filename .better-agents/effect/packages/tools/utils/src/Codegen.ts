...
const findAnnotation = (
  content: string
): { pattern: string; offset: number } | undefined => {
  ...
  while (
    commentEnd < lines.length - 1 &&
    lines[commentEnd + 1].trimStart().startsWith("//")
  ) {
    commentEnd++;
  }
...
const extractModuleSince = (
  file: string,
  content: string
): Effect.Effect<string, BarrelCodegenError> => {
...
export interface BarrelGenerator {
  ...
  readonly processFile: (
    file: BarrelFile
  ) => Effect.Effect<void, PlatformError | Glob.GlobError | BarrelCodegenError>;
}
 ...
 * @since 4.0.0
 */
export const BarrelGenerator: Context.Service<
  BarrelGenerator,
  BarrelGenerator
> = Context.Service("@effect/utils/BarrelGenerator");
...
export const layer: Layer.Layer<
  BarrelGenerator,
  never,
  FileSystem.FileSystem | Path.Path | Glob.Glob
> = Effect.gen(function* () {
  ...
  const toPosix =
    path.sep === "/"
      ? (file: string) => file
      : (file: string) => file.split(path.sep).join("/");
  ...
  const processModule = Effect.fn("processModule")(function* (
    directory: string,
    file: string
  ) {
  ...
  const discoverFiles = Effect.fn("discoverFiles")(function* (
    pattern: string,
    cwd: string
  ) {
    ...
    const results = yield* Effect.forEach(indexFiles, (file) =>
      discoverFile(path.join(cwd, file))
    );
  ...
  const processFile = Effect.fn("processFile")(function* (file: BarrelFile) {
    ...
    const moduleContents = yield* Effect.forEach(matchedFiles, (file) =>
      processModule(directory, file)
    );
