...
export class Reporter extends Context.Service<Reporter>()(
  "@effect/bundle/Reporter",
  {
    make: Effect.gen(function* () {
      ...
      const createReport = (
        ...
        for (const current of curr) {
          const previous =
            prev.find((previous) => {
              return (
                path.basename(previous.path) === path.basename(current.path)
              );
            }) ?? current;
      ...
      const createSelectedReport = (
        stats: ReadonlyArray<BundleStats>
      ): string => {
        const lines: Array<string> = [
      ...
      const report = Effect.fn("Reporter.report")(function* (
        ...
        const [currentStats, previousStats] = yield* Effect.all(
          [
            ...
            rollup.bundleAll({
              paths: fixtures.map((fixture) =>
                path.join(options.baseDirectory, fixture)
              ),
            }),
          ],
          { concurrency: 2 }
        ...
        yield* Effect.logInfo(
          "Bundling complete! Generating bundle size report..."
        );

        return createReport(currentStats, previousStats);
      });

      const visualize = Effect.fn("Reporter.visualize")(function* (
