...
export interface CodeGenerator {
  readonly generate: (
    ...
    string,
    GenerationError | PatchError,
    FileSystem.FileSystem | Path_.Path
  >;
}
 ...
 * @since 4.0.0
 */
export const CodeGenerator: Context.Service<CodeGenerator, CodeGenerator> =
  Context.Service("@effect/ai-codegen/CodeGenerator");
...
> = Effect.gen(function* () {
  ...
  const applyPatches = Effect.fn("applyPatches")(function* (
    ...
    const parsedPatches = yield* Effect.forEach(patchInputs, (input) => {
      ...
      const resolvedInput =
        !input.startsWith("[") && !pathService.isAbsolute(input)
      ...
      Effect.mapError(
        (cause) => new PatchError({ provider: provider.name, cause })
      )
    );
  ...
  const generate = Effect.fn("generate")(function* (
    ...
    const disableAdditionalProperties =
      provider.config.shouldDisableAdditionalProperties;

    const exclude = excludeAnnotations
      ? new Set(excludeAnnotations)
      : undefined;
    const onEnter =
      exclude || disableAdditionalProperties
    ...
    return yield* openApiGen
      .generate(
        patchedSpec as unknown as Parameters<typeof openApiGen.generate>[0],
        {
          name: provider.config.clientName,
          format: provider.config.isTypeOnly
      ...
      .pipe(
        Effect.mapError(
          (cause) => new GenerationError({ provider: provider.name, cause })
        )
      );
  });
 ...
 * @since 4.0.0
 */
export const layerSchema: Layer.Layer<
  CodeGenerator,
  never,
  FileSystem.FileSystem | Path_.Path
> = layer.pipe(Layer.provide(OpenApiGenerator.layerTransformerSchema));
 ...
 * @since 4.0.0
 */
export const layerTypeScript: Layer.Layer<
  CodeGenerator,
  never,
  FileSystem.FileSystem | Path_.Path
> = layer.pipe(Layer.provide(OpenApiGenerator.layerTransformerTs));
