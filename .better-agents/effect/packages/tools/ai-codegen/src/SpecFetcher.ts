 ...
 * @since 4.0.0
 */
export const SpecFetcher: Context.Service<SpecFetcher, SpecFetcher> =
  Context.Service("@effect/ai-codegen/SpecFetcher");
...
> = Effect.gen(function* () {
  ...
  const fetchFromFile = Effect.fn("fetchFromFile")(function* (
    ...
    return yield* fs
      ...
      .pipe(
        Effect.mapError(
          (cause) => new SpecFetchError({ provider, source: path, cause })
        )
      );
  });
  ...
  const fetchFromUrl = Effect.fn("fetchFromUrl")(function* (
    ...
    return yield* httpClient.get(url).pipe(
      ...
      Effect.mapError(
        (cause) => new SpecFetchError({ provider, source: url, cause })
      )
    );
  });
  ...
  const parseSpec = (content: string, sourceUrl: string, provider: string) => {
    ...
    return Effect.try({
      try: () => (isYaml ? Yaml.parse(content) : JSON.parse(content)),
        ...
        new SpecFetchError({ provider, source: sourceUrl, cause }),
    });
  };
  ...
  const fetchViaStainlessStats = Effect.fn("fetchViaStainlessStats")(function* (
    ...
    const statsContent = yield* httpClient.get(statsUrl).pipe(
      ...
      Effect.mapError(
        (cause) =>
          new SpecFetchError({ provider, source: `stats:${statsUrl}`, cause })
      )
    );
