...
const formatSpecSource = (
  source: ProviderDiscovery.DiscoveredProvider["specSource"]
): string => {
  switch (source._tag) {
    case "Url":
      return source.url;
...
const generateProvider = Effect.fn("generateProvider")(function* (
  ...
  if (replacements.length > 0) {
    yield* Console.log(
      `  ${colors.dim}Applying ${replacements.length} replacement(s)...${colors.reset}`
    );
  ...
  Command.withHandler(
    Effect.fnUntraced(function* ({ provider, skipLint, skipFormat }) {
      ...
      const providers = yield* Option.match(provider, {
        ...
        onSome: (name) =>
          discovery.discoverOne(name).pipe(Effect.map((p) => [p])),
      });

      if (providers.length === 0) {
        yield* Console.log(
          `${colors.yellow}No providers found.${colors.reset}`
        );
        return;
      }
...
const list = Command.make("list").pipe(
  Command.withHandler(
    Effect.fnUntraced(function* () {
      ...
      yield* Console.log(
        `${colors.green}Found ${providers.length} provider(s):${colors.reset}\n`
      );

      for (const p of providers) {
        yield* Console.log(
          `${colors.bold}${colors.cyan}${p.name}${colors.reset}`
        );
        yield* Console.log(
          `  ${colors.gray}spec:${colors.reset}     ${formatSpecSource(p.specSource)}`
        );
        yield* Console.log(
          `  ${colors.gray}output:${colors.reset}   ${p.config.output}`
        );
        yield* Console.log(
          `  ${colors.gray}client:${colors.reset}   ${colors.white}${p.config.clientName}${colors.reset}`
        );
        yield* Console.log(
          `  ${colors.gray}typeOnly:${colors.reset} ${p.config.isTypeOnly}`
        );
