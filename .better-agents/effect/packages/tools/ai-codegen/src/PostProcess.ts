 ...
 * @since 4.0.0
 */
export const PostProcessor: Context.Service<PostProcessor, PostProcessor> =
  Context.Service("@effect/ai-codegen/PostProcessor");
...
> = Effect.gen(function* () {
  ...
  const lint = Effect.fn("lint")(function* (filePath: string) {
    yield* runCommand(
      "pnpm",
      ["exec", "oxlint", "--silent", "--quiet", "--fix", filePath],
      "lint",
      filePath
    );
  });
  ...
  const format = Effect.fn("format")(function* (filePath: string) {
    yield* runCommand(
      "pnpm",
      ["exec", "dprint", "--log-level", "silent", "fmt", filePath],
      "format",
      filePath
    );
  });
