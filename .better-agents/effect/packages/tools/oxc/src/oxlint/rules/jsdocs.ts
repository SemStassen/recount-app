...
function readCachedModel(modelPath: string): CachedModel["result"] {
  ...
  modelCache.set(modelPath, {
    ...
    filesByPath:
      result._tag === "Success"
        ? new Map(result.value.files.map((file) => [file.file, file]))
        : new Map(),
...
function getCachedFile(
  ...
  ? R extends {
      ...
      readonly value: { readonly files: ReadonlyArray<infer A> };
    }
    ? A | undefined
    : never
  : never {
...
function rangeNode(range: readonly [number, number]): {
    ...
    return {
      Program(node: ESTree.Node) {
        if (file.hash !== Jsdocs.sourceHash(source)) {
          context.report({
            node,
            message: "JSDoc model is stale for this file; run `pnpm jsdocs`",
          });
          return;
        }
