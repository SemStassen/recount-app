...
const queryFailureReasonTag = (cause: unknown) =>
  Effect.map(queryFailureReason(cause), (reason) => reason._tag);
...
const makeFailingPool = (cause: unknown) => ({
  options: {},
  ending: false,
  connect: (cb: (cause: unknown, client: any) => void) =>
    cb(null, makeFailingClient(cause)),
  query: () => undefined,
});
...
describe("PgClient SqlError classification", () => {
  ...
  it.effect(
    "classifies 23505 as UniqueViolation and trims the constraint name",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "uses unknown for missing, non-string, or blank unique violation constraints",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "keeps non-unique integrity constraints classified as ConstraintError",
    () =>
      Effect.gen(function* () {
        const tag = yield* queryFailureReasonTag({
          code: "23503",
