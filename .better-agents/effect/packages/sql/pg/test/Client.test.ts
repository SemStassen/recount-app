  ...
  (it) => {
    ...
    it.effect("update helper", () =>
      Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        let result =
          sql`UPDATE people SET ${sql.update({ name: "Tim" })}`.compile();
        expect(result[0]).toEqual(`UPDATE people SET "name" = $1`);
        expect(result[1]).toEqual(["Tim"]);

        result =
          sql`UPDATE people SET ${sql.update({ name: "Tim", age: 10 }, ["age"])}`.compile();
        expect(result[0]).toEqual(`UPDATE people SET "name" = $1`);
        expect(result[1]).toEqual(["Tim"]);
      })
    );
    ...
    it.effect("update helper returning", () =>
      Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const result =
          sql`UPDATE people SET ${sql.update({ name: "Tim" }).returning("*")}`.compile();
    ...
    it.effect("array helper with column", () =>
      Effect.gen(function* () {
        ...
        result =
          sql`SELECT * FROM ${sql("people")} WHERE ${sql.in("id", [])}`.compile();
    ...
    it.effect("update fragments", () =>
      Effect.gen(function* () {
        ...
        const now = new Date();
        const [query, params] =
          sql`UPDATE people SET json = data.json FROM ${sql.updateValues(
    ...
    it.effect("jsonb", () =>
      Effect.gen(function* () {
        ...
        const rows = yield* sql<{
          json: unknown;
        }>`select ${{ testValue: 123 }}::jsonb as json`;
  ...
  (it) => {
    ...
    it.effect("Should populate config", () =>
      Effect.gen(function* () {
        ...
        const parsedConfig = parsePgConnectionString(
          Redacted.value(sql.config.url)
        );
        ...
        expect(Redacted.value(sql.config.password)).toEqual(
        ...
        Effect.gen(function* () {
          ...
          const rows = yield* sql<{ value: number }>`SELECT 1 as value`.pipe(
            Effect.timeoutOrElse({
              ...
              orElse: () =>
                Effect.fail(
                  new Error("query timed out while listener was active")
                ),
            })
          );
          ...
          const payloads = yield* Fiber.join(listenFiber).pipe(
            Effect.timeoutOrElse({
              ...
              orElse: () =>
                Effect.fail(
                  new Error("listener did not receive notification in time")
                ),
            })
          );
