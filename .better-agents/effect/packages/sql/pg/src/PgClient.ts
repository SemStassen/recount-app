...
export const fromPool = Effect.fnUntraced(function* (options: {
    ...
    new ConnectionImpl(
      function runWithClient<A>(
        f: (
          ...
          resume: (_: Effect.Effect<A, SqlError>) => void
        ) => void
      ) {
        if (client !== undefined) {
  ...
  const reserveRaw = Effect.callback<Pg.PoolClient, SqlError, Scope.Scope>(
    (resume) => {
  ...
  let config: PgClientConfig = {
    url: pool.options.connectionString
      ? Redacted.make(pool.options.connectionString)
      : undefined,
    ...
    password:
      typeof pool.options.password === "string"
    ...
    try {
      ...
      config = {
        ...
        port:
          config.port ??
          (parsed.port
            ? Option.getOrUndefined(Number.parse(parsed.port))
            : undefined),
        username: config.username ?? parsed.user ?? undefined,
        password:
          config.password ??
          (parsed.password ? Redacted.make(parsed.password) : undefined),
 ...
 * @since 4.0.0
 */
export const fromClient = Effect.fnUntraced(function* (options: {
  ...
  let streamClient = options.acquireForStream
    ? acquireWithErrorHandler
    : Effect.acquireRelease(Effect.as(semaphore.take(1), client), () =>
        semaphore.release(1)
      );

  const makeConection = (client: Pg.Client) =>
    new ConnectionImpl(function runWithClient<A>(
      f: (
        client: Pg.ClientBase,
        resume: (_: Effect.Effect<A, SqlError>) => void
      ) => void
          ...
          >((resume) => {
            cursor.read(128, (err, rows) => {
              if (err) {
                ...
                resume(
                  Effect.succeed(
                    transformRows ? (transformRows(rows) as any) : rows
                  )
                );
              } else {
                resume(Cause.done());
...
const cancelEffects = new WeakMap<
  Pg.PoolClient,
