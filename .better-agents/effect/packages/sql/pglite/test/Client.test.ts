  ...
  layer(ClientLayer, { timeout: "30 seconds" })((it) => {
    ...
    it.effect("in helper", () =>
      Effect.gen(function* () {
        const sql = yield* PgliteClient.PgliteClient;
        const [query, params] =
          sql`SELECT * FROM ${sql("people")} WHERE id IN ${sql.in([1, 2, "x"])}`.compile();
        assert.strictEqual(
          query,
    ...
    it.effect("and helper", () =>
      Effect.gen(function* () {
        ...
        const now = new Date();
        const [query, params] =
          sql`SELECT * FROM ${sql("people")} WHERE ${sql.and([
    ...
    it.effect("identifier transform", () =>
      Effect.gen(function* () {
        const sql = yield* PgliteClient.PgliteClient;
        const compiler = PgliteClient.makeCompiler((s) =>
    ...
    it.effect("json fragment", () =>
      Effect.gen(function* () {
        ...
        const rows = yield* sql<{
          json: unknown;
        }>`SELECT ${sql.json({ a: 1 })}::jsonb AS json`;
      ...
      () =>
        Effect.gen(function* () {
          ...
          const unsub = yield* Effect.tryPromise({
            try: () =>
              sql.pglite.listen("ch1", (payload) =>
                Effect.runFork(Deferred.succeed(deferred, payload))
              ),
            catch: (cause) => cause,
          });
