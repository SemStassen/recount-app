  ...
  layer(ClientLayer, { timeout: "30 seconds" })((it) => {
    it.effect("withTransaction commit", () =>
      Effect.gen(function* () {
        ...
        yield* sql.withTransaction(
          sql.unsafe(`INSERT INTO tx_commit (name) VALUES ('hello')`)
        );
        const rows = yield* sql.unsafe<{ name: string }>(
          `SELECT name FROM tx_commit`
        );
    ...
    it.effect("withTransaction rollback", () =>
      Effect.gen(function* () {
        ...
        yield* sql
          .unsafe(`INSERT INTO tx_rollback (name) VALUES ('hello')`)
          .pipe(
    ...
    it.effect("nested transaction commits both", () =>
      Effect.gen(function* () {
        ...
        const stmt = sql.unsafe(
          `INSERT INTO tx_nested_commit (name) VALUES ('hello')`
        );
        yield* stmt.pipe(
    ...
    it.effect("nested transaction rollback via savepoint", () =>
      Effect.gen(function* () {
        ...
        const stmt = sql.unsafe(
          `INSERT INTO tx_nested_rollback (name) VALUES ('hello')`
        );
        yield* stmt.pipe(
          Effect.andThen(() =>
            stmt.pipe(
