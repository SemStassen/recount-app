...
export interface PgliteClient extends Client.SqlClient {
  ...
  readonly dumpDataDir: (
    compression?: "none" | "gzip" | "auto"
  ) => Effect.Effect<File | Blob, SqlError>;
 ...
 * @since 4.0.0
 */
export const PgliteClient = Context.Service<PgliteClient>(
  "@effect/sql-pglite/PgliteClient"
);
 ...
 * @since 4.0.0
 */
export type PgliteClientConfig =
  | PgliteClientConfig.Create
  | PgliteClientConfig.Live;
              ...
              try: async () => {
                ...
                new SqlError({
                  reason: classifyError(
                    cause,
                    "PgliteClient: Failed to connect",
                    "connect"
                  ),
                }),
            }),
            (pg) =>
              Effect.promise(() => pg.close()).pipe(Effect.timeoutOption(1000)),
            { interruptible: true }
          );
...
export const fromClient = (
  options: PgliteClientConfig.Base & {
    readonly liveClient: PGliteInterface;
  }
): Effect.Effect<PgliteClient, SqlError, Scope.Scope | Reactivity.Reactivity> =>
  Effect.gen(function* () {
    ...
    return Object.assign(client, {
      ...
      notify: (channel: string, payload: string) =>
        Effect.tryPromise({
          try: () =>
            pglite.exec(`NOTIFY ${escape(channel)}, ${escapeLiteral(payload)}`),
          catch: (cause) =>
            new SqlError({
              reason: classifyError(cause, "Failed to notify", "notify"),
            }),
        }).pipe(semaphore.withPermit, Effect.asVoid),
...
class PgliteConnection implements Connection {
    ...
    return Effect.map(
      Effect.tryPromise({
        try: () => this.pglite.query<any>(sql, params as Array<any>),
          ...
          new SqlError({
            reason: classifyError(cause, "Failed to execute statement", method),
          }),
      }),
      (result) => result.rows
    );
  ...
  executeValues(sql: string, params: ReadonlyArray<unknown>) {
    return Effect.map(
      Effect.tryPromise({
        try: () =>
          this.pglite.query<any>(sql, params as Array<any>, {
            rowMode: "array",
          }),
        catch: (cause) =>
          new SqlError({
...
export const layerFrom = <E, R>(
  ...
  PgliteClient | Client.SqlClient,
  E,
  Exclude<R, Scope.Scope | Reactivity.Reactivity>
> =>
  Layer.effectContext(
...
> =>
  layerFrom(
    Effect.flatMap(Config.unwrap(config), (resolved) =>
      make(resolved as PgliteClientConfig)
    )
  );
...
): Layer.Layer<PgliteClient | Client.SqlClient, SqlError> =>
