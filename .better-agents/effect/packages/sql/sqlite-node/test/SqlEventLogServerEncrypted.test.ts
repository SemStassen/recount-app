...
const persistEntries = (
  ...
  Effect.gen(function* () {
    ...
    return encrypted.encryptedEntries.map(
...
const getIdentityRootSecretMaterial = makeGetIdentityRootSecretMaterial(
  globalThis.crypto
);
...
const makeAuthenticateRequest = Effect.fnUntraced(function* (options: {
  ...
  const rootSecretMaterial = yield* getIdentityRootSecretMaterial(
    options.identity
  );
...
describe("SqlEventLogServer", () => {
  ...
  it.effect("rejects session-auth rebinding for an existing publicKey", () =>
    Effect.gen(function* () {
      ...
      const rpcClient = yield* RpcTest.makeClient(
      ...
      const secondMaterial =
        yield* getIdentityRootSecretMaterial(secondIdentity);
      const sameSigningPublicKey =
        firstMaterial.signingPublicKey.byteLength ===
          secondMaterial.signingPublicKey.byteLength &&
        firstMaterial.signingPublicKey.every(
          (byte, index) => byte === secondMaterial.signingPublicKey[index]
        );
  ...
  it.effect("writes entries and streams changes", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        written.map((entry) => entry.sequence),
        [1, 2]
      );

      const changes = yield* storage
        .changes(identity.publicKey, storeIdA, 0)
        .pipe(Stream.toQueue({ capacity: "unbounded" }));
      const taken = yield* Queue.takeAll(changes);
      assert.deepStrictEqual(
        taken.map((entry) => entry.sequence),
        [1, 2]
      );

      const nextEntry = makeEntry(3);
      const nextPersisted = yield* persistEntries(encryption, identity, [
        nextEntry,
      ]);
      const updated = yield* storage.write(
        identity.publicKey,
        storeIdA,
        nextPersisted
      );
      assert.deepStrictEqual(
        updated.map((entry) => entry.sequence),
        [3]
      );
  ...
  it.effect("isolates same publicKey across storeIds", () =>
    Effect.gen(function* () {
      ...
      const storeAEntries = yield* storage
        .changes("client-1", storeIdA, 0)
        .pipe(Stream.take(1), Stream.runCollect);
      const storeBEntries = yield* storage
        .changes("client-1", storeIdB, 0)
        .pipe(Stream.take(1), Stream.runCollect);

      assert.deepStrictEqual(
        storeAEntries.map((entry) => entry.sequence),
        [1]
      );
      assert.deepStrictEqual(
        storeBEntries.map((entry) => entry.sequence),
        [1]
      );
    }).pipe(Effect.provide([Reactivity.layer, EventLogEncryption.layerSubtle]))
  );
  ...
  it.effect("isolates same storeId across publicKeys", () =>
    Effect.gen(function* () {
      ...
      const clientOneEntries = yield* storage
        .changes("client-1", storeIdA, 0)
        .pipe(Stream.take(1), Stream.runCollect);
      const clientTwoEntries = yield* storage
        .changes("client-2", storeIdA, 0)
        .pipe(Stream.take(1), Stream.runCollect);

      assert.deepStrictEqual(
        clientOneEntries.map((entry) => entry.sequence),
        [1]
      );
      assert.deepStrictEqual(
        clientTwoEntries.map((entry) => entry.sequence),
        [1]
      );
    }).pipe(Effect.provide([Reactivity.layer, EventLogEncryption.layerSubtle]))
  );
  ...
  it.effect("keeps deduplication isolated per encrypted scope", () =>
    Effect.gen(function* () {
      ...
      yield* storage.write("client-1", storeIdA, [
        makePersistedEntry(1, sharedEntryId),
      ]);
      yield* storage.write("client-1", storeIdA, [
        makePersistedEntry(2, sharedEntryId),
      ]);
      yield* storage.write("client-1", storeIdB, [
        makePersistedEntry(3, sharedEntryId),
      ]);
      yield* storage.write("client-2", storeIdA, [
      ...
      assert.deepStrictEqual(
        (yield* storage
          .changes("client-1", storeIdA, 0)
          .pipe(Stream.take(1), Stream.runCollect)).map(
