...
describe("Client", () => {
  it.effect("should work", () =>
    Effect.gen(function* () {
      ...
      response = yield* sql`INSERT INTO test (name) VALUES ('world')`.pipe(
        sql.withTransaction
      );
      assert.deepStrictEqual(response, []);
      response = yield* sql`SELECT * FROM test`;
  ...
  it.effect("should work with raw", () =>
    Effect.gen(function* () {
      ...
      let response;
      response =
        yield* sql`CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)`.raw;
      ...
      assert.deepStrictEqual(response, [{ id: 1, name: "hello" }]);
      response = yield* sql`INSERT INTO test (name) VALUES ('world')`.raw.pipe(
        sql.withTransaction
      );
