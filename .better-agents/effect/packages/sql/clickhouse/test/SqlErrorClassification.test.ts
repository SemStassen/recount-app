...
vi.mock("@clickhouse/client", () => ({
  createClient: () => ({
    exec: () =>
      state.connectCause
        ? Promise.reject(state.connectCause)
        : Promise.resolve({}),
...
const connectFailureReasonTag = (code: number) =>
  Effect.gen(function* () {
    ...
    const { ClickhouseClient } = yield* Effect.promise(
      () => import("@effect/sql-clickhouse")
    );
    const error = yield* Effect.flip(
      ClickhouseClient.make({ url: "http://localhost:8123" })
    );
...
describe("ClickhouseClient SqlError classification", () => {
  ...
  it.effect(
    "falls back to UnknownError for unmapped codes during execution",
    () =>
      Effect.gen(function* () {
