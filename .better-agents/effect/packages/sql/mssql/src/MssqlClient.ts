...
const mssqlCauseProperty = (
  cause: unknown,
  property: "constraint" | "message"
): unknown => {
...
const mssqlUniqueViolationConstraintFromMessage = (
  number: 2601 | 2627,
  message: unknown
): string => {
  ...
  return match === null
    ? UNKNOWN_CONSTRAINT
    : normalizeConstraintIdentifier(match[1]);
};

const mssqlUniqueViolationConstraintFromCause = (
  number: 2601 | 2627,
  cause: unknown
): string => {
  const constraint = normalizeConstraintIdentifier(
    mssqlCauseProperty(cause, "constraint")
  );
  if (constraint !== UNKNOWN_CONSTRAINT) {
    return constraint;
  }
  return mssqlUniqueViolationConstraintFromMessage(
    number,
    mssqlCauseProperty(cause, "message")
  );
};
  ...
  if (number !== undefined) {
    ...
    if (number === 2601 || number === 2627) {
      return new UniqueViolation({
        ...props,
        constraint: mssqlUniqueViolationConstraintFromCause(number, cause),
      });
    }
 ...
 * @since 4.0.0
 */
export const MssqlClient = Context.Service<MssqlClient>(
  "@effect/sql-mssql/MssqlClient"
);
...
export interface MssqlClientConfig {
  ...
  readonly parameterTypes?:
    | Record<Statement.PrimitiveKind, DataType>
    | undefined;
...
interface MssqlConnection extends Connection {
  readonly call: (
    ...
    transformRows:
      | ((rows: ReadonlyArray<any>) => ReadonlyArray<any>)
      | undefined
  ...
  Effect.gen(function* () {
    ...
    yield* Pool.get(pool).pipe(
      Effect.tap((connection) =>
        connection.executeUnprepared("SELECT 1", [], undefined)
      ),
      Effect.mapError(
        (cause) =>
          new SqlError({
      ...
      Object.assign(
        ...
        }),
        {
          ...
          >(
            ...
            Effect.scoped(
              Effect.flatMap(Pool.get(pool), (_) =>
                _.call(procedure, transformRows)
              )
            ),
          withoutTransforms() {
            const statement = Statement.make(
              Pool.get(pool),
              compiler.withoutTransform,
              spanAttributes,
              undefined
            );
            const client = Object.assign(statement, this, statement, {
              call: <
                I extends Record<string, Parameter<any>>,
                O extends Record<string, Parameter<any>>,
                A,
...
interface MssqlParam extends Statement.Custom<
  "MssqlParam",
  DataType,
  unknown,
  ParameterOptions
