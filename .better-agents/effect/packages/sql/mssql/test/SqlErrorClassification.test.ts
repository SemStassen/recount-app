...
class MockConnection {
  ...
  execSql(request: MockRequest) {
    const cause =
      state.requestCauses.length > 0 ? state.requestCauses.shift() : null;
...
const queryFailureReasonTag = (number: number) =>
  Effect.map(queryFailureReason({ number }), (reason) => reason._tag);
...
describe("MssqlClient SqlError classification", () => {
  ...
  it.effect(
    "classifies duplicate-key number 2601 as UniqueViolation and extracts the unique index",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "classifies constraint number 2627 as UniqueViolation and extracts the constraint",
    () =>
      Effect.gen(function* () {
        const reason = yield* queryFailureReason({
          number: 2627,
  ...
  it.effect(
    "uses unknown for blank, missing, malformed, or non-string unique violation metadata",
    () =>
      Effect.gen(function* () {
        ...
        const blank = yield* queryFailureReason({
          ...
          constraint: "   ",
          message:
            "Violation of UNIQUE KEY constraint '   '. Cannot insert duplicate key in object 'dbo.Users'.",
        });
        ...
        const nonString = yield* queryFailureReason({
          ...
          constraint: 2627,
          message: {
            text: "Violation of UNIQUE KEY constraint 'UQ_Users_Email'.",
          },
        });
  ...
  it.effect(
    "keeps non-unique constraint number 547 classified as ConstraintError",
    () =>
      Effect.gen(function* () {
