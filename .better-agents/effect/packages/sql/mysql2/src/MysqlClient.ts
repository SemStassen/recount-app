...
const mysqlConstraintErrorCodes = new Set([
  1022, 1048, 1169, 1216, 1217, 1451, 1452, 1557,
]);
...
const mysqlCauseProperty = (
  cause: unknown,
  property: "constraint" | "message" | "sqlMessage"
): unknown => {
...
const mysqlDuplicateEntryConstraintFromMessage = (message: unknown): string => {
  ...
  const match =
    /\bfor key\s+(?:'([^']*)'|\x60([^\x60]*)\x60|([^\s'\x60]+))/i.exec(message);
  return match === null
    ? UNKNOWN_CONSTRAINT
...
const mysqlDuplicateEntryConstraintFromCause = (cause: unknown): string => {
  const constraint = normalizeConstraintIdentifier(
    mysqlCauseProperty(cause, "constraint")
  );
  if (constraint !== UNKNOWN_CONSTRAINT) {
    return constraint;
  }
  const sqlMessageConstraint = mysqlDuplicateEntryConstraintFromMessage(
    mysqlCauseProperty(cause, "sqlMessage")
  );
  if (sqlMessageConstraint !== UNKNOWN_CONSTRAINT) {
    return sqlMessageConstraint;
  }
  return mysqlDuplicateEntryConstraintFromMessage(
    mysqlCauseProperty(cause, "message")
  );
};
 ...
 * @since 4.0.0
 */
export const MysqlClient = Context.Service<MysqlClient>(
    ...
    class ConnectionImpl implements Connection {
      ...
      private runRaw(
        ...
        return Effect.callback<unknown, SqlError>((resume) => {
          const operation =
            method === "query" ? "executeUnprepared" : "execute";
...
function queryStream(
  ...
  return asyncPauseResume<any, SqlError>(
    Effect.fnUntraced(function* (emit) {
      const query = (conn as any).query(sql, params).stream();
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => query.destroy() as void)
      );

      let buffer: Array<any> = [];
      let taskPending = false;
      query.on("error", (cause: unknown) =>
        emit.fail(
          new SqlError({
            reason: classifyError(
              cause,
              "Failed to stream statement",
              "stream"
            ),
          })
        )
