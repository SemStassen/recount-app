...
describe("SqlModel", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* repo.insert(
          User.insert.make({ name: "Alice", age: 30 })
        );
  ...
  it.effect(
    "insert returns result with transforms",
    () =>
      Effect.gen(function* () {
        const repo = yield* SqlModel.makeRepository(User, {
          tableName: "users",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* repo.insertVoid(
          User.insert.make({ name: "Alice", age: 30 })
        );
  ...
  it.live(
    "insert data loader returns result",
    () =>
      Effect.gen(function* () {
        ...
        const [alice, john] = yield* Effect.all(
          [
            ...
            SqlResolver.request(
              User.insert.make({ name: "John", age: 30 }),
              repo.insert
            ),
          ],
  ...
  it.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const sql = yield* SqlClient.SqlClient;
        yield* sql`CREATE TABLE users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), age INT)`;
        const alice = yield* SqlResolver.request(
          User.insert.make({ name: "Alice", age: 30 }),
          repo.insert
        );
        const john = yield* SqlResolver.request(
          User.insert.make({ name: "John", age: 30 }),
          repo.insert
        );
  ...
  it.effect(
    "findById ignores soft deleted rows",
    () =>
      Effect.gen(function* () {
        ...
        const alice = yield* repo.insert(
          SoftDeleteUser.insert.make({ name: "Alice" })
        );
        const bob = yield* repo.insert(
          SoftDeleteUser.insert.make({ name: "Bob" })
        );
        ...
        assert.deepStrictEqual(
          aliceResult,
          new SoftDeleteUser({ id: alice.id, name: "Alice", deletedAt: null })
        );
  ...
  it.live(
    "findById data loader ignores soft deleted rows",
    () =>
      Effect.gen(function* () {
        ...
        const bob = yield* SqlResolver.request(
          SoftDeleteUser.insert.make({ name: "Bob" }),
          repo.insert
        );
        ...
        const error = yield* Effect.flip(
          SqlResolver.request(bob.id, repo.findById)
        );
        assert.isTrue(Cause.isNoSuchElementError(error));
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        let result = yield* repo.insert(
          User.insert.make({ name: "Alice", age: 30 })
        );
        result = yield* repo.update(
          User.update.make({ ...result, name: "Bob" })
        );
        assert.deepStrictEqual(
          result,
          new User({ id: 1, name: "Bob", age: 30 })
        );
      }).pipe(
        Effect.provide(MysqlContainer.layerClient),
        Effect.catchTag("ContainerError", () => Effect.void)
  ...
  it.effect(
    "update returns result with transforms",
    () =>
      Effect.gen(function* () {
        const repo = yield* SqlModel.makeRepository(User, {
          tableName: "users",
