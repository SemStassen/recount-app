...
vi.mock("mysql2", () => ({
  createPool: () => ({
    query: (_sql: string, cb: (cause: unknown) => void) =>
      cb(state.connectCause),
    end: (cb: () => void) => cb(),
...
const connectFailureReason = (cause: unknown) =>
  Effect.gen(function* () {
    ...
    const { MysqlClient } = yield* Effect.promise(
      () => import("@effect/sql-mysql2")
    );
...
const connectFailureReasonTag = (errno: number) =>
  Effect.map(connectFailureReason({ errno }), (reason) => reason._tag);
...
describe("MysqlClient SqlError classification", () => {
  ...
  it.effect(
    "extracts duplicate-entry constraints from structured fields and common messages",
    () =>
      Effect.gen(function* () {
        const explicit = yield* connectFailureReason({
          ...
          sqlMessage:
            "Duplicate entry 'user@example.com' for key 'ignored_key'",
        });
        ...
        const singleQuoted = yield* connectFailureReason({
          errno: 1062,
          sqlMessage:
            "Duplicate entry 'user@example.com' for key 'users_email_key'",
        });
  ...
  it.effect(
    "uses unknown for blank, missing, malformed, or non-string duplicate-entry metadata",
    () =>
      Effect.gen(function* () {
        ...
        const nonString = yield* connectFailureReason({
          ...
          sqlMessage: 1062,
          message: {
            text: "Duplicate entry 'user@example.com' for key 'users_email_key'",
          },
        });
  ...
  it.effect(
    "keeps non-unique constraint errno values classified as ConstraintError",
    () =>
      Effect.gen(function* () {
