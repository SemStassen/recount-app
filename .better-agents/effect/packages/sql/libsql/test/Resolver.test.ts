...
layer(LibsqlContainer.layerClient, { timeout: "30 seconds" })(
  "Resolver",
  (it) => {
      ...
      it.effect("result length mismatch", () =>
        Effect.gen(function* () {
          ...
          const error = yield* Effect.all(
            [execute(1), execute(2), execute(3), execute(101)],
            { concurrency: "unbounded" }
          ).pipe(Effect.flip);
      ...
      it.effect("find by name", () =>
        Effect.gen(function* () {
          ...
          const FindByName = SqlResolver.grouped({
            ...
            execute: (names) =>
              sql`SELECT * FROM test WHERE name IN ${sql.in(names)}`,
          });
      ...
      it.effect("find by id", () =>
        Effect.gen(function* () {
          ...
          const FindById = SqlResolver.findById({
            ...
            execute: (ids) =>
              sql`SELECT * FROM test WHERE id IN ${sql.in(ids)}`,
          });
