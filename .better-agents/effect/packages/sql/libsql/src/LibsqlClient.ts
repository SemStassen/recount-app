 ...
 * @since 4.0.0
 */
export const LibsqlClient = Context.Service<LibsqlClient>(
  "@effect/sql-libsql/LibsqlClient"
);

const LibsqlTransaction = Context.Service<
  readonly [LibsqlConnection, counter: number]
...
export type LibsqlClientConfig =
  | LibsqlClientConfig.Full
  | LibsqlClientConfig.Live;
  ...
  Effect.gen(function* () {
    ...
    class LibsqlConnectionImpl implements LibsqlConnection {
      ...
      executeValues(sql: string, params: ReadonlyArray<unknown>) {
        return Effect.map(this.run(sql, params), (rows) =>
      ...
      Option.match({
        onNone: () =>
          semaphore.withPermits(1)(
            Effect.succeed(connection as LibsqlConnection)
          ),
