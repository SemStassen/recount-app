 ...
 * @since 4.0.0
 */
export const make = Effect.fnUntraced(function* <R extends string>(
  ...
  const handlerStack: Array<
    (
      request: Request,
      server: BunServer<WebSocketContext>
      ...
      function handler(request: Request, server: BunServer<WebSocketContext>) {
        return new Promise<Response>((resolve, _reject) => {
          ...
          const fiber = Fiber.runIn(
            Effect.runForkWith(Context.makeUnsafe<any>(map))(httpEffect),
            scope
          );
...
> = Layer.mergeAll(Platform.layer, Etag.layerWeak, BunServices.layer);
...
export const layer = <R extends string>(
  ...
  Server.HttpServer | HttpPlatform | Etag.Generator | BunServices.BunServices
  ...
  implements ServerRequest.HttpServerRequest
{
  ...
  get remoteAddress(): Option.Option<string> {
    return (
      this.remoteAddressOverride ??
      Option.fromNullishOr(this.bunServer.requestIP(this.source)?.address)
    );
  }
  ...
  > {
    ...
    this.multipartEffect = Effect.runSync(
      Effect.cached(BunMultipart.persisted(this.source))
    );
    return this.multipartEffect;
  }
  ...
  private arrayBufferEffect:
    | Effect.Effect<ArrayBuffer, Error.HttpServerError>
    | undefined;
  get arrayBuffer(): Effect.Effect<ArrayBuffer, Error.HttpServerError> {
    ...
    this.textEffect = Effect.map(this.arrayBufferEffect, (_) =>
      ...
      resume(
        Effect.map(Deferred.await(deferred), (ws) => {
          ...
          const runRaw = Effect.fnUntraced(
            ...
            Effect.scoped,
            Effect.onExit((exit) =>
              Effect.sync(() => ws.close(exit._tag === "Success" ? 1000 : 1011))
            ),
