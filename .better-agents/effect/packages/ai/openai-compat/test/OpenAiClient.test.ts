...
describe("OpenAiClient", () => {
  describe("request behavior", () => {
    it.effect(
      "sets auth and OpenAI headers on /chat/completions requests",
      () =>
        Effect.gen(function* () {
              ...
              Layer.succeed(
                ...
                makeHttpClient((request) => {
                  ...
                  return Effect.succeed(
                    jsonResponse(request, 200, makeChatCompletion())
                  );
                })
              )
            )
          ...
          assert.isTrue(
            capturedRequest.url.startsWith("https://compat.example.test/v1")
          );
          assert.strictEqual(
    ...
    it.effect(
      "uses /embeddings path and decodes permissive embedding payloads",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
    ...
    it.effect(
      "sets stream=true for createResponseStream and returns chat chunks",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
          ...
          const eventsChunk = yield* client
            .createResponseStream({
              model: "gpt-4o-mini",
              messages: [{ role: "user", content: "hello" }],
            })
            .pipe(Effect.flatMap(([_, stream]) => Stream.runCollect(stream)));
    ...
    it.effect(
      "passes chat-completions tool_choice payload through unchanged",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
    ...
    it.effect("accepts assistant tool-call and tool result chat history", () =>
      Effect.gen(function* () {
        ...
        const patchMessage = assistantMessages.find(
          (message: any) =>
        ...
        assert.deepStrictEqual(
          JSON.parse(patchMessage.tool_calls[0].function.arguments),
          {
            call_id: "patch_call_1",
            operation: {
              type: "delete_file",
        ...
        const patchOutput = toolMessages.find(
          (message: any) => message.tool_call_id === "patch_call_1"
        );
