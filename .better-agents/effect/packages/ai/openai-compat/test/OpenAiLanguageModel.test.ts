...
describe("OpenAiLanguageModel", () => {
  describe("generateText", () => {
    it.effect("sends model in request and decodes text output", () =>
      Effect.gen(function* () {
        ...
        const layer = OpenAiClient.layer({
    ...
    it.effect("forwards reasoning config to chat completions request", () =>
      Effect.gen(function* () {
        ...
        yield* LanguageModel.generateText({ prompt: "hello" }).pipe(
          Effect.provide(
            OpenAiLanguageModel.model("gpt-5", {
              reasoning: {
                effort: "medium",
                summary: "auto",
              },
    ...
    it.effect(
      "forwards custom model config properties to chat completions request",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
    ...
    it.effect(
      "maps function_call output to tool-call part and sends function tool schema",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
    ...
    it.effect(
      "maps provider apply_patch function call back to custom provider-defined tool",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
  ...
  describe("generateObject", () => {
    it.effect("uses json_schema format for structured output", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          requestBody.response_format.json_schema.strict,
          true
        );
      })
    );
    ...
    it.effect(
      "uses OpenAI codec transformer for optional structured fields",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
          ...
          assert.deepStrictEqual(
            requestBody.response_format.json_schema.schema.properties.nickname,
            {
  ...
  describe("streamText", () => {
    ...
    it.effect(
      "maps local shell stream tool calls to local_shell call outputs",
      () =>
        Effect.gen(function* () {
          ...
          const httpClient = HttpClient.makeWith(
            Effect.fnUntraced(function* (requestEffect) {
              ...
              yield* Ref.update(capturedRequests, (requests) => [
                ...requests,
                request,
              ]);
          ...
          const toolCall = globalThis.Array.from(partsChunk).find(
            (part) => part.type === "tool-call"
          );
          assert.isDefined(toolCall);
          if (toolCall?.type !== "tool-call") {
          ...
          const localShellCall = followUpBody.messages.find(
            (item: any) =>
              item.role === "assistant" &&
              item.tool_calls?.[0]?.function?.name === "local_shell"
          );
          assert.isDefined(localShellCall);
          assert.strictEqual(localShellCall.tool_calls[0].id, toolCall.id);

          const localShellOutput = followUpBody.messages.find(
    ...
    it.effect(
      "maps apply_patch stream tool calls to custom provider-defined tool",
      () =>
        Effect.gen(function* () {
          const layer = OpenAiClient.layer({
            apiKey: Redacted.make("sk-test-key"),
    ...
    it.effect("preserves fragmented stream tool call ids and names", () =>
      Effect.gen(function* () {
        ...
        const start = parts.find(
          (part) =>
            part.type === "tool-params-start" && part.id === "patch_call_2"
        );
        ...
        assert.deepStrictEqual(
          decodeToolParamsFromStream(parts, "patch_call_2"),
          expectedParams
        );
          ...
          Layer.provide(
            Layer.succeed(
              ...
              makeHttpClient((request) => {
                capturedRequest = request;
                return Effect.succeed(
                  jsonResponse(request, makeChatCompletion())
                );
              })
            )
          )
