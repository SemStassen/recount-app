...
export const make = Effect.fnUntraced(
  function* (
    ...
    const httpClient = baseClient.pipe(
      ...
      options.transformClient !== undefined ? options.transformClient : identity
    );
...
export const layerConfig = (options?: {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      const apiKey =
        options?.apiKey !== undefined ? yield* options.apiKey : undefined;
      const apiUrl =
        options?.apiUrl !== undefined ? yield* options.apiUrl : undefined;
      const organizationId =
        options?.organizationId !== undefined
          ? yield* options.organizationId
          : undefined;
      const projectId =
        options?.projectId !== undefined ? yield* options.projectId : undefined;
      return yield* make({
        apiKey,
        apiUrl,
        organizationId,
...
type FunctionCallOutput = {
  ...
  readonly output:
    | string
    | ReadonlyArray<InputTextContent | InputImageContent | InputFileContent>;
...
export type ChatCompletionRequestMessage =
  | {
      ...
      readonly content:
        | string
        | ReadonlyArray<ChatCompletionContentPart>
        | null;
      readonly tool_calls?:
        | ReadonlyArray<ChatCompletionRequestToolCall>
        | undefined;
    }
  | {
      readonly role: "tool";
...
const decodeChatCompletionSseData = (
  ...
  return isChatCompletionChunk(parsed) ? parsed : undefined;
};
