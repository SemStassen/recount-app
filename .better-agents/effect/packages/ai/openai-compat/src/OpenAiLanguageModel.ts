...
>({
  ...
  if (message !== undefined) {
    if (
      ...
      Predicate.isNotNull(message.content) &&
      ...
      Effect.fnUntraced(function* (event) {
        ...
        if (event === "[DONE]") {
          ...
          for (const toolCall of Object.values(activeToolCalls)) {
            const toolParams =
              toolCall.arguments.length > 0 ? toolCall.arguments : "{}";
            const params = yield* Effect.try({
              try: () => Tool.unsafeSecureJsonParse(toolParams),
              catch: (cause) =>
                AiError.make({
        ...
        if (!metadataEmitted) {
          ...
          parts.push({
            ...
            timestamp: DateTime.formatIso(
              DateTime.fromDateUnsafe(new Date(event.created * 1000))
            ),
            request: buildHttpRequestDetails(response.request),
          });
        }
        ...
        if (
          ...
          Predicate.isNotNull(choice.delta.content)
        ) {
          if (!textStarted) {
            textStarted = true;
            parts.push({
        ...
        if (
          ...
          Predicate.isNotNull(choice.finish_reason)
        ) {
  ...
  for (const tool of allowedTools) {
    ...
    if (Tool.isProviderDefined(tool)) {
      tools.push({
        ...
        description: Tool.getDescription(tool) ?? null,
        parameters: Tool.getJsonSchema(tool) as {
  ...
  const tools =
    payload.tools !== undefined
      ? payload.tools
          .map(toChatTool)
          .filter(
            (tool): tool is NonNullable<ReturnType<typeof toChatTool>> =>
              tool !== undefined
          )
      : [];
...
const extractCustomRequestProperties = (
  payload: CreateResponse
...
const toChatToolChoice = (
  ...
  const functionName =
    Predicate.hasProperty(toolChoice, "name") &&
...
const getUsage = (usage: CreateResponse200["usage"]): Response.Usage => {
  ...
  const cachedTokens =
    getUsageDetailNumber(usage.prompt_tokens_details, "cached_tokens") ?? 0;
  const reasoningTokens =
    getUsageDetailNumber(usage.completion_tokens_details, "reasoning_tokens") ??
    0;
