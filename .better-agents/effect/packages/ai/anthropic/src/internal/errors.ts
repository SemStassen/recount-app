...
export const mapClientError = dual<
  (
    ...
    error: Generated.AnthropicClientError<string, AnthropicClientErrorBody>
  ) => AiError.AiError,
  (
    error: Generated.AnthropicClientError<string, AnthropicClientErrorBody>,
    method: string
...
export const parseRateLimitHeaders = (headers: Record<string, string>) => {
  ...
  return {
    retryAfter,
    requestsLimit: Predicate.isNotUndefined(requestsLimitRaw)
      ? Option.getOrNull(Number.parse(requestsLimitRaw))
      : null,
    ...
    tokensLimit: Predicate.isNotUndefined(tokensLimitRaw)
      ? Option.getOrNull(Number.parse(tokensLimitRaw))
      : null,
...
export const buildHttpContext = (params: {
  ...
  response: Predicate.isNotUndefined(params.response)
    ? {
        ...
        headers: Redactable.redact(params.response.headers) as Record<
          string,
          string
        >,
      }
...
export const mapStatusCodeToReason = ({
  ...
  switch (status) {
    ...
    case 429: {
      const { retryAfter, ...rateLimitMetadata } =
