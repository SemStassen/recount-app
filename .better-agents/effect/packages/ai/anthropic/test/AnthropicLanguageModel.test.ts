  ...
  describe("streamText", () => {
    it.effect("decodes tool call params in content_block_stop", () =>
      Effect.gen(function* () {
        ...
        const layer = AnthropicClient.layer({
        ...
        const partsChunk = yield* LanguageModel.streamText({
          ...
          Stream.runCollect,
          Effect.provide(
            AnthropicLanguageModel.model("claude-sonnet-4-20250514")
          ),
  ...
  describe("generateText", () => {
    it.effect("encodes dynamic tools", () =>
      Effect.gen(function* () {
        ...
        const dynamicTool = body.tools.find(
          (tool: any) => tool.name === "DynamicTool"
        );
...
const toSseBody = (events: ReadonlyArray<unknown>): string =>
  events
    .map((event) => `event: message_stream\ndata: ${JSON.stringify(event)}\n\n`)
    .join("");
