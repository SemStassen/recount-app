...
describe("OpenAiEmbeddingModel", () => {
  ...
  it.effect("reorders embeddings by provider index", () =>
    Effect.gen(function* () {
      ...
      const response = yield* Effect.gen(function* () {
        ...
        Effect.provide(
          OpenAiEmbeddingModel.layer({ model: "text-embedding-3-small" })
        ),
          ...
          Layer.succeed(
            ...
            makeHttpClient((request) => {
              ...
              return Effect.succeed(
                jsonResponse(request, {
                  data: [
                    { index: 0, embedding: [1, 2, 3], object: "embedding" },
                  ],
  ...
  it.effect(
    "fails with InvalidOutputError when provider returns duplicate indices",
    () =>
      Effect.gen(function* () {
        const clientLayer = OpenAiClient.layer({
          apiKey: Redacted.make("sk-test-key"),
  ...
  it.effect(
    "fails with InvalidOutputError when provider returns out-of-range indices",
    () =>
      Effect.gen(function* () {
        const clientLayer = OpenAiClient.layer({
          apiKey: Redacted.make("sk-test-key"),
  ...
  it.effect(
    "fails with InvalidOutputError when provider returns wrong embedding count",
    () =>
      Effect.gen(function* () {
        const clientLayer = OpenAiClient.layer({
          apiKey: Redacted.make("sk-test-key"),
  ...
  it.effect(
    "fails with InvalidOutputError when provider returns base64 embeddings",
    () =>
      Effect.gen(function* () {
        const clientLayer = OpenAiClient.layer({
          apiKey: Redacted.make("sk-test-key"),
            ...
            Layer.succeed(
              ...
              makeHttpClient((request) =>
                Effect.succeed(
                  jsonResponse(request, {
                    data: [
                      { index: 0, embedding: "AQID", object: "embedding" },
                    ],
                    model: "text-embedding-3-small",
                    object: "list",
                    usage: {
