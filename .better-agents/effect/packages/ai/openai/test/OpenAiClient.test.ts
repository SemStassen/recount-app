  ...
  describe("make", () => {
      ...
      Effect.gen(function* () {
        ...
        const mockClient = makeMockHttpClient((request) => {
          ...
          return Effect.succeed(
            makeMockResponse({ status: 200, body: {}, request })
          );
        });
        ...
        yield* client
          .createResponse({ model: "gpt-4o", input: "test" })
          .pipe(Effect.ignore);
    ...
    it.effect("prepends default URL", () =>
      Effect.gen(function* () {
        ...
        assert.isDefined(capturedRequest);
        assert.isTrue(
          capturedRequest!.url.startsWith("https://api.openai.com/v1")
        );
      })
    );
    ...
    it.effect("uses custom apiUrl when provided", () =>
      Effect.gen(function* () {
        ...
        assert.isDefined(capturedRequest);
        assert.isTrue(
          capturedRequest!.url.startsWith("https://custom.api.com/v2")
        );
      })
    );

    it.effect(
      "sets OpenAI-Organization header when organizationId provided",
      () =>
        Effect.gen(function* () {
          let capturedRequest: HttpClientRequest.HttpClientRequest | undefined;
          const mockClient = makeMockHttpClient((request) => {
          ...
          assert.isDefined(capturedRequest);
          assert.strictEqual(
            capturedRequest!.headers["openai-organization"],
            "org-12345"
          );
        })
    );
    ...
    it.effect("sets OpenAI-Project header when projectId provided", () =>
      Effect.gen(function* () {
        ...
        assert.isDefined(capturedRequest);
        assert.strictEqual(
          capturedRequest!.headers["openai-project"],
          "proj-67890"
        );
      })
    );
    ...
    it.effect("exposes transformed HttpClient via client field", () =>
      Effect.gen(function* () {
          ...
          transformClient: (client) =>
            client.pipe(
              HttpClient.mapRequest(
                HttpClientRequest.setHeader("x-client-field", "enabled")
              )
            ),
        }).pipe(
          Effect.provide(Layer.succeed(HttpClient.HttpClient, mockClient))
        ...
        yield* client.client
          .execute(HttpClientRequest.get("/responses"))
          .pipe(Effect.ignore);
        ...
        assert.strictEqual(
          capturedRequest!.headers["x-client-field"],
          "enabled"
        );
      })
    );
    ...
    it.effect(
      "applies OpenAiConfig transformClient after options transformClient",
      () =>
        Effect.gen(function* () {
          ...
          const client = yield* OpenAiClient.make({
            ...
            transformClient: (client) => {
              ...
              return client.pipe(
                HttpClient.mapRequest(
                  HttpClientRequest.setHeader("x-openai-transform", "options")
                )
              );
            },
          }).pipe(
          ...
          yield* client
            ...
            .pipe(
              OpenAiConfig.withClientTransform((client) => {
                ...
                return client.pipe(
                  HttpClient.mapRequest(
                    HttpClientRequest.setHeader("x-openai-transform", "config")
                  )
                );
              })
            );
          ...
          assert.isDefined(capturedRequest);
          assert.strictEqual(
            capturedRequest!.headers["x-openai-transform"],
            "config"
          );
        })
    );
  ...
  describe("OpenAiClientGenerated", () => {
    it.effect("sets Bearer token from apiKey", () =>
      Effect.gen(function* () {
        ...
        const mockClient = makeMockHttpClient((request) => {
          capturedRequest = request;
          return Effect.succeed(
            makeMockResponse({ status: 200, body: makeResponseBody(), request })
          );
        });
    ...
    it.effect("prepends custom apiUrl", () =>
      Effect.gen(function* () {
        ...
        assert.isDefined(capturedRequest);
        assert.isTrue(
          capturedRequest!.url.startsWith("https://generated.example.test/v2")
        );
      })
    );
  ...
  describe("layer", () => {
    it.effect("creates working service", () => {
      const HttpClientLayer = Layer.succeed(
        HttpClient.HttpClient,
        makeMockHttpClient(() =>
          Effect.succeed(makeMockResponse({ status: 200, body: {} }))
        )
      );
    ...
    it.effect("layerConfig loads from Config", () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.strictEqual(
          capturedRequest!.headers["authorization"],
          "Bearer sk-config-key"
        );
        assert.isTrue(
          capturedRequest!.url.startsWith("https://config.api.com/v1")
        );
      }).pipe(Effect.provide(MainLayer));
    });
  });
  ...
  describe("error mapping", () => {
    ...
    it.effect("maps 401 status to AuthenticationError reason", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          (result.reason as AiError.AuthenticationError).kind,
          "InvalidKey"
        );
      })
    );
    ...
    it.effect(
      "maps 403 status to AuthenticationError with InsufficientPermissions",
      () =>
        Effect.gen(function* () {
          const mockClient = makeMockHttpClient((request) =>
            Effect.succeed(
    ...
    it.effect(
      "maps 429 with insufficient_quota code to QuotaExhaustedError",
      () =>
        Effect.gen(function* () {
          const mockClient = makeMockHttpClient((request) =>
            Effect.succeed(
