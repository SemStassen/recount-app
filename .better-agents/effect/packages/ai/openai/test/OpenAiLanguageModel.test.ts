...
describe("OpenAiLanguageModel", () => {
  describe("make", () => {
    ...
    it.effect("sends custom model string in request", () =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          makeTestLayer({ body: { model: "ft:gpt-4o-mini:custom" as any } })
        )
      )
    );
  });
      ...
      describe("system messages", () => {
        it.effect("uses system role for standard models", () =>
          Effect.gen(function* () {
            ...
            const systemMessage = body.input.find(
              (m: any) => m.role === "system"
            );
        ...
        it.effect("uses developer role for reasoning models", () =>
          Effect.gen(function* () {
            ...
            const devMessage = body.input.find(
              (m: any) => m.role === "developer"
            );
      ...
      describe("user messages", () => {
        ...
        it.effect("handles image base64 data", () =>
          Effect.gen(function* () {
            ...
            const userMessage = body.input.find((m: any) => m.role === "user");
            assert.isTrue(
              userMessage.content[0].image_url.startsWith(
        ...
        it.effect("handles PDF base64 data with filename", () =>
          Effect.gen(function* () {
            ...
            assert.isTrue(
              userMessage.content[0].file_data.startsWith(
      ...
      describe("assistant messages", () => {
        ...
        it.effect("converts reasoning parts", () =>
          Effect.gen(function* () {
            ...
            const reasoningItem = body.input.find(
              (m: any) => m.type === "reasoning"
            );
    ...
    describe("tool preparation", () => {
      ...
      it.effect("empty object on properties for empty parameters", () =>
        Effect.gen(function* () {
          ...
          const tool = body.tools?.find(
            (t: any) => t.type === "function" && t.name === "EmptyParamsTool"
          );
          assert.isDefined(tool);
      ...
      it.effect("converts dynamic tools to function type", () =>
        Effect.gen(function* () {
          ...
          const tool = body.tools?.find(
            (entry: any) =>
              entry.type === "function" && entry.name === "DynamicTool"
          );
          assert.isDefined(tool);
      ...
      it.effect("handles specific tool choice", () =>
        Effect.gen(function* () {
          ...
          deepStrictEqual(body.tool_choice, {
            type: "function",
            name: "TestTool",
          });
        }).pipe(Effect.provide([makeTestLayer(), TestToolkitLayer]))
      ...
      it.effect("adds code_interpreter tool", () =>
        Effect.gen(function* () {
          const toolkit = Toolkit.make(
            OpenAiTool.CodeInterpreter({ container: { type: "auto" } })
          );
        ...
        Effect.gen(function* () {
          ...
          Effect.provide(
            makeTestLayer({
              body: {
                output: [
                  makeTextOutput(JSON.stringify({ name: "John", age: 30 })),
                ],
              },
            })
          )
      ...
      it.effect("parses function call arguments", () =>
        Effect.gen(function* () {
          ...
          Effect.provide([
            makeTestLayer({
              body: {
                output: [makeFunctionCall("TestTool", { input: "hello" })],
              },
            }),
            TestToolkitLayer,
          ])
        ...
        Effect.gen(function* () {
          ...
          Effect.provide(
            makeTestLayer({
              body: {
                output: [
                  makeMcpCall("CheckPackage", { packageName: "effect" }),
                ],
              },
            })
          )
      ...
      it.effect("uses canonical OpenAiMcp name for mcp_approval_request", () =>
        Effect.gen(function* () {
          ...
          if (
            ...
            approvalRequest?.type === "tool-approval-request"
          ) {
          ...
          Effect.provide(
            makeTestLayer({
              body: {
                output: [
                  makeMcpApprovalRequest("CheckPackage", {
                    packageName: "effect",
                  }),
                ],
              },
          ...
          Effect.provide(
            makeTestLayer({
              body: {
                model: "o1",
                output: [
                  makeReasoningOutput(["First thought", "Second thought"]),
                ],
              },
            })
          )
      ...
      it.effect("extracts usage information", () =>
        Effect.gen(function* () {
          ...
          if (finishPart?.type === "finish") {
            ...
            deepStrictEqual(finishPart.usage.outputTokens, {
              total: 20,
              text: 20,
              reasoning: 0,
            });
      ...
      it.effect("sets finish reason to tool-calls when has tool calls", () =>
        Effect.gen(function* () {
          ...
          Effect.provide([
            makeTestLayer({
              body: {
                output: [makeFunctionCall("TestTool", { input: "test" })],
              },
            }),
            TestToolkitLayer,
          ])
  ...
  describe("streamText", () => {
    it.effect(
      "emits valid apply_patch tool params JSON for update_file diffs",
      () =>
        Effect.gen(function* () {
          const diff = "@@ -1 +1 @@\n-old\n+new\n";
          const outputItem = {
          ...
          const streamEvents = [
            ...
            typeof Generated.ResponseStreamEvent.Type
          >;
    ...
    it.effect(
      "emits tool call from function_call_arguments.done when output_item.done is missing",
      () =>
        Effect.gen(function* () {
          const streamEvents = [
            {
          ...
          const toolCalls = parts.filter(
            (part) => part.type === "tool-call" && part.id === "call_1"
          );
          ...
          const toolParamsEnd = parts.find(
            (part) => part.type === "tool-params-end" && part.id === "call_1"
          );
    ...
    it.effect(
      "handles reasoning summary events when reasoning state is missing",
      () =>
        Effect.gen(function* () {
          const streamEvents = [
            {
          ...
          assert.isDefined(
            parts.find(
              (part) =>
                part.type === "reasoning-start" && part.id === "rs_missing:1"
            )
          );
          assert.isDefined(
            parts.find(
              (part) =>
                part.type === "reasoning-end" && part.id === "rs_missing:1"
            )
          );
    ...
    it.effect(
      "uses canonical OpenAiMcp name for streamed mcp_approval_request",
      () =>
        Effect.gen(function* () {
          const outputItem = makeMcpApprovalRequest(
            "CheckPackage",
            { packageName: "effect" },
            { id: "approval_1" }
          );
    ...
    it.effect(
      "pre-resolves denied OpenAiMcp approvals without lookup failure",
      () =>
        Effect.gen(function* () {
          const result = yield* LanguageModel.generateText({
            prompt: Prompt.make([
    ...
    it.effect("override takes precedence", () =>
      Effect.gen(function* () {
        yield* LanguageModel.generateText({ prompt: "test" }).pipe(
          ...
          Effect.provide(
            OpenAiLanguageModel.model("gpt-4o-mini", { temperature: 0.5 })
          )
        );
...
class MockOpenAiResponse extends Context.Service<
  MockOpenAiResponse,
  {
...
class MockHttpClient extends Context.Service<
  MockHttpClient,
  {
...
const makeHttpClient = Effect.gen(function* () {
  const capturedRequests = yield* Ref.make<
    ReadonlyArray<HttpClientRequest.HttpClientRequest>
  >([]);
  ...
  return Context.make(HttpClient.HttpClient, httpClient).pipe(
    Context.add(
      MockHttpClient,
      MockHttpClient.of({ requests: Ref.get(capturedRequests) })
    )
  );
});
...
const makeStreamTestLayer = (
  events: ReadonlyArray<typeof Generated.ResponseStreamEvent.Type>
) => {
  ...
  return Layer.succeed(
    ...
    OpenAiClient.OpenAiClient.of({
      ...
      createResponse: () =>
        Effect.die(new Error("unexpected createResponse call")),
      createResponseStream: () =>
        Effect.succeed([response, Stream.fromIterable(events)]),
      createEmbedding: () =>
        Effect.die(new Error("unexpected createEmbedding call")),
    })
  );
};
...
const makeTestLayer = (
  ...
  OpenAiClient.layer({ apiKey: Redacted.make("sk-test-key") }).pipe(
    ...
    Layer.provide(
      Layer.succeed(MockOpenAiResponse, {
...
const decodeToolParamsFromStream = (
  ...
  const start = parts.find(
    (part) => part.type === "tool-params-start" && part.id === toolCallId
  );
  const end = parts.find(
    (part) => part.type === "tool-params-end" && part.id === toolCallId
  );
  ...
  const deltas = parts
    .filter(
      (part) => part.type === "tool-params-delta" && part.id === toolCallId
    )
    .map((part) => part.delta)
    .join("");
...
const makeMcpApprovalRequest = (
  name: string,
  args: Record<string, unknown>,
  overrides: Partial<Generated.MCPApprovalRequest> & {
