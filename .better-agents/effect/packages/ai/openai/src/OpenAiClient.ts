...
export interface Service {
  ...
  readonly createResponse: (
    ...
    readonly [
      body: typeof OpenAiSchema.Response.Type,
  ...
  readonly createResponseStream: (
    ...
    readonly [
      ...
      stream: Stream.Stream<
        typeof OpenAiSchema.ResponseStreamEvent.Type,
        AiError.AiError
      >,
    ],
    AiError.AiError
  ...
  readonly createEmbedding: (
    ...
    typeof OpenAiSchema.CreateEmbeddingResponse.Type,
    AiError.AiError
  >;
}
...
export const make = Effect.fnUntraced(
  function* (
    ...
    const decodeResponse = HttpClientResponse.schemaBodyJson(
      OpenAiSchema.Response
    );
          ...
          .pipe(
            ...
            Effect.catchTags({
              HttpClientError: (error) =>
                Errors.mapHttpClientError(error, "createResponse"),
              SchemaError: (error) =>
                Effect.fail(Errors.mapSchemaError(error, "createResponse")),
            })
          )
      );
    ...
    const buildResponseStream = (
      ...
      const stream = response.stream.pipe(
        ...
        Stream.pipeThroughChannel(
          Sse.decodeDataSchema(OpenAiSchema.ResponseStreamEvent)
        ),
        Stream.takeUntil(
          (event) =>
    ...
    const decodeEmbedding = HttpClientResponse.schemaBodyJson(
      ...
      Effect.flatMap(resolveHttpClient, (client) =>
        client
          ...
          .pipe(
            ...
            Effect.catchTags({
              HttpClientError: (error) =>
                Errors.mapHttpClientError(error, "createEmbedding"),
              SchemaError: (error) =>
                Effect.fail(Errors.mapSchemaError(error, "createEmbedding")),
            })
          )
      );
 ...
 * @since 4.0.0
 */
export class OpenAiSocket extends Context.Service<
  OpenAiSocket,
  {
    /**
     * Create a streaming response using the OpenAI responses endpoint.
...
const makeSocket = Effect.gen(function* () {
  ...
  const makeRequest = Effect.flatMap(OpenAiConfig.getOrUndefined, (config) => {
    const httpClient = Predicate.isNotUndefined(config?.transformClient)
      ...
      : client.client;
    return Effect.orDie(
      httpClient.preprocess(HttpClientRequest.post("/responses"))
    );
  });
    ...
    acquire: Effect.gen(function* () {
      ...
      yield* socket
        .runRaw((msg) => {
          ...
          try {
            ...
            if (event.type === "error" && "status" in event) {
              ...
              const error =
                "error" in event
                  ? (event.error as typeof ErrorEvent.Type.error)
                  : event;
              ...
              return Effect.fail(
                AiError.make({
                  ...
                  reason: AiError.reasonFromHttpStatus({
                    description: json,
                    status: isNaN(status)
                      ? (errorTypeToStatus[error.type] ?? 500)
                      : status,
        ...
        .pipe(
          ...
          Effect.ensuring(
            Effect.forkIn(RcRef.invalidate(queueRef), socketScope, {
...
const ErrorEvent = Schema.Struct({
  ...
  status: Schema.Number.pipe(Schema.withDecodingDefault(Effect.succeed(500))),
  error: Schema.Struct({
    type: Schema.String,
...
export const withWebSocketMode = <A, E, R>(
  ...
  A,
  E,
  | Exclude<R, OpenAiSocket | ResponseIdTracker.ResponseIdTracker>
  | OpenAiClient
  | Socket.WebSocketConstructor
> =>
  Effect.scopedWith((scope) =>
    Effect.flatMap(Scope.provide(makeSocket, scope), (services) =>
