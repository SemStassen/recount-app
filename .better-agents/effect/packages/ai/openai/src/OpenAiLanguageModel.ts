 ...
 */
export type Model =
  | typeof ResponseModelIds.Encoded
  | typeof SharedModelIds.Encoded;
  ...
  export interface TextPartOptions extends ProviderOptions {
    ...
    readonly openai?: {
      ...
      readonly annotations?: ReadonlyArray<
        typeof OpenAiSchema.Annotation.Encoded
      > | null;
    } | null;
  }
}
...
const prepareMessages = Effect.fnUntraced(function* <
  Tools extends ReadonlyArray<Tool.Any>,
>({
  config,
  options,
  ...
  ReadonlyArray<typeof OpenAiSchema.InputItem.Encoded>,
  AiError.AiError
> {
  ...
  const applyPatchTool = options.tools.find(
    (tool): tool is ReturnType<typeof OpenAiTool.ApplyPatch> =>
      Tool.isProviderDefined(tool) && tool.name === "OpenAiApplyPatch"
  );
  const codeInterpreterTool = options.tools.find(
    (tool): tool is ReturnType<typeof OpenAiTool.CodeInterpreter> =>
      Tool.isProviderDefined(tool) && tool.name === "OpenAiCodeInterpreter"
  );
  const shellTool = options.tools.find(
    (tool): tool is ReturnType<typeof OpenAiTool.Shell> =>
      Tool.isProviderDefined(tool) && tool.name === "OpenAiFunctionShell"
  );
  const localShellTool = options.tools.find(
    (tool): tool is ReturnType<typeof OpenAiTool.LocalShell> =>
      Tool.isProviderDefined(tool) && tool.name === "OpenAiLocalShell"
  );
  const webSearchTool = options.tools.find(
    (tool): tool is ReturnType<typeof OpenAiTool.WebSearch> =>
      Tool.isProviderDefined(tool) && tool.name === "OpenAiWebSearch"
  );
  const webSearchPreviewTool = options.tools.find(
    (tool): tool is ReturnType<typeof OpenAiTool.WebSearchPreview> =>
    ...
    switch (message.role) {
      ...
      case "assistant": {
        const reasoningMessages: Record<
          string,
          DeepMutable<typeof OpenAiSchema.ReasoningItem.Encoded>
        > = {};
        ...
        for (const part of message.content) {
          switch (part.type) {
              ...
              if (Predicate.isNotNull(id)) {
                ...
                if (config.store === true) {
                  ...
                  const summaryParts: Array<
                    typeof OpenAiSchema.SummaryTextContent.Encoded
                  > = [];
            ...
            case "tool-call": {
              ...
              if (
                Predicate.isNotUndefined(localShellTool) &&
                toolName === "local_shell"
              ) {
                const params = yield* Schema.decodeUnknownEffect(
                  localShellTool.parametersSchema
                )(part.params).pipe(
      ...
      case "tool": {
        for (const part of message.content) {
          ...
          if (
            Predicate.isNotUndefined(applyPatchTool) &&
  ...
  for (const part of rawResponse.output) {
    switch (part.type) {
      ...
      case "function_call": {
        ...
        const params = yield* transformToolCallParams(
          options.tools,
          part.name,
          toolParams
        );
...
const makeStreamResponse = Effect.fnUntraced(function* <
  Tools extends ReadonlyArray<Tool.Any>,
>({
  stream,
  response,
...
> {
  ...
  const getOrCreateReasoningPart = (
    ...
    const reasoningPart: ReasoningPart = {
      encryptedContent: Predicate.isNotNullish(encryptedContent)
        ? encryptedContent
        : undefined,
      summaryParts: {},
    };
  ...
  const webSearchTool = options.tools.find(
    ...
    | ReturnType<typeof OpenAiTool.WebSearch>
    | ReturnType<typeof OpenAiTool.WebSearchPreview>
    | undefined;
        ...
        switch (event.type) {
          ...
          case "response.output_item.added": {
            switch (event.item.type) {
              case "apply_patch_call": {
                ...
                if (operation.type === "delete_file") {
                  ...
                  parts.push({
                    ...
                    id: toolId,
                    delta:
                      `{"call_id":"${InternalUtilities.escapeJSONDelta(toolId)}",` +
              ...
              case "code_interpreter_call": {
                const toolName =
                  toolNameMapper.getCustomName("code_interpreter");
                activeToolCalls[event.output_index] = {
                  id: event.item.id,
                  name: toolName,
              ...
              case "image_generation_call": {
                const toolName =
                  toolNameMapper.getCustomName("image_generation");
                parts.push({
                  type: "tool-call",
                  id: event.item.id,
                  name: toolName,
                    ...
                    metadata: {
                      openai: {
                        ...
                        ...makeEncryptedContentMetadata(
                          reasoningPart.encryptedContent
                        ),
                      },
                    },
                  });
          ...
          case "response.output_item.done": {
            switch (event.item.type) {
              case "apply_patch_call": {
                ...
                if (
                  ...
                  if (!toolCall.applyPatch.hasDiff) {
                    parts.push({
                      type: "tool-params-delta",
                      id: toolCall.id,
                      delta: InternalUtilities.escapeJSONDelta(
                        event.item.operation.diff ?? ""
                      ),
                    });
                  }
                  parts.push({
                ...
                if (
                  Predicate.isNotUndefined(toolCall) &&
              ...
              case "function_call": {
                const toolCall = activeToolCalls[event.output_index];
                if (
                  Predicate.isNotUndefined(toolCall?.functionCall?.emitted) &&
              ...
              case "mcp_call": {
                ...
                const toolId = Predicate.isNotNullish(approvalRequestId)
                  ? (streamApprovalRequests.get(approvalRequestId) ??
                    approvalRequests.get(approvalRequestId) ??
                    event.item.id)
                  : event.item.id;
              ...
              case "mcp_approval_request": {
                ...
                const approvalRequestId =
                  (event.item as any).approval_request_id ?? event.item.id;
              ...
              case "reasoning": {
                ...
                for (const [summaryIndex, status] of Object.entries(
                  reasoningPart.summaryParts
                )) {
          ...
          case "response.output_text.annotation.added": {
            const annotation =
              event.annotation as typeof OpenAiSchema.Annotation.Encoded;
          ...
          case "response.apply_patch_call_operation_diff.done": {
            ...
            if (
              ...
              if (
                !toolCall.applyPatch.hasDiff &&
                Predicate.isNotUndefined(event.delta)
              ) {
                parts.push({
                  type: "tool-params-delta",
                  id: toolCall.id,
          ...
          case "response.code_interpreter_call_code.done": {
            ...
            if (
              Predicate.isNotUndefined(toolCall) &&
              Predicate.isNotUndefined(toolCall.codeInterpreter)
            ) {
              const toolName = toolNameMapper.getCustomName("code_interpreter");
              parts.push({
                type: "tool-params-delta",
          ...
          case "response.reasoning_summary_part.added": {
            ...
            if (event.summary_index > 0) {
              ...
              for (const [summaryIndex, status] of Object.entries(
                ...
                if (status === "can-conclude") {
                  ...
                  reasoningPart.summaryParts[Number(summaryIndex)] =
                    "concluded";
                }
              }
            }
            ...
            if (
              Predicate.isUndefined(
                reasoningPart.summaryParts[event.summary_index]
              )
            ) {
...
const annotateResponse = (
  ...
  const finishReason = response.incomplete_details?.reason as
    | string
    | undefined;
  addGenAIAnnotations(span, {
    response: {
      ...
      finishReasons: Predicate.isNotUndefined(finishReason)
        ? [finishReason]
        : undefined,
    },
    usage: {
...
const annotateStreamResponse = (
  ...
  if (part.type === "finish") {
    const serviceTier = (part.metadata as any)?.openai?.serviceTier as
      | string
      | undefined;
    addGenAIAnnotations(span, {
      response: {
...
type OpenAiToolChoice =
  (typeof OpenAiSchema.CreateResponse.Encoded)["tool_choice"];

const prepareTools = Effect.fnUntraced(function* <
  Tools extends ReadonlyArray<Tool.Any>,
>({
  config,
  options,
...
> {
  ...
  for (const tool of allowedTools) {
    if (Tool.isUserDefined(tool) || Tool.isDynamic(tool)) {
      const strict =
        Tool.getStrictMode(tool) ?? config.strictJsonSchema ?? true;
    ...
    if (Tool.isProviderDefined(tool)) {
      ...
      switch (openAiTool.name) {
        ...
        case "OpenAiCodeInterpreter": {
          const args = yield* Schema.decodeUnknownEffect(openAiTool.argsSchema)(
            tool.args
          ).pipe(
            Effect.mapError((error) =>
              AiError.make({
...
const isFileId = (data: string, config: typeof Config.Service): boolean =>
  config.fileIdPrefixes != null &&
  config.fileIdPrefixes.some((prefix) => data.startsWith(prefix));
...
): typeof OpenAiSchema.MessageStatus.Encoded | null =>
  part.options.openai?.status ?? null;
const getEncryptedContent = (part: Prompt.ReasoningPart): string | null =>
  part.options.openai?.encryptedContent ?? null;

const getImageDetail = (part: Prompt.FilePart): ImageDetail =>
  part.options.openai?.imageDetail ?? "auto";

const makeItemIdMetadata = (itemId: string | undefined) =>
...
const tryJsonSchema = <S extends Schema.Top>(schema: S, method: string) =>
  Effect.try({
    try: () =>
      Tool.getJsonSchemaFromSchema(schema, { transformer: toCodecOpenAI }),
    catch: (error) => unsupportedSchemaError(error, method),
  });
...
const prepareResponseFormat = Effect.fnUntraced(function* ({
  ...
  typeof OpenAiSchema.TextResponseFormatConfiguration.Encoded,
  AiError.AiError
> {
  if (options.responseFormat.type === "json") {
    ...
    return {
      ...
      name,
      description:
        AST.resolveDescription(schema.ast) ?? "Response with a JSON object",
      schema: jsonSchema,
...
const getApprovalRequestIdMapping = (
...
const getUsage = (
  usage: OpenAiSchema.ResponseUsage | null | undefined
): Response.Usage => {
  ...
  const cachedTokens = getUsageTokenDetail(
    usage.input_tokens_details,
    "cached_tokens"
  );
  const reasoningTokens = getUsageTokenDetail(
...
const getUsageTokenDetail = (details: unknown, key: string): number =>
  Predicate.hasProperty(details, key) && typeof details[key] === "number"
    ? details[key]
    : 0;

const transformToolCallParams = Effect.fnUntraced(function* <
  Tools extends ReadonlyArray<Tool.Any>,
>(
  tools: Tools,
  toolName: string,
  ...
  const { codec } = yield* tryCodecTransform(
