 ...
 * @since 4.0.0
 */
export type ReasoningDetails = Exclude<
  (typeof Generated.AssistantMessage.Encoded)["reasoning_details"],
  undefined
>;
...
export type FileAnnotation = Extract<
  NonNullable<
    typeof Generated.AssistantMessage.fields.annotations.Type
  >[number],
  { type: "file" }
>;
  ...
  export interface SystemMessageOptions extends ProviderOptions {
    ...
    readonly openrouter?: {
      ...
      readonly cacheControl?:
        | typeof Generated.ChatMessageContentItemCacheControl.Encoded
        | null;
    } | null;
  }
...
export const make = Effect.fnUntraced(function* ({
  ...
  > {
    ...
    const { tools, toolChoice } = yield* prepareTools({
      options,
    ...
    streamText: Effect.fnUntraced(
      function* (options) {
        ...
        const [response, stream] =
          yield* client.createChatCompletionStream(request);
    ...
    switch (message.role) {
      ...
      case "assistant": {
        ...
        const messageReasoningDetails =
          message.options.openrouter?.reasoningDetails;
        ...
        const candidateReasoningDetails: ReasoningDetails | null =
          Predicate.isNotNullish(messageReasoningDetails) &&
        ...
        if (
          Predicate.isNotNull(candidateReasoningDetails) &&
...
> {
  ...
  if (Predicate.isNotNullish(reasoningDetails) && reasoningDetails.length > 0) {
    ...
    Predicate.isNotNullish(message.reasoning) &&
  ...
  if (Predicate.isNotNullish(toolCalls) && toolCalls.length > 0) {
    ...
    for (let index = 0; index < toolCalls.length; index++) {
      ...
      parts.push({
        ...
        ...(index === 0 &&
        Predicate.isNotNullish(reasoningDetails) &&
  ...
  parts.push({
    ...
    metadata: {
      openrouter: {
        ...
        ...(Predicate.isNotUndefined(fileAnnotations) &&
        fileAnnotations.length > 0
          ? { annotations: fileAnnotations }
          : undefined),
        ...(Predicate.hasProperty(rawResponse, "provider") &&
        Predicate.isString(rawResponse.provider)
      ...
      Effect.fnUntraced(function* (event) {
        ...
        if (Predicate.isNotUndefined(choice)) {
          ...
          const emitReasoning = Effect.fnUntraced(function* (
            delta: string,
            metadata?: Response.ReasoningDeltaPart["metadata"] | undefined
          ) {
            if (!reasoningStarted) {
              activeReasoningId =
                openRouterResponseId ?? (yield* idGenerator.generateId());
              parts.push({
          ...
          if (
            Predicate.isNotUndefined(reasoningDetails) &&
            reasoningDetails.length > 0
          ) {
            ...
            for (const detail of reasoningDetails) {
              if (detail.type === "reasoning.text") {
                ...
                if (
                  Predicate.isNotUndefined(lastDetail) &&
                  ...
                  lastDetail.signature =
                    lastDetail.signature ?? detail.signature ?? null;
                  lastDetail.format =
                    lastDetail.format ?? detail.format ?? null;
                } else {
          ...
          if (Predicate.isNotNullish(toolCalls)) {
            for (const toolCall of toolCalls) {
              ...
              if (Predicate.isUndefined(activeToolCall)) {
                if (toolCall.type !== "function") {
                  return yield* AiError.make({
                    ...
                    reason: new AiError.InvalidOutputError({
                      description:
                        "Received tool call delta that was not of type: 'function'",
                    }),
                  });
                }
                ...
                if (Predicate.isNullish(toolCall.id)) {
                  return yield* AiError.make({
                    ...
                    reason: new AiError.InvalidOutputError({
                      description:
                        "Received tool call delta without a tool call identifier",
                    }),
                  });
                }
                ...
                if (Predicate.isNullish(toolCall.function?.name)) {
                  return yield* AiError.make({
                    ...
                    reason: new AiError.InvalidOutputError({
                      description:
                        "Received tool call delta without a tool call name",
                    }),
                  });
                }
              ...
              try {
                const params = Tool.unsafeSecureJsonParse(
                  activeToolCall.params
                );
        ...
        if (Predicate.isNotUndefined(event.usage)) {
          ...
          const hasEncryptedReasoning = accumulatedReasoningDetails.some(
            (detail) =>
              detail.type === "reasoning.encrypted" && detail.data.length > 0
          );
          if (
            totalToolCalls > 0 &&
          ...
          const metadata: Response.FinishPart["metadata"] = {
            openrouter: {
              ...
              ...(Predicate.isNotUndefined(event.usage)
                ? { usage: event.usage }
                : undefined),
              ...(Predicate.hasProperty(event, "provider") &&
              Predicate.isString(event.provider)
...
const prepareTools = Effect.fnUntraced(function* ({
  ...
  {
    readonly tools:
      | ReadonlyArray<typeof Generated.ToolDefinitionJson.Encoded>
      | undefined;
...
> {
  ...
  const hasProviderDefinedTools = options.tools.some((tool) =>
  ...
  if (hasProviderDefinedTools) {
    return yield* AiError.make({
      ...
      reason: new AiError.InvalidUserInputError({
        description:
          "Provider-defined tools are unsupported by the OpenRouter " +
  ...
  let toolChoice: typeof Generated.ToolChoiceOption.Encoded | undefined =
    undefined;

  for (const tool of options.tools) {
    const description = Tool.getDescription(tool);
    const parameters = yield* tryJsonSchema(
...
): typeof Generated.ChatMessageContentItemCacheControl.Encoded | null =>
  part.options.openrouter?.cacheControl ?? null;

const findFirstReasoningDetails = (
  ...
  for (const part of content) {
    ...
    if (part.type === "tool-call") {
      ...
      if (
        Predicate.isNotNullish(details) &&
...
const getResponseFormat = Effect.fnUntraced(function* ({
  config,
  options,
  transformer,
}: {
  ...
  typeof Generated.ResponseFormatJSONSchema.Encoded | undefined,
  AiError.AiError
> {
  if (options.responseFormat.type === "json") {
    const description = SchemaAST.resolveDescription(
      options.responseFormat.schema.ast
    );
    const jsonSchema = yield* tryJsonSchema(
...
const getUsage = (
  usage: Generated.ChatGenerationTokenUsage | undefined
): Response.Usage => {
