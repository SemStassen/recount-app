...
export type ChatStreamingResponseChunkData =
  typeof Generated.ChatStreamingResponseChunk.fields.data.Type;
...
export const make = Effect.fnUntraced(function* (
    ...
    HttpClient.mapRequest((request) =>
      request.pipe(
        ...
        options.siteReferrer
          ...
          : identity,
        options.siteTitle
          ? HttpClientRequest.setHeader("X-Title", options.siteTitle)
          : identity,
  ...
  const createChatCompletion: Service["createChatCompletion"] = (payload) =>
    client
      .sendChatCompletionRequest({ payload, config: { includeResponse: true } })
      .pipe(
        Effect.catchTags({
          ...
          HttpClientError: (error) =>
            Errors.mapHttpClientError(error, "createChatCompletion"),
          SchemaError: (error) =>
            Effect.fail(Errors.mapSchemaError(error, "createChatCompletion")),
        })
      );
  ...
  const buildChatCompletionStream = (
    ...
    const stream = response.stream.pipe(
      ...
      Stream.catchTags({
        ...
        SchemaError: (error) =>
          Stream.fail(
            Errors.mapSchemaError(error, "createChatCompletionStream")
          ),
      })
    ) as any;
  ...
  const createChatCompletionStream: Service["createChatCompletionStream"] = (
    payload
  ) =>
    httpClientOk
      .execute(
...
const ChatStreamingResponseChunkDataFromString = Schema.fromJsonString(
  Generated.ChatStreamingResponseChunk.fields.data
);
...
const decodeChatCompletionSseData = (
  ...
  ChatStreamingResponseChunkData | "[DONE]",
  Schema.SchemaError
> =>
  data === "[DONE]"
    ? Effect.succeed(data)
