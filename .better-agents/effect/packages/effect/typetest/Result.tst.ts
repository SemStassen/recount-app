...
describe("Result", () => {
  it("flip", () => {
    expect(Result.flip(number$string)).type.toBe<
      Result.Result<string, number>
    >();
    expect(pipe(number$string, Result.flip)).type.toBe<
      Result.Result<string, number>
    >();
  });
  ...
  it("try", () => {
    ...
    expect(Result.try({ try: () => 1, catch: () => new Error() })).type.toBe<
  ...
  describe("all", () => {
    it("tuple", () => {
      ...
      expect(Result.all([number$string])).type.toBe<
        Result.Result<[number], string>
      >();
      expect(Result.all([number$string, boolean$string])).type.toBe<
        Result.Result<[number, boolean], string>
      >();
      expect(Result.all([number$string, boolean$Error])).type.toBe<
        Result.Result<[number, boolean], string | Error>
      >();
      expect(
        pipe([number$string, boolean$string] as const, Result.all)
      ).type.toBe<Result.Result<[number, boolean], string>>();
      expect(
        pipe([number$string, boolean$Error] as const, Result.all)
      ).type.toBe<Result.Result<[number, boolean], string | Error>>();
    });

    it("struct", () => {
      expect(Result.all({})).type.toBe<Result.Result<{}, never>>();
      expect(Result.all({ a: number$string })).type.toBe<
        Result.Result<{ a: number }, string>
      >();
      ...
      >();
      expect(
        pipe({ a: number$string, b: boolean$string }, Result.all)
      ).type.toBe<Result.Result<{ a: number; b: boolean }, string>>();
      expect(
        pipe({ a: number$string, b: boolean$Error }, Result.all)
      ).type.toBe<Result.Result<{ a: number; b: boolean }, string | Error>>();
    });

    it("array", () => {
      const resultArray = hole<Array<Result.Result<number, string>>>();
      expect(Result.all(resultArray)).type.toBe<
        Result.Result<Array<number>, string>
      >();
      expect(pipe(resultArray, Result.all)).type.toBe<
        Result.Result<Array<number>, string>
      >();
    });
    ...
    it("record", () => {
      const resultRecord =
        hole<Record<string, Result.Result<number, string>>>();
      expect(Result.all(resultRecord)).type.toBe<
        Result.Result<{ [x: string]: number }, string>
      >();
      expect(pipe(resultRecord, Result.all)).type.toBe<
        Result.Result<{ [x: string]: number }, string>
      >();
    });
  });
  ...
  it("andThen", () => {
    expect(Result.andThen(string$string, number$string)).type.toBe<
      Result.Result<number, string>
    >();
    expect(string$string.pipe(Result.andThen(number$string))).type.toBe<
      Result.Result<number, string>
    >();
  ...
  it("liftPredicate", () => {
    ...
    const predicateNumberOrString =
      hole<Predicate.Predicate<number | string>>();
    const refinementNumberOrStringToNumber =
      hole<Predicate.Refinement<number | string, number>>();
  ...
  it("fromNullishOr", () => {
    ...
    expect(Result.fromNullishOr(nullableString, () => new Error())).type.toBe<
    ...
    expect(Result.fromNullishOr(nullableObject, () => new Error())).type.toBe<
  ...
  it("filterOrFail", () => {
    ...
    const readonlyArrayOfStrings$Error =
      hole<Result.Result<ReadonlyArray<string>, Error>>();
    expect(
      Result.filterOrFail(
      ...
      Result.Result<readonly [string, ...Array<string>], "b" | Error>
    >();
    expect(
      pipe(
        readonlyArrayOfStrings$Error,
...
it("transposeMapOption", () => {
  expect(
    Result.transposeMapOption(Option.none(), (value) => {
      expect(value).type.toBe<never>();
      return string$string;
    })
  ).type.toBe<Result.Result<Option.Option<string>, string>>();
  expect(
    pipe(
      Option.none(),
  ...
  expect(
    Result.transposeMapOption(Option.some(42), (value) => {
