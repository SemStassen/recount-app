...
describe("Tuple", () => {
  ...
  it("appendElement", () => {
    expect(pipe(tuple, Tuple.appendElement("b"))).type.toBe<
      [string, number, boolean, "b"]
    >();
    expect(Tuple.appendElement(tuple, "b")).type.toBe<
      [string, number, boolean, "b"]
    >();
  });

  it("appendElements", () => {
    expect(pipe(tuple, Tuple.appendElements(["b", 2]))).type.toBe<
      [string, number, boolean, "b", 2]
    >();
    expect(Tuple.appendElements(tuple, ["b", 2])).type.toBe<
      [string, number, boolean, "b", 2]
    >();
  });

  it("evolve", () => {
    expect(pipe(tuple, Tuple.evolve([(s) => s.length]))).type.toBe<
      [number, number, boolean]
    >();
    expect(Tuple.evolve(tuple, [(s) => s.length] as const)).type.toBe<
      [number, number, boolean]
    >();

    expect(
      pipe(tuple, Tuple.evolve([(s) => s.length, undefined, (b) => `b: ${b}`]))
  ...
  describe("renameIndices", () => {
    ...
    it("partial index mapping", () => {
      expect(pipe(tuple, Tuple.renameIndices(["1", "0"]))).type.toBe<
        [number, string, boolean]
      >();
      expect(Tuple.renameIndices(tuple, ["1", "0"])).type.toBe<
        [number, string, boolean]
      >();
    });

    it("full index mapping", () => {
      expect(pipe(tuple, Tuple.renameIndices(["2", "1", "0"]))).type.toBe<
        [boolean, number, string]
      >();
      expect(Tuple.renameIndices(tuple, ["2", "1", "0"])).type.toBe<
        [boolean, number, string]
      >();
    });
  });
  ...
  it("makeReducer", () => {
    expect(Tuple.makeReducer([Number.ReducerSum, Str.ReducerConcat])).type.toBe<
      Reducer.Reducer<[number, string]>
    >();
    expect(
      Tuple.makeReducer<readonly [number, string]>([
        Number.ReducerSum,
        Str.ReducerConcat,
      ])
