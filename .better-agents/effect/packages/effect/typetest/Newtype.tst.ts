...
describe("Newtype", () => {
  ...
  it("two newtypes with different keys are not assignable", () => {
    ...
    expect(acceptUserId).type.not.toBeCallableWith(
      Newtype.makeIso<OrderId>().set(1)
    );
  ...
  it("makeCombiner", () => {
    ...
    expect(
      combiner.combine(userIdIso.set(10), userIdIso.set(20))
    ).type.toBe<UserId>();
  });
  ...
  it("makeReducer", () => {
    ...
    expect(
      reducer.combineAll([userIdIso.set(10), userIdIso.set(20)])
    ).type.toBe<UserId>();
  });
});
