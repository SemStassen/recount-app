...
class Metrics extends Context.Service<
  Metrics,
  {
    readonly value: string;
  }
...
describe("RpcGroup", () => {
  it("toLayer includes handler dependencies in layer services", () => {
    ...
    type HasExpectedServices = [Layer.Services<typeof layer>] extends [Db]
      ? [Db] extends [Layer.Services<typeof layer>]
        ? true
        : false
      : false;
  ...
  it("toLayerHandler includes handler dependencies in layer services", () => {
    const layer = Group.toLayerHandler("GetUser", () =>
      Metrics.use((metrics) => Effect.succeed(metrics.value))
    );

    type HasExpectedServices = [Layer.Services<typeof layer>] extends [Metrics]
      ? [Metrics] extends [Layer.Services<typeof layer>]
        ? true
        : false
      : false;
