...
class DbConnection extends Context.Service<DbConnection, { url: string }>()(
  "DbConnection"
) {}
...
class DbMiddleware extends RpcMiddleware.Service<
  DbMiddleware,
  {
...
const AuthedStream = Rpc.make("AuthedStream", {
  success: Schema.String,
  stream: true,
}).middleware(AuthMiddleware);
const AuthedGetUser = Rpc.make("AuthedGetUser", {
  success: Schema.String,
}).middleware(AuthMiddleware);
const MultiMiddleware = Rpc.make("MultiMiddleware", {
  success: Schema.String,
  stream: true,
})
  .middleware(AuthMiddleware)
...
describe("Rpc", () => {
  ...
  describe("ExtractProvides", () => {
    it("extracts provided service from stream RPC with middleware", () => {
      expect<
        Rpc.ExtractProvides<typeof AuthedStream, "AuthedStream">
      >().type.toBe<CurrentUser>();
    });

    it("extracts provided service from non-stream RPC with middleware", () => {
      expect<
        Rpc.ExtractProvides<typeof AuthedGetUser, "AuthedGetUser">
      >().type.toBe<CurrentUser>();
    });

    it("returns never for RPC without middleware", () => {
      expect<
        Rpc.ExtractProvides<typeof GetUser, "GetUser">
      >().type.toBe<never>();
    });

    it("extracts union of provides from multiple middlewares", () => {
      expect<
        Rpc.ExtractProvides<typeof MultiMiddleware, "MultiMiddleware">
      >().type.toBe<CurrentUser | DbConnection>();
    });

    it("works in a mixed group", () => {
      expect<
        Rpc.ExtractProvides<MixedWithMiddleware, "AuthedStream">
      >().type.toBe<CurrentUser>();
    });

    it("returns never for non-middleware RPC in mixed group", () => {
      expect<
        Rpc.ExtractProvides<MixedWithMiddleware, "GetUser">
      >().type.toBe<never>();
    });
  });
});
