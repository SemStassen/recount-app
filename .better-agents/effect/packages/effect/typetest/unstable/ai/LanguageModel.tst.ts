...
describe("LanguageModel", () => {
  describe("generateText", () => {
    it("returns an empty tool record when no toolkit is provided", () => {
      ...
      type ProgramSuccess =
        typeof program extends Effect.Effect<infer A, any, any> ? A : never;

      expect<ProgramSuccess>().type.toBe<
        LanguageModel.GenerateTextResponse<{}>
      >();
      expect<
        IsExact<ProgramSuccess, LanguageModel.GenerateTextResponse<{}>>
      >().type.toBe<true>();
    });
    ...
    it("tool handlers do not leak AiErrorReason into the error channel", () => {
      ...
      type ProgramError =
        typeof program extends Effect.Effect<any, infer E, any> ? E : never;

      expect<ProgramError>().type.toBe<
        AiError.AiError | { readonly message: string }
      >();
    ...
    it("includes tool request dependencies when a toolkit is provided", () => {
      ...
      type ProgramRequirements =
        typeof program extends Effect.Effect<any, any, infer R> ? R : never;
    ...
    it("includes tool request dependencies for resolved toolkits with handlers", () => {
      ...
      expect<ProgramRequirements>().type.toBe<
        LanguageModel.LanguageModel | RequestContext
      >();
    });
    ...
    it("extracts services and tools from toolkit unions", () => {
      ...
      expect<
        LanguageModel.ExtractServices<{ readonly toolkit: ToolkitUnion }>
      >().type.toBe<RequestContext | ToolkitUnionHandlers>();
      expect<
        LanguageModel.ExtractTools<{ readonly toolkit: ToolkitUnion }>
      >().type.toBe<ToolkitUnionTools>();
    });

    it("preserves generic toolkit tool records", () => {
      const helper = <Tools extends Record<string, Tool.Any>>(
  ...
  describe("streamText", () => {
    it("returns an empty tool record when no toolkit is provided", () => {
      ...
      type StreamPart =
        typeof stream extends Stream.Stream<infer A, any, any> ? A : never;
    ...
    it("preserves generic toolkit tool records", () => {
      ...
      type StreamPart =
        ...
        > extends Stream.Stream<infer A, any, any>
          ? A
          : never;
