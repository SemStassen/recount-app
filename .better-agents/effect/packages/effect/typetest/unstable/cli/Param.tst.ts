...
describe("Param", () => {
  it("accepts effectful fallback prompts for flags and arguments", () => {
    ...
    const argument = Argument.string("name").pipe(
      Argument.withFallbackPrompt(prompt)
    );
    const integerFlag = Flag.string("count").pipe(
      Flag.withFallbackPrompt(integerPrompt)
    );
    const integerArgument = Argument.string("count").pipe(
      Argument.withFallbackPrompt(integerPrompt)
    );
