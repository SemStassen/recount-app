...
describe("HttpApiClient", () => {
  ...
  describe("urlBuilder", () => {
    it("should mirror client shape and use schema input types", () => {
      ...
      expect(builder.users.getUser).type.not.toBeCallableWith({
        params: { id: "123" },
        query: { page: 1 },
      });
      expect(builder.users.getUser).type.not.toBeCallableWith({
    ...
    it("should support prefixes and top-level endpoints", () => {
      const Api = HttpApi.make("Api")
        ...
        .add(
          HttpApiGroup.make("top", { topLevel: true }).add(
  ...
  describe("payload option", () => {
    it("should default to void", () => {
      const Api = HttpApi.make("Api").add(
        HttpApiGroup.make("group").add(HttpApiEndpoint.get("a", "/a"))
      );
      const client = Effect.runSync(
        HttpApiClient.make(Api).pipe(Effect.provide(FetchHttpClient.layer))
      );
  ...
  describe("client middleware", () => {
    it("requiredForClient requires layer and includes required client errors", () => {
      class RequiredClientError extends Schema.ErrorClass<RequiredClientError>(
        "RequiredClientError"
      )({
        _tag: Schema.tag("RequiredClientError"),
      }) {}

      class OptionalClientError extends Schema.ErrorClass<OptionalClientError>(
        "OptionalClientError"
      )({
        _tag: Schema.tag("OptionalClientError"),
      }) {}

      class RequiredMiddleware extends HttpApiMiddleware.Service<
        RequiredMiddleware,
        {
      ...
      class OptionalMiddleware extends HttpApiMiddleware.Service<
        OptionalMiddleware,
        {
    ...
    it("requiredForClient is enforced for makeWith, group, and endpoint", () => {
      ...
      expect(Effect.runSync).type.not.toBeCallableWith(
        HttpApiClient.makeWith(Api, { httpClient: TestHttpClient })
      );
      expect(Effect.runSync).type.not.toBeCallableWith(
      ...
      const withGroup = Effect.runSync(
        HttpApiClient.group(Api, {
          group: "group",
          httpClient: TestHttpClient,
        }).pipe(Effect.provide(middlewareLayer))
      );
