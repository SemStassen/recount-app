...
describe("Predicate", () => {
  ...
  it("isUndefined", () => {
    expect(unknowns.filter(Predicate.isUndefined)).type.toBe<
      Array<undefined>
    >();
  });

  it("isNotUndefined", () => {
    expect(numberOrUndefined.filter(Predicate.isNotUndefined)).type.toBe<
      Array<number>
    >();
    expect(numberOrNullOrUndefined.filter(Predicate.isNotUndefined)).type.toBe<
      Array<number | null>
    >();
  });
  ...
  it("isNotNull", () => {
    ...
    expect(numberOrNullOrUndefined.filter(Predicate.isNotNull)).type.toBe<
  ...
  it("isTagged", () => {
    expect(anys.filter(Predicate.isTagged("a"))).type.toBe<
      Array<{ _tag: "a" }>
    >();
  });
  ...
  it("isNullish", () => {
    ...
    expect(numberOrUndefined.filter(Predicate.isNullish)).type.toBe<
      Array<undefined>
    >();
    expect(numberOrNullOrUndefined.filter(Predicate.isNullish)).type.toBe<
  ...
  it("isNotNullish", () => {
    expect(numberOrNull.filter(Predicate.isNotNullish)).type.toBe<
      Array<number>
    >();
    expect(numberOrUndefined.filter(Predicate.isNotNullish)).type.toBe<
      Array<number>
    >();
    expect(numberOrNullOrUndefined.filter(Predicate.isNotNullish)).type.toBe<
      Array<number>
    >();
  ...
  it("isObject", () => {
    expect(unknowns.filter(Predicate.isObject)).type.toBe<
      Array<{ [x: PropertyKey]: unknown }>
    >();
  });

  it("isReadonlyObject", () => {
    expect(unknowns.filter(Predicate.isReadonlyObject)).type.toBe<
      Array<{ readonly [x: PropertyKey]: unknown }>
    >();
  });
  ...
  it("isTupleOfAtLeast", () => {
    if (Predicate.isTupleOfAtLeast(unknowns, 3)) {
      expect(unknowns).type.toBe<
        [unknown, unknown, unknown, ...Array<unknown>]
      >();
    }
  });
  ...
  it("compose", () => {
    ...
    const isNonEmptyString =
      hole<Predicate.Refinement<string, NonEmptyString>>();
  ...
  it("and", () => {
    ...
    expect(pipe(isPositive, Predicate.and(isLessThan2))).type.toBe<
      Predicate.Predicate<number>
    >();

    expect(Predicate.and(isPositive, isLessThan2)).type.toBe<
      Predicate.Predicate<number>
    >();

    expect(pipe(Predicate.isNumber, Predicate.and(isPositive))).type.toBe<
      Predicate.Predicate<number>
    >();

    expect(Predicate.and(Predicate.isNumber, isPositive)).type.toBe<
      Predicate.Predicate<number>
    >();
    ...
    expect(pipe(hasa, Predicate.and(hasb))).type.toBe<
      Predicate.Refinement<unknown, { a: unknown } & { b: unknown }>
    >();

    expect(Predicate.and(hasa, hasb)).type.toBe<
      Predicate.Refinement<unknown, { a: unknown } & { b: unknown }>
    >();
  });
  ...
  it("Tuple", () => {
    ...
    expect(Predicate.Tuple([isOdd, isOdd])).type.toBe<
      Predicate.Predicate<readonly [number, number]>
    >();
    ...
    expect(
      Predicate.Tuple([
        ...hole<
          Array<
            Predicate.Predicate<number> | Predicate.Refinement<boolean, true>
          >
        >(),
      ])
    ).type.toBe<
      Predicate.Refinement<ReadonlyArray<never>, ReadonlyArray<never>>
    >();
  ...
  it("Struct", () => {
    ...
    expect(
      ...
      Predicate.Predicate<{ readonly odd: number; readonly odd1: number }>
    >();
  });

  it("isUint8Array", () => {
    expect(unknowns.filter(Predicate.isUint8Array)).type.toBe<
      Array<Uint8Array>
    >();
  });
});
