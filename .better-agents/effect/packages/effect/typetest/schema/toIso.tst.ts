...
describe("toIso", () => {
  ...
  it("encodedCodec(Class)", () => {
    ...
    expect(optic).type.toBe<
      Optic.Iso<{ readonly a: Date }, { readonly a: Date }>
    >();
  });
  ...
  it("Array", () => {
    ...
    expect(optic).type.toBe<
      Optic.Iso<ReadonlyArray<Value>, ReadonlyArray<{ readonly a: Date }>>
    >();
  });
  ...
  it("Record", () => {
    ...
    expect(optic).type.toBe<
      Optic.Iso<
        ...
        { readonly [x: string]: { readonly a: Date } }
      >
    >();
  });
  ...
  it("Union", () => {
    ...
    expect(optic).type.toBe<
      Optic.Iso<string | Value, string | { readonly a: Date }>
    >();
  });
  ...
  it("Opaque", () => {
    class Value extends Schema.Opaque<Value>()(
      Schema.Struct({ a: Schema.Date })
    ) {}
    const schema = Value;
    const optic = Schema.toIso(schema);
...
it("Cause", () => {
  ...
  expect(optic).type.toBe<
    Optic.Iso<
      Cause.Cause<Value>,
      ReadonlyArray<Schema.CauseReasonIso<typeof Value, typeof Value>>
    >
  >();
});
...
it("Exit", () => {
  ...
  expect(optic).type.toBe<
    Optic.Iso<
      ...
      Schema.ExitIso<typeof Value, Schema.Error, Schema.Defect>
    >
  >();
});
...
it("ReadonlyMap", () => {
  ...
  expect(optic).type.toBe<
    Optic.Iso<
      ...
      ReadonlyArray<readonly [string, { readonly a: Date }]>
    >
  >();
});
