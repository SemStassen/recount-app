...
describe("Union", () => {
  it("empty", () => {
    ...
    expect(Schema.revealCodec(schema)).type.toBe<
      Schema.Codec<never, never, never>
    >();
  ...
  it("string | number", () => {
    ...
    expect(Schema.revealCodec(schema)).type.toBe<
      Schema.Codec<string | number, string | number, never>
    >();
    expect(schema).type.toBe<
      Schema.Union<readonly [Schema.String, Schema.Number]>
    >();
    expect(schema.annotate({})).type.toBe<
      Schema.Union<readonly [Schema.String, Schema.Number]>
    >();
  ...
  describe("mapMembers", () => {
    it("appendElement", () => {
      ...
      >();
      expect(schema).type.toBe<
        Schema.Union<readonly [Schema.String, Schema.Number, Schema.Boolean]>
      >();
    });
    ...
    it("Array", () => {
      ...
      expect(schema).type.toBe<
        Schema.Union<
          readonly [Schema.$Array<Schema.String>, Schema.$Array<Schema.Number>]
        >
      >();
    });
    ...
    it("NonEmptyArray", () => {
      const schema = Schema.Union([Schema.String, Schema.Number]).mapMembers(
        Tuple.map(Schema.NonEmptyArray)
      );
      expect(Schema.revealCodec(schema)).type.toBe<
        Schema.Codec<
  ...
  describe("Tagged unions", () => {
    describe("asTaggedUnion", () => {
      it("should not be callable if the tag field is invalid", () => {
        ...
        expect(original.pipe).type.toBeCallableWith(
          Schema.toTaggedUnion("_tag")
        );
        expect(original.pipe).type.not.toBeCallableWith(
          Schema.toTaggedUnion("a")
        );
        ...
        >();
        expect(B).type.toBe<
          Schema.TaggedStruct<"B", { readonly b: Schema.FiniteFromString }>
        >();
        expect(C).type.toBe<
          Schema.TaggedStruct<"C", { readonly c: Schema.Boolean }>
        >();
      });
    });
    ...
    describe("match", () => {
      it("should allow distinct Effect branch outputs", () => {
        ...
        expect(handler).type.toBeAssignableTo<
          (
            value: Schema.Schema.Type<typeof schema>
          ) => Effect.Effect<"ok", "nope", never>
        >();
      });
    });
