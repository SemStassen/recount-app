...
describe("decoding / encoding API", () => {
  ...
  it("decoding / encoding APIs should work with a union of schemas", () => {
    ...
    expect(decodeUnknownEffect).type.toBe<
      Effect.Effect<{ readonly a: number }, Schema.SchemaError>
    >();

    const decodeEffect = (<K extends "a" | "b">(key: K) => {
      return Schema.decodeEffect(schemas[key]);
    })("a")({ a: "1" });
    expect(decodeEffect).type.toBe<
      Effect.Effect<{ readonly a: number }, Schema.SchemaError>
    >();
    ...
    expect(decodeUnknownExit).type.toBe<
      Exit.Exit<{ readonly a: number }, Schema.SchemaError>
    >();

    const decodeExit = (<K extends "a" | "b">(key: K) => {
      return Schema.decodeExit(schemas[key]);
    })("a")({ a: "1" });
    expect(decodeExit).type.toBe<
      Exit.Exit<{ readonly a: number }, Schema.SchemaError>
    >();
    ...
    expect(decodeUnknownResult).type.toBe<
      Result.Result<{ readonly a: number }, Schema.SchemaError>
    >();

    const decodeResult = (<K extends "a" | "b">(key: K) => {
      return Schema.decodeResult(schemas[key]);
    })("a")({ a: "1" });
    expect(decodeResult).type.toBe<
      Result.Result<{ readonly a: number }, Schema.SchemaError>
    >();
    ...
    expect(decodeUnknownOption).type.toBe<
      Option.Option<{ readonly a: number }>
    >();
    ...
    expect(encodeUnknownEffect).type.toBe<
      Effect.Effect<{ readonly a: string }, Schema.SchemaError>
    >();

    const encodeEffect = (<K extends "a" | "b">(key: K) => {
      return Schema.encodeEffect(schemas[key]);
    })("a")({ a: 1 });
    expect(encodeEffect).type.toBe<
      Effect.Effect<{ readonly a: string }, Schema.SchemaError>
    >();
    ...
    expect(encodeUnknownExit).type.toBe<
      Exit.Exit<{ readonly a: string }, Schema.SchemaError>
    >();

    const encodeExit = (<K extends "a" | "b">(key: K) => {
      return Schema.encodeExit(schemas[key]);
    })("a")({ a: 1 });
    expect(encodeExit).type.toBe<
      Exit.Exit<{ readonly a: string }, Schema.SchemaError>
    >();
    ...
    expect(encodeUnknownResult).type.toBe<
      Result.Result<{ readonly a: string }, Schema.SchemaError>
    >();

    const encodeResult = (<K extends "a" | "b">(key: K) => {
      return Schema.encodeResult(schemas[key]);
    })("a")({ a: 1 });
    expect(encodeResult).type.toBe<
      Result.Result<{ readonly a: string }, Schema.SchemaError>
    >();
    ...
    expect(encodeUnknownOption).type.toBe<
      Option.Option<{ readonly a: string }>
    >();
