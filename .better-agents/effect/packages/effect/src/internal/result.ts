...
const SuccessProto = Object.assign(Object.create(CommonProto), {
  ...
  [Equal.symbol]<A, E>(this: Result.Success<A, E>, that: unknown): boolean {
    return (
      isResult(that) &&
      isSuccess(that) &&
      Equal.equals(this.success, that.success)
    );
  },
...
const FailureProto = Object.assign(Object.create(CommonProto), {
  ...
  [Equal.symbol]<A, E>(this: Result.Failure<A, E>, that: unknown): boolean {
    return (
      isResult(that) &&
      isFailure(that) &&
      Equal.equals(this.failure, that.failure)
    );
  },
...
export const isResult = (
  input: unknown
): input is Result.Result<unknown, unknown> => hasProperty(input, TypeId);

/** @internal */
export const isFailure = <A, E>(
  result: Result.Result<A, E>
): result is Result.Failure<A, E> => result._tag === "Failure";

/** @internal */
export const isSuccess = <A, E>(
  result: Result.Result<A, E>
): result is Result.Success<A, E> => result._tag === "Success";
