...
const ProtoUtc = {
  ...
  [Equal.symbol](this: DateTime.Utc, that: unknown) {
    return (
      isDateTime(that) &&
      that._tag === "Utc" &&
      this.epochMilliseconds === that.epochMilliseconds
    );
  },
...
const ProtoZoned = {
  ...
  [Hash.symbol](this: DateTime.Zoned) {
    return Hash.combine(Hash.number(this.epochMilliseconds))(
      Hash.hash(this.zone)
    );
  },
...
const ProtoTimeZoneOffset = {
  ...
  [Equal.symbol](this: DateTime.TimeZone.Offset, that: unknown) {
    return (
      isTimeZone(that) && that._tag === "Offset" && this.offset === that.offset
    );
  },
...
/** @internal */
export const isDateTime = (u: unknown): u is DateTime.DateTime =>
  Predicate.hasProperty(u, TypeId);

const isDateTimeArgs = (args: IArguments) => isDateTime(args[0]);

/** @internal */
export const isTimeZone = (u: unknown): u is DateTime.TimeZone =>
  Predicate.hasProperty(u, TimeZoneTypeId);

/** @internal */
export const isTimeZoneOffset = (u: unknown): u is DateTime.TimeZone.Offset =>
  isTimeZone(u) && u._tag === "Offset";

/** @internal */
export const isTimeZoneNamed = (u: unknown): u is DateTime.TimeZone.Named =>
  isTimeZone(u) && u._tag === "Named";

/** @internal */
export const isUtc = (self: DateTime.DateTime): self is DateTime.Utc =>
  self._tag === "Utc";

/** @internal */
export const isZoned = (self: DateTime.DateTime): self is DateTime.Zoned =>
  self._tag === "Zoned";
...
/** @internal */
export const Equivalence: Equ.Equivalence<DateTime.DateTime> = Equ.make(
  (a, b) => a.epochMilliseconds === b.epochMilliseconds
);

/** @internal */
export const Order: order.Order<DateTime.DateTime> = order.make((self, that) =>
  self.epochMilliseconds < that.epochMilliseconds
    ? -1
    : self.epochMilliseconds > that.epochMilliseconds
      ? 1
      : 0
...
export const clamp: {
  ...
  <
    ...
    Min extends DateTime.DateTime,
    Max extends DateTime.DateTime,
  >(
    self: A,
...
/** @internal */
export const makeUnsafe = <A extends DateTime.DateTime.Input>(
  input: A
 ...
 */
const hasZone = (input: string): boolean =>
  /Z|GMT|[+-]\d{2}$|[+-]\d{2}:?\d{2}$|\]$/.test(input);
...
export const makeZonedUnsafe = (
  ...
  if (
    self.epochMilliseconds < minEpochMillis ||
    self.epochMilliseconds > maxEpochMillis
  ) {
    throw new RangeError(
      `Epoch millis out of range: ${self.epochMilliseconds}`
    );
  }
  if (
  ...
  if (timeZoneOption === undefined) {
    const offset =
      new Date(self.epochMilliseconds).getTimezoneOffset() * -60 * 1000;
...
/** @internal */
export const make: <A extends DateTime.DateTime.Input>(
  input: A
) => Option.Option<DateTime.DateTime.PreserveZone<A>> =
...
export const makeZonedFromString = (
  ...
  if (match === null) {
    ...
    return offset !== null
      ? makeZoned(input, { timeZone: offset })
      : Option.none();
  }
...
/** @internal */
export const nowAsDate: Effect.Effect<Date> = effect.map(
  Clock.currentTimeMillis,
...
/** @internal */
export const toUtc = (self: DateTime.DateTime): DateTime.Utc =>
  makeUtc(self.epochMilliseconds);
...
/** @internal */
export const zoneMakeNamedUnsafe = (
  zoneId: string
...
export const zoneMakeNamed: (
  zoneId: string
) => Option.Option<DateTime.TimeZone.Named> =
  Option.liftThrowable(zoneMakeNamedUnsafe);

/** @internal */
export const zoneMakeNamedEffect = (
  zoneId: string
): Effect.Effect<DateTime.TimeZone.Named, IllegalArgumentError> =>
  effect.try({
...
export const zoneFromString = (
  ...
  if (offsetZoneRegExp.test(zone)) {
    ...
    return offset === null
      ? Option.none()
      : Option.some(zoneMakeOffset(offset));
  }
...
export const setZoneNamed: {
  ...
  (
    ...
    Option.map(zoneMakeNamed(zoneId), (zone) => setZone(self, zone, options))
);
...
/** @internal */
export const isFuture = (self: DateTime.DateTime): Effect.Effect<boolean> =>
  effect.map(now, isLessThan(self));

/** @internal */
export const isFutureUnsafe = (self: DateTime.DateTime): boolean =>
  isLessThan(nowUnsafe(), self);

/** @internal */
export const isPast = (self: DateTime.DateTime): Effect.Effect<boolean> =>
  effect.map(now, isGreaterThan(self));

/** @internal */
export const isPastUnsafe = (self: DateTime.DateTime): boolean =>
  isGreaterThan(nowUnsafe(), self);
...
/** @internal */
export const toDateUtc = (self: DateTime.DateTime): Date =>
  new Date(self.epochMilliseconds);
...
export const toDate = (self: DateTime.DateTime): Date => {
  ...
  const parts = self.zone.format
    .formatToParts(self.epochMilliseconds)
...
/** @internal */
export const zonedOffsetIso = (self: DateTime.Zoned): string =>
  offsetToString(zonedOffset(self));

/** @internal */
export const toEpochMillis = (self: DateTime.DateTime): number =>
  self.epochMilliseconds;
...
const makeZonedFromAdjusted = (
    ...
    if (adjustedMillis !== beforeAdjustedMillis) {
      switch (disambiguation) {
        case "reject": {
          ...
          throw new RangeError(
            `Gap time: ${formatted} does not exist in time zone ${zone.id}`
          );
        }
        case "earlier":
          return makeZonedProto(adjustedMillis - afterOffset, zone);
  ...
  if (currentBeforeOffset === beforeOffset) {
    ...
    if (disambiguation === "reject") {
      const formatted = new Date(adjustedMillis).toISOString();
      throw new RangeError(
        `Ambiguous time: ${formatted} occurs twice in time zone ${zone.id}`
      );
    }
...
const calculateNamedOffset = (
  ...
  const offset =
    zone.format.formatToParts(utcMillis).find((_) => _.type === "timeZoneName")
      ?.value ?? "";
  if (offset === "GMT") {
    return 0;
  }
...
export const match: {
  ...
  <A, B>(
    ...
    self._tag === "Utc" ? options.onUtc(self) : options.onZoned(self)
);
...
export const addDuration: {
  ...
  (self: DateTime.DateTime, duration: Duration.Input): DateTime.DateTime =>
    mapEpochMillis(
      self,
      (millis) => millis + Duration.toMillis(Duration.fromInputUnsafe(duration))
    )
);
...
export const subtract: {
  ...
  (
    ...
    for (const key in parts) {
      newParts[key as keyof DateTime.DateTime.PartsForMath] =
        -1 * parts[key as keyof DateTime.DateTime.PartsForMath]!;
    }
...
export const startOf: {
  ...
  (
    ...
    mutate(self, (date) => startOfDate(date, part, options))
);
...
export const formatLocal: {
  ...
  (
    ...
    new Intl.DateTimeFormat(options?.locale, options).format(
      self.epochMilliseconds
    )
);
...
} = dual(2, (self: DateTime.DateTime, format: Intl.DateTimeFormat): string =>
  format.format(self.epochMilliseconds)
);

/** @internal */
export const formatIso = (self: DateTime.DateTime): string =>
  toDateUtc(self).toISOString();

/** @internal */
export const formatIsoDate = (self: DateTime.DateTime): string =>
  toDate(self).toISOString().slice(0, 10);

/** @internal */
export const formatIsoDateUtc = (self: DateTime.DateTime): string =>
  toDateUtc(self).toISOString().slice(0, 10);
