...
export const repeatOrElse: {
  <R2, A, B, E, E2, E3, R3>(
    ...
    orElse: (
      error: E | E2,
      meta: Option.Option<Schedule.Metadata<B, A>>
    ) => Effect<B, E3, R3>
      ...
      return effect.catch_(
        effect.forever(
          effect.tap(
            effect.flatMap(
              effect.suspend(() =>
                effect.provideService(self, Schedule.CurrentMetadata, meta)
              ),
              step
            ),
            (meta_) =>
        ...
        (error) =>
          core.isDone(error)
            ...
            : orElse(
                error as E | E2,
                meta.attempt === 0 ? Option.none() : Option.some(meta as any)
              )
      );
    })
);
...
export const retryOrElse: {
  ...
  <A, E, R, A1, E1, R1, A2, E2, R2>(
    ...
    effect.flatMap(Schedule.toStepWithMetadata(policy), (step) => {
      ...
      return Pull.catchDone(loop, (out) =>
        internalCall(() => orElse(lastError, out as A1))
      );
    })
);
...
export const repeat = dual<
  {
    ...
    <Output, Input, Error, Env>(
      builder: (
        $: <O, E, R>(
          _: Schedule.Schedule<O, NoInfer<Input>, E, R>
        ) => Schedule.Schedule<O, Input, E, R>
...
>(
  ...
  (
    ...
    options:
      ...
      | ((
          $: <O, E, R>(
            _: Schedule.Schedule<O, any, E, R>
          ) => Schedule.Schedule<O, any, E, R>
        ) => Schedule.Schedule<any, any, any, any>)
  ) => {
    const schedule =
...
export const retry = dual<
  {
    ...
    <B, E, Error, Env>(
      builder: (
        $: <O, SE, R>(
          _: Schedule.Schedule<O, NoInfer<E>, SE, R>
        ) => Schedule.Schedule<O, E, SE, R>
...
>(
  ...
  (
    ...
    options:
      ...
      | ((
          $: <O, SE, R>(
            _: Schedule.Schedule<O, any, SE, R>
          ) => Schedule.Schedule<O, any, SE, R>
        ) => Schedule.Schedule<any, any, any, any>)
  ) => {
    const schedule =
      typeof options === "function"
...
>(
  ...
  <Input, E, R, Output, Error, Env>(
    ...
    effect.flatMap(Schedule.toStepWithMetadata(schedule), (step) => {
      let meta = Schedule.CurrentMetadata.defaultValue();
      const selfWithMeta = effect.suspend(() =>
        effect.provideService(self, Schedule.CurrentMetadata, meta)
      );
      return effect.catch_(
        effect.flatMap(step(initial), (meta_) => {
          meta = meta_;
        ...
        (error) =>
          core.isDone(error)
            ? effect.succeed(error.value as Output)
