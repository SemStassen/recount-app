...
export interface HashMap<out Key, out Value>
  extends Iterable<[Key, Value]>, Equal_.Equal, Pipeable, Inspectable {
...
const mask = (hash: number, shift: number): number =>
  (hash >>> shift) & BITMAP_INDEX_MASK;

/** @internal */
const bitpos = (hash: number, shift: number): number => 1 << mask(hash, shift);

/** @internal */
const index = (bitmap: number, bit: number): number =>
...
function mergeLeaves<K, V>(
  ...
  const children: Array<Node<K, V>> =
    bit1 >>> 0 < bit2 >>> 0 ? [node1, node2] : [node2, node1];
...
class LeafNode<K, V> extends Node<K, V> {
  ...
  set(
    ...
    if (newBit === existingBit) {
      return new IndexedNode(edit, newBit, [
...
class IndexedNode<K, V> extends Node<K, V> {
  ...
  private expand(
    ...
    const nodes: Array<Node<K, V> | undefined> = new globalThis.Array(
      BUCKET_SIZE
    );
    let j = 0;
...
class ArrayNode<K, V> extends Node<K, V> {
  ...
  get size(): number {
    if (this._size === undefined) {
      this._size = this.children.reduce<number>(
  ...
  [Equal_.symbol](that: Equal_.Equal): boolean {
    if (isHashMap(that)) {
      ...
      for (const [key, value] of this) {
        ...
        if (
          Option.isNone(otherValue) ||
          !Equal_.equals(value, otherValue.value)
        ) {
          return false;
        }
      }
...
export const isHashMap: {
  ...
  hasProperty(u, HashMapTypeId);

/** @internal */
export const empty = <K = never, V = never>(): HashMap<K, V> =>
  new HashMapImpl(false, 0, emptyNode, 0);
...
/** @internal */
export const isEmpty = <K, V>(self: HashMap<K, V>): boolean =>
  (self as HashMapImpl<K, V>).size === 0;
...
export const getHash = dual<
  <K1 extends K, K>(
    key: K1,
    hash: number
  ) => <V>(self: HashMap<K, V>) => Option.Option<V>,
  <K1 extends K, K, V>(
    self: HashMap<K, V>,
    key: K1,
    hash: number
...
/** @internal */
export const size = <K, V>(self: HashMap<K, V>): number =>
  (self as HashMapImpl<K, V>).size;
...
export const filterMap = dual<
  <A, K, B, X>(
    f: (input: A, key: K) => Result.Result<B, X>
  ) => (self: HashMap<K, A>) => HashMap<K, B>,
  <K, A, B, X>(
    self: HashMap<K, A>,
    f: (input: A, key: K) => Result.Result<B, X>
...
export const findFirst = dual<
  ...
  ) => (self: HashMap<K, A>) => Option.Option<[K, A]>,
  <K, A>(
    self: HashMap<K, A>,
