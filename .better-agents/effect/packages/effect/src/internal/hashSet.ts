...
/** @internal */
export interface HashSet<out V>
  extends Iterable<V>, Equal.Equal, Pipeable, Inspectable {
...
const HashSetProto: Omit<HashSet<unknown>, HashSetTypeId> = {
  ...
  [Equal.symbol]<V>(this: HashSet<V>, that: unknown): boolean {
    return (
      isHashSet(that) &&
      size(this) === size(that) &&
      every(this, (value) => has(that, value))
    );
  },
...
/** @internal */
export const isHashSet = (u: unknown): u is HashSet<unknown> =>
  hasProperty(u, HashSetTypeId);

/** @internal */
export const keyMap = <V>(self: HashSet<V>): HashMap.HashMap<V, boolean> =>
  (self as any).keyMap;

/** @internal */
export const empty = <V = never>(): HashSet<V> =>
  makeImpl(HashMap.empty<V, boolean>());
...
/** @internal */
export const has = <V>(self: HashSet<V>, value: V): boolean =>
  HashMap.has(keyMap(self), value);

/** @internal */
export const add = <V>(self: HashSet<V>, value: V): HashSet<V> => {
  const map = keyMap(self);
  return HashMap.has(map, value)
    ? self
    : makeImpl(HashMap.set(map, value, true));
};
...
/** @internal */
export const isEmpty = <V>(self: HashSet<V>): boolean =>
  HashMap.isEmpty(keyMap(self));

// Helper function for building new HashSets from iteration
const fromPredicate = <V>(
  self: HashSet<V>,
...
/** @internal */
export const intersection = <V0, V1>(
  self: HashSet<V0>,
  that: HashSet<V1>
): HashSet<V0 & V1> => {
...
/** @internal */
export const difference = <V0, V1>(
  self: HashSet<V0>,
  that: HashSet<V1>
): HashSet<V0> => fromPredicate(self, (value) => !has(that, value as any));

/** @internal */
export const isSubset = <V0, V1>(
  self: HashSet<V0>,
  that: HashSet<V1>
): boolean => {
  for (const value of self) {
...
export const filter = <V>(
  self: HashSet<V>,
  predicate: (value: V) => boolean
): HashSet<V> => fromPredicate(self, predicate);
