  ...
  [Equal.symbol]<V>(this: TrieImpl<V>, that: unknown): boolean {
    if (isTrie(that)) {
      ...
      return Array.from(this).every((itemSelf, i) => {
        ...
        return (
          Equal.equals(itemSelf[0], itemThat[0]) &&
          Equal.equals(itemSelf[1], itemThat[1])
        );
      });
    }
    return false;
...
/** @internal */
export const size = <V>(self: TR.Trie<V>): number =>
  (self as TrieImpl<V>)._root?.count ?? 0;
...
>(2, (self, f) =>
  reduce(self, empty(), (trie, value, key) => insert(trie, key, f(value, key)))
);
...
export const filter: {
  ...
  <A>(
    f: (a: NoInfer<A>, k: string) => boolean
  ): (self: TR.Trie<A>) => TR.Trie<A>;
  <A, B extends A>(
    self: TR.Trie<A>,
  ...
  <A>(self: TR.Trie<A>, f: (a: A, k: string) => boolean): TR.Trie<A> =>
    reduce(self, empty(), (trie, value, key) =>
      f(value, key) ? insert(trie, key, value) : trie
    )
);
...
>(
  ...
  <A, B, X>(
    ...
    reduce(self, empty<B>(), (trie, value, key) => {
      const result = f(value, key);
      return Result.isSuccess(result)
        ? insert(trie, key, result.success)
        : trie;
    })
);
...
export const compact = <A>(self: TR.Trie<Option.Option<A>>) =>
  reduce(self, empty<A>(), (trie, option, key) =>
    Option.isSome(option) ? insert(trie, key, option.value) : trie
  );
...
>(2, (self, f) =>
  reduce(self, void 0 as void, (_, value, key) => f(value, key))
);
...
>(
  2,
  <V>(self: TR.Trie<V>, prefix: string): Array<[string, V]> =>
    Array.from(entriesWithPrefix(self, prefix))
);
...
>(2, <V>(self: TR.Trie<V>, key: string): Option.Option<[string, V]> => {
  let n: Node<V> | undefined = (self as TrieImpl<V>)._root;
  if (n === undefined || key.length === 0) return Option.none();
