...
function applyChecks(
  ...
  return filters
    .map(
      (filter) => (a: any) =>
        filter.run(a, ast, SchemaAST.defaultParseOptions) === undefined
    )
...
): constraint is Schema.Annotations.ToArbitrary.ArrayConstraints &
  FastCheck.UniqueArrayConstraintsCustomCompare<any> {
...
function isConstraintKey(
  key: string
): key is keyof Schema.Annotations.ToArbitrary.Constraint {
...
export function constraintContext(
  ...
  ctx: Schema.Annotations.ToArbitrary.Context
) => Schema.Annotations.ToArbitrary.Context {
  const annotations = filters
    .map((filter) => filter.annotations?.toArbitraryConstraint)
    .filter(Predicate.isNotUndefined);
  return (ctx) => {
    const constraints = annotations.reduce(
      (acc: Schema.Annotations.ToArbitrary.Constraint, c) => {
...
function toIntegerConstraints(
  ...
  if (constraint.min !== undefined) {
    out.min = constraint.minExcluded
      ? Math.floor(constraint.min) + 1
      : Math.ceil(constraint.min);
  }
  if (constraint.max !== undefined) {
    out.max = constraint.maxExcluded
      ? Math.ceil(constraint.max) - 1
      : Math.floor(constraint.max);
  }
  return out;
}
...
/** @internal */
export const memoized = memoize(
  (ast: SchemaAST.AST): LazyArbitraryWithContext<any> => {
    return recur(ast, []);
  }
);
...
function recur(
  ...
  const annotation = InternalAnnotations.resolve(ast)?.toArbitrary as
    ...
    | undefined;
  if (annotation) {
    const typeParameters = SchemaAST.isDeclaration(ast)
      ? ast.typeParameters.map((tp) => recur(tp, path))
      : [];
    ...
    return (fc, ctx) =>
      applyChecks(
        ast,
        filters,
        annotation(typeParameters.map((tp) => tp(fc, resetContext(ctx))))(
          fc,
          f(ctx)
        )
      );
  ...
  switch (ast._tag) {
    ...
    case "String":
      return (fc, ctx) => {
        ...
        if (patterns) {
          return fc.oneof(
            ...patterns.map((pattern) => fc.stringMatching(new RegExp(pattern)))
          );
        }
    ...
    case "Arrays":
      return (fc, ctx) => {
        ...
        // handle elements
        // ---------------------------------------------
        const elements: Array<FastCheck.Arbitrary<Option.Option<any>>> =
          ast.elements.map((e, i) => {
            const out = recur(e, [...path, i])(fc, reset);
            if (!SchemaAST.isOptional(e)) {
              return out.map(Option.some);
            }
            return out.chain((a) =>
              fc.boolean().map((b) => (b ? Option.some(a) : Option.none()))
            );
          });
        ...
        if (Array.isReadonlyArrayNonEmpty(ast.rest)) {
          ...
          const [head, ...tail] = ast.rest.map((r, i) =>
            recur(r, [...path, len + i])(fc, reset)
          );
      ...
      return (fc, ctx) => {
        ...
        for (const is of ast.indexSignatures) {
          const entry = fc.tuple(
            recur(is.parameter, path)(fc, reset),
            recur(is.type, path)(fc, reset)
          );
          const entries = array(
            fc,
            ast.propertySignatures.length === 0 ? ctx : reset,
            entry
          );
          out = out.chain((o) => {
    ...
    case "Union":
      return (fc, ctx) =>
        fc.oneof(...ast.types.map((ast) => recur(ast, path)(fc, ctx)));
    case "Suspend": {
