...
export function fromASTs(
  ...
  function fromASTChecks(
    checks:
      | readonly [SchemaAST.Check<any>, ...Array<SchemaAST.Check<any>>]
      | undefined
    ...
    function getCheck(
      c: SchemaAST.Check<any>
    ): SchemaRepresentation.Check<any> | undefined {
      switch (c._tag) {
        case "Filter": {
...
function fromASTAnnotations(
  ...
  if (annotations !== undefined) {
    const filtered = Rec.filter(
      annotations,
      (_, k) => !fromASTBlacklist.has(k)
    );
...
export function toJsonSchemaMultiDocument(
  ...
  function recur(
    ...
    if ("checks" in s) {
      const checks = collectJsonSchemaChecks<SchemaRepresentation.Meta>(
        s.checks,
        js.type
      );
  ...
  function on(
    schema: SchemaRepresentation.Representation
  ): JsonSchema.JsonSchema {
    switch (schema._tag) {
      case "Any":
      case "Unknown":
      ...
      case "Arrays": {
        ...
        const prefixItems: Array<JsonSchema.JsonSchema> = schema.elements.map(
          (e) => {
            if (e.isOptional) {
              minItems--;
            }
      ...
      case "Objects": {
        if (
          schema.propertySignatures.length === 0 &&
          schema.indexSignatures.length === 0
        ) {
        ...
        for (const ps of schema.propertySignatures) {
          ...
          if (typeof name !== "string") {
            throw new globalThis.Error(
              `Unsupported property signature name: ${format(name)}`
            );
          }
        ...
        if (
          Predicate.isObject(out.additionalProperties) &&
          Rec.isEmptyRecord(out.additionalProperties)
        ) {
          delete out.additionalProperties;
        }
  ...
  function collectJsonSchemaChecks<M>(
    ...
    function collectJsonSchemaCheck<M>(
  ...
  function filterToJsonSchema(
    ...
    function on(
      ...
      switch (meta._tag) {
        ...
        case "isBetween": {
          return {
            [meta.exclusiveMinimum ? "exclusiveMinimum" : "minimum"]:
              meta.minimum,
            [meta.exclusiveMaximum ? "exclusiveMaximum" : "maximum"]:
              meta.maximum,
          };
        }
  ...
  function getParameterPatterns(
    ...
    switch (parameter._tag) {
      default:
        throw new globalThis.Error(
          `Unsupported index signature parameter: ${parameter._tag}`
        );
      case "Reference":
...
function getPatterns(s: SchemaRepresentation.String): Array<string> {
  ...
  function recur(
    checks: ReadonlyArray<
      SchemaRepresentation.Check<SchemaRepresentation.StringMeta>
    >
  ): Array<string> {
...
function hasCheck(
  checks: ReadonlyArray<SchemaRepresentation.Check<SchemaRepresentation.Meta>>,
  tag: string
): boolean {
