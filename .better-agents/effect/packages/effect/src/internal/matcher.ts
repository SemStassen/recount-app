...
export const value = <const I>(
  ...
  makeValueMatcher(i, Result.fail(i));
...
export const discriminatorStartsWith =
  ...
  >(
    ...
    const pred = (_: any) =>
      _ != null && typeof _[field] === "string" && _[field].startsWith(pattern);
...
/** @internal */
export const nonEmptyString: SafeRefinement<string, never> = ((u: unknown) =>
...
export const orElse =
  ...
  <I, R, A, Pr>(
    ...
    if (Result.isResult(toResult)) {
      return toResult._tag === "Success"
        ? (toResult.success as any)
        : (f(toResult.failure) as any);
    }
...
export const result: <I, F, R, A, Pr, Ret>(
  ...
  : Result.Result<Unify<A>, R> = (<I, R, RA, A>(
