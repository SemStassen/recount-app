...
export const StructuralProto = {
  ...
  [Equal.symbol](this: any, that: any): boolean {
    ...
    for (let i = 0; i < selfKeys.length; i++) {
      if (
        selfKeys[i] !== thatKeys[i] &&
        !Equal.equals(this[selfKeys[i]], that[selfKeys[i]])
      ) {
        return false;
      }
    }
...
/** @internal */
export const isCauseReason = (self: unknown): self is Cause.Reason<unknown> =>
  hasProperty(self, CauseReasonTypeId);
...
export class CauseImpl<E> implements Cause.Cause<E> {
  ...
  constructor(
    failures: ReadonlyArray<Cause.Fail<E> | Cause.Die | Cause.Interrupt>
  ) {
...
/** @internal */
export abstract class ReasonBase<
  Tag extends string,
> implements Cause.Cause.ReasonProto<Tag> {
...
export const causeAnnotate: {
  ...
  <E>(
    ...
    return new CauseImpl(
      self.reasons.map((f) => f.annotate(annotations, options))
    );
  }
);

/** @internal */
export const isFailReason = <E>(self: Cause.Reason<E>): self is Cause.Fail<E> =>
  self._tag === "Fail";

/** @internal */
export const isDieReason = <E>(self: Cause.Reason<E>): self is Cause.Die =>
  self._tag === "Die";

/** @internal */
export const isInterruptReason = <E>(
  self: Cause.Reason<E>
...
/** @internal */
export const isDone = (u: unknown): u is Cause.Done =>
  hasProperty(u, DoneTypeId);
