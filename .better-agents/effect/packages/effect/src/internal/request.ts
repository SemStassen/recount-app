...
export const request: {
  ...
  > => {
    ...
    return isEffect(resolver)
      ? effect.flatMap(resolver, withResolver)
  ...
  if (!batch) {
    if (batchPool.length > 0) {
      ...
      const newBatch: Batch = {
        ...
        run: effect.onExit(
          effect.suspend(() =>
            newBatch.resolver.runAll(
              Array.from(newBatch.entries) as NonEmptyArray<Request.Entry<any>>,
              newBatch.key
            )
          ),
          (exit) => {
            for (const entry of newBatch.entrySet) {
              entry.completeUnsafe(
                exit._tag === "Success"
                  ? exitDie(
                      new Error(
                        "Effect.request: RequestResolver did not complete request",
                        { cause: entry.request }
                      )
                    )
                  : exit
