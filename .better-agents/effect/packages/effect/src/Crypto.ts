 ...
 * @since 4.0.0
 */
export const Crypto: Context.Service<Crypto, Crypto> =
  Context.Service("effect/Crypto");
...
export const make = (impl: {
  ...
  return Crypto.of({
    ...
    randomBetween: (min, max) =>
      Effect.sync(() => nextDoubleUnsafe() * (max - min) + min),
    randomIntBetween(min, max, options) {
      ...
      return Effect.sync(() => {
        ...
        return (
          Math.floor(nextDoubleUnsafe() * (maxInt - minInt + extra)) + minInt
        );
      });
    },
    ...
    randomUUIDv7: Effect.clockWith((clock) =>
      Effect.succeed(
        formatUUIDv7(clock.currentTimeMillisUnsafe(), randomBytesUnsafe(16))
      )
    ),
  });
};
...
const formatUUIDv7 = (timestampMillis: number, bytes: Uint8Array): string => {
  const timestamp = Math.min(
    Math.max(0, Math.trunc(timestampMillis)),
    maxUUIDv7Timestamp
  );
