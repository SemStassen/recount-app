 ...
 * @since 3.19.0
 */
export interface HashRing<A extends PrimaryKey.PrimaryKey>
  extends Pipeable, Iterable<A> {
 ...
 * @since 3.19.0
 */
export const isHashRing = (u: unknown): u is HashRing<any> =>
...
const Proto = {
  ...
  [TypeId]: TypeId,
  [Symbol.iterator]<A extends PrimaryKey.PrimaryKey>(
    this: HashRing<A>
  ): Iterator<A> {
...
function addNodesToRing<A extends PrimaryKey.PrimaryKey>(
  self: HashRing<A>,
  keys: Array<string>,
  weight: number
) {
 ...
 * @since 3.19.0
 */
export const get = <A extends PrimaryKey.PrimaryKey>(
  self: HashRing<A>,
  input: string
): A | undefined => {
 ...
 * @since 3.19.0
 */
export const getShards = <A extends PrimaryKey.PrimaryKey>(
  self: HashRing<A>,
  count: number
): Array<A> | undefined => {
  if (self.ring.length === 0) {
  ...
  // max
  const distances = new Array<[shard: number, node: string, distance: number]>(
    count
  );
  ...
  for (let i = 0; i < count; i++) {
    ...
    const maxPerNode = Math.max(
      1,
      Math.floor(count * (weight / self.totalWeightCache))
    );
  ...
  remaining.forEach((shard) => {
    const index = getIndexForInput(
      self,
      shardHashes[shard],
      allAtMax ? undefined : exclude
    )[0];
    const node = self.ring[index][1];
    const [value, weight] = self.nodes.get(node)!;
    shards[shard] = value;
