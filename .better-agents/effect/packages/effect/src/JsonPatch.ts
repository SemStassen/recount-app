...
export function apply(patch: JsonPatch, oldValue: Schema.Json): Schema.Json {
  ...
  for (const op of patch) {
    switch (op.op) {
      case "replace": {
        doc =
          op.path === "" ? op.value : setAt(doc, op.path, op.value, "replace");
        break;
      }
      case "add": {
...
function tokenize(pointer: string): Array<string> {
  ...
  if (pointer.charCodeAt(0) !== 47 /* "/" */) {
    throw new Error(
      `Invalid JSON Pointer, it must start with "/": ${format(pointer)}`
    );
  }
...
function addAt(
  ...
  if (resolved === null) {
    throw new Error(
      `Cannot add at "${pointer}" (parent not found or not a container).`
    );
  }
...
function setAt(
  ...
  if (resolved === null) {
    throw new Error(
      `Cannot ${mode} at "${pointer}" (parent not found or not a container).`
    );
  }
  ...
  if (isJsonObject(parent)) {
    if (!Object.hasOwn(parent, lastToken)) {
      throw new Error(
        `Property "${lastToken}" does not exist at "${pointer}".`
      );
    }
    const updated = { ...parent };
