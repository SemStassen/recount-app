 ...
 * @since 2.0.0
 */
export interface StreamUnify<
  A extends { [Unify.typeSymbol]?: any },
> extends Effect.EffectUnify<A> {
  Stream?: () => A[Unify.typeSymbol] extends
    | Stream<infer A0, infer E0, infer R0>
    | (infer _)
    ? Stream<A0, E0, R0>
    : never;
}
 ...
 * @since 3.4.0
 */
export type Success<T extends Stream<any, any, any>> = [T] extends [
 ...
 * @since 3.4.0
 */
export type Error<T extends Stream<any, any, any>> = [T] extends [
  Stream<infer _A, infer _E, infer _R>,
]
  ? _E
  : never;
 ...
 * @since 4.0.0
 */
export type Services<T extends Stream<any, any, any>> = [T] extends [
  Stream<infer _A, infer _E, infer _R>,
]
  ? _R
  : never;
 ...
 * @since 4.0.0
 */
export const isStream = (u: unknown): u is Stream<unknown, unknown, unknown> =>
...
): Stream<S, never, I> => fromEffect(Effect.service(service));
...
export const serviceOption = <I, S>(
  service: Context.Key<I, S>
): Stream<Option.Option<S>> => fromEffect(Effect.serviceOption(service));
 ...
 * @since 4.0.0
 */
export const fromEffectRepeat = <A, E, R>(
  effect: Effect.Effect<A, E, R>
...
export const fromEffectSchedule = <A, E, R, X, AS extends A, ES, RS>(
  ...
  fromPull(
    Effect.gen(function* () {
      ...
      let s = yield* Effect.provideService(
        effect,
        Schedule.CurrentMetadata,
        Schedule.CurrentMetadata.defaultValue()
      );
      let initial = true;
      const pull = Effect.suspend(() => step(s as AS)).pipe(
        Effect.flatMap((meta) =>
          Effect.provideService(effect, Schedule.CurrentMetadata, meta)
        ),
        Effect.map((next) => {
          s = next;
          return Arr.of(next);
...
export const fromPull = <A, E, R, EX, RX>(
  ...
  fromChannel(Channel.fromPull(pull));
...
export const transformPull = <A, E, R, B, E2, R2, EX, RX>(
  ...
  fromChannel(
    Channel.fromTransform((_, scope) =>
      Effect.flatMap(Channel.toPullScoped(self.channel, scope), (pull) =>
        f(pull as any, scope)
      )
    )
  );
...
export const callback = <A, E = never, R = never>(
  ...
  fromChannel(Channel.callbackArray(f, options));
 ...
 * @since 2.0.0
 */
export const succeed = <A>(value: A): Stream<A> =>
  fromChannel(Channel.succeed(Arr.of(value)));
 ...
 * @since 2.0.0
 */
export const make = <const As extends ReadonlyArray<any>>(
  ...values: As
): Stream<As[number]> => fromArray(values);
 ...
 * @since 2.0.0
 */
export const sync = <A>(evaluate: LazyArg<A>): Stream<A> =>
  fromChannel(Channel.sync(() => Arr.of(evaluate())));
...
): Stream<A, E, R> => fromChannel(Channel.suspend(() => stream().channel));
 ...
 * @since 2.0.0
 */
export const fail = <E>(error: E): Stream<never, E> =>
  fromChannel(Channel.fail(error));
 ...
 * @since 2.0.0
 */
export const failSync = <E>(evaluate: LazyArg<E>): Stream<never, E> =>
 ...
 * @since 2.0.0
 */
export const failCause = <E>(cause: Cause.Cause<E>): Stream<never, E> =>
 ...
 * @since 2.0.0
 */
export const die = (defect: unknown): Stream<never> =>
  fromChannel(Channel.die(defect));
...
): Stream<never, E> => fromChannel(Channel.failCauseSync(evaluate));
...
): Stream<A, E, R> => unwrap(Effect.map(iterable, fromIterable));
...
export const fromIterableEffectRepeat = <A, E, R>(
  ...
  flatMap(fromEffectRepeat(iterable), fromIterable);
...
export const fromArray = <A>(array: ReadonlyArray<A>): Stream<A> =>
  Arr.isReadonlyArrayNonEmpty(array)
    ? fromChannel(Channel.succeed(array))
    : empty;
...
export const fromArrayEffect = <A, E, R>(
  ...
  unwrap(Effect.map(effect, fromArray)) as any;
...
export const fromArrays = <Arr extends ReadonlyArray<ReadonlyArray<any>>>(
  ...
  fromChannel(
    Channel.fromArray(Arr.filter(arrays, Arr.isReadonlyArrayNonEmpty))
  );
 ...
 * @since 2.0.0
 */
export const fromPubSub = <A>(pubsub: PubSub.PubSub<A>): Stream<A> =>
  fromChannel(Channel.fromPubSubArray(pubsub));
 ...
 * @since 4.0.0
 */
export const fromPubSubTake = <A, E>(
  pubsub: PubSub.PubSub<Take.Take<A, E>>
): Stream<A, E> => fromChannel(Channel.fromPubSubTake(pubsub));
...
): Stream<A> => fromChannel(Channel.fromSubscriptionArray(pubsub));
...
export const unwrap = <A, E2, R2, E, R>(
  ...
  fromChannel(Channel.unwrap(Effect.map(effect, toChannel)));
...
>(
  ...
  Stream<Effect.Effect<infer _A, infer _EX, infer _RX>, infer _E, infer _R>,
]
  ? Stream<_A, _EX | _E, _RX | _R>
  : <A, EX, RX, E, R>(
      self: Stream<Effect.Effect<A, EX, RX>, E, R>
    ) => Stream<A, EX | E, RX | R> = dual(
  (args) => isStream(args[0]),
  <A, E, R, EX, RX>(
    self: Stream<Effect.Effect<A, EX, RX>, E, R>,
    options?:
...
export const mapArrayEffect: {
  ...
  <A, E, R, B, E2, R2>(
    ...
    fromChannel(Channel.mapEffect(self.channel, f))
);
  ...
  <A, E, R, E2, R2>(
    ...
    transformPullBracket(
      ...
      Effect.fnUntraced(function* (pull, _, scope) {
        ...
        return Effect.suspend(
          (): Pull.Pull<Arr.NonEmptyReadonlyArray<A>, E | E2, void, R> => {
            if (causeSink) {
 ...
 * @since 4.0.0
 */
export const flattenArray = <A, E, R>(
  self: Stream<Arr.NonEmptyReadonlyArray<A>, E, R>
): Stream<A, E, R> => fromChannel(Channel.flattenArray(self.channel));
 ...
 * @since 2.0.0
 */
export const drain = <A, E, R>(self: Stream<A, E, R>): Stream<never, E, R> =>
  fromChannel(Channel.drain(self.channel));
...
export const drainFork: {
  ...
  <A, E, R, A2, E2, R2>(
    self: Stream<A, E, R>,
    that: Stream<A2, E2, R2>
  ): Stream<A, E | E2, R | R2> => mergeEffect(self, runDrain(that))
);
...
export const repeat: {
  ...
  <A, E, R, B, E2, R2>(
    ...
    fromChannel(Channel.repeat(self.channel, schedule))
);
  ...
  <A, E, R, B, E2, R2>(
    ...
    return catchCause(
      ...
      (cause): Stream<B, E | E2, R2> => {
        const isTimeout = cause.reasons.find(
          (r) => r._tag === "Die" && r.defect === timeoutSymbol
        );
  ...
  <A, E, R, B, E2, R2>(
    ...
    fromChannel(
      Channel.fromTransform((upstream, scope) =>
        Effect.map(
          Channel.toTransform(Channel.flattenArray(self.channel))(
            upstream,
            scope
          ),
          (pullElement) => {
            let pullRepeat:
              | Pull.Pull<Arr.NonEmptyReadonlyArray<A>, E | E2, void, R | R2>
              | undefined = undefined;
 ...
 * @since 2.0.0
 */
export const forever = <A, E, R>(self: Stream<A, E, R>): Stream<A, E, R> =>
  fromChannel(Channel.forever(self.channel));
 ...
 * @since 4.0.0
 */
export const flattenIterable = <A, E, R>(
  self: Stream<Iterable<A>, E, R>
): Stream<A, E, R> => flatMap(self, fromIterable);
 ...
 * @since 2.0.0
 */
export const flattenTake = <A, E, E2, R>(
...
export const prepend: {
  <B>(
    values: Iterable<B>
  ): <A, E, R>(self: Stream<A, E, R>) => Stream<B | A, E, R>;
...
export const merge: {
  ...
  <A, E, R, A2, E2, R2>(
    ...
    fromChannel(Channel.merge(toChannel(self), toChannel(that), options))
);
...
export const mergeResult: {
  <A2, E2, R2>(
    ...
    self: Stream<A, E, R>
  ) => Stream<Result.Result<A, A2>, E2 | E, R2 | R>;
  <A, E, R, A2, E2, R2>(
    self: Stream<A, E, R>,
    that: Stream<A2, E2, R2>
  ...
  <A, E, R, A2, E2, R2>(
    ...
    merge(map(self, Result.succeed), map(that, Result.fail))
);
...
export const mergeLeft: {
  ...
  <AL, EL, RL, AR, ER, RR>(
    left: Stream<AL, EL, RL>,
    right: Stream<AR, ER, RR>
  ): Stream<AL, EL | ER, RL | RR> => mergeEffect(left, runDrain(right))
);
...
export const mergeRight: {
  ...
  <AL, EL, RL, AR, ER, RR>(
    left: Stream<AL, EL, RL>,
    right: Stream<AR, ER, RR>
  ): Stream<AR, EL | ER, RL | RR> => mergeEffect(right, runDrain(left))
);
...
export const crossWith: {
  ...
  <AL, EL, RL, AR, ER, RR, A>(
    ...
    flatMap(left, (l) => map(right, (r) => f(l, r)))
);
 ...
 * @since 2.0.0
 */
export const zipWithIndex = <A, E, R>(
  self: Stream<A, E, R>
): Stream<[A, number], E, R> => map(self, (a, i) => [a, i]);
    ...
    },
    {
      onHalt(acc) {
        return acc.current._tag === "Some"
          ? [[acc.prev, acc.current.value, Option.none<A>()]]
          : [];
      },
    }
  );
...
export const zipLatestWith: {
  ...
  <AL, EL, RL, AR, ER, RR, A>(
    ...
    map(zipLatestAll(left, right), ([a, a2]) => f(a, a2))
);
  ...
  fromChannel(
    Channel.fromTransform((_, scope) =>
      Effect.sync(() => {
        let winner:
          | Pull.Pull<
              Arr.NonEmptyReadonlyArray<Success<S[number]>>,
              Error<S[number]>,
              void,
              Services<S[number]>
            >
...
export const filterEffect: {
  ...
  <A, E, R, EX, RX>(
    ...
    fromChannel(Channel.filterArrayEffect(toChannel(self), predicate))
);
...
export const filterMapEffect: {
  ...
  <A, E, R, B, X, EX, RX>(
    ...
    fromChannel(Channel.filterMapArrayEffect(toChannel(self), filter))
);
...
export const partitionQueue: {
  ...
  (args) => isStream(args[0]),
  Effect.fnUntraced(function* <A, E, R, Pass, Fail>(
    self: Stream<A, E, R>,
    filter: Filter.Filter<NoInfer<A>, Pass, Fail>,
    options?: {
      readonly capacity?: number | "unbounded" | undefined;
  ...
  > {
    ...
    const capacity =
      options?.capacity === "unbounded"
        ? undefined
        : (options?.capacity ?? DefaultChunkSize);
      ...
      while (true) {
        ...
        if (satisfying.length > 0) {
          ...
          if (leftover.length > 0) {
            passFiber = yield* Effect.forkChild(
              Queue.offerAll(passes, leftover)
            );
          }
        }
...
export const peel: {
  <A2, A, E2, R2>(
    ...
    self: Stream<A, E, R>
  ) => Effect.Effect<[A2, Stream<A, E, never>], E2 | E, Scope.Scope | R2 | R>;
  <A, E, R, A2, E2, R2>(
    self: Stream<A, E, R>,
    sink: Sink.Sink<A2, A, A, E2, R2>
  ...
  Effect.fnUntraced(function* <A, E, R, A2, E2, R2>(
    ...
    const pull: Pull.Pull<Arr.NonEmptyReadonlyArray<A>, E> = Effect.catchCause(
      originalPull,
      (cause_) => {
        cause = cause_;
...
export const tapCause: {
  ...
  <A, E, R, A2, E2, R2>(
    ...
    self.channel.pipe(Channel.tapCause(f), fromChannel)
);
...
export const catchTags: {
  ...
  >(
    cases: Cases,
    orElse?:
      | ((e: Exclude<E, { _tag: keyof Cases }>) => Stream<A2, E2, R2>)
      | undefined
  ): <A, R>(
    self: Stream<A, E, R>
  ) => Stream<
...
export const catchReasons: {
  ...
  (self, errorTag, cases, orElse) => {
    ...
    const orElseHandler =
      orElse && ((reason: any, error: any) => orElse(reason, error).channel);
    return fromChannel(
      Channel.catchReasons(
        self.channel,
        errorTag as any,
        handlers as any,
        orElseHandler as any
      ) as Channel.Channel<
...
export const catchCauseIf: {
  ...
  <A, E, R, A2, E2, R2>(
    ...
    fromChannel(
      Channel.catchCauseIf(self.channel, predicate, (cause) => f(cause).channel)
    )
);
 ...
 * @since 2.0.0
 */
export const orDie = <A, E, R>(self: Stream<A, E, R>): Stream<A, never, R> =>
  fromChannel(Channel.orDie(self.channel));
...
>(
  ...
  <A, E, R>(
    ...
    fromChannel(Channel.ignoreCause(self.channel, options))
);
...
export const retry: {
  ...
  <A, E, R, X, E2, R2>(
    ...
    fromChannel(Channel.retry(self.channel, policy))
);
...
export const withExecutionPlan: {
  ...
  <A, E extends Input, R, R2, Input, Provides, PolicyE>(
    ...
    suspend(() => {
      const preventFallbackOnPartialStream =
        options?.preventFallbackOnPartialStream ?? false;
      let i = 0;
      ...
      const loop: Stream<A, E | PolicyE, R2 | Exclude<R, Provides>> = suspend(
        () => {
          ...
          let nextStream: Stream<A, E | PolicyE, R2 | Exclude<R, Provides>> =
            provideMeta(provide(self, step.provide));
          ...
          if (Option.isSome(lastError)) {
            ...
            nextStream = retry(
              nextStream,
              internalExecutionPlan.scheduleFromStep(step, false) as any
            );
          } else {
            const schedule = internalExecutionPlan.scheduleFromStep(step, true);
            nextStream = schedule
              ? retry(nextStream, schedule as any)
              : nextStream;
          }
        ...
        > = Effect.flatMap(
          ...
          (chunk) => {
            ...
            if (index >= 0) {
              ...
              const arr = chunk.slice(
                0,
                options?.excludeLast ? index : index + 1
              );
...
export const takeWhileEffect: {
  ...
  <A, E, R, E2, R2>(
    ...
    takeUntilEffect(self, (a, n) => Effect.map(predicate(a, n), (b) => !b), {
      excludeLast: true,
    })
);
        ...
        > = pull.pipe(
          Effect.flatMap((chunk) => {
            ...
            return Effect.succeed(
              chunk.slice(n - dropped) as Arr.NonEmptyArray<A>
            );
          })
        );
        return pump;
...
export const dropRight: {
  ...
  return transformPull(self, (pull, _scope) =>
    Effect.sync(() => {
      ...
      const emit: Pull.Pull<Arr.NonEmptyReadonlyArray<A>, E> = Effect.flatMap(
        pull,
        (arr) => {
...
export const rechunk: {
  ...
  2,
  <A, E, R>(self: Stream<A, E, R>, target: number): Stream<A, E, R> => {
      ...
      Effect.sync(() => {
        ...
        > = Effect.matchCauseEffect(upstream, {
          onSuccess(arr) {
            ...
            emitted = true;
            const chunks = [] as any as Arr.NonEmptyArray<
              Arr.NonEmptyReadonlyArray<A>
            >;
...
export const combine: {
  ...
  <A, E, R, A2, E2, R2, S, A3, E3, R3>(
    ...
    Channel.combine(
      ...
      s,
      f
    ).pipe(Channel.map(Arr.of), fromChannel)
);
...
export const combineArray: {
  ...
  <R, A2, E2, R2, S, E, A, A3, E3, R3>(
    ...
    fromChannel(Channel.combine(self.channel, that.channel, s, f))
);
...
export const mapAccumEffect: {
  <S, A, B, E2, R2>(
    ...
    f: (
      s: S,
      a: A
    ) => Effect.Effect<readonly [state: S, values: ReadonlyArray<B>], E2, R2>,
    options?: {
      readonly onHalt?: ((state: S) => ReadonlyArray<B>) | undefined;
    }
  ): <E, R>(self: Stream<A, E, R>) => Stream<B, E | E2, R | R2>;
        ...
        (state, a) =>
          Effect.map(f(state, a), ([state, values]) => [
            ...
            Arr.isReadonlyArrayNonEmpty(values)
              ? Arr.of(values)
              : Arr.empty<Arr.NonEmptyReadonlyArray<B>>(),
          ]),
        options?.onHalt
          ? {
              onHalt(state) {
    ...
    transformPull(
      ...
      Effect.fnUntraced(function* (pull, scope) {
        ...
        const durationMs = Duration.toMillis(
          Duration.fromInputUnsafe(duration)
        );
        ...
        const sleepLoop = Effect.suspend(function loop(): Pull.Pull<
          Arr.NonEmptyReadonlyArray<A>,
          E,
          void,
          R
        > {
          ...
          const timeMs =
            emitAtMs < now ? durationMs : Math.min(durationMs, emitAtMs - now);
          return Effect.flatMap(
            Effect.raceFirst(Effect.sleep(timeMs), endLatch.await),
            () => {
              const now = clock.currentTimeMillisUnsafe();
              if (now < emitAtMs) {
                return loop();
...
const throttleEnforceEffect = <A, E, R, E2, R2>(
  ...
  transformPull(self, (pull) =>
    Effect.clockWith((clock) => {
      ...
      return Effect.succeed(
        Effect.flatMap(pull, function loop(arr): Pull.Pull<
          Arr.NonEmptyReadonlyArray<A>,
          E | E2,
          void,
          R | R2
...
export const grouped: {
  ...
  <A, E, R>(
    self: Stream<A, E, R>,
    n: number
  ): Stream<Arr.NonEmptyReadonlyArray<A>, E, R> => chunks(rechunk(self, n))
);
      ...
      Effect.sync(() => {
        ...
        return Pull.catchDone(
          ...
          () => {
            ...
            const out = group;
            group = undefined;
            return out && Arr.isArrayNonEmpty(out)
              ? Effect.succeed(Arr.of([currentKey, out]))
              : Cause.done();
          }
        );
      })
  ...
  <A, E, R, A2, E2, R2>(
    ...
    transformPull(self, (upstream, scope) =>
      Effect.sync(() => {
        ...
        return Effect.suspend(
          (): Pull.Pull<Arr.NonEmptyReadonlyArray<A2>, E | E2, void, R2> =>
            done ? done : pull
        );
      })
    )
    ...
    fromChannel(
      Channel.fromTransformBracket(
        Effect.fnUntraced(function* (_upstream, _, scope) {
          ...
          const step = yield* Schedule.toStepWithSleep(schedule);
          const stepToBuffer = Effect.suspend(function loop(): Pull.Pull<
            never,
            E3,
            void,
            R3
          > {
            return step(lastOutput).pipe(
              Effect.flatMap(() =>
                !sinkHasInput ? loop() : Queue.offer(buffer, scheduleStep)
              ),
          ...
          const sinkUpstream = Effect.suspend(
            (): Pull.Pull<Arr.NonEmptyReadonlyArray<A | A2>, E> => {
              if (leftover !== undefined) {
                const chunk = leftover;
                leftover = undefined;
                sinkHasInput = true;
          ...
          const catchSinkHalt = Effect.flatMap(
            ([value, leftover_]: Sink.End<B, A2>) => {
              // ignore the last output if the upstream only pulled a halt
              if (!sinkHasInput && buffer.state._tag === "Done")
                return Cause.done();
          ...
          return Effect.suspend(() => {
            ...
            if (buffer.state._tag === "Done" && leftover === undefined) {
              return buffer.state.exit as Exit.Exit<
                never,
                Cause.Done<void> | E
              >;
            }
            ...
            return Effect.succeed(
              Effect.suspend(() => sink.transform(sinkUpstream as any, scope))
            );
          }).pipe(
            Effect.flatMap((pull) =>
              Effect.raceFirst(catchSinkHalt(pull), stepToBuffer)
            )
          );
        })
      )
...
export const broadcastN: {
  ...
  Effect.fnUntraced(function* <A, E, R, const N extends number>(
    ...
    yield* Channel.runForEach(self.channel, (value) =>
...
export const broadcast: {
  ...
  <A, E, R>(
    ...
    Effect.map(toPubSubTake(self, options), fromPubSubTake)
);
...
export const pipeThroughChannel: {
  ...
  <R, R2, E, E2, A, A2>(
    ...
    fromChannel(Channel.pipeTo(self.channel, channel))
);
...
export const pipeThroughChannelOrFail: {
  ...
  <R, R2, E, E2, A, A2>(
    ...
    fromChannel(Channel.pipeToOrFail(self.channel, channel))
);
...
export const pipeThrough: {
  ...
  <A, E, R, A2, L, E2, R2>(
    ...
    self.channel.pipe(
      ...
      Channel.concatWith(([_, leftover]) =>
        leftover ? Channel.succeed(leftover) : Channel.empty
      ),
      fromChannel
    )
);
...
export const collect = <A, E, R>(
  self: Stream<A, E, R>
): Stream<Array<A>, E, R> => fromEffect(runCollect(self));
 ...
 * @since 2.0.0
 */
export const accumulate = <A, E, R>(
  self: Stream<A, E, R>
): Stream<Arr.NonEmptyArray<A>, E, R> =>
 ...
 * @since 2.0.0
 */
export const changes = <A, E, R>(self: Stream<A, E, R>): Stream<A, E, R> =>
  changesWith(self, Equal.equals);
  ...
  <A, E, R>(
    ...
    transformPull(self, (pull, _scope) =>
      Effect.sync(() => {
        let first = true;
        let last: A;
        return Effect.flatMap(pull, function loop(arr): Pull.Pull<
          Arr.NonEmptyReadonlyArray<A>,
          E
        > {
          const out: Array<A> = [];
          ...
          return Arr.isArrayNonEmpty(out)
            ? Effect.succeed(out)
      ...
      Effect.sync(() => {
        ...
        return Effect.flatMap(
          ...
          > {
            ...
            return Arr.isArrayNonEmpty(out)
              ? out
              : yield* Effect.flatMap(pull, Effect.fnUntraced(loop));
          })
        );
      })
    )
    ...
    fromChannel(
      Channel.fromTransform(
        Effect.fnUntraced(function* (upstream, scope) {
          const pullDecider = yield* Channel.toTransform(
            Channel.flattenArray(decider.channel)
          )(upstream, scope);
          const retry = Symbol();
          type retry = typeof retry;
          let leftDone = false;
          let rightDone = false;
          const pullLeft = (yield* Channel.toTransform(
            Channel.flattenArray(self.channel)
          )(upstream, scope)).pipe(
            Pull.catchDone(() => {
              leftDone = true;
          ...
          const pullRight = (yield* Channel.toTransform(
            Channel.flattenArray(that.channel)
          )(upstream, scope)).pipe(
            Pull.catchDone(() => {
              rightDone = true;
...
export const onExit: {
  ...
  <A, E, R, R2>(
    ...
    fromChannel(Channel.onExit(self.channel, finalizer))
);
...
export const onFirst: {
  ...
  <A, E, R, X, EX, RX>(
    ...
    fromChannel(Channel.onFirst(self.channel, (arr) => onFirst(arr[0])))
);
...
export const ensuring: {
  ...
  ): <A, E, R>(self: Stream<A, E, R>) => Stream<A, E, R | R2>;
  <A, E, R, R2>(
    self: Stream<A, E, R>,
    finalizer: Effect.Effect<unknown, never, R2>
  ): Stream<A, E, R | R2>;
...
export const provide: {
  ...
  <A, E, R, AL, EL = never, RL = never>(
    ...
    fromChannel(Channel.provide(self.channel, layer, options))
);
...
export const provideContext: {
  ...
  ): <A, E, R>(self: Stream<A, E, R>) => Stream<A, E, Exclude<R, R2>>;
  <A, E, R, R2>(
    self: Stream<A, E, R>,
    context: Context.Context<R2>
...
export const provideService: {
  ...
  <A, E, R, I, S>(
    ...
    fromChannel(Channel.provideService(self.channel, key, service))
);
...
export const provideServiceEffect: {
  ...
  <A, E, R, I, S, ES, RS>(
    ...
    fromChannel(Channel.provideServiceEffect(self.channel, key, service))
);
...
export const updateService: {
  ...
  <A, E, R, I, S>(
    ...
    updateContext(self, (context) =>
      Context.add(context, service, f(Context.get(context, service)))
    )
);
...
export const withSpan: {
  ...
  return (self: Stream<any, any, any>) =>
...
export const bindEffect: {
  ...
  > =>
    mapEffect(
      self,
      (a) => Effect.map(f(a), (b) => ({ ...a, [tag]: b }) as any),
      options
    )
);
 ...
 * @since 2.0.0
 */
export const runSum = <E, R>(
  self: Stream<number, E, R>
): Effect.Effect<number, E, R> =>
  Channel.runFold(
    self.channel,
    () => 0,
...
export const runFold: {
  <Z, A>(
    initial: LazyArg<Z>,
    f: (acc: Z, a: A) => Z
  ): <E, R>(self: Stream<A, E, R>) => Effect.Effect<Z, E, R>;
  <A, E, R, Z>(
    self: Stream<A, E, R>,
    initial: LazyArg<Z>,
    f: (acc: Z, a: A) => Z
 ...
 * @since 2.0.0
 */
export const runDrain = <A, E, R>(
  self: Stream<A, E, R>
): Effect.Effect<void, E, R> => Channel.runDrain(self.channel);
...
export const toReadableStreamEffect: {
  ...
  <A, E, R>(
    ...
    Effect.map(Effect.context<R>(), (context) =>
      toReadableStreamWith(self, context, options)
    )
);
...
export const toAsyncIterableWith: {
  <XR>(
    context: Context.Context<XR>
  ): <A, E, R extends XR>(self: Stream<A, E, R>) => AsyncIterable<A>;
  <A, E, XR, R extends XR>(
    self: Stream<A, E, R>,
    context: Context.Context<XR>
  ): AsyncIterable<A>;
  ...
  <A, E, XR, R extends XR>(
    ...
    [Symbol.asyncIterator]() {
      ...
      return {
        ...
        return(_) {
          return runPromise(
            Effect.as(Scope.close(scope, Exit.void), {
              done: true,
              value: undefined,
            })
          );
...
export const toAsyncIterableEffect = <A, E, R>(
  ...
  Effect.map(Effect.context<R>(), (context) =>
    toAsyncIterableWith(self, context)
  );
...
export const runIntoPubSub: {
  ...
  <A, E, R>(
    ...
    Channel.runIntoPubSubArray(self.channel, pubsub, options)
);
...
export const toPubSub: {
  ...
  <A, E, R>(
    ...
    Channel.toPubSubArray(self.channel, options)
);
...
export const runIntoQueue: {
  <A, E>(
    queue: Queue.Queue<A, E | Cause.Done>
  ): <R>(self: Stream<A, E, R>) => Effect.Effect<void, never, R>;
  <A, E, R>(
    self: Stream<A, E, R>,
    queue: Queue.Queue<A, E | Cause.Done>
  ): Effect.Effect<void, never, R>;
