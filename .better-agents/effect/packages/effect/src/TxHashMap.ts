...
export declare namespace TxHashMap {
   ...
   * @since 4.0.0
   */
  export type Key<T extends TxHashMap<any, any>> =
    T extends TxHashMap<infer K, any> ? K : never;
   ...
   * @since 4.0.0
   */
  export type Value<T extends TxHashMap<any, any>> =
    T extends TxHashMap<any, infer V> ? V : never;
 ...
 * @since 4.0.0
 */
export const isNonEmpty = <K, V>(
  self: TxHashMap<K, V>
): Effect.Effect<boolean> => Effect.map(isEmpty(self), (empty) => !empty);
  ...
  <K, V>(
    ...
    Effect.gen(function* () {
      ...
      if (Option.isSome(newValue)) {
        yield* TxRef.set(
          self.ref,
          HashMap.set(currentMap, key, newValue.value)
        );
...
export const removeMany: {
  <K1 extends K, K>(
    keys: Iterable<K1>
  ): <V>(self: TxHashMap<K, V>) => Effect.Effect<void>;
  <K1 extends K, K, V>(
    self: TxHashMap<K, V>,
    keys: Iterable<K1>
  ): Effect.Effect<void>;
...
export const setMany: {
  ...
  <K1 extends K, K, V1 extends V, V>(
    ...
    TxRef.update(self.ref, (map) => HashMap.setMany(map, entries))
);
...
export const getHash: {
  ...
  <K1 extends K, K, V>(
    ...
    TxRef.get(self.ref).pipe(
      Effect.map((map) => HashMap.getHash(map, key, hash))
    )
);
...
export const hasHash: {
  ...
  <K1 extends K, K, V>(
    ...
    TxRef.get(self.ref).pipe(
      Effect.map((map) => HashMap.hasHash(map, key, hash))
    )
);
...
export const findFirst: {
  ...
  <K, V>(
    ...
    TxRef.get(self.ref).pipe(
      Effect.map((map) => HashMap.findFirst(map, predicate))
    )
);
 ...
 * @since 4.0.0
 */
export const toValues = <K, V>(
  self: TxHashMap<K, V>
): Effect.Effect<Array<V>> => values(self);

/**
 * Helper function to create a TxHashMap from an existing HashMap
 */
const fromHashMap = <K, V>(
