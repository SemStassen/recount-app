...
class OptionalNode<S, A> {
  ...
  constructor(
    get: (s: S) => Result.Result<A, string>,
...
function pushNormalized(
  ...
  if (last) {
    ...
    if (last._tag === "CheckNode" && node._tag === "CheckNode") {
      // fuse Checks
      acc[acc.length - 1] = new CheckNode<any>([
        ...last.checks,
        ...node.checks,
      ]);
      return;
...
type ForbidUnion<A, Message extends string> =
...
export interface Optional<in out S, in out A> {
  ...
  tag<
    S,
    A extends { readonly _tag: SchemaAST.LiteralValue },
    Tag extends A["_tag"],
  >(
    this: Prism<S, A>,
    tag: Tag
 ...
 * @since 4.0.0
 */
export interface Traversal<in out S, in out A> extends Optional<
  S,
  ReadonlyArray<A>
> {}
...
class OptionalImpl<S, A> implements Optional<S, A> {
  ...
  refine<B extends A>(
    ...
    return make(
      compose(
        this.node,
        new CheckNode([SchemaAST.makeFilterByGuard(refinement, annotations)])
      )
    );
  }
  tag(tag: string): any {
        ...
        new PrismNode(
          (s) =>
            s._tag === tag
              ? Result.succeed(s)
              : Result.fail(
                  `Expected ${format(tag)} tag, got ${format(s._tag)}`
                ),
          identity
        )
      )
  ...
  pick(keys: any) {
    return this.compose(
      makeLens(Struct.pick(keys), (p, a) => ({ ...a, ...p }))
    );
  }
  omit(keys: any) {
    return this.compose(
      makeLens(Struct.omit(keys), (o, a) => ({ ...a, ...o }))
    );
  }
  notUndefined(): Prism<S, Exclude<A, undefined>> {
  ...
  switch (node._tag) {
    ...
    case "CheckNode":
      return {
        _tag: "PrismNode",
        get: (s: any) =>
          Result.mapError(SchemaAST.runChecks(node.checks, s), String),
        set: identity,
      };
    case "CompositionNode": {
      const ops = node.nodes.map(recur);
      const _tag = ops.reduce<Op["_tag"]>(
        (tag, op) => getCompositionTag(tag, op._tag),
        "IsoNode"
      );
      return {
        _tag,
