...
export interface Duration
  extends Equal.Equal, Pipeable, Inspectable.Inspectable {
 ...
 * @since 4.0.0
 */
export const fromInput: (u: Input) => Option.Option<Duration> =
  Option.liftThrowable(fromInputUnsafe);

const zeroDurationValue: DurationValue = { _tag: "Millis", millis: 0 };
const infinityDurationValue: DurationValue = { _tag: "Infinity" };
const negativeInfinityDurationValue: DurationValue = {
  _tag: "NegativeInfinity",
};
...
const make = (input: number | bigint): Duration => {
  ...
  if (typeof input === "number") {
    if (isNaN(input) || input === 0 || Object.is(input, -0)) {
      ...
      duration.value =
        input > 0 ? infinityDurationValue : negativeInfinityDurationValue;
 ...
 * @since 2.0.0
 */
export const toNanos: (self: Input) => Option.Option<bigint> =
  Option.liftThrowable(toNanosUnsafe);
...
export const match: {
  ...
  <A, B, C, D = C>(
    ...
    switch (self.value._tag) {
      ...
      case "NegativeInfinity":
        return (
          options.onNegativeInfinity ??
          (options.onInfinity as unknown as () => D)
        )();
    }
  }
);
...
export const matchPair: {
  ...
  <A, B, C>(
    ...
    if (
      ...
      self.value._tag === "NegativeInfinity" ||
      that.value._tag === "Infinity" ||
      that.value._tag === "NegativeInfinity"
    )
...
} = dual(2, (self: Duration, by: number): Option.Option<Duration> => {
...
export const divideUnsafe: {
  ...
  return match(self, {
    ...
    onNegativeInfinity: () =>
      by > 0 ? negativeInfinity : by < 0 ? infinity : zero,
  });
});
...
export const times: {
  ...
  (self: Duration, times: number): Duration =>
    match(self, {
      ...
      onInfinity: () =>
        times > 0 ? infinity : times < 0 ? negativeInfinity : zero,
      onNegativeInfinity: () =>
        times > 0 ? negativeInfinity : times < 0 ? infinity : zero,
    })
);
...
export const equals: {
  (that: Duration): (self: Duration) => boolean;
  (self: Duration, that: Duration): boolean;
} = dual(2, (self: Duration, that: Duration): boolean =>
