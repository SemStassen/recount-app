 ...
 * @since 2.0.0
 */
export interface MutableHashMap<out K, out V>
  extends Iterable<[K, V]>, Pipeable, Inspectable {
 ...
 * @since 4.0.0
 */
export const isMutableHashMap = <K, V>(
  value: unknown
): value is MutableHashMap<K, V> => hasProperty(value, TypeId);

const MutableHashMapProto: Omit<
  MutableHashMap<unknown, unknown>,
  "backing" | "buckets" | "bucketsSize"
> = {
  [TypeId]: TypeId,
  [Symbol.iterator](
    this: MutableHashMap<unknown, unknown>
...
>(2, <K, V>(self: MutableHashMap<K, V>, key: K): Option.Option<V> => {
  ...
  if (refKey !== undefined) {
    return self.backing.has(refKey)
      ? Option.some(self.backing.get(refKey)!)
      : Option.none();
  }
...
const isSimpleKey = (u: unknown): boolean =>
 ...
 * @since 3.8.0
 */
export const keys = <K, V>(self: MutableHashMap<K, V>): Iterable<K> =>
  self.backing.keys();
 ...
 * @since 3.8.0
 */
export const values = <K, V>(self: MutableHashMap<K, V>): Iterable<V> =>
  self.backing.values();
 ...
 * @since 2.0.0
 */
export const isEmpty = <K, V>(self: MutableHashMap<K, V>): boolean =>
  self.backing.size === 0;
...
export const forEach: {
  ...
  2,
  <K, V>(self: MutableHashMap<K, V>, f: (value: V, key: K) => void) => {
