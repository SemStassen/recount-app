 ...
 * @since 4.0.0
 */
export type PrimitiveTree = Schema.Tree<
  null | number | boolean | bigint | symbol | string
>;
 ...
 * @since 4.0.0
 */
export const $DeclarationMeta = Schema.Union([$DateMeta, $SizeMeta]).annotate({
 ...
 * @since 4.0.0
 */
export const fromAST: (ast: SchemaAST.AST) => Document =
  InternalRepresentation.fromAST;
 ...
 * @since 4.0.0
 */
export const fromASTs: (
  asts: readonly [SchemaAST.AST, ...Array<SchemaAST.AST>]
) => MultiDocument = InternalRepresentation.fromASTs;
 ...
 * @since 4.0.0
 */
export const DocumentFromJson: Schema.Codec<Document, Schema.Json> =
  Schema.toCodecJson($Document);
 ...
 * @since 4.0.0
 */
export const MultiDocumentFromJson: Schema.Codec<MultiDocument, Schema.Json> =
  Schema.toCodecJson($MultiDocument);
...
export const toSchemaDefaultReviver: Reviver<Schema.Top> = (s, recur) => {
  ...
  if (
    Predicate.isObject(typeConstructor) &&
    typeof typeConstructor._tag === "string"
  ) {
    ...
    switch (r._tag) {
      ...
      case "Objects": {
        ...
        for (const ps of r.propertySignatures) {
          ...
          fields[ps.name] = ps.isMutable
            ? Schema.mutableKey(withOptional)
            : withOptional;
        }

        const indexSignatures = r.indexSignatures.map((is) =>
          Schema.Record(
            recur(is.parameter) as Schema.Record.Key,
            recur(is.type)
          )
        );
    ...
    switch (check._tag) {
      ...
      case "FilterGroup": {
        return Schema.makeFilterGroup(
          Arr.map(check.checks, toSchemaCheck),
          check.annotations
        );
      }
    }
  ...
  function toSchemaFilter(filter: Filter<Meta>): SchemaAST.Check<any> {
    ...
    switch (filter.meta._tag) {
      ...
      case "isPropertiesLengthBetween":
        return Schema.isPropertiesLengthBetween(
          filter.meta.minimum,
          filter.meta.maximum,
          a
        );
      case "isPropertyNames":
        return Schema.isPropertyNames(
          recur(filter.meta.propertyNames) as Schema.Record.Key,
          a
        );
...
export const toJsonSchemaDocument: (
  ...
  InternalRepresentation.toJsonSchemaDocument;
...
export const toJsonSchemaMultiDocument: (
  ...
  InternalRepresentation.toJsonSchemaMultiDocument;
...
export function toCodeDocument(
  ...
  return {
    ...
    references: {
      nonRecursives: nonRecursives.filter(
        ({ $ref }) => (referenceCount.get($ref) ?? 0) > 0
      ),
      recursives: Rec.filter(
        recursives,
        (_, $ref) => (referenceCount.get($ref) ?? 0) > 0
      ),
    },
    artifacts,
  };
  ...
  function addImport(importDeclaration: string) {
    if (
      !artifacts.some(
        (a) => a._tag === "Import" && a.importDeclaration === importDeclaration
      )
    ) {
  ...
  function recur(s: Representation): Code {
    ...
    switch (s._tag) {
      default:
        return makeCode(
          g.runtime +
            toRuntimeAnnotate(s.annotations) +
            toRuntimeBrand(s.annotations),
    ...
    switch (s._tag) {
      ...
      case "String": {
        ...
        if (
          ...
          return makeCode(
            `Schema.fromJsonString(${recur(contentSchema)})`,
            "string"
          );
        } else {
      ...
      case "UniqueSymbol": {
        ...
        return makeCode(
          `Schema.UniqueSymbol(${identifier})`,
          `typeof ${identifier}`
        );
      }
      case "ObjectKeyword":
      ...
      case "TemplateLiteral": {
        ...
        const type = toTypeParts(s.parts)
          ...
          .join(" | ");
        return makeCode(
          `Schema.TemplateLiteral([${parts.map((p) => p.runtime).join(", ")}])`,
          type
        );
      }
      case "Arrays": {
        ...
        if (Arr.isArrayNonEmpty(rest)) {
          ...
          return makeCode(
            `Schema.TupleWithRest(Schema.Tuple([${elements
              .map(
                (e) =>
                  toRuntimeIsOptional(e.isOptional, e.type.runtime) +
                  toRuntimeAnnotateKey(e.annotations)
              )
        ...
        if (iss.length === 0) {
          ...
          return makeCode(
            `Schema.StructWithRest(Schema.Struct({ ${pss.map((p) => p.runtime).join(", ")} }), [${iss
              .map(
                (is) =>
                  `Schema.Record(${is.parameter.runtime}, ${is.type.runtime})`
              )
              .join(", ")}])`,
            `{ ${pss.map((p) => p.Type).join(", ")}, ${iss
              .map(
                (is) => `readonly [x: ${is.parameter.Type}]: ${is.type.Type}`
              )
              .join(", ")} }`
          );
        }
  ...
  function toRuntimeChecks(checks: ReadonlyArray<Check<Meta>>): string {
    return checks
      .map(
        (c) => `.check(${toRuntimeCheck(c)})` + toRuntimeBrand(c.annotations)
      )
      .join("");
  }
...
const VALID_ASCII_UPPER_JAVASCRIPT_IDENTIFIER_REGEXP =
  /^[A-Z_$][A-Za-z0-9_$]*$/;
...
function toRuntimeBrand(
  annotations: Schema.Annotations.Annotations | undefined
): string {
  const brands = collectBrands(annotations);
  return brands.length > 0
    ? `.pipe(${brands.map((b) => `Schema.brand(${format(b)})`).join(", ")})`
    : "";
}

function toRuntimeAnnotate(
  annotations: Schema.Annotations.Annotations | undefined
): string {
...
function toRuntimeValue(
  value: undefined | number | boolean | bigint | Date
): string {
...
export function fromJsonSchemaMultiDocument(
  ...
  function resolveReference(
    $ref: string
  ): Exclude<Representation, { _tag: "Reference" }> {
    ...
    slot.value =
      value._tag === "Reference" ? resolveReference(value.$ref) : value;
    slot.state = 2;
    return slot.value;
  }
      ...
      switch (type) {
        ...
        case "array": {
          ...
          const elements: Array<Element> = (
            Array.isArray(js.prefixItems) ? js.prefixItems : []
          ).map((e, i) => ({
  ...
  function collectObjectsChecks(
    js: JsonSchema.JsonSchema
  ): Array<Check<ObjectsMeta>> {
    const checks: Array<Check<ObjectsMeta>> = [];
    if (typeof js.minProperties === "number") {
      checks.push({
        _tag: "Filter",
        meta: { _tag: "isMinProperties", minProperties: js.minProperties },
      });
    }
    if (typeof js.maxProperties === "number") {
      checks.push({
        _tag: "Filter",
        meta: { _tag: "isMaxProperties", maxProperties: js.maxProperties },
      });
    }
    if (js.propertyNames !== undefined) {
      ...
      checks.push({
        _tag: "Filter",
        meta: { _tag: "isPropertyNames", propertyNames },
      });
    }
    return checks;
  }
    ...
    switch (a._tag) {
        ...
        switch (b._tag) {
          ...
          case "String": {
            ...
            return {
              ...
              ...combineAnnotations(
                a.annotations,
                checks ? undefined : b.annotations
              ),
            };
          }
          case "Literal":
      ...
      case "Number":
        switch (b._tag) {
          ...
          case "Number": {
            const checks = combineNumberChecks(
              a.checks,
              b.checks,
              b.annotations
            );
      ...
      case "Arrays":
        switch (b._tag) {
          ...
          case "Arrays": {
            const checks = combineArraysChecks(
              a.checks,
              b.checks,
              b.annotations
            );
      ...
      case "Objects":
        switch (b._tag) {
          ...
          case "Objects": {
            ...
            return {
              _tag: "Objects",
              propertySignatures: combinePropertySignatures(
                a.propertySignatures,
                b.propertySignatures
              ),
              indexSignatures: combineIndexSignatures(
                a.indexSignatures,
                b.indexSignatures
              ),
              checks: checks ?? a.checks,
  ...
  function collectProperties(
    ...
    const properties: Record<string, unknown> = Predicate.isObject(
      js.properties
    )
      ? js.properties
      : {};
  ...
  function collectIndexSignatures(
    ...
    if (
      js.additionalProperties === undefined ||
  ...
  function combineRest(
    a: ReadonlyArray<Representation>,
    b: ReadonlyArray<Representation>
...
function collectStringChecks(
  ...
  if (typeof js.minLength === "number") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isMinLength", minLength: js.minLength },
    });
  }
  if (typeof js.maxLength === "number") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isMaxLength", maxLength: js.maxLength },
    });
  }
  if (typeof js.pattern === "string") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isPattern", regExp: new RegExp(js.pattern) },
    });
  }
  return checks;
}

function collectNumberChecks(
  js: JsonSchema.JsonSchema
  ...
  if (typeof js.minimum === "number") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isGreaterThanOrEqualTo", minimum: js.minimum },
    });
  }
  if (typeof js.maximum === "number") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isLessThanOrEqualTo", maximum: js.maximum },
    });
  }
  if (typeof js.exclusiveMinimum === "number") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isGreaterThan", exclusiveMinimum: js.exclusiveMinimum },
    });
  }
  if (typeof js.exclusiveMaximum === "number") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isLessThan", exclusiveMaximum: js.exclusiveMaximum },
    });
  }
  if (typeof js.multipleOf === "number") {
    checks.push({
      _tag: "Filter",
      meta: { _tag: "isMultipleOf", divisor: js.multipleOf },
    });
  }
  return checks;
}

function collectArraysChecks(
  js: JsonSchema.JsonSchema
  ...
  if (js.prefixItems === undefined) {
    if (typeof js.minItems === "number") {
      checks.push({
        _tag: "Filter",
        meta: { _tag: "isMinLength", minLength: js.minItems },
      });
    }
    if (typeof js.maxItems === "number") {
      checks.push({
        _tag: "Filter",
        meta: { _tag: "isMaxLength", maxLength: js.maxItems },
      });
    }
  }
...
function collectAnnotations(
  ...
  if (typeof schema.contentMediaType === "string")
    as.contentMediaType = schema.contentMediaType;
    ...
    while (stack.length > 0) {
      ...
      switch (r._tag) {
        ...
        case "Arrays":
          ...
          break;
        case "Objects":
          for (const propertySignature of r.propertySignatures)
            stack.push(propertySignature.type);
