...
export const releaseSubscriber: {
  ...
  <A>(
    ...
    Effect.gen(function* () {
      yield* TxRef.update(self.subscribersRef, (subs) =>
        subs.filter((q) => q !== queue)
      );
 ...
 * @since 4.0.0
 */
export const isTxPubSub = (u: unknown): u is TxPubSub<unknown> =>
