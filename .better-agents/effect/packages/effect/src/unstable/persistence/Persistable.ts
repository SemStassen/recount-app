 ...
 * @since 4.0.0
 */
export interface Persistable<A extends Schema.Top, E extends Schema.Top>
  extends PrimaryKey.PrimaryKey {
  readonly [symbol]: {
    readonly success: A;
    readonly error: E;
  };
...
export type SuccessSchema<A extends Any> =
  A["~effect/persistence/Persistable"]["success"];
 ...
 * @since 4.0.0
 */
export type Success<A extends Any> =
  A["~effect/persistence/Persistable"]["success"]["Type"];
...
export type ErrorSchema<A extends Any> =
  A["~effect/persistence/Persistable"]["error"];
 ...
 * @since 4.0.0
 */
export type Error<A extends Any> =
  A["~effect/persistence/Persistable"]["error"]["Type"];
 ...
 * @since 4.0.0
 */
export type TimeToLiveFn<K extends Any> = (
...
export const Class =
  ...
  >(
    ...
    args: Types.EqualsWith<
      ...
      void,
      {
        readonly [P in keyof Config["payload"] as P extends "_tag"
          ? never
          : P]: Config["payload"][P];
      }
    >
    ...
    Request.Request<
      A["Type"],
      | E["Type"]
      | ("requestError" extends keyof Config
          ? Config["requestError"]
...
const exitSchemaCache = new WeakMap<
  Persistable<any, any>,
  Schema.Exit<any, any, Schema.Defect>
>();
