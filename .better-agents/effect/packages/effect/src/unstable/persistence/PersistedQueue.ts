          ...
          Effect.uninterruptibleMask(
            Effect.fnUntraced(function* (restore) {
              ...
              const exit = yield* Effect.exit(
                restore(f(decoded, { id: item.id, attempts: item.attempts }))
              );
 ...
 * @since 4.0.0
 */
export const layerStoreMemory: Layer.Layer<PersistedQueueStore> = Layer.sync(
  PersistedQueueStore,
  () => {
    type Entry = {
      readonly id: string;
...
export const makeStoreRedis = Effect.fnUntraced(function* (options?: {
    ...
    lookup: Effect.fnUntraced(function* (name: string) {
      ...
      yield* Effect.gen(function* () {
        while (true) {
          ...
          const results =
            takers.current === 0 ? null : yield* poll(takers.current);
          if (results === null) {
  ...
  yield* Effect.gen(function* () {
    while (true) {
      ...
      yield* Effect.ignore(
        expireAll(Array.from(activeLockKeys), lockExpirationMillis)
      );
    }
  }).pipe(
    Effect.forkScoped,
            ...
            return Effect.addFinalizer(
              Exit.match({
                ...
                onSuccess: () => {
                  activeLockKeys.delete(lock);
                  return Effect.orDie(
                    complete(keyPending(options.name), lock, element.id)
                  );
                },
              })
            );
...
const offerRedis = Redis.script(
  (...args: [keyQueue: string, keyIds: string, id: string, payload: string]) =>
    args,
  {
    lua: `
...
> = Effect.fnUntraced(function* (options) {
  ...
  const lockExpirationSql = sql.literal(
    Math.ceil(Duration.toSeconds(lockExpiration)).toString()
  );
  const workerId = crypto.randomUUID();
  ...
  yield* sql.onDialectOrElse({
    ...
    mysql: () =>
      sql`CREATE UNIQUE INDEX ${sql(`idx_${tableName}_id`)} ON ${tableNameSql} (id)`.pipe(
        Effect.ignore
      ),
    orElse: () =>
      sql`CREATE UNIQUE INDEX IF NOT EXISTS ${sql(`idx_${tableName}_id`)} ON ${tableNameSql} (id)`,
  });
  ...
  yield* sql.onDialectOrElse({
    ...
    mysql: () =>
      ...
      )} ON ${tableNameSql} (queue_name, completed, attempts, acquired_at)`.pipe(
        Effect.ignore
      ),
    orElse: () =>
    ...
    lookup: Effect.fnUntraced(function* ({ maxAttempts, name }: QueueKey) {
      ...
      yield* Effect.gen(function* () {
        while (true) {
          ...
          yield* Effect.yieldNow;
          const results =
            takers.current === 0 ? [] : yield* poll(takers.current);
          if (results.length === 0) {
            yield* Effect.sleep(pollInterval);
            continue;
          }
