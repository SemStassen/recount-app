...
>(
  lookup: (
    key: K
  ) => Effect.Effect<Persistable.Success<K>, Persistable.Error<K>, R>,
  options: {
...
>(
  ...
  const inMemory = yield* Cache.makeWith(
    Effect.fnUntraced(function* (key: K) {
      const exit = yield* store.get(key) as Effect.Effect<
        Exit<Persistable.Success<K>, Persistable.Error<K>>
      >;
      if (exit) {
        return yield* exit;
      }
  ...
  return identity<PersistedCache<K, "lookup" extends ServiceMode ? R : never>>({
    ...
    invalidate: (key) =>
      Effect.flatMap(store.remove(key), () => Cache.invalidate(inMemory, key)),
  });
});
