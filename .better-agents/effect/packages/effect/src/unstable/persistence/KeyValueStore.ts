...
const ErrorTypeId =
  "~effect/persistence/KeyValueStore/KeyValueStoreError" as const;
 ...
 * @since 4.0.0
 */
export const KeyValueStore: Context.Service<KeyValueStore, KeyValueStore> =
      ...
      return make({
        ...
        set: (key: string, value: string | Uint8Array) =>
          upsert({
            ...
            value_type:
              typeof value === "string" ? ValueTypeString : ValueTypeUint8Array,
          }).pipe(
            Effect.mapError(
              (cause) =>
                new KeyValueStoreError({
        ...
        size: sql<{
          count: number;
        }>`SELECT COUNT(*) as count FROM ${table}`.pipe(
          Effect.mapError(
            (cause) =>
              new KeyValueStoreError({
                method: "size",
...
const SchemaStoreTypeId =
  "~effect/persistence/KeyValueStore/SchemaStore" as const;
 ...
 * @since 4.0.0
 */
export const toSchemaStore = <S extends Schema.Top>(
  self: KeyValueStore,
  schema: S
): SchemaStore<S> => {
