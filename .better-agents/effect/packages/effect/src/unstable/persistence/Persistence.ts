...
export interface PersistenceStore {
  ...
  readonly setMany: <A extends Schema.Top, E extends Schema.Top>(
    entries: Iterable<
      readonly [Persistable.Persistable<A, E>, Exit.Exit<A["Type"], E["Type"]>]
    >
  ) => Effect.Effect<
    void,
    PersistenceError | Schema.SchemaError,
...
export class BackingPersistence extends Context.Service<
  BackingPersistence,
  {
    readonly make: (
      storeId: string
    ) => Effect.Effect<BackingPersistenceStore, never, Scope.Scope>;
  }
...
export interface BackingPersistenceStore {
  readonly get: (
    key: string
  ) => Effect.Effect<object | undefined, PersistenceError>;
  readonly getMany: (
        ...
        return identity<PersistenceStore>({
          get: (key) =>
            Effect.flatMap(storage.get(PrimaryKey.value(key)), (result) =>
              result
                ? Persistable.deserializeExit(key, result)
                : Effect.undefined
            ),
          getMany: Effect.fnUntraced(function* (keys) {
            ...
            const out = new Array<Exit.Exit<unknown, unknown> | undefined>(
              primaryKeys.length
            );
          ...
          set(key, value) {
            ...
            if (Duration.isZero(ttl) || Duration.isNegative(ttl))
              return Effect.void;
          ...
          setMany: Effect.fnUntraced(function* (entries) {
            const encodedEntries =
              Arr.empty<
                readonly [string, object, Duration.Duration | undefined]
              >();
 ...
 * @since 4.0.0
 */
export const layerBackingMemory: Layer.Layer<BackingPersistence> = Layer.sync(
  BackingPersistence
)(() => {
  const stores = new Map<
    string,
    Map<string, readonly [object, expires: number | null]>
  >();
      ...
      Effect.clockWith((clock) => {
        ...
        const unsafeGet = (key: string): object | undefined => {
          ...
          if (value === undefined) {
            ...
            value[1] !== null &&
            value[1] <= clock.currentTimeMillisUnsafe()
          ) {
            map.delete(key);
            return undefined;
          }
            ...
            sql<{
              ...
              Effect.flatMap((rows) => {
                ...
                return Effect.succeed(
                  out as Arr.NonEmptyArray<object | undefined>
                );
              })
            ),
          set: (key, value, ttl) =>
        ...
        Effect.sync(() => {
          ...
          return identity<BackingPersistenceStore>({
            ...
            getMany: (keys) =>
              Effect.flatMap(
                Effect.mapError(
                  redis.send<Arr.NonEmptyArray<string>>(
                    "mget",
                    ...keys.map(prefixed)
                  ),
                  ({ cause }) =>
                ...
                (values) => {
                  const out = new Array<object | undefined>(
                    keys.length
                  ) as Arr.NonEmptyArray<object | undefined>;
      ...
      make: (storeId) =>
        Effect.sync(() => {
            ...
            Effect.flatMap(
              ...
              (str) => {
                ...
                try {
                  ...
                  if (
                    expires !== null &&
                    expires <= clock.currentTimeMillisUnsafe()
                  ) {
                    return Effect.as(
                      Effect.ignore(store.remove(key)),
                      undefined
                    );
                  }
          ...
          return identity<BackingPersistenceStore>({
            get,
            getMany: (keys) =>
              Effect.forEach(keys, get, { concurrency: "unbounded" }),
            set: (key, value, ttl) =>
              Effect.suspend(() => {
                try {
                  return Effect.mapError(
                    store.set(
                      key,
                      JSON.stringify([value, unsafeTtlToExpires(clock, ttl)])
                    ),
                    (cause) =>
                      new PersistenceError({
                        message: `Failed to set key ${key} in backing store`,
 ...
 * @since 4.0.0
 */
export const layerKvs: Layer.Layer<
  Persistence,
  never,
  KeyValueStore.KeyValueStore
> = layer.pipe(Layer.provide(layerBackingKvs));
 ...
 * @since 4.0.0
 */
export const layerRedis: Layer.Layer<Persistence, never, Redis.Redis> =
  layer.pipe(Layer.provide(layerBackingRedis));
 ...
 * @since 4.0.0
 */
export const layerSqlMultiTable: Layer.Layer<
  Persistence,
  never,
  SqlClient.SqlClient
> = layer.pipe(Layer.provide(layerBackingSqlMultiTable));
 ...
 * @since 4.0.0
 */
export const layerSql: Layer.Layer<Persistence, never, SqlClient.SqlClient> =
  layer.pipe(Layer.provide(layerBackingSql));
