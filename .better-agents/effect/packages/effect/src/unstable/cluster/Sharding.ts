...
import {
  ...
  SingletonRegistered,
} from "./ShardingRegistrationEvent.ts";
 ...
 * @since 4.0.0
 */
export class Sharding extends Context.Service<
  Sharding,
  {
    /**
...
const make = Effect.gen(function* () {
  ...
  const services = Context.omit(Scope.Scope)(
    yield* Effect.context<ShardingConfig>()
  );
  const runFork = flow(
    ...
    const releaseShard = Effect.fnUntraced(
      function* (shardId: ShardId) {
        ...
        for (const state of entityManagers.values()) {
          ...
          fibers.push(
            yield* Effect.forkScoped(state.manager.interruptShard(shardId))
          );
        }
    ...
    yield* Effect.gen(function* () {
      ...
      while (true) {
        ...
        if (acquired.length > 0) {
          ...
          // update metrics
          ClusterMetrics.shards.updateUnsafe(
    ...
    yield* Effect.gen(function* () {
      ...
      const send = Effect.catchCause(
        Effect.suspend(() => {
          ...
          if (!state) {
            const sinceStart =
              clock.currentTimeMillisUnsafe() - storageStartMillis;
          ...
          const sendWithRetry: Effect.Effect<void, EntityNotAssignedToRunner> =
            Effect.catchTags(
              Effect.suspend(() => {
                if (!MutableHashSet.has(acquiredShards, address.shardId)) {
                  return Effect.fail(
                    new EntityNotAssignedToRunner({ address })
                  );
                }
    ...
    while (true) {
      ...
      if (changed) {
        ...
        hashRings.forEach((ring, group) => {
          const newAssignments = HashRing.getShards(
            ring,
            config.shardsPerGroup
          );
          ...
          switch (options.message._tag) {
            case "Request": {
              ...
              const message = new Message.OutgoingRequest({
                ...
                )(rpc.annotations, envelope as any),
              });
              if (!options.discard) {
  ...
  const registerSingleton: Sharding["Service"]["registerSingleton"] =
  ...
  const registerEntity: Sharding["Service"]["registerEntity"] =
    Effect.fnUntraced(function* (entity, build, options) {
      const runnerAddress = getRunnerAddress();
...
const ClientAddressTag = Context.Service<EntityAddress>(
