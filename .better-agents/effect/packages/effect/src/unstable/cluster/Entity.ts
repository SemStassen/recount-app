...
const Proto = {
  ...
  annotateRpcsMerge<S>(
    ...
    return fromRpcGroup(
      this.type,
      this.protocol.annotateRpcsMerge(annotations)
    );
  },
  ...
  toLayerQueue<Rpcs extends Rpc.Any, R, RX = never>(
    ...
    const buildHandlers = Effect.gen({ self: this }, function* () {
      ...
      const streamHandler = (envelope: any) =>
        Effect.callback<any, any>((resume) => {
          ...
          Effect.map((streamOrQueue) =>
            Stream.isStream(streamOrQueue)
              ? streamOrQueue
              : Stream.fromQueue(streamOrQueue)
          ),
          Stream.unwrap
        );
      ...
      for (const rpc_ of this.protocol.requests.values()) {
        ...
        handlers[rpc._tag] = RpcSchema.isStreamSchema(rpc.successSchema)
          ? streamHandler
          : handler;
      }
...
> {
  ...
  get lastSentChunkValue(): Option.Option<Rpc.SuccessChunk<Rpc>> {
    return Option.map(this.lastSentChunk, (chunk) =>
      Arr.lastNonEmpty(chunk.values)
    );
  }
...
const shardingTag = Context.Service<Sharding, Sharding["Service"]>(
 ...
 * @since 4.0.0
 */
export class KeepAliveLatch extends Context.Service<
  KeepAliveLatch,
  Latch.Latch
