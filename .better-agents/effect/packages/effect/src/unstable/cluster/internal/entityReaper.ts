...
/** @internal */
export class EntityReaper extends Context.Service<EntityReaper>()(
      ...
      const register = (options: {
        ...
        Effect.suspend(() => {
          currentResolution = Math.max(
            Math.min(currentResolution, options.maxIdleTime),
            5000
          );
  ...
  static readonly layer: Layer.Layer<EntityReaper> = Layer.effect(this)(
    this.make
  );
}
