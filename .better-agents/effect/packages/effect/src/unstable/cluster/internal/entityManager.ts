...
>(
  ...
  options: {
    ...
    readonly defectRetryPolicy?:
      | Schedule.Schedule<any, unknown, never, never>
      | undefined;
  ...
  const mailboxCapacity =
    options.mailboxCapacity ?? config.entityMailboxCapacity;
  const clock = yield* Clock;
  const context = yield* Effect.context<
    Rpc.Services<Rpcs> | Rpc.Middleware<Rpcs> | RX
  >();
      ...
      function* (address: EntityAddress) {
            ...
            const handlers = yield* entity.protocol
              ...
              .pipe(
                ...
                Effect.tapError((cause) =>
                  Effect.logError("Defect building entity handlers", cause)
                ),
                Effect.retry(defectRetryPolicy)
              ) as Effect.Effect<Context.Context<Rpc.ToHandler<Rpcs>>>;

            const server = yield* RpcServer.makeNoSerialization(
              entity.protocol,
              {
                  ...
                  switch (response._tag) {
                    case "Exit": {
                      ...
                      if (
                        ...
                        return options.storage.unregisterReplyHandler(
                          request.message.envelope.requestId
                        );
                      }
                      return retryRespond(
                        4,
                    ...
                    case "Defect": {
                      return Effect.forkIn(
                        onDefect(Cause.die(response.defect)),
                        managerScope
                      );
                    }
                    case "ClientEnd": {
        ...
        function onDefect(cause: Cause.Cause<never>): Effect.Effect<void> {
          ...
          return Effect.logError("Defect in entity, restarting", cause).pipe(
            ...
            Effect.flatMap(() =>
              activeServers.has(address.entityId) ? effect : endLatch.open
            ),
            Effect.annotateLogs({
        ...
        const state: EntityState = {
          ...
          write(clientId, message) {
            if (writeRef.state.current._tag !== "Acquired") {
              return Effect.flatMap(writeRef.await, (write) =>
                write(clientId, message)
              );
            }
        ...
        > => {
          switch (message._tag) {
            case "IncomingRequestLocal": {
              ...
              const rpc = entityRpcs.get(
                message.envelope.tag
              ...
              if (rpc._tag === KeepAliveRpc._tag) {
                const msg = message as unknown as Message.IncomingRequestLocal<
                  typeof KeepAliveRpc
                >;
              ...
              if (
                ...
                return Effect.fail(
                  new MailboxFull({ address: message.envelope.address })
                );
              }
              ...
              entry = {
                ...
                lastSentChunk: Option.filter(
                  ...
                  (reply): reply is Reply.Chunk<Rpc.Any> =>
                    reply._tag === "Chunk"
                ),
                sequence: Option.match(message.lastSentReply, {
                  onNone: () => 0,
                  onSome: (reply) =>
                    reply._tag === "Chunk" ? reply.sequence + 1 : 0,
                }),
              };
            ...
            case "IncomingEnvelope": {
              const entry = server.activeRequests.get(
          ...
          onSuccess: (decoded) => {
            ...
            return sendLocal(
              new Message.IncomingRequestLocal({
                annotations: Context.get(
                  rpc.annotations,
                  ClusterSchema.Dynamic
                )(rpc.annotations, decoded.envelope as any),
...
const makeMessageDecode = <Type extends string, Rpcs extends Rpc.Any>(
  ...
  const decodeRequest = Effect.fnUntracedEager(function* (
    ...
    const lastSentReply = Option.isNone(message.lastSentReply)
      ? message.lastSentReply
      : Option.some(
          yield* Schema.decodeEffect(Reply.Reply(rpc))(
            message.lastSentReply.value
          )
        );
    return {
...
const retryRespond = <A, E, R>(
  ...
  times === 0
    ? effect
    : Effect.catch(effect, () =>
        Effect.delay(retryRespond(times - 1, effect), 200)
      );
