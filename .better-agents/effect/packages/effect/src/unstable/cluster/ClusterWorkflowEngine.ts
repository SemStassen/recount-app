...
export const make = Effect.gen(function* () {
    ...
    Entity.Entity<
      ...
      | Rpc.Rpc<
          ...
          Schema.Struct<{
            ...
            withTransaction: typeof Schema.Boolean;
          }>,
          Schema.declare<Workflow.Result<any, any>>
        >
  ...
  const sendDiscard = Effect.fnUntraced(function* (options: {
    ...
    yield* sharding.sendOutgoing(
      new Message.OutgoingRequest({
        ...
        )(options.rpc.annotations, envelope as any),
      }),
      true
    );
  });
  ...
  const replyForRequestId = Effect.fnUntraced(function* (
    ...
    if (last && last._tag === "WithExit") {
      return Option.some(
        last as WithExitEncoded<Workflow.ResultEncoded<any, any>>
      );
    }
  ...
  const engine = WorkflowEngine.makeUnsafe({
            ...
            Effect.gen(function* () {
              ...
              return {
                run: (request: Entity.Request<any>) => {
                  ...
                  return execute(
                    workflow.payloadSchema.make(payload) as object,
                    executionId
                  ).pipe(
                    Effect.onExit((exit) => {
                ...
                activity(request: Entity.Request<any>) {
                  ...
                  return Effect.gen(function* () {
                    ...
                    Effect.catchCause((cause) => {
                      ...
                      // client requested it
                      const suspend = cause.reasons.some(
                        (f) =>
                          f._tag === "Interrupt" &&
                          f.annotations.has(ClientAbort.key)
                      );
                      if (suspend) {
    ...
    poll: Effect.fnUntraced(function* (workflow, executionId) {
      ...
      const exitSchema = Schema.toCodecJson(
      ...
      const exit = yield* Schema.decodeUnknownEffect(exitSchema)(
        reply.value.exit
      ) as Effect.Effect<Exit.Exit<any, any>, Schema.SchemaError>;
      return Option.some(yield* exit);
    }, Effect.orDie),

    interrupt: (workflow, executionId) =>
      Effect.asVoid(interrupt(workflow, executionId)),
    ...
    deferredResult: (deferred) =>
      WorkflowEngine.WorkflowInstance.pipe(
        ...
        Effect.map((reply) => {
          ...
          return Option.some(
            decoded.exit._tag === "Success" ? decoded.exit.value : decoded.exit
          );
        }),
        Effect.retry({
          while: (e) => e._tag === "PersistenceError",
...
const ActivityRpc = Rpc.make("activity", {
  ...
  .annotate(ClusterSchema.Dynamic, (annotations, request) =>
...
const decodeDeferredWithExit = Schema.decodeSync(
  Schema.toCodecJson(Reply.WithExit.schema(DeferredRpc))
);
