 ...
 * @since 4.0.0
 */
export class Runners extends Context.Service<
  Runners,
  {
    /**
    ...
    ) => Effect.Effect<void, RunnerUnavailable>;
    ...
    if (message._tag === "OutgoingEnvelope") {
      ...
      return storage
        ...
        .pipe(
          ...
          Effect.andThen(
            entry
              ? Effect.andThen(entry.latch.open, afterPersist(message, false))
    ...
    return Effect.flatMap(
      ...
      MessageStorage.SaveResult.$match({
        ...
        Duplicate: ({ lastReceivedReply, originalId }) => {
          ...
          // as the response.
          if (
            Option.isSome(lastReceivedReply) &&
            lastReceivedReply.value._tag === "WithExit"
          ) {
            return message.respond(
              lastReceivedReply.value.withRequestId(message.envelope.requestId)
            );
          }
          ...
          return afterPersist(
            new Message.OutgoingRequest({
              ...
              respond(reply) {
                ...
                return message.respond(
                  reply.withRequestId(message.envelope.requestId)
                );
              },
            }),
            true
  ...
  if (storage !== MessageStorage.noop) {
    yield* Effect.gen(function* () {
      ...
      while (true) {
        ...
        const replies = yield* storage
          .repliesFor(waitingStorageRequests.values())
          .pipe(
  ...
  return Runners.of({
    ...
    notify(options_) {
      ...
      return notifyWith(message, (message, duplicate) => {
        ...
        return options
          .notify(options_)
          .pipe(Effect.andThen(replyFromStorage(message)));
      });
    },
    notifyLocal(options) {
...
> = make({
  send: ({ message }) =>
    Effect.fail(
      new EntityNotAssignedToRunner({ address: message.envelope.address })
    ),
  notify: () => Effect.void,
  ping: () => Effect.void,
...
> = Layer.effect(Runners, makeNoop).pipe(
 ...
 * @since 4.0.0
 */
export interface RpcClient extends RpcClient_.FromGroup<
  typeof Rpcs,
  RpcClientError
> {}
...
> = Effect.gen(function* () {
  ...
  return yield* make({
    ...
    notify({ address, message }) {
      ...
      const encode: Effect.Effect<
        Envelope.AckChunk | Envelope.Interrupt | Envelope.PartialRequest
      > =
...
export class RpcClientProtocol extends Context.Service<
  ...
  (
    address: RunnerAddress
  ) => Effect.Effect<RpcClient_.Protocol["Service"], never, Scope>
