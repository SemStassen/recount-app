...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const token = yield* fs
      .readFileString("/var/run/secrets/kubernetes.io/serviceaccount/token")
      .pipe(Effect.option);
    return (yield* HttpClient.HttpClient).pipe(
      HttpClient.mapRequest(
        HttpClientRequest.prependUrl("https://kubernetes.default.svc/api")
      ),
      token._tag === "Some"
        ? HttpClient.mapRequest(
            HttpClientRequest.bearerToken(token.value.trim())
          )
        : identity,
...
> = Effect.fnUntraced(function* (options?: {
  ...
  const getPods = HttpClientRequest.get(
    ...
    options?.labelSelector
      ? HttpClientRequest.setUrlParam("labelSelector", options.labelSelector)
      : identity
  );
  ...
  return yield* client.execute(getPods).pipe(
    ...
    Effect.tapCause((cause) =>
      Effect.logWarning("Failed to fetch pods from Kubernetes API", cause)
    ),
...
export const makeCreatePod = Effect.gen(function* () {
  ...
  return Effect.fnUntraced(function* (spec: v1.Pod) {
    ...
    const readPodRaw = HttpClientRequest.get(
    ...
    const createPod = HttpClientRequest.post(
    ...
    const deletePod = HttpClientRequest.delete(
      `/v1/namespaces/${namespace}/pods/${name}`
    ).pipe(
      client.execute,
 ...
 * @since 4.0.0
 */
export class PodStatus extends Schema.Class<PodStatus>(
  "@effect/cluster/K8sHttpClient/PodStatus"
)({
  phase: Schema.String,
