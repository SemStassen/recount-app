...
export class MessageStorage extends Context.Service<
  MessageStorage,
  {
     ...
     */
    readonly unregisterReplyHandler: (
      requestId: Snowflake.Snowflake
    ) => Effect.Effect<void>;
 ...
 * @since 4.0.0
 */
export type SaveResult<R extends Rpc.Any> =
  | SaveResult.Success
  | SaveResult.Duplicate<R>;
...
export declare namespace SaveResult {
  ...
  export interface Constructor extends Data.TaggedEnum.WithGenerics<1> {
    readonly taggedEnum: SaveResult<
      this["A"] extends Rpc.Any ? this["A"] : never
    >;
  }
}
...
export const make = (
  ...
  Effect.sync(() => {
    type ReplyHandler = {
      readonly message:
        | Message.OutgoingRequest<any>
        | Message.IncomingRequest<any>;
      readonly shardSet: Set<ReplyHandler>;
      readonly respond: (
        reply: Reply.ReplyWithContext<any>
      ) => Effect.Effect<void, PersistenceError | MalformedMessage>;
      readonly resume: (
        effect: Effect.Effect<void, EntityNotAssignedToRunner>
      ) => void;
    };
...
export const makeEncoded: (
  ...
  Effect.fnUntraced(function* (encoded: Encoded) {
    ...
    const storage: MessageStorage["Service"] = yield* make({
      saveRequest: (message) =>
        Message.serializeEnvelope(message).pipe(
          ...
          Effect.flatMap((result) => {
            if (
              result._tag === "Success" ||
              Option.isNone(result.lastReceivedReply)
            ) {
            ...
            return Schema.decodeEffect(schema)(
              result.lastReceivedReply.value
            ).pipe(
      ...
      saveReply: (reply) =>
        Effect.flatMap(Reply.serialize(reply), encoded.saveReply),
      ...
      unprocessedMessages(shardIds) {
        ...
        return Effect.flatMap(
          Effect.suspend(() =>
            encoded.unprocessedMessages(shards, clock.currentTimeMillisUnsafe())
          ),
      ...
      unprocessedMessagesById(messageIds) {
        ...
        return Effect.flatMap(
          Effect.suspend(() =>
            encoded.unprocessedMessagesById(
              ids,
...
export class MemoryDriver extends Context.Service<MemoryDriver>()(
      ...
      const unprocessedWith = (predicate: Predicate<Envelope.Encoded>) => {
        ...
        for (const envelope of unprocessed) {
          ...
          if (envelope._tag === "Request") {
            ...
            messages.push({
              envelope,
              lastSentReply: Option.fromNullishOr(
      ...
      const encoded: Encoded = {
            ...
            if (existing) {
              return SaveResultEncoded.Duplicate({
                ...
                lastReceivedReply: Option.fromNullishOr(
                  existing.replies.length === 1 &&
                    existing.replies[0]._tag === "WithExit"
                    ? existing.replies[0]
        ...
        requestIdForPrimaryKey: (primaryKey) =>
          Effect.sync(() => {
            ...
            return Option.map(
              Option.fromNullishOr(entry?.envelope.requestId),
              Snowflake.Snowflake
            );
          }),
        ...
        repliesForUnfiltered: (requestIds) =>
          Effect.sync(() =>
            requestIds.flatMap((id) => requests.get(String(id))?.replies ?? [])
          ),
        unprocessedMessages: (shardIds) =>
          Effect.sync(() => {
            ...
            for (let index = 0; index < journal.length; index++) {
              ...
              if (
                !unprocessed.has(envelope as any) ||
                !shardIds.includes(shardId.toString())
              ) {
                continue;
              }
              if (envelope._tag === "Request") {
        ...
        unprocessedMessagesById: (ids) =>
          Effect.sync(() => {
            ...
            return unprocessedWith((envelope) =>
              envelopeIds.has(envelope.requestId)
            );
          }),
        resetAddress: () => Effect.void,
   ...
   * @since 4.0.0
   */
  static readonly layer: Layer.Layer<MemoryDriver> = Layer.effect(this)(
    this.make
...
const EnvelopeWithReply: Schema.Struct<{
  readonly envelope: Schema.Decoder<
