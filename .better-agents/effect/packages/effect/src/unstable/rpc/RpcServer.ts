...
> = Effect.fnUntraced(function* <Rpcs extends Rpc.Any>(
      ...
      Effect.withFiber((requestFiber) => {
        ...
        switch (message._tag) {
          ...
          case "Interrupt": {
            ...
            if (fiber) {
              fiber.interruptUnsafe(
                requestFiber.id,
                RpcSchema.ClientAbort.annotation
              );
              return Effect.void;
            }
          ...
          default: {
            return sendDefect(
              client,
              `Unknown request tag: ${(message as any)._tag}`
            );
          }
        }
      }),
  ...
  const handleRequest = (
    ...
    requestFiber.context.mapUnsafe.forEach((value, key) =>
...
> = Effect.fnUntraced(function* <Rpcs extends Rpc.Any>(
  ...
  type Schemas = {
    readonly decode: (
      u: unknown
    ) => Effect.Effect<Rpc.Payload<Rpcs>, Schema.SchemaError>;
    ...
    if (!schemas) {
      ...
      schemas = {
        ...
        encodeChunk: Schema.encodeUnknownEffect(
          Schema.toCodecJson(
            Schema.Array(
              Option.isSome(streamSchemas)
                ? streamSchemas.value.success
                : Schema.Any
            )
          )
        ) as any,
        encodeExit: Schema.encodeUnknownEffect(
          Schema.toCodecJson(Rpc.exitSchema(rpc as any))
        ) as any,
        encodeDefect: Schema.encodeUnknownEffect(
          Schema.toCodecJson(rpc.defectSchema)
        ) as any,
  ...
  const handleEncode = <A, R>(
    ...
    (collector
      ? Effect.provideService(effect, Transferable.Collector, collector)
      : effect
    ).pipe(
      Effect.flatMap((a) =>
        send(client.id, onSuccess(a), collector && collector.clearUnsafe())
      ),
      Effect.catchCause((cause) => {
        client.schemas.delete(requestId);
        const defect = Cause.squash(
          Cause.map(cause, (e) => e.issue.toString())
        );
        return Effect.andThen(
          sendRequestDefect(client, requestId, encodeDefect, defect),
          server.write(client.id, {
            _tag: "Interrupt",
            requestId,
            interruptors: [],
          })
  ...
  const sendDefect = (client: Client, defect: unknown) =>
    Effect.catchCause(send(client.id, ResponseDefectEncoded(defect)), (cause) =>
  ...
  return yield* run((clientId, request) => {
    ...
    switch (request._tag) {
      case "Request": {
        const tag = Predicate.hasProperty(request, "tag")
          ? (request.tag as string)
          : "";
        ...
        return Effect.matchEffect(
          Effect.provideContext(
            schemas.decode(request.payload),
            schemas.context
          ),
          {
            onFailure: (error) =>
      ...
      default: {
        return sendDefect(
          client,
          `Unknown request tag: ${(request as any)._tag}`
        );
      }
    }
  }).pipe(
    Effect.tapCause((cause) =>
      Effect.logFatal("BUG: RpcServer protocol crashed", cause)
    ),
...
> = Effect.gen(function* () {
  ...
  > = Effect.gen(function* () {
    ...
    const data = yield* Effect.orDie<string | Uint8Array, any, never>(
      isBinary
        ? Effect.map(request.arrayBuffer, (buf) => new Uint8Array(buf))
        : request.text
    );
    const id = clientId++;
    const queue = yield* Queue.make<
      Uint8Array | FromServerEncoded,
      Cause.Done
    >();
    ...
    yield* Scope.addFinalizerExit(scope, () => {
      ...
      return Effect.forEach(
        requestIds,
        (requestId) =>
          writeRequest(id, { _tag: "Interrupt", requestId: String(requestId) }),
        { discard: true }
      );
    });
    ...
    try {
      ...
      for (let i = 0; i < decoded.length; i++) {
        ...
        if (message._tag === "Request") {
          ...
          (message as Types.Mutable<RequestEncoded>).headers =
  ...
  return yield* Protocol.make(
    Effect.fnUntraced(function* (writeRequest) {
      ...
      yield* stdio.stdin.pipe(
        ...
        Effect.ensuring(
          Effect.forkDetach(Fiber.interrupt(fiber), { startImmediately: true })
        ),
        Effect.forkScoped
      );
