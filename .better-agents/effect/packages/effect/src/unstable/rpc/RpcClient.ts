...
export declare namespace RpcClient {
  ...
  export type From<Rpcs extends Rpc.Any, E = never> = {
    ...
    >(
      ...
      options?: Rpc.Success<Current> extends Stream.Stream<
        infer _A,
        infer _E,
        infer _R
      >
  ...
  >(
    ...
    options?: Rpc.Success<Rpc.ExtractTag<Rpcs, Tag>> extends Stream.Stream<
      infer _A,
      infer _E,
      infer _R
    >
...
>(
  ...
  {
    readonly client: Flatten extends true
      ? RpcClient.Flat<Rpcs, E>
      : RpcClient<Rpcs, E>;
...
>(
  ...
  const generateRequestId =
    options?.generateRequestId ?? (() => requestIdCounter++ as RequestId);
      ...
      if (entry._tag === "Queue") {
        yield* (
          exit._tag === "Success"
            ...
            : Queue.failCause(entry.queue as any, exit.cause)
  ...
  const onRequest = (rpc: Rpc.AnyWithProps) => {
    ...
    return (
      ...
      const headers = opts?.headers
        ? Headers.fromInput(opts.headers)
        : Headers.empty;
  ...
  const onStreamRequest = Effect.fnUntraced(function* (
    ...
    const span = disableTracing
      ? undefined
      : yield* Effect.makeSpanScoped(`${spanPrefix}.${rpc._tag}`, {
          attributes: options.spanAttributes,
        });
    ...
    yield* Scope.addFinalizerExit(scope, (exit) => {
      ...
      return sendInterrupt(
        id,
        Exit.isFailure(exit) ? Array.from(Cause.interruptors(exit.cause)) : [],
        context
      );
    });
...
>(
  ...
  Flatten extends true
    ? RpcClient.Flat<Rpcs, RpcClientError>
    : RpcClient<Rpcs, RpcClientError>,
  never,
...
>(
  ...
  const { client, write } = yield* makeNoSerialization(group, {
    ...
    onFromClient({ message }) {
      switch (message._tag) {
        case "Request": {
          const rpc = group.requests.get(
            message.tag
          )! as any as Rpc.AnyWithProps;
          const collector = supportsTransferables
          ...
          const entry: ClientEntry = {
            rpc,
            context: collector
              ? Context.add(fiber.context, Transferable.Collector, collector)
              : fiber.context,
  ...
  yield* run(clientId, (message) => {
    switch (message._tag) {
      case "Chunk": {
        ...
        const entry = entries.get(requestId);
        if (!entry || Option.isNone(entry.schemas.decodeChunk))
          return Effect.void;
      ...
      case "Exit": {
        ...
        return entry.schemas.decodeExit(message.exit).pipe(
          ...
          Effect.matchCauseEffect({
            onSuccess: (exit) =>
              write({ _tag: "Exit", clientId: 0, requestId, exit }),
            onFailure: (cause) =>
              write({
                _tag: "Exit",
                clientId: 0,
      ...
      case "Defect": {
        return write({
          _tag: "Defect",
          clientId: 0,
          defect: decodeDefect(message.defect as any),
        });
      }
      case "ClientProtocolError": {
        const exit = Exit.fail(message.error);
        return Effect.forEach(entries.keys(), (requestId) =>
          write({ _tag: "Exit", clientId: 0, requestId, exit: exit as any })
        );
      }
      default: {
        return Effect.void;
...
interface RpcSchemas {
  readonly decodeChunk: Option.Option<
    ...
    ) => Effect.Effect<NonEmptyReadonlyArray<any>, Schema.SchemaError, unknown>
  >;
  readonly encodePayload: (
    payload: any
  ) => Effect.Effect<any, Schema.SchemaError, unknown>;
  readonly decodeExit: (
    encoded: unknown
  ) => Effect.Effect<Exit.Exit<any, any>, Schema.SchemaError, unknown>;
}
...
const rpcSchemas = (rpc: Rpc.AnyWithProps) => {
  ...
  entry = {
    decodeChunk: Option.map(streamSchemas, (streamSchemas) =>
      Schema.decodeUnknownEffect(
        Schema.toCodecJson(Schema.NonEmptyArray(streamSchemas.success))
      )
    ),
    encodePayload: Schema.encodeEffect(Schema.toCodecJson(rpc.payloadSchema)),
    decodeExit: Schema.decodeUnknownEffect(
      Schema.toCodecJson(Rpc.exitSchema(rpc as any))
    ),
  };
      ...
      const send = Effect.fnUntraced(function* (
        ...
        if (!isFramed) {
          ...
          const responses = yield* Effect.try({
            try: () => parser.decode(text),
              ...
              protocolDefect("Error decoding HTTP response", cause),
          });
          if (!Array.isArray(responses)) {
            return yield* protocolDefect(
              "Expected an array of responses",
              responses
            );
          }
        ...
        yield* Stream.runForEachArray(response.stream, (chunk) =>
          ...
          Effect.mapError((cause) =>
            cause instanceof RpcClientError ? cause : httpClientError(cause)
          )
        );
        if (!hasResponse) {
          return yield* emptyResponseError(request);
...
> =>
  Layer.effect(Protocol)(
    Effect.flatMap(HttpClient.HttpClient, (client) => {
      client = HttpClient.mapRequest(
        client,
        HttpClientRequest.prependUrl(options.url)
      );
      return makeProtocolHttp(
        options.transformClient ? options.transformClient(client) : client
      );
    })
  );
  ...
  Protocol.make(
    Effect.fnUntraced(function* (writeResponse, clientIds) {
      ...
      yield* Effect.suspend(() => {
        ...
        Option.isSome(hooks)
          ? Effect.ensuring(hooks.value.onDisconnect)
          : identity,
...
> =>
  Protocol.make(
    Effect.fnUntraced(function* (writeResponse, clientIds) {
      ...
      let workerId = 0;
      const initialMessage = yield* Effect.serviceOption(
      ...
      const acquire = Effect.gen(function* () {
        const id = workerId++;
        const backing = yield* worker.spawn<
          FromServerEncoded,
          FromClientEncoded | RpcWorker.InitialMessage.Encoded
        >(id);
          ...
          .run(
            ...
            },
            {
              onSpawn: Option.isSome(initialMessage)
                ? Effect.flatMap(initialMessage.value, ([value, transfers]) =>
                    Effect.orDie(
                      backing.send({ _tag: "InitialMessage", value }, transfers)
                    )
                  )
                : undefined,
            }
            ...
            return Pool.get(pool).pipe(
              Effect.flatMap((worker) => {
                ...
                return Effect.flatMap(
                  worker.send(request, transferables),
                  () => latch.await
                );
              }),
              Effect.scoped,
 ...
 * @since 4.0.0
 */
export class ConnectionHooks extends Context.Service<
  ConnectionHooks,
  {
