...
export interface RpcGroup<in out R extends Rpc.Any> extends Pipeable {
  ...
  middleware<M extends RpcMiddleware.AnyService>(
    middleware: M
  ): RpcGroup<Rpc.AddMiddleware<R, M>>;
  ...
  prefix<const Prefix extends string>(
    prefix: Prefix
  ): RpcGroup<Rpc.Prefixed<R, Prefix>>;
  ...
  toHandlers<Handlers extends HandlersFrom<R>, EX = never, RX = never>(
    ...
    EX,
    RX | HandlersServices<R, Handlers>
  >;
  ...
  toLayer<Handlers extends HandlersFrom<R>, EX = never, RX = never>(
    ...
    Rpc.ToHandler<R>,
    EX,
    Exclude<RX, Scope> | HandlersServices<R, Handlers>
  >;
  ...
  >(
    ...
    Rpc.Handler<Tag>,
    EX,
    Exclude<RX, Scope> | HandlerServices<R, Tag, Handler>
  >;
  ...
  true extends Rpc.IsStream<Rpcs, K>
    ? Handler extends (
      ...
      ?
          ...
          | Rpc.ExtractRequires<Rpcs, K>
      : never
    : Handler extends (
          ...args: any
        ) =>
          | Effect.Effect<infer _A, infer _E, infer _R>
          | Rpc.Wrapper<Effect.Effect<infer _A, infer _E, infer _R>>
      ?
          | Exclude<Rpc.ExcludeProvides<_R, Rpcs, K>, Scope>
          | Rpc.ExtractRequires<Rpcs, K>
      : never;
...
export type Rpcs<Group> =
  Group extends RpcGroup<infer R>
    ? string extends R["_tag"]
      ? never
      : R
    : never;
...
const RpcGroupProto = {
  ...
  toHandlers(
    this: RpcGroup<any>,
    build: Effect.Effect<Record<string, (request: any) => any>>
  ) {
  ...
  accessHandler(this: RpcGroup<any>, service: string) {
    return Effect.contextWith((parentContext: Context.Context<any>) => {
      ...
      const { handler, context } = parentContext.mapUnsafe.get(
        rpc.key
      ) as Rpc.Handler<any>;
  ...
  annotateRpcsMerge(this: RpcGroup<any>, context: Context.Context<any>) {
    ...
    for (const [tag, rpc] of this.requests) {
      requests.set(
        tag,
        rpc.annotateMerge(Context.merge(context, rpc.annotations))
      );
    }
    return makeProto({
      requests,
