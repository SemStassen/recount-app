 ...
 * @since 4.0.0
 */
export const isAtomRegistry = (u: unknown): u is AtomRegistry =>
  ...
  options?:
    | {
        readonly initialValues?:
          | Iterable<readonly [Atom.Atom<any>, any]>
          | undefined;
...
export const toStreamResult: {
  ...
  ): (self: AtomRegistry) => Stream.Stream<A, E>;
  <A, E>(
    self: AtomRegistry,
    atom: Atom.Atom<Result.AsyncResult<A, E>>
  ): Stream.Stream<A, E>;
...
export const mount: {
  <A>(
    atom: Atom.Atom<A>
  ): (self: AtomRegistry) => Effect.Effect<void, never, Scope.Scope>;
  <A>(
    self: AtomRegistry,
    atom: Atom.Atom<A>
...
class RegistryImpl implements AtomRegistry {
  ...
  readonly timeoutBuckets = new Map<
    number,
    readonly [nodes: Set<NodeImpl<any>>, handle: number]
  >();
  ...
  atomHasTtl(atom: Atom.Atom<any>): boolean {
    return (
      !atom.keepAlive &&
      atom.idleTTL !== 0 &&
      (atom.idleTTL !== undefined || this.defaultIdleTTL !== undefined)
    );
  }
  ...
  ensureNode<A>(atom: Atom.Atom<A>): NodeImpl<A> {
    ...
    if (typeof key === "string" && this.preloadedSerializable.has(key)) {
      ...
      const decoded = (atom as any as Atom.Serializable<any>)[
        SerializableTypeId
      ].decode(encoded);
  ...
  setNodeTimeout(node: NodeImpl<any>): void {
    ...
    const ttl =
      Math.ceil(idleTTL! / this.timeoutResolution) * this.timeoutResolution;
    const timestamp = Date.now() + ttl;
    const bucket =
      timestamp - (timestamp % this.timeoutResolution) + this.timeoutResolution;
...
class NodeImpl<A> {
  ...
  addParent(parent: NodeImpl<any>): void {
    ...
    if (this.previousParents !== undefined) {
      ...
      if (index !== -1) {
        this.previousParents[index] =
          this.previousParents[this.previousParents.length - 1];
...
const LifetimeProto: Omit<
  Lifetime<any>,
  "node" | "finalizers" | "disposed" | "isFn"
> = {
