...
export type Success<T extends Atom<any>> =
  T extends Atom<AsyncResult.AsyncResult<infer A, infer _>> ? A : never;
...
export type PullSuccess<T extends Atom<any>> =
  T extends Atom<PullResult<infer A, infer _>> ? A : never;
...
export type Failure<T extends Atom<any>> =
  T extends Atom<AsyncResult.AsyncResult<infer _, infer E>> ? E : never;
...
export const WritableTypeId: WritableTypeId =
  "~effect/reactivity/Atom/Writable";
...
export interface AtomContext {
  ...
  setResult<A, E, W>(
    this: AtomContext,
    atom: Writable<AsyncResult.AsyncResult<A, E>, W>,
    value: W
  ): Effect.Effect<A, E>;
  some<A>(this: AtomContext, atom: Atom<Option.Option<A>>): Effect.Effect<A>;
  someOnce<A>(
    this: AtomContext,
    atom: Atom<Option.Option<A>>
  ): Effect.Effect<A>;
  stream<A>(
    this: AtomContext,
    atom: Atom<A>,
    ...
    options?.reactivityKeys
      ? (((a: any, get: FnContext) => {
          ...
          return Effect.isEffect(effect)
            ...
            : Stream.ensuring(
                effect,
                Reactivity.invalidate(options.reactivityKeys!)
              );
        }) as any)
      : arg,
    options
...
const makeRead: {
  ...
  ): (get: AtomContext, services?: Context.Context<any>) => A;
  <A>(initialValue: A): Writable<A>;
} = <A, E>(
  arg:
    | Effect.Effect<A, E, Scope.Scope | AtomRegistry>
  ...
  if (
    ...
    return function (
      get: AtomContext,
      providedServices?: Context.Context<any>
    ) {
...
export const context: (options: {
  ...
  function factory<E, R>(
    ...
    self.read = function read(get: AtomContext) {
      ...
      const build = Effect.flatMap(Effect.scope, (scope) =>
  ...
  factory.addGlobalLayer = (
    layer: Layer.Layer<any, any, AtomRegistry | Reactivity.Reactivity>
  ) => {
    globalLayer = Layer.provideMerge(
      globalLayer,
      Layer.provide(layer, Reactivity.layer)
    );
  };
  const reactivityAtom = removeTtl(
    make(
      ...
      ).pipe(Effect.map(Context.get(Reactivity.Reactivity)))
    )
  );
...
const readSubscriptionRef = (
  ...
  return makeStream(
    get,
    SubscriptionRef.changes(sub.value),
    AsyncResult.initial(true),
    services
  );
};
...
const makeSubRef = (
  ...
  read: (
    ...
    ref:
      | SubscriptionRef.SubscriptionRef<any>
      | AsyncResult.Success<SubscriptionRef.SubscriptionRef<any>, any>
  ) => any
) => {
  function write(
    ctx: WriteContext<SubscriptionRef.SubscriptionRef<any>>,
    value: any
  ) {
...
function makeResultFn<Arg, E, A>(
  ...
  const argAtom = removeTtl(
    state<[number, Arg | Interrupt]>([0, undefined as any])
  );
  const initialValue =
    options?.initialValue !== undefined
      ? AsyncResult.success<A, E>(options.initialValue)
...
> =>
  Effect.flatMap(
    ...
    (pullChunk) => {
      ...
      const services = fiber.context as Context.Context<
        AtomRegistry | Scope.Scope
      >;
        ...
        > {
          if (Pull.isDoneCause(cause)) {
            if (!Arr.isReadonlyArrayNonEmpty(acc)) {
              return Effect.fail(
                new Cause.NoSuchElementError(`Atom.pull: no items`)
              );
            }
            return Effect.succeed({
              done: true,
              items: acc as Arr.NonEmptyArray<A>,
            });
          }
...
export const transform: {
  ...
  if (options?.initialValueTarget) {
    (atom as Mutable<Atom<B>>).initialValueTarget = getInitialValueTarget(
      options.initialValueTarget
    );
  }
  return atom;
}) as any);
...
export const map: {
  ...
  2,
  <A, B>(self: Atom<A>, f: (_: A) => B): Atom<B> =>
    transform(self, (get) => f(get(self)))
);
...
export const mapResult: {
  <R extends Atom<AsyncResult.AsyncResult<any, any>>, B>(
    ...
    ? Writable<
        AsyncResult.AsyncResult<B, AsyncResult.AsyncResult.Failure<Type<R>>>,
        RW
      >
    : Atom<
        AsyncResult.AsyncResult<B, AsyncResult.AsyncResult.Failure<Type<R>>>
...
} = dual(2, <A>(self: Atom<A>, duration: Duration.Input): Atom<A> => {
...
export const swr: {
  ...
  <A, E>(
    ...
    const staleTime = Duration.toMillis(
      Duration.fromInputUnsafe(options.staleTime)
    );
    return transform(
      self,
      (get) => {
...
const isFreshWithin = (
  ...
  staleTime: number,
  now: number
): boolean => now - timestamp < staleTime;
...
export const optimistic = <A>(
  self: Atom<A>
): Writable<A, Atom<AsyncResult.AsyncResult<A, unknown>>> => {
  let counter = 0;
  const writeAtom = removeTtl(
    state([
      counter,
    ...
    (get) => {
      ...
      get.subscribe(self, (value) => {
        ...
        const current = Option.getOrUndefined(
          get.self<AsyncResult.AsyncResult<any, any>>()
        )!;
        switch (value._tag) {
          case "Initial": {
...
export const optimisticFn: {
  ...
  <A, W, XA, XE, OW = void>(
    ...
    const transition = removeTtl(
      state<AsyncResult.AsyncResult<W, unknown>>(AsyncResult.initial())
    );
 ...
 * @since 4.0.0
 */
export const refreshOnWindowFocus: <A extends Atom<any>>(
  self: A
) => WithoutSerializable<A> = makeRefreshOnSignal(windowFocusSignal);
...
export const kvs = <
  ...
  const Mode extends "sync" | "async" = never,
>(options: {
  ...
  "async" extends Mode ? AsyncResult.AsyncResult<S["Type"]> : S["Type"],
  S["Type"]
> => {
  const setAtom = options.runtime.fn((value: S["Type"]) =>
  ...
  const resultAtom = options.runtime.atom(
    KeyValueStore.KeyValueStore.use((store) =>
      KeyValueStore.toSchemaStore(store, options.schema).get(options.key)
    )
  );
  return writable(
    options.mode === "async"
...
export const searchParam = <S extends Schema.Codec<any, string> = never>(
  ...
  return writable(
    ...
    (ctx, value: any) => {
      ...
      if (encode) {
        const encoded = Option.flatMap(value, (v) =>
          Exit.getSuccess(encode(v as S["Type"]))
        );
...
export const toStream = <A>(
  self: Atom<A>
): Stream.Stream<A, never, AtomRegistry> =>
  Stream.unwrap(
    AtomRegistry.use((r) => Effect.succeed(Registry.toStream(r, self)))
  );
 ...
 * @since 4.0.0
 */
export const toStreamResult = <A, E>(
  self: Atom<AsyncResult.AsyncResult<A, E>>
): Stream.Stream<A, E, AtomRegistry> =>
  Stream.unwrap(
    AtomRegistry.use((r) => Effect.succeed(Registry.toStreamResult(r, self)))
  );
...
export const modify: {
  ...
  <R, W, A>(
    self: Writable<R, W>,
    f: (_: R) => [returnValue: A, nextValue: W]
  ): Effect.Effect<A, never, AtomRegistry>;
} = dual(
  2,
  <R, W, A>(
    self: Writable<R, W>,
...
export const set: {
  <W>(
    value: W
  ): <R>(self: Writable<R, W>) => Effect.Effect<void, never, AtomRegistry>;
  <R, W>(
    self: Writable<R, W>,
    value: W
...
export const update: {
  <R, W>(
    f: (_: R) => W
  ): (self: Writable<R, W>) => Effect.Effect<void, never, AtomRegistry>;
  <R, W>(
    self: Writable<R, W>,
    f: (_: R) => W
  ): Effect.Effect<void, never, AtomRegistry>;
...
export const getResult = <A, E>(
  ...
  AtomRegistry.use(Registry.getResult(self, options));
...
export const mount = <A>(
  self: Atom<A>
): Effect.Effect<void, never, AtomRegistry | Scope.Scope> =>
...
export const isSerializable = (
  self: Atom<any>
): self is Atom<any> & Serializable<any> => SerializableTypeId in self;
 ...
 * @since 4.0.0
 */
export const withServerValueInitial = <
  A extends Atom<AsyncResult.AsyncResult<any, any>>,
>(
  self: A
): A => withServerValue(self, constant(AsyncResult.initial(true)) as any);
...
export const getServerValue: {
  ...
  <A>(self: Atom<A>, registry: Registry.AtomRegistry): A =>
    ServerValueTypeId in self
      ? (self as any)[ServerValueTypeId]((atom: Atom<any>) =>
          registry.get(atom)
        )
      : registry.get(self)
);
