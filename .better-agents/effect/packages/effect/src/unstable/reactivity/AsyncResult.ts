...
const ResultProto = {
  ...
  [Equal.symbol](
    this: AsyncResult<any, any>,
    that: AsyncResult<any, any>
  ): boolean {
 ...
 * @since 4.0.0
 */
export const isNotInitial = <A, E>(
  result: AsyncResult<A, E>
): result is Success<A, E> | Failure<A, E> => result._tag !== "Initial";
...
} = dual(2, <A, E, B>(self: AsyncResult<A, E>, orElse: LazyArg<B>): A | B =>
...
export const getOrThrow = <A, E>(self: AsyncResult<A, E>): A =>
  Option.getOrThrowWith(
    value(self),
    () => new Cause.NoSuchElementError("AsyncResult.getOrThrow: no value found")
  );
    ...
    switch (self._tag) {
      ...
      case "Failure":
        return failure(self.cause, {
          previousSuccess: Option.map(self.previousSuccess, (s) =>
            success(f(s.value), s)
          ),
          waiting: self.waiting,
        });
...
export type Builder<Out, A, E, I, F> = Pipeable & {
  onWaiting<B>(
    f: (result: AsyncResult<A, E>) => B
  ): Builder<Out | B, A, E, I, F>;
  ...
  ([I] extends [never]
    ...
    : {
        onInitial<B>(
          f: (result: Initial<A, E>) => B
        ): Builder<Out | B, A, E, never, F>;
        onInitialOrWaiting<B>(
          f: (result: AsyncResult<A, E>) => B
        ): Builder<Out | B, A, E, never, F>;
      }) &
  ([A] extends [never]
  ...
  (Defect extends F
    ? {
        ...
        ): Builder<Out | B, A, E, I, Exclude<F, Defect>>;
      }
    : unknown);
...
class BuilderImpl<Out, A, E> {
  ...
  onDefect<B>(
    ...
    return this.when(isFailure, (result) => {
      ...
      return Result.isFailure(defect)
        ? Option.none()
        : Option.some(f(defect.success, result));
    });
  }
  ...
  onInterrupt<B>(
    ...
    return this.when(isFailure, (result) => {
      ...
      return Result.isFailure(interruptors)
        ? Option.none()
        : Option.some(f(interruptors.success, result));
    });
  }
            ...
            const prevSuccessEffect = input.previousSuccess.pipe(
              Option.map((ps) =>
                Effect.mapBothEager(
                  ...
                  {
                    onSuccess: (value) =>
                      Option.some(success<A["Type"], E["Type"]>(value, ps)),
                    onFailure: (issue) =>
                      new SchemaIssue.Composite(ast, Option.some(input), [
                        new SchemaIssue.Pointer(
                          ["previousSuccess", "value"],
                          issue
                        ),
                      ]),
                  }
              ...
              switch (encoded._tag) {
                ...
                case "Failure": {
                  return failure(encoded.cause, {
                    previousSuccess: Option.map(encoded.previousSuccess, (ps) =>
                      success(ps.value, ps)
                    ),
                    waiting: encoded.waiting,
                  });
