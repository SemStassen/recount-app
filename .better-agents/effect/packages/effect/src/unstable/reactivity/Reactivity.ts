...
export const make = Effect.sync(() => {
  ...
  const invalidate = (
    ...
    Effect.contextWith((services) => {
      const pending = services.mapUnsafe.get(PendingInvalidation.key) as
        | Set<string | number>
        | undefined;
      if (pending) {
        keysToHashes(keys, (hash) => {
...
class PendingInvalidation extends Context.Service<
  PendingInvalidation,
  Set<string | number>
...
export const mutation: {
  ...
  <A, E, R>(
    ...
    Reactivity.use((_) => _.mutation(keys, effect))
);
...
export const invalidate = (
  ...
  Reactivity.use((r) => r.invalidate(keys));
