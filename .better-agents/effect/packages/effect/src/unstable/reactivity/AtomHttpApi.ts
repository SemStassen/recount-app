...
> extends Context.Service<Self, HttpApiClient.Client<Groups, never, never>> {
  new (
    _: never
  ): Context.ServiceClass.Shape<Id, HttpApiClient.Client<Groups, never, never>>;
...
export const Service =
  ...
  >(
    ...
    options: {
      ...
      readonly httpClient:
        | Layer.Layer<
            HttpApiGroup.ClientServices<Groups> | HttpClient.HttpClient
          >
        | ((
    ...
    const mutationFamily = Atom.family(
      ({ endpoint, group, responseMode }: MutationKey) => {
        ...
        if (responseMode === "decoded-only") {
          const definition = options.api.groups[group]!.endpoints[
            endpoint
          ]! as HttpApiEndpoint.AnyWithProps;
          return Atom.serializable(atom, {
            key: `AtomHttpApi:mutation:${group}:${endpoint}`,
            schema: AsyncResult.Schema({
              success: Schema.Union(
                HttpApiEndpoint.getSuccessSchemas(definition)
              ),
    ...
    const queryFamily = Atom.family((opts: QueryKey) => {
      let atom = self.runtime.atom(
        self.use((client_) => {
          ...
          return catchErrors(
            client[opts.group][opts.endpoint](opts) as Effect.Effect<
              any,
      ...
      if (opts.responseMode === "decoded-only" && opts.serializationKey) {
        const endpoint = options.api.groups[opts.group]!.endpoints[
          opts.endpoint
        ]! as HttpApiEndpoint.AnyWithProps;
...
> = [ResponseMode] extends ["decoded-and-response"]
