...
const policy = Schedule.forever.pipe(
  ...
  Schedule.addDelay((error) => {
    if (
      ...
      const retryAfter = Option.fromUndefinedOr(
        error.reason.response.headers["retry-after"]
      ).pipe(
...
> = Effect.fnUntraced(function* (options) {
  ...
  const runExport = Effect.suspend(() => {
    if (
      ...
      clock.currentTimeMillisUnsafe() < disabledUntil
    ) {
      return Effect.void;
