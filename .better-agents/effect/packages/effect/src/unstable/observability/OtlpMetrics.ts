  ...
  const snapshot = (): HttpBody => {
    ...
    for (let i = 0, len = snapshot.length; i < len; i++) {
      ...
      const attributes = state.attributes
        ? OtlpResource.entriesToAttributes(Object.entries(state.attributes))
        : [];
      ...
      switch (state.type) {
        ...
        case "Histogram": {
          ...
          if (metricDataByName.has(state.id)) {
            metricDataByName
              .get(state.id)!
              .histogram!.dataPoints.push(dataPoint);
          } else {
            addMetricData({
              name: state.id,
              description: state.description!,
        ...
        case "Frequency": {
          ...
          for (const [freqKey, value] of state.state.occurrences) {
            ...
            dataPoints.push({
              attributes: [
                ...attributes,
                { key: "key", value: { stringValue: freqKey } },
              ],
              startTimeUnixNano: intervalStartTime,
              timeUnixNano: nowTime,
              asInt: reportValue,
        ...
        case "Summary": {
          ...
          const dataPoints: Array<INumberDataPoint> = [
            {
              attributes: [
                ...attributes,
                { key: "quantile", value: { stringValue: "min" } },
              ],
              startTimeUnixNano: intervalStartTime,
              timeUnixNano: nowTime,
          ...
          for (const [quantile, value] of state.state.quantiles) {
            dataPoints.push({
              attributes: [
                ...
                {
                  key: "quantile",
                  value: { stringValue: quantile.toString() },
                },
              ],
              startTimeUnixNano: intervalStartTime,
              timeUnixNano: nowTime,
          ...
          dataPoints.push({
            attributes: [
              ...attributes,
              { key: "quantile", value: { stringValue: "max" } },
            ],
            startTimeUnixNano: intervalStartTime,
            timeUnixNano: nowTime,
          ...
          if (metricDataByName.has(`${state.id}_quantiles`)) {
            metricDataByName
              .get(`${state.id}_quantiles`)!
              .sum!.dataPoints.push(...dataPoints);
            metricDataByName
              .get(`${state.id}_count`)!
              .sum!.dataPoints.push(countDataPoint);
            metricDataByName
              .get(`${state.id}_sum`)!
              .sum!.dataPoints.push(sumDataPoint);
          } else {
            addMetricData({
...
type EAggregationTemporality =
  (typeof EAggregationTemporality)[keyof typeof EAggregationTemporality];
