...
const makeEffect = Effect.gen(function* () {
  ...
  yield* Effect.addFinalizer(() =>
    offerMetricsSnapshot.pipe(
      Effect.andThen(
        Effect.flatMap(Effect.fiberId, (id) =>
          Queue.failCause(requests, Cause.interrupt(id))
        )
      ),
 ...
 * @since 4.0.0
 */
export const layer: Layer.Layer<DevToolsClient, never, Socket.Socket> =
 ...
 * @since 4.0.0
 */
export const layerTracer: Layer.Layer<never, never, Socket.Socket> =
  Layer.effect(Tracer.Tracer, makeTracer).pipe(Layer.provide(layer));
