  ...
  Effect.gen(function* () {
    ...
    const withSoftDeleteFilter = (where: any) =>
      softDeleteColumn === undefined
        ? where
        : sql.and([where, sql`${sql(softDeleteColumn)} is null`]);
    const setSoftDeleted =
      softDeleteColumn === undefined
        ? undefined
...
select * from ${sql(options.tableName)} where ${withSoftDeleteFilter(sql`${sql(idColumn)} = LAST_INSERT_ID()`)};`.unprepared.pipe(
    ...
    const updateSchema = SqlSchema.findOne({
      ...
      execute: (request: any) =>
        sql.onDialectOrElse({
          mysql: () =>
            sql`update ${sql(options.tableName)} set ${sql.update(request, [idColumn])} where ${withSoftDeleteFilter(
              sql`${sql(idColumn)} = ${request[idColumn]}`
            )};
select * from ${sql(options.tableName)} where ${withSoftDeleteFilter(sql`${sql(idColumn)} = ${request[idColumn]}`)};`.unprepared.pipe(
              Effect.map(([, results]) => results as any)
            ),
          orElse: () =>
            sql`update ${sql(options.tableName)} set ${sql.update(request, [idColumn])} where ${withSoftDeleteFilter(
    ...
    const deleteSchema = SqlSchema.void({
      ...
      execute: (id: any) =>
        softDeleteColumn === undefined
          ...
          : sql`update ${sql(options.tableName)} set ${setSoftDeleted} where ${withSoftDeleteFilter(
              sql`${sql(idColumn)} = ${id}`
            )}`,
    });
    const delete_ = (
...
select * from ${sql(options.tableName)} where ${withSoftDeleteFilter(sql`${sql(idColumn)} = LAST_INSERT_ID()`)};`.unprepared.pipe(
          ...
          orElse: () =>
            sql`insert into ${sql(options.tableName)} ${sql.insert(request).returning("*")}`,
        }),
