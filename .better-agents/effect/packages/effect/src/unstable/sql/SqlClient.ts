...
export interface SqlClient extends Constructor {
   ...
   */
  readonly transactionService: Context.Service<
    TransactionConnection,
    TransactionConnection.Service
  >;
...
export const make = Effect.fnUntraced(function* (
  options: SqlClient.MakeOptions
) {
  const transactionService =
    options.transactionService ?? TransactionConnection(clientIdCounter++);
  ...
  const savepoint =
    options.savepoint ?? ((name: string) => `SAVEPOINT ${name}`);
  const rollback = options.rollback ?? "ROLLBACK";
  const rollbackSavepoint =
    options.rollbackSavepoint ??
    ((name: string) => `ROLLBACK TO SAVEPOINT ${name}`);
  ...
  const withTransaction = makeWithTransaction({
    ...
    rollbackSavepoint: (conn, id) =>
      conn.executeUnprepared(
        rollbackSavepoint(`effect_sql_${id}`),
        [],
        undefined
      ),
  });
...
export const makeWithTransaction =
  <I, S>(options: {
    readonly transactionService: Context.Key<
      I,
      readonly [conn: S, counter: number]
    >;
    readonly spanAttributes: ReadonlyArray<readonly [string, unknown]>;
    readonly acquireConnection: Effect.Effect<
      readonly [Scope.Closeable | undefined, S],
      SqlError
    >;
    ...
    return Effect.uninterruptibleMask((restore) =>
      Effect.useSpan("sql.transaction", { kind: "client" }, (span) =>
        Effect.withFiber<A, E | SqlError, R>((fiber) => {
          ...
          const connOption = Context.getOption(
            services,
 ...
 * @since 4.0.0
 */
export const SafeIntegers = Context.Reference<boolean>(
