 ...
 * @since 4.0.0
 */
export const CurrentTransformer = Context.Reference<Transformer | undefined>(
  "effect/sql/CurrentTransformer",
  {
 ...
 * @since 4.0.0
 */
export const isFragment = (u: unknown): u is Fragment =>
  hasProperty(u, FragmentTypeId);
...
export interface RecordInsertHelper {
  ...
  readonly returning: (
    sql: string | Identifier | Fragment
  ) => RecordInsertHelper;
}
...
export const make = (
  ...
  const cache =
    transformRows === undefined
      ? constructorCache.noTransforms
      : constructorCache.transforms;
  ...
  const self = Object.assign(
    ...
    },
    {
      ...
      onDialectOrElse(options: any) {
        return options[compiler.dialect] !== undefined
          ? options[compiler.dialect]()
          : options.orElse();
      },
    }
  );
 ...
 * @since 4.0.0
 */
export const or: (clauses: ReadonlyArray<string | Fragment>) => Fragment = join(
  " OR ",
  true,
  "1=1"
);
...
const statementCacheSymbol = Symbol.for(
  "effect/unstable/sql/Statement/statementCache"
);
const statementCacheNoTransformSymbol = Symbol.for(
  "effect/unstable/sql/Statement/statementCacheNoTransform"
);
...
const CompilerProto = {
  compile(
    ...
    const cacheSymbol = withoutTransform
      ? statementCacheNoTransformSymbol
    ...
    const placeholder =
      placeholderOverride ??
      ((u: unknown) => opts.placeholder(++placeholderCount, u));
    const placeholderNoIncrement = (u: unknown) =>
      opts.placeholder(placeholderCount, u);
    ...
    for (let i = 0; i < len; i++) {
      ...
      switch (segment._tag) {
        ...
        case "RecordInsertHelper": {
          ...
          if (opts.onInsert) {
            const values: Array<ReadonlyArray<unknown>> = new Array(
            ...
            for (let i = 0; i < segment.value.length; i++) {
              ...
              for (let j = 0; j < keys.length; j++) {
                ...
                const primitive = extractPrimitive(
                  value,
                  opts.onCustom,
                ...
                row[j] = primitive;
                placeholders +=
                  j === 0 ? placeholder(value) : `,${placeholder(value)}`;
              }
              placeholders += ")";
            }
 ...
 * @since 4.0.0
 */
export const makeCompilerSqlite = (
...
> = {
      ...
      return Effect.onExit(
        this.withConnectionSpan(
          ...
          (connection, sql, params) =>
            connection.execute(sql, params, this.transformRows),
          false,
          span
        ),
  ...
  withConnectionSpan<XA, E>(
    ...
    return withStatement(this, span, (statement) => {
      ...
      return Effect.scoped(
        Effect.flatMap(this.acquirer, (_) => f(_, sql, params))
      );
    });
  },
      ...
      Effect.flatMap(
        ...
        (span) =>
          withStatement(self, span, (statement) => {
            ...
            span.attribute(ATTR_DB_QUERY_TEXT, sql);
            return Effect.map(self.acquirer, (_) =>
              _.executeStream(sql, params, self.transformRows)
            );
          })
      )
    );
  ...
  get values(): Effect.Effect<ReadonlyArray<ReadonlyArray<unknown>>, SqlError> {
    return this.withConnection("executeValues", (connection, sql, params) =>
      connection.executeValues(sql, params)
    );
  },
...
function in_(): Fragment | ArrayHelper {
  ...
  return values.length === 0
    ? neverFragment
    : fragment([identifier(column), literal(" IN "), arrayHelper(values)]);
}
