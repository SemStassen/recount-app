...
> {
  let context =
    yield* Effect.context<HttpApiGroup.ToService<ApiId, SelectedGroups>>();
  ...
  for (const name in api.groups) {
    ...
    for (const endpointName in group.endpoints) {
      ...
      const handler: Handlers.Item<never> = {
        ...
        handler: () =>
          Effect.die(new Error(`Unhandled endpoint: ${endpointName}`)),
        isRaw: false,
        uninterruptible: false,
      };
      handlers.set(endpointName, handler);
      routes.push(
        HttpApiBuilder.handlerToRoute(group as any, handler, context)
      );
    }
