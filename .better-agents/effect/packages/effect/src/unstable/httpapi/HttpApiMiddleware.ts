...
export type Provides<A> = A extends {
  readonly [TypeId]: { readonly provides: infer P };
}
  ? P
  : never;
...
export type Requires<A> = A extends {
  readonly [TypeId]: { readonly requires: infer R };
}
  ? R
  : never;
 ...
 * @since 4.0.0
 */
export type ApplyServices<A extends AnyId, R> =
  | Exclude<R, Provides<A>>
  | Requires<A>;
...
export type ErrorSchema<A> = A extends {
  readonly [TypeId]: { readonly error: infer E };
}
...
export const Service =
  ...
  >(
    ...
    Id,
    {
      ...
      error: Error;
      clientError: "clientError" extends keyof Config
        ? Config["clientError"]
        : never;
  ...
  (
    ...
    options?:
      | {
          readonly security?:
            | Record<string, HttpApiSecurity.HttpApiSecurity>
            | undefined;
    ...
    if (options?.security !== undefined) {
      if (Object.keys(options.security).length === 0) {
        throw new Error(
          "HttpApiMiddleware.Service: security object must not be empty"
        );
      }
 ...
 * @since 4.0.0
 */
export const layerSchemaErrorTransform = <
  Id,
  E extends ErrorConstraint,
  Requires,
>(
