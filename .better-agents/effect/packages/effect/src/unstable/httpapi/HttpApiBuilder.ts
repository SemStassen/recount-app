  ...
  HttpRouter.use(
    Effect.fnUntraced(function* (router) {
      ...
      const availableGroups = Array.from(services.mapUnsafe.keys()).filter(
        (key) => key.startsWith("effect/httpapi/HttpApiGroup/")
      );
      for (const group of Object.values(api.groups)) {
        const groupRoutes = services.mapUnsafe.get(group.key)?.routes as Array<
      ...
      if (options?.openapiPath) {
        ...
        yield* router.add(
          "GET",
          options.openapiPath,
          Effect.succeed(Response.jsonUnsafe(spec))
        );
      }
    })
  );
...
> extends Pipeable {
  ...
  handle<Name extends HttpApiEndpoint.Name<Endpoints>, R1>(
    name: Name,
    handler: HttpApiEndpoint.HandlerWithName<
      Endpoints,
      Name,
      HttpApiEndpoint.ErrorsWithName<Endpoints, Name>,
      R1
    >,
  ...
  handleRaw<Name extends HttpApiEndpoint.Name<Endpoints>, R1>(
    name: Name,
    handler: HttpApiEndpoint.HandlerRawWithName<
      Endpoints,
      Name,
      HttpApiEndpoint.ErrorsWithName<Endpoints, Name>,
      R1
...
> =>
  Effect.contextWith((context: Context.Context<any>) => {
    ...
    const endpoint = group.endpoints[
      endpointName
    ] as unknown as HttpApiEndpoint.AnyWithProps;
 ...
 * @since 4.0.0
 */
export const securityDecode = <
  Security extends HttpApiSecurity.HttpApiSecurity,
>(
  self: Security
): Effect.Effect<
  ...
  switch (self._tag) {
    case "Http": {
      return Effect.map(
        ...
        (request) =>
          Redacted.make(
            (request.headers.authorization ?? "").slice(self.schemeLength)
          ) as any
      );
    }
    case "ApiKey": {
        ...
        Effect.flatMap((request) =>
          Effect.fromResult(
            Encoding.decodeBase64String(
              (request.headers.authorization ?? "").slice(basicLen)
            )
          )
        ),
        Effect.match({
  ...
  payloadMap.forEach(({ encoding, schemas }, contentType) => {
    ...
    if (encoding._tag === "Multipart") {
      ...
      result.set(contentType, {
        _tag: encoding._tag,
        decode,
        nullOnEmpty: schemas.some((s) =>
          SchemaAST.isNull(SchemaAST.toEncoded(s.ast))
        ),
      });
    }
  });
...
function decodePayload(
  ...
  switch (_tag) {
    ...
    case "Json":
      const json = Effect.orDie(
        Effect.flatMap(httpRequest.text, (text) => {
          if (text === "") {
    ...
    case "FormUrlEncoded": {
      const source = hasBody
        ? Effect.map(
            Effect.orDie(httpRequest.urlParamsBody),
            UrlParams.toRecord
          )
    ...
    case "Uint8Array":
      return Effect.flatMap(
        Effect.map(
          Effect.orDie(httpRequest.arrayBuffer),
...
function handlerToHttpEffect(
  ...
  const payloadBy = shouldParsePayload
    ? buildPayloadDecoders(endpoint.payload)
    : undefined;
  ...
  return applyMiddleware(
    ...
    Effect.gen(function* () {
      ...
      if (decodeParams) {
        request.params = yield* HttpApiSchemaError.wrap(
          "Params",
          decodeParams(routeContext.params)
        );
      }
      if (decodeHeaders) {
        request.headers = yield* HttpApiSchemaError.wrap(
          "Headers",
          decodeHeaders(httpRequest.headers)
        );
      }
      if (decodeQuery) {
...
const applyMiddleware = <A extends Effect.Effect<any, any, any>>(
  ...
  for (const key_ of endpoint.middlewares) {
    ...
    const service = Context.getUnsafe(
      context,
      key as any
    ) as HttpApiMiddleware.HttpApiMiddleware<any, any, any>;
...
const toResponseSuccessSchema = toResponseSchema(
...
function makeSuccessSchema(
  endpoint: HttpApiEndpoint.AnyWithProps
): Schema.Encoder<HttpServerResponse, unknown> {
  const schemas = HttpApiEndpoint.getSuccessSchemas(endpoint).map(
    toResponseSuccessSchema
  );
...
function makeErrorSchema(
  ...
  const schemas = HttpApiEndpoint.getErrorSchemas(endpoint).map(
    toResponseErrorSchema
  );
...
function getResponseTransformation(
  ...
  return SchemaTransformation.transformOrFail({
    decode: (res) =>
      Effect.fail(
        new SchemaIssue.Forbidden(Option.some(res), {
...
> {
  switch (encoding._tag) {
    case "Json": {
      return (e) => {
        ...
        try {
          ...
          return Effect.succeed(
            Response.text(s, { status, contentType: encoding.contentType })
          );
        } catch (error) {
          return Effect.fail(
    ...
    case "Uint8Array":
      return (e) =>
        Effect.succeed(
          Response.uint8Array(e as Uint8Array, {
            status,
            contentType: encoding.contentType,
          })
        );
