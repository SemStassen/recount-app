...
export class McpServer extends Context.Service<
  McpServer,
  {
    readonly notifications: RpcClient.RpcClient<
...
>()("effect/ai/McpServer") {
  ...
  static readonly layer: Layer.Layer<McpServer | McpServerClient> =
    Layer.effect(McpServer)(McpServer.make) as any;
}
...
export const run: (options: {
  ...
  Effect.fnUntraced(function* (options: {
    ...
    const isHttp = Option.isSome(
      yield* Effect.serviceOption(HttpRouter.HttpRouter)
    );
    const clientSessions = new Map<
      string,
      typeof Initialize.payloadSchema.Type
    >();
    const handlers = yield* Layer.build(
      layerHandlers(options, { clientSessions })
    );
    ...
    const clientMiddleware = McpServerClientMiddleware.of(
      (effect, { client, headers, rpc }) => {
        const initializePayload = getInitializedClient(
          clientSessions,
          client.id,
          headers
        );
        ...
        if (!isInitialize && !initializePayload) {
          ...
          const httpRequest = Context.getOrUndefined(
            fiber.context,
            HttpServerRequest.HttpServerRequest
          );
          if (httpRequest) {
            appendPreResponseHandlerUnsafe(httpRequest, () =>
        ...
        protocol.run((clientId, request_) => {
          ...
          switch (request._tag) {
            case "Request": {
              if (isHttp) {
                const fiber = Fiber.getCurrent()!;
                const httpRequest = Context.getUnsafe(
                  fiber.context,
                  HttpServerRequest.HttpServerRequest
                );
              ...
              if (rpc) {
                ...
                const handler = handlers.mapUnsafe.get(
                  request.tag
...
export const registerToolkit: <Tools extends Record<string, Tool.Any>>(
  ...
  | Exclude<Tool.HandlerServices<Tools>, McpServerClient>
...
export const registerResource: {
  ...
  <const Schemas extends ReadonlyArray<Schema.Top>>(
      ...
      yield* registry.addResource({
        ...
        handle: options.content.pipe(
          ...
          Effect.catchCause((cause) => {
            ...
            return Effect.fail(
              new InternalError({ message: prettyError.message })
            );
          })
        ),
  ...
  return Effect.fnUntraced(function* <E, R>(options: {
    ...
    for (const [param, handle] of Object.entries(options.completion ?? {})) {
      const encodeArray = Schema.encodeUnknownEffect(
        Schema.Array(params[param])
      );
...
export const resource: {
  ...
  return (options: any) =>
    Layer.effectDiscard(register(options)).pipe(Layer.provide(McpServer.layer));
} as any;
...
>(options: {
  ...
  void,
  never,
  | Exclude<Schema.Struct.DecodingServices<Params> | R, McpServerClient>
  | McpServer
> => {
  ...
  const completion: Record<string, (input: string) => Effect.Effect<any>> =
    options.completion ?? {};
  return Effect.gen(function* () {
    const registry = yield* McpServer;
    const services =
...
> = Effect.fnUntraced(function* <
  S extends Schema.Encoder<Record<string, unknown>, unknown>,
>(options: { readonly message: string; readonly schema: S }) {
  ...
  switch (res.action) {
    case "accept":
      return yield* Effect.orDie(
        Schema.decodeUnknownEffect(schema)(res.content)
      );
    case "cancel":
...
const compileUriTemplate = (
  ...
  if (schemas.length > 0) {
    ...
    for (let i = 0; i < schemas.length; i++) {
      ...
      const paramName = isParam(schema)
        ? (schema as Param<string, Schema.Top>).name
        : `param${key}`;
    ...
    Effect.gen(function* () {
      ...
      return ClientRpcs.of({
        ...
        initialize(params, { client }) {
          const requestedVersion = SUPPORTED_PROTOCOL_VERSIONS.includes(
        ...
        "completion/complete": (r) =>
          server
            .completion(r)
            .pipe(Effect.provideService(CurrentLogLevel, currentLogLevel)),
