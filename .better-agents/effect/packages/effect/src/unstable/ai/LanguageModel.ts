  ...
  readonly generateText: {
    ...
    <
      Options extends NoExcessProperties<
        GenerateTextOptionsWithoutToolkit,
        Options
      >,
    >(
...
type GenerateTextOptionsWithoutToolkit = Omit<
  GenerateTextOptions<{}>,
  "toolkit"
> & {
...
type ExtractErrorFromToolkitOption<
  ToolkitValue,
...
export const make: (params: {
  ...
  > =>
    Effect.useSpan(
      ...
      Effect.fnUntraced(
        ...
        (effect, span) =>
          Effect.withParentSpan(effect, span, { captureStackTrace: false }),
  ...
  >(
    ...
    if (Predicate.isUndefined(options.toolkit)) {
      ...
      if (tracker) {
        ...
        if (Option.isSome(prepared)) {
          providerOptions.previousResponseId =
            prepared.value.previousResponseId;
      ...
      if (tracker) {
        const responseMetadata = content.find(
    ...
    const tools =
      typeof toolChoice === "object" && "oneOf" in toolChoice
        ? Object.values(toolkit.tools).filter((tool) =>
            toolChoice.oneOf.includes(tool.name)
          )
  ...
  >(
    ...
    // Check for pending approvals that need resolution
    const { approved: pendingApproved, denied: pendingDenied } =
      collectToolApprovals(providerOptions.prompt.content, {
        excludeResolved: true,
      });
    const hasPendingApprovals =
      pendingApproved.length > 0 || pendingDenied.length > 0;
              ...
              ? Effect.tap((decodedParts) => {
                  for (const part of decodedParts) {
                    if (
                      ...
                      Predicate.isNotUndefined(part.id)
                    ) {
                      tracker.markParts(
                        providerOptions.prompt.content,
                        part.id
    ...
    const handleToolCall = Effect.fnUntraced(function* (
      part: Response.ToolCallPartEncoded
    ) {
...
): Effect.Effect<GenerateTextResponse<any>, AiError.AiError, LanguageModel> =>
  Effect.flatMap(Effect.service(LanguageModel), (model) =>
    model.generateText(options as any)
  );
...
>(
  options: Options &
    GenerateObjectOptions<ExtractTools<Options>, StructuredOutputSchema>
): Effect.Effect<
...
export const streamText: {
  ...
  Stream.unwrap(
    Effect.map(Effect.service(LanguageModel), (model) =>
      model.streamText(options as any)
    )
  ) as any;
  ...
  const executeTool = Effect.fnUntraced(function* (approval: ApprovalResult) {
    ...
    const terminalResult = yield* resultStream.pipe(
      ...
      Effect.flatMap(
        Option.match({
          onNone: () =>
            Effect.die("Tool handler did not produce a final result"),
