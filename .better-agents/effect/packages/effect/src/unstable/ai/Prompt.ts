...
): ReasoningPart => makePart("reasoning", params as any);
...
export const FilePart: Schema.Struct<{
  ...
  readonly data: Schema.Union<
    readonly [Schema.String, Schema.Uint8Array, Schema.URL]
  >;
  readonly "~effect/ai/Prompt/Part": Schema.withDecodingDefaultKey<
    Schema.Literal<"~effect/ai/Prompt/Part">
  >;
...
): ToolCallPart => makePart("tool-call", params as any);
...
): ToolResultPart => makePart("tool-result", params as any);
...
export const toolApprovalResponsePart = (
  ...
  makePart("tool-approval-response", params as any);
...
const BaseMessage = Schema.Struct({
  [MessageTypeId]: Schema.Literal(MessageTypeId).pipe(
    Schema.withDecodingDefaultKey(Effect.succeed(MessageTypeId), {
      encodingStrategy: "omit",
    })
  ),
  options: ProviderOptions.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
  ...
  Schema.decodeTo(
    ...
    SchemaTransformation.transform({
      decode: (text) =>
        Arr.of(
          makePart("text", { text })
        ) as Arr.NonEmptyReadonlyArray<TextPart>,
 ...
 * @since 4.0.0
 */
export interface SystemMessageEncoded extends BaseMessageEncoded<
  "system",
  SystemMessageOptions
> {
  /**
 ...
 * @since 4.0.0
 */
export interface UserMessageEncoded extends BaseMessageEncoded<
  "user",
  UserMessageOptions
> {
  /**
 ...
 * @since 4.0.0
 */
export interface ToolMessageEncoded extends BaseMessageEncoded<
  "tool",
  ToolMessageOptions
> {
  /**
 ...
 * @since 4.0.0
 */
export type ToolMessagePartEncoded =
  | ToolResultPartEncoded
  | ToolApprovalResponsePartEncoded;
