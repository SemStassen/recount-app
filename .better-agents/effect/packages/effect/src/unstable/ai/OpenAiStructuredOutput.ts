...
export function toCodecOpenAI<T, E, RD, RE>(
  ...
  const codec =
    from === to
      ? schema
      : Schema.make<typeof schema>(
          SchemaAST.decodeTo(from, to, SchemaTransformation.passthrough())
        );
...
function recurOpenAI(ast: SchemaAST.AST): SchemaAST.AST {
  switch (ast._tag) {
    ...
    case "Literal": {
      ...
      throw new Error(
        `${errorPrefix}: Unsupported literal type ${typeof literal} (value: ${String(
          literal
        ...
        if (ast.rest.length === 1) {
          propertySignatures.push(
            new SchemaAST.PropertySignature(
              REST_PROPERTY_NAME,
              new SchemaAST.Arrays(false, [], ast.rest)
            )
          );
        }
        return SchemaAST.decodeTo(
          recurOpenAI(
            new SchemaAST.Objects(propertySignatures, [], annotations, filters)
          ),
          ast,
          SchemaTransformation.transform({
            ...
            encode: (t) => {
              ...
              if (ast.rest.length === 1) {
                o[REST_PROPERTY_NAME] =
                  t.length >= ast.elements.length
                    ? t.slice(ast.elements.length)
                    : [];
              }
              return o;
            },
    ...
    case "Objects": {
      ...
      if (ast.indexSignatures.length === 0) {
        const propertySignatures = SchemaAST.mapOrSame(
        ...
        return SchemaAST.decodeTo(
          recurOpenAI(
            new SchemaAST.Arrays(
              false,
              [],
              [new SchemaAST.Arrays(false, [is.parameter, is.type], [])],
              annotations
            )
          ),
          ast,
...
const getAnnotations = (
  ...
  if (annotations !== undefined) {
    const description =
      ...
      annotations.meta?._tag === "isFinite"
        ? undefined
