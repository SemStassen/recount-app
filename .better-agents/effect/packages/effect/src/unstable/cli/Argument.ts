 ...
 * @since 4.0.0
 */
export interface Argument<A> extends Param.Param<
  typeof Param.argumentKind,
  A
> {}
...
export const string = (name: string): Argument<string> =>
  Param.string(Param.argumentKind, name);
...
export const integer = (name: string): Argument<number> =>
  Param.integer(Param.argumentKind, name);
...
export const float = (name: string): Argument<number> =>
  Param.float(Param.argumentKind, name);
 ...
 * @since 4.0.0
 */
export const optional = <A>(arg: Argument<A>): Argument<Option.Option<A>> =>
  Param.optional(arg);
...
export const between: {
  <A>(
    min: number,
    max: number
  ): (self: Argument<A>) => Argument<ReadonlyArray<A>>;
  <A>(self: Argument<A>, min: number, max: number): Argument<ReadonlyArray<A>>;
} = dual(3, <A>(self: Argument<A>, min: number, max: number) =>
...
} = dual(2, <A, B>(self: Argument<A>, schema: Schema.Codec<B, A>) =>
...
>(
  ...
  Param.choiceWithValue(Param.argumentKind, name, choices);
...
} = dual(2, <A>(self: Argument<A>, metavar: string) =>
...
export const orElseResult: {
  <B>(
    that: LazyArg<Argument<B>>
  ): <A>(self: Argument<A>) => Argument<Result.Result<A, B>>;
  <A, B>(
    self: Argument<A>,
    that: LazyArg<Argument<B>>
  ): Argument<Result.Result<A, B>>;
} = dual(2, <A, B>(self: Argument<A>, that: LazyArg<Argument<B>>) =>
