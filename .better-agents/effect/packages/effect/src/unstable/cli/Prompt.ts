 ...
 * @since 4.0.0
 */
export interface Prompt<Output> extends Effect.Effect<
  Output,
  Terminal.QuitError,
  Environment
> {
 ...
 * @since 4.0.0
 */
export const isPrompt = (u: unknown): u is Prompt<unknown> =>
...
export declare namespace All {
   ...
   * @since 4.0.0
   */
  export type ReturnIterable<T extends Iterable<Any>> = [T] extends [
    Iterable<Prompt<infer A>>,
  ]
    ? Prompt<Array<A>>
    : never;
  ...
  if (arguments.length === 1) {
    if (isPrompt(arguments[0])) {
      ...
      const entries = Object.entries(
        arguments[0] as Readonly<{ [K: string]: Prompt<any> }>
      );
...
export const file = (options: FileOptions = {}): Prompt<string> => {
  ...
  const initialState: Effect.Effect<FileState, never, Environment> = Effect.gen(
    function* () {
      const currentPath = yield* resolveCurrentPath(Option.none(), opts);
      const path = yield* Path.Path;
      const defaultPath = Option.map(opts.default, (defaultValue) =>
        path.resolve(currentPath, defaultValue)
      );
...
export const hidden = (options: TextOptions): Prompt<Redacted.Redacted> =>
  basePrompt(options, "hidden").pipe(map(Redacted.make));
...
export const list = (options: ListOptions): Prompt<Array<string>> =>
  text(options).pipe(map((output) => output.split(options.delimiter || ",")));
...
export const map: {
  ...
  2,
  <Output, Output2>(self: Prompt<Output>, f: (output: Output) => Output2) =>
...
export const password = (options: TextOptions): Prompt<Redacted.Redacted> =>
  basePrompt(options, "password").pipe(map(Redacted.make));
...
export const run: <Output>(
  self: Prompt<Output>
) => Effect.Effect<Output, Terminal.QuitError, Environment> = Effect.fnUntraced(
...
const getSelectInitialIndex = <A>(
  ...
  for (let i = 0; i < choices.length; i++) {
    ...
    if (choice.selected === true) {
      if (seenSelected !== -1) {
        throw new Error(
          "InvalidArgumentException: only a single choice can be selected by default for Prompt.select"
        );
      }
      seenSelected = i;
    }
 ...
 * @since 4.0.0
 */
export const autoComplete = <const A>(
 ...
 * @since 4.0.0
 */
export const text = (options: TextOptions): Prompt<string> =>
...
type Op<Tag extends string, Body = {}> = Prompt<never> &
  Body & {
    readonly _tag: Tag;
  };
...
/** @internal */
export interface OnSuccess extends Op<
  "OnSuccess",
  {
...
const allTupled = <const T extends ArrayLike<Prompt<any>>>(
  arg: T
): Prompt<{
...
const runWithInput = <Output>(
  ...
  Effect.suspend(() => {
    ...
    switch (op._tag) {
      ...
      case "OnSuccess": {
        return Effect.flatMap(runWithInput(op.prompt, terminal, input), (a) =>
          runWithInput(op.onSuccess(a), terminal, input)
        ) as any;
      }
      case "Succeed": {
...
const runLoop = Effect.fnUntraced(
  function* (
    ...
    let state = Effect.isEffect(loop.initialState)
    ...
    while (true) {
      ...
      switch (action._tag) {
        ...
        case "NextFrame": {
          yield* Effect.orDie(
            terminal.display(yield* loop.clear(state, action))
          );
          state = action.state;
          continue;
        }
  ...
  (effect, _, terminal) =>
    Effect.ensuring(effect, Effect.orDie(terminal.display(Ansi.cursorShow)))
);
...
const clearOutputWithError = (
  ...
  if (errorText !== undefined && errorText.length > 0) {
    return (
      Ansi.cursorDown(lines(errorText, columns)) +
      eraseText(`\n${errorText}`, columns) +
...
const renderConfirmNextFrame = Effect.fnUntraced(function* (
  state: ConfirmState,
  options: ConfirmOptionsReq
) {
  const figures = yield* platformFigures;
  ...
  const promptMsg = renderConfirmOutput(
    confirm,
    leadingSymbol,
    trailingSymbol,
    options
...
const renderConfirmSubmission = Effect.fnUntraced(function* (
  value: boolean,
  options: ConfirmOptionsReq
) {
  const figures = yield* platformFigures;
...
const handleDateClear = (options: DateOptionsReq) => {
  return Effect.fnUntraced(function* (
    state: DateState,
    _: Action<DateState, globalThis.Date>
  ) {
    const terminal = yield* Terminal.Terminal;
    const columns = yield* terminal.columns;
    const figures = yield* platformFigures;
    const resetCurrentLine = Ansi.eraseLine + Ansi.cursorLeft;
    const parts = Arr.reduce(
      state.dateParts,
      "",
      (doc, part) => doc + part.toString()
    );
    const promptText = renderDateOutput(
      "?",
      figures.pointerSmall,
      parts,
      options,
...
const renderDateNextFrame = Effect.fnUntraced(function* (
  state: DateState,
  options: DateOptionsReq
) {
  const figures = yield* platformFigures;
  ...
  const promptMsg = renderDateOutput(
    leadingSymbol,
    trailingSymbol,
    parts,
    options
...
const renderDateSubmission = Effect.fnUntraced(function* (
  state: DateState,
  options: DateOptionsReq
) {
  const figures = yield* platformFigures;
...
const makeDateParts = (
  ...
  while ((result = DATE_PART_REGEXP.exec(dateMask))) {
    ...
    if (index in regExpGroups) {
      ...
      parts.push(
        new Token({ token: (result[index] || match)!, date, parts, locales })
      );
    }
  }
...
abstract class DatePart {
  ...
  nextPart(): Option.Option<DatePart> {
    ...
    return Arr.findFirst(
      this.parts.slice(currentPartIndex + 1),
      (part) => !part.isToken()
    );
  }
  ...
  previousPart(): Option.Option<DatePart> {
    const currentPartIndex = Arr.findFirstIndex(
      this.parts,
      (part) => part === this
    );
...
interface FileOptionsReq extends Required<
  Omit<FileOptions, "startingPath" | "default">
> {
...
const getFileList = Effect.fnUntraced(function* (
  directory: string,
  options: FileOptionsReq
) {
...
const renderPrompt = (
  ...
  const annotate =
    options?.plain === true ? (line: string) => line : annotateLine;
...
const renderPrefix = (
  ...
  if (currentIndex === toDisplay.startIndex && toDisplay.startIndex > 0) {
    ...
    currentIndex === toDisplay.endIndex - 1 &&
    toDisplay.endIndex < length
  ) {
...
const renderFileNextFrame = Effect.fnUntraced(function* (
  state: FileState,
  options: FileOptionsReq
) {
  const path = yield* Path.Path;
  ...
  const resolvedPathMsg = Ansi.annotate(
    figures.pointerSmall + " " + resolvedPath,
    Ansi.blackBright
  );

  if (showConfirmation(state.confirm)) {
    const leadingSymbol = Ansi.annotate("?", Ansi.cyanBright);
    const trailingSymbol = Ansi.annotate(
      figures.pointerSmall,
      Ansi.blackBright
    );
...
const processFileBackspace = (state: FileState) => {
  ...
  return Effect.succeed(
    Action.NextFrame({ state: updateFileState(state, query) })
  );
};

const processFileClear = (state: FileState) =>
  Effect.succeed(Action.NextFrame({ state: updateFileState(state, "") }));
...
const processSelection = Effect.fnUntraced(function* (
  state: FileState,
  options: FileOptionsReq
) {
  if (state.files.length === 0) {
...
const renderMultiSelectChoices = <A>(
  ...
  for (let index = toDisplay.startIndex; index < toDisplay.endIndex; index++) {
    ...
    if (index === toDisplay.startIndex && toDisplay.startIndex > 0) {
      ...
      index === toDisplay.endIndex - 1 &&
      toDisplay.endIndex < allChoices.length
    ) {
      prefix = figures.arrowDown;
    }
    if (index < metaOptions.length) {
      // Meta options
      const title = renderMultiSelectTitle(
        choice.title,
        isHighlighted,
        renderOptions
      );
      ...
      const annotatedCheckbox =
        isHighlighted && renderOptions?.plain !== true
      ...
      documents.push(
        prefix + " " + annotatedCheckbox + " " + title + " " + description
      );
    }
  }
  return documents.join("\n");
...
const renderMultiSelectNextFrame = Effect.fnUntraced(function* <A>(
  state: MultiSelectState,
...
const renderMultiSelectSubmission = Effect.fnUntraced(function* <A>(
  state: MultiSelectState,
  options: SelectOptionsReq<A>
) {
  const figures = yield* platformFigures;
  const selectedChoices = Array.from(state.selectedIndices)
    .sort(EffectNumber.Order)
    .map((index) => options.choices[index].title);
...
const processMultiSelectCursorUp = (
  state: MultiSelectState,
  totalChoices: number
) => {
  const newIndex = state.index === 0 ? totalChoices - 1 : state.index - 1;
  return Effect.succeed(
    Action.NextFrame({ state: { ...state, index: newIndex } })
  );
};

const processMultiSelectCursorDown = (
  state: MultiSelectState,
  totalChoices: number
) => {
...
const processSpace = <A>(
  ...
  return Effect.succeed(
    Action.NextFrame({ state: { ...state, selectedIndices } })
  );
};
...
const handleMultiSelectClear = <A>(options: SelectOptionsReq<A>) =>
  Effect.fnUntraced(function* (
    state: MultiSelectState,
    _: Action<MultiSelectState, Array<A>>
  ) {
    const terminal = yield* Terminal.Terminal;
    const columns = yield* terminal.columns;
    const figures = yield* platformFigures;
...
const handleMultiSelectProcess = <A>(
  options: SelectOptionsReq<A> & MultiSelectOptionsReq
) => {
          ...
          return Effect.succeed(
            Action.NextFrame({
              state: {
                ...state,
                error: Option.some(
                  `At most ${options.max} choices are allowed`
                ),
              },
            })
          );
        }
        const selectedValues = Array.from(state.selectedIndices)
          .sort(EffectNumber.Order)
...
const handleMultiSelectRender = <A>(options: SelectOptionsReq<A>) => {
  return (
    state: MultiSelectState,
    action: Action<MultiSelectState, Array<A>>
  ) => {
    return Action.$match(action, {
      Beep: () => Effect.succeed(renderBeep),
...
const handleNumberClear = (options: IntegerOptionsReq) => {
  return Effect.fnUntraced(function* (
    ...
    const promptText = renderNumberOutput(
      state,
      "?",
      figures.pointerSmall,
      options,
...
const renderNumberNextFrame = Effect.fnUntraced(function* (
  state: NumberState,
  options: IntegerOptionsReq
) {
  const figures = yield* platformFigures;
  ...
  const promptMsg = renderNumberOutput(
    state,
    leadingSymbol,
    trailingSymbol,
    options
...
const renderNumberSubmission = Effect.fnUntraced(function* (
...
const filterAutoCompleteChoices = <A>(
  choices: ReadonlyArray<SelectChoice<A>>,
  query: string
) => {
  const normalizedQuery = query.toLowerCase();
...
const renderChoicePrefix = <A>(
  ...
  if (currentIndex === toDisplay.startIndex && toDisplay.startIndex > 0) {
    ...
    currentIndex === toDisplay.endIndex - 1 &&
    toDisplay.endIndex < choices.length
  ) {
    prefix = figures.arrowDown;
  }
  if (renderOptions?.plain === true) {
...
const renderAutoCompleteChoicePrefix = <A>(
  ...
  if (currentIndex === toDisplay.startIndex && toDisplay.startIndex > 0) {
    ...
    currentIndex === toDisplay.endIndex - 1 &&
    toDisplay.endIndex < state.filtered.length
  ) {
    prefix = figures.arrowDown;
  }
...
const renderSelectChoices = <A>(
  ...
  for (let index = toDisplay.startIndex; index < toDisplay.endIndex; index++) {
    ...
    const prefix = renderChoicePrefix(
      state,
      choices,
      toDisplay,
      index,
...
const renderSelectNextFrame = Effect.fnUntraced(function* <A>(
  state: SelectState,
  options: SelectOptionsReq<A>
) {
  const figures = yield* platformFigures;
...
const renderAutoCompleteNextFrame = Effect.fnUntraced(function* <A>(
  ...
  const leadingSymbol = Ansi.annotate("?", Ansi.cyanBright);
  const trailingSymbol = Ansi.annotate(figures.pointerSmall, Ansi.blackBright);
  const promptMsg = renderAutoCompleteOutput(
    state,
    leadingSymbol,
    trailingSymbol,
    options
...
const processSelectNext = <A>(
  state: SelectState,
  choices: SelectOptionsReq<A>["choices"]
) => {
  return Effect.succeed(
    Action.NextFrame({ state: (state + 1) % choices.length })
  );
};
...
const processAutoCompleteCursorUp = (state: AutoCompleteState) => {
  ...
  return Effect.succeed(
    Action.NextFrame({ state: { ...state, index: state.filtered[nextCursor] } })
  );
};
...
const processAutoCompleteNext = (state: AutoCompleteState) =>
...
const processAutoCompleteBackspace = <A>(
  ...
  const query = state.query.slice(0, state.query.length - 1);
  return Effect.succeed(
    Action.NextFrame({ state: updateAutoCompleteState(state, options, query) })
  );
};
...
const processAutoCompleteClear = <A>(
  ...
  Effect.succeed(
    Action.NextFrame({ state: updateAutoCompleteState(state, options, "") })
  );
...
const handleAutoCompleteProcess = <A>(options: AutoCompleteOptionsReq<A>) => {
  return (input: Terminal.UserInput, state: AutoCompleteState) => {
    ...
    switch (input.key.name) {
      ...
      default: {
        return processAutoCompleteInput(
          Option.getOrElse(input.input, () => ""),
          state,
          options
        );
...
const renderClearScreen = Effect.fnUntraced(function* (
  state: TextState,
  ...
  const clearOutput = clearOutputWithError(
    renderTextOutput(state, "?", figures.pointerSmall, options, {
      plain: true,
    }),
    columns,
    errorText
...
const renderTextNextFrame = Effect.fnUntraced(function* (
  state: TextState,
  options: TextOptionsReq
) {
  const figures = yield* platformFigures;
  const leadingSymbol = Ansi.annotate("?", Ansi.cyanBright);
  const trailingSymbol = Ansi.annotate(figures.pointerSmall, Ansi.blackBright);
  const promptMsg = renderTextOutput(
    state,
    leadingSymbol,
    trailingSymbol,
    options
...
const renderTextSubmission = Effect.fnUntraced(function* (
  state: TextState,
  options: TextOptionsReq
) {
  const figures = yield* platformFigures;
...
const handleToggleClear = Effect.fnUntraced(function* (
...
const renderToggle = (
  ...
  const selectedAnnotation = Ansi.combine(
    Ansi.underlined,
    submitted ? Ansi.white : Ansi.cyanBright
  );
...
const renderToggleNextFrame = Effect.fnUntraced(function* (
  state: ToggleState,
  options: ToggleOptionsReq
) {
  const figures = yield* platformFigures;
  ...
  const promptMsg = renderToggleOutput(
    toggle,
    leadingSymbol,
    trailingSymbol,
    options
...
const renderToggleSubmission = Effect.fnUntraced(function* (
  value: boolean,
  options: ToggleOptionsReq
) {
  const figures = yield* platformFigures;
