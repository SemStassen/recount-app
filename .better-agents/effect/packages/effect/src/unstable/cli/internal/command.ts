...
interface SubcommandGroup {
  ...
  readonly commands: Arr.NonEmptyReadonlyArray<
    Command<any, unknown, any, unknown, unknown>
  >;
}
...
> extends Command<Name, Input, ContextInput, E, R> {
  ...
  readonly parseContext: (
    input: ParsedTokens
  ) => Effect.Effect<ContextInput, CliError.CliError, Environment>;
  readonly handle: (
    input: Input,
...
): CommandInternal<Name, Input, E, R, ContextInput> =>
  self as CommandInternal<Name, Input, E, R, ContextInput>;
...
>(options: {
  ...
  readonly service?:
    | Context.Key<CommandContext<Name>, ContextInput>
    | undefined;
  ...
  readonly handle?:
    | ((
        input: Input,
        commandPath: ReadonlyArray<string>
      ) => Effect.Effect<void, E, R | Environment>)
    | undefined;
  ...
  const service =
    options.service ??
    Context.Service<CommandContext<Name>, ContextInput>(
      `${TypeId}/${options.name}`
    );
  ...
  const parseContext =
    options.parseContext ?? (makeParser(contextConfig) as any);
  ...
  const buildHelpDoc = (commandPath: ReadonlyArray<string>): HelpDoc => {
    ...
    for (const group of subcommands) {
      ...
      subcommandDocs.push({
        group: group.group,
        commands: Arr.map(
          visible as unknown as Arr.NonEmptyReadonlyArray<Command.Any>,
          (subcommand) => ({
...
export const toFlagDoc = (
  ...
  const formattedAliases = single.aliases.map((alias) =>
  ...
  return {
    ...
    type: single.typeName ?? Primitive.getTypeName(single.primitiveType),
    description: appendChoiceKeys(
      single.description,
      Primitive.getChoiceKeys(single.primitiveType)
    ),
 ...
 */
export const checkForDuplicateFlags = <
  Name extends string,
  Input,
  ContextInput,
  E,
