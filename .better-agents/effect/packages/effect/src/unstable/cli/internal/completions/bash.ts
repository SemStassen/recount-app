...
const sanitizeFunctionName = (s: string): string =>
  s.replace(/[^a-zA-Z0-9_]/g, "_");

const flagNamesForWordlist = (
  flag: Completions.FlagDescriptor
): Array<string> => {
...
const buildFlagGroupDeclarations = (
  ...
  lines.push(
    `  for _f in ${flags.flatMap(flagNamesForWordlist).join(" ")}; do`
  );
  lines.push(`    local _g="\${_flag_groups[$_f]:-}"`);
  lines.push(
    `    [[ -z "$_g" || -z "\${_used_groups[$_g]:-}" ]] && _filtered_flags+=" $_f"`
  );
  lines.push(`  done`);
  lines.push(``);
};

const flagValueCompletion = (
  type: Completions.FlagType
): string | undefined => {
  switch (type._tag) {
    case "Boolean":
      return undefined;
...
const generateFunction = (
  ...
  const flagsWithValues = descriptor.flags.filter(
    (f) => f.type._tag !== "Boolean"
  );
  ...
  const argsWithCompletions = descriptor.arguments.filter(
    (a) => argCompletion(a.type) !== undefined
  );
...
export const generate = (
  ...
  lines.push(
    `#   ${escapeForBash(executableName)} --completions bash >> ~/.bashrc`
  );
  lines.push(`#`);
  lines.push(``);
