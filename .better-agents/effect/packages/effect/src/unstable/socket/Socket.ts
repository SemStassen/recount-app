 ...
 * @since 4.0.0
 */
export const isSocket = (u: unknown): u is Socket =>
 ...
 * @since 4.0.0
 */
export const Socket: Context.Service<Socket, Socket> = Context.Service<Socket>(
...
export const make = (options: {
  ...
  Socket.of({
    ...
    runString:
      ...
      (options.run
        ? (handler, opts) =>
            options.run!((data) => handler(decoder.decode(data)), opts)
        : (handler, opts) =>
            options.runRaw(
              (data) =>
                typeof data === "string"
                  ...
                  : data instanceof Uint8Array
                    ...
                    : handler(decoder.decode(new Uint8Array(data))),
              opts
            )),
 ...
 * @since 4.0.0
 */
export const SocketErrorTypeId: SocketErrorTypeId =
 ...
 * @since 4.0.0
 */
export class SocketReadError extends Schema.ErrorClass<SocketReadError>(
 ...
 * @since 4.0.0
 */
export class SocketWriteError extends Schema.ErrorClass<SocketWriteError>(
 ...
 * @since 4.0.0
 */
export class SocketOpenError extends Schema.ErrorClass<SocketOpenError>(
 ...
 * @since 4.0.0
 */
export class SocketCloseError extends Schema.ErrorClass<SocketCloseError>(
  ...
  ): <E>(u: E) => Result.Result<SocketCloseError, E> {
    return function <E>(u: E) {
      return SocketError.is(u) &&
        u.reason._tag === "SocketCloseError" &&
        isClean(u.reason.code)
 ...
 * @since 4.0.0
 */
export class SocketError extends Schema.TaggedErrorClass<SocketError>(
  SocketErrorTypeId
)("SocketError", {
 ...
 * @since 4.0.0
 */
export class WebSocket extends Context.Service<
  WebSocket,
 ...
 * @since 4.0.0
 */
export const layerWebSocketConstructorGlobal: Layer.Layer<WebSocketConstructor> =
  Layer.succeed(WebSocketConstructor)(
...
export const fromWebSocket = <RO>(
  ...
  Effect.withFiber((fiber) => {
    ...
    const closeCodeIsError =
      options?.closeCodeIsError ?? defaultCloseCodeIsError;
    ...
    const runRaw = <_, E, R>(
      ...
      Effect.scopedWith(
        ...
        Effect.updateContext((input: Context.Context<R>) =>
...
export const layerWebSocket: (
  ...
  options?:
    ...
    | undefined
) => Layer.Layer<Socket, never, WebSocketConstructor> = flow(
  makeWebSocket,
  Layer.effect(Socket)
);
 ...
 * @since 4.0.0
 */
export const SendQueueCapacity = Context.Reference<number>(
  "~effect/socket/Socket/SendQueueCapacity",
  {
    defaultValue: () => 16,
  }
);
              ...
              while (true) {
                ...
                if (done) {
                  throw new SocketError({
                    reason: new SocketCloseError({ code: 1000 }),
                  });
                }
