    ...
    middleware === undefined
      ...
      : Effect.matchCauseEffect(tracer(middleware(responded)), {
          ...
          onSuccess(response): Effect.Effect<void, EH, RH> {
            ...
            return handledSymbol in request
              ? Effect.void
              : handleResponse(request, response);
          },
        });
 ...
 * @since 4.0.0
 */
export const appendPreResponseHandler = (
...
export const toWebHandlerWith =
  ...
  <E>(
    ...
    return (
      request: Request,
      reqContext?: Context.Context<never> | undefined
    ): Promise<globalThis.Response> =>
      new Promise((resolve) => {
...
export const toWebHandler: <E>(
  ...
  request: Request,
  context?: Context.Context<never> | undefined
) => Promise<globalThis.Response> = toWebHandlerWith(Context.empty());
...
>(
  ...
  function handler(
    ...
    handlerPromise ??= Effect.runPromise(
      Effect.gen(function* () {
...
export const fromWebHandler = (
  ...
  Effect.callback((resume, signal) => {
    ...
    if (requestResult._tag === "Failure") {
      return resume(
        Effect.fail(new HttpServerError({ reason: requestResult.failure }))
      );
    }
