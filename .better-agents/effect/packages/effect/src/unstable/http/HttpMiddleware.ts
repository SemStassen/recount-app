...
export declare namespace HttpMiddleware {
  ...
  export interface Applied<
    A extends Effect.Effect<HttpServerResponse, any, any>,
    E,
    R,
  > {
...
export const SpanNameGenerator = Context.Reference<
  (request: HttpServerRequest) => string
...
) => Effect.Effect<HttpServerResponse, E, HttpServerRequest | R> = make(
  (httpApp) =>
    Effect.withFiber((fiber) => {
      const request = Context.getUnsafe(fiber.context, HttpServerRequest);
...
export const tracer: <E, R>(
  ...
  (httpApp) =>
    Effect.withFiber((fiber) => {
      ...
      const span = internalEffect.makeSpanUnsafe(
        fiber,
        nameGenerator(request),
        {
          parent: Option.getOrUndefined(
            TraceContext.fromHeaders(request.headers)
          ),
          kind: "server",
        }
      );
      ...
      return Effect.onExitPrimitive(
        ...
        (exit) => {
          ...
          fiber.currentDispatcher.scheduleTask(() => {
            ...
            if (
              Option.isSome(url) &&
              (url.value.username !== "" || url.value.password !== "")
            ) {
            ...
            const requestHeaders = Headers.redact(
              request.headers,
              redactedHeaderNames
            );
            ...
            const responseHeaders = Headers.redact(
              response.headers,
              redactedHeaderNames
            );
...
export const cors = (options?: {
  ...
  const isAllowedOrigin =
    typeof opts.allowedOrigins === "function"
      ? opts.allowedOrigins
      : (origin: string) =>
          (opts.allowedOrigins as ReadonlyArray<string>).includes(origin);
