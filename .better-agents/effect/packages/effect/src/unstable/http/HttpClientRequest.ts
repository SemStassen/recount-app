 ...
 * @since 4.0.0
 */
export const isHttpClientRequest = (u: unknown): u is HttpClientRequest =>
...
const del: (url: string | URL, options?: Options.NoUrl) => HttpClientRequest =
  make("DELETE");
...
export const setUrl: {
  ...
  const hash = Option.fromNullishOr(
    clone.hash === "" ? undefined : clone.hash.slice(1)
  );
  clone.search = "";
  clone.hash = "";
  return makeWith(
...
export const bodyUint8Array: {
  ...
  (
    ...
    body: Uint8Array,
    contentType?: string
  ): HttpClientRequest => setBody(self, HttpBody.uint8Array(body, contentType))
);
...
export const bodyText: {
  ...
  (
    ...
    body: string,
    contentType?: string
  ): HttpClientRequest => setBody(self, HttpBody.text(body, contentType))
);
...
export const bodyFormDataRecord: {
  ...
  (
    self: HttpClientRequest,
    entries: HttpBody.FormDataInput
  ): HttpClientRequest => setBody(self, HttpBody.formDataRecord(entries))
);
...
export function toUrl(self: HttpClientRequest): Option.Option<URL> {
  const r = UrlParams.makeUrl(
    self.url,
    self.urlParams,
    Option.getOrUndefined(self.hash)
  );
