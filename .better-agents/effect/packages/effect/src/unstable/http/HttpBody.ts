 ...
 * @since 4.0.0
 */
export const isHttpBody = (u: unknown): u is HttpBody =>
...
export const json = (
  body: unknown,
  contentType?: string
): Effect.Effect<Uint8Array, HttpBodyError> =>
  Effect.try({
    try: () => text(JSON.stringify(body), contentType ?? "application/json"),
    catch: (cause) =>
      new HttpBodyError({ reason: { _tag: "JsonError" }, cause }),
  });
...
export const jsonSchema = <S extends Schema.Top>(
  ...
  return (
    body: S["Type"],
    contentType?: string
  ): Effect.Effect<Uint8Array, HttpBodyError, S["EncodingServices"]> =>
    encode(body, options).pipe(
      Effect.mapError(
        (issue) =>
          new HttpBodyError({
            reason: { _tag: "SchemaError", issue },
            cause: issue,
          })
      ),
 ...
 * @since 4.0.0
 */
export const formData = (body: globalThis.FormData): FormData =>
  new FormData(body);
 ...
 * @since 4.0.0
 */
export type FormDataInput = Record<
  string,
  FormDataCoercible | ReadonlyArray<FormDataCoercible>
>;
...
export const stream = (
  ...
  new Stream(body, contentType ?? "application/octet-stream", contentLength);
