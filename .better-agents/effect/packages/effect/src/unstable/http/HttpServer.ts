...
export const serve: {
  (): <E, R>(
    ...
    never,
    never,
    HttpServer | Exclude<R, HttpServerRequest | Scope.Scope>
  >;
  <E, R, App extends Effect.Effect<HttpServerResponse, any, any>>(
    middleware: Middleware.HttpMiddleware.Applied<App, E, R>
  ): (
...
export const serveEffect: {
  (): <E, R>(
    ...
    void,
    never,
    Scope.Scope | HttpServer | Exclude<R, HttpServerRequest>
  >;
  <E, R, App extends Effect.Effect<HttpServerResponse, any, any>>(
    middleware: Middleware.HttpMiddleware.Applied<App, E, R>
  ): (
 ...
 * @since 4.0.0
 */
export const logAddress: Effect.Effect<void, never, HttpServer> =
  addressFormattedWith((_) => Effect.log(`Listening on ${_}`));
...
> = Effect.gen(function* () {
  ...
  if (address._tag === "UnixAddress") {
    return yield* Effect.die(
      new Error("HttpServer.layerTestClient: UnixAddress not supported")
    );
  }
