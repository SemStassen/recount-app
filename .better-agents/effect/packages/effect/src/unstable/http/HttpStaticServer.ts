...
> = Effect.fnUntraced(function* (options) {
  ...
  const setFileHeaders = (
    ...
    if (cacheControl !== undefined) {
      currentResponse = HttpServerResponse.setHeader(
        currentResponse,
        "Cache-Control",
        cacheControl
      );
  ...
  const serveFile: (
    ...
    HttpServerResponse.HttpServerResponse,
    HttpServerError.HttpServerError
  > = Effect.fnUntraced(function* (request, filePath, fileSize) {
    ...
    if (shouldEvaluateConditionals) {
      fullResponse = setFileHeaders(
        yield* handlePlatformError(request, platform.fileResponse(filePath)),
        filePath
      );
      const conditionalResponse = evaluateConditionalRequest(
        request,
        fullResponse
      );
    ...
    const resolvedFileSize =
      rangeHeader === undefined
        ...
        : (fileSize ??
          Number(
            (yield* handlePlatformError(request, fileSystem.stat(filePath)))
              .size
          ));
    const parsedRange =
      rangeHeader === undefined || resolvedFileSize === undefined
        ? undefined
...
const matchesIfNoneMatch = (
  ...
  const normalizedEtag =
    etag === undefined ? undefined : stripWeakEtagPrefix(etag);
  for (const candidate of ifNoneMatch.split(",")) {
    ...
    if (
      ...
      stripWeakEtagPrefix(value) === normalizedEtag
    ) {
      return true;
    }
  }
...
const evaluateConditionalRequest = (
  ...
  if (ifNoneMatch !== undefined) {
    return matchesIfNoneMatch(ifNoneMatch, response.headers["etag"])
  ...
  if (
    ...
    isNotModifiedSince(ifModifiedSince, response.headers["last-modified"])
  ) {
