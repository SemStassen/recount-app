...
/** @internal */
export const EncryptionDerivationLabelV1 =
...
const makeEd25519Pkcs8FromSeed = (
  seed: Uint8Array
): Uint8Array<ArrayBuffer> => {
  const key = new Uint8Array(
    Ed25519Pkcs8SeedPrefix.byteLength + seed.byteLength
  );
...
const deriveSecretBytes = Effect.fnUntraced(function* (options: {
  ...
  const derivationInput = new Uint8Array(
    labelBytes.byteLength + 1 + options.rootSecret.byteLength
  );
  ...
  const digest = yield* Effect.promise(() =>
    options.crypto.subtle.digest("SHA-256", toArrayBuffer(derivationInput))
  );
...
/** @internal */
export const deriveIdentityRootSecretMaterial = Effect.fnUntraced(
  function* (options: {
    readonly crypto: Crypto;
    readonly rootSecret: Uint8Array;
  }) {
    ...
    const encryptionKey = yield* Effect.promise(() =>
      options.crypto.subtle.importKey(
        "raw",
        toArrayBuffer(encryptionKeyMaterial),
        "AES-GCM",
        true,
        ["encrypt", "decrypt"]
      )
    ...
    const signingPrivateKey = yield* Effect.promise(() =>
      options.crypto.subtle.importKey(
        "pkcs8",
        toArrayBuffer(signingPrivateKeyBytes),
        "Ed25519",
        true,
        ["sign"]
      )
    );
    const signingJwk = yield* Effect.promise(() =>
      options.crypto.subtle.exportKey("jwk", signingPrivateKey)
    );

    if (typeof signingJwk.x !== "string") {
      return yield* Effect.die(
        new Error("Unable to export deterministic Ed25519 public key")
      );
    }
    ...
    if (signingPublicKey.byteLength !== Ed25519PublicKeyLength) {
      return yield* Effect.die(
        new Error(
          `Expected derived signing public key to be ${Ed25519PublicKeyLength} bytes`
        )
      );
    }
