    ...
    return EventLogServer.layerRpcHandlers({
      ...
      onWrite: Effect.fnUntraced(function* (data) {
        ...
        return yield* storage
          .write(request.publicKey, request.storeId, entries)
          .pipe(
            Effect.catchCause((_) =>
              Effect.fail(
                new EventLogProtocolError({
 ...
 * @since 4.0.0
 */
export const layer: Layer.Layer<never, never, RpcServer.Protocol | Storage> =
  RpcServer.layer(EventLogRemoteRpcs).pipe(Layer.provide(layerRpcHandlers));
  ...
  return Storage.of({
    ...
    changes: Effect.fnUntraced(function* (publicKey, storeId, startSequence) {
      ...
      return Stream.fromArray(
        ensureJournal(scopeKey).slice(startSequence)
