 ...
 * @since 4.0.0
 */
export class EventLogServerUnencrypted extends Context.Service<
...
export const layerRpcHandlers: Layer.Layer<
  | Rpc.ToHandler<RpcGroup.Rpcs<typeof EventLogRemoteRpcs>>
  ...
  Effect.gen(function* () {
    ...
    return EventLogServer.layerRpcHandlers({
      ...
      onWrite: Effect.fnUntraced(function* (data) {
        ...
        const resolvedStoreId = yield* mapping
          .resolve({
        ...
        yield* processEntries({
          ...
          Effect.provideService(
            EventLog.Identity,
            makeClientIdentity(request.publicKey)
          )
        );
      }),
        ...
        return storage
          ...
          .pipe(
            Stream.mapArrayEffect((entries) =>
              Effect.map(ChangesRpc.encodeUnencrypted(entries), Arr.of)
            )
          );
      }, Stream.unwrap),
    });
 ...
 * @since 4.0.0
 */
export class EventLogServerStoreError extends Data.TaggedError(
 ...
 * @since 4.0.0
 */
export class EventLogServerAuthError extends Data.TaggedError(
 ...
 * @since 4.0.0
 */
export class EventLogServerAuthorization extends Context.Service<
 ...
 * @since 4.0.0
 */
export class StoreMapping extends Context.Service<
  StoreMapping,
  {
...
const representativeSequences = (options: {
  ...
  const maxSequence =
    options.remoteEntries[options.remoteEntries.length - 1]!.remoteSequence;
 ...
 * @since 4.0.0
 */
export const makeStorageMemory: Effect.Effect<
  Storage["Service"],
  never,
  Scope.Scope
> = Effect.gen(function* () {
  ...
  const pubsubs = yield* RcMap.make({
    lookup: (_storeId: string) =>
      Effect.acquireRelease(PubSub.unbounded<RemoteEntry>(), PubSub.shutdown),
  ...
  return Storage.of({
    ...
    entriesAfter: (storeId, entry) =>
      Effect.sync(() => {
        const journal = ensureJournal(storeId);
        return journal
          .filter((e) => Entry.Order(e.entry, entry) >= 0)
          .map((e) => e.entry);
      }),
    write,
    changes: Effect.fnUntraced(function* ({
      ...
      const replayedUpTo =
        backlog.length > 0
          ? backlog[backlog.length - 1].remoteSequence
          : startSequence - 1;
...
export const layerStorageMemory: Layer.Layer<Storage> =
  Layer.effect(Storage)(makeStorageMemory);
...
export const make = Effect.gen(function* () {
  ...
  return EventLogServerUnencrypted.of({
    makeWrite<Groups extends EventGroup.Any>(
      schema: EventLog.EventLogSchema<Groups>
    ) {
      const events = new Map<string, Event.AnyWithProps>();
      for (const group of schema.groups as unknown as ReadonlyArray<
        EventGroup.EventGroup<Event.Any>
      >) {
      ...
      return Effect.fnUntraced(function* (options: {
        ...
        if (schemaEvent === undefined) {
          return yield* Effect.die(
            `Event schema not found for: "${options.event}"`
          );
        }
        ...
        const entry = new EventJournal.Entry(
          {
            ...
            payload: yield* Schema.encodeUnknownEffect(
              schemaEvent.payloadMsgPack
            )(options.payload).pipe(
              Effect.mapError(
                (_) =>
                  new EventLogServerStoreError({
...
export const layerNoRpcServer = <Groups extends EventGroup.Any, E, R>(
  ...
  | Storage
  | StoreMapping
> => layerRpcHandlers.pipe(Layer.merge(layer), Layer.provide(layerServer));
  ...
  return Effect.fnUntraced(function* (options: {
    ...
    if (handler === undefined) {
      return yield* Effect.logDebug(
        `Event handler not found for: "${options.entry.event}"`
      );
    }
