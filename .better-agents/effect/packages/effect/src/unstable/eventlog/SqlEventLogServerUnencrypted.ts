    ...
    const remoteId = yield* selectRemoteId.pipe(
      Effect.flatMap((rows) => {
        ...
        return sql`
          ...
          VALUES (1, ${created})
        `.pipe(
          Effect.catchIf(isConstraintConflict, () => Effect.void),
...
const decodeRemoteEntryRows = Schema.decodeUnknownEffect(
  Schema.mutable(Schema.Array(RemoteEntrySql))
);
const decodeStoreSequenceRows = Schema.decodeUnknownEffect(
  Schema.Array(StoreSequenceSql)
);
...
const decodeStoreSequence = (
  rows: unknown
): Effect.Effect<number, Schema.SchemaError> =>
  decodeStoreSequenceRows(rows).pipe(
    Effect.flatMap((rows) => {
      const row = rows[0];
      if (row === undefined) {
        return Effect.die(
          "SqlEventLogServerUnencrypted missing store sequence row"
        );
      }
