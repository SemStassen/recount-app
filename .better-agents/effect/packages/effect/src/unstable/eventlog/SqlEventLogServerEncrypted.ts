...
> =>
  Effect.gen(function* () {
    ...
    const remoteId = yield* sql<{
      remote_id: Uint8Array;
    }>`SELECT remote_id FROM ${remoteIdTableSql}`.pipe(
    ...
    const resources = yield* RcMap.make({
      lookup: Effect.fnUntraced(function* (scopeKey: string) {
        const scopeHash = (yield* encryptions.sha256String(
      ...
      sql`
        ...
        Effect.map((rows) => {
          ...
          return row === undefined
            ? undefined
            : (row.signing_public_key as Uint8Array<ArrayBuffer>);
        }),
        Effect.orDie
      );
    ...
    return EventLogServerEncrypted.Storage.of({
      ...
      changes: Effect.fnUntraced(
        function* (publicKey, storeId, startSequence) {
          ...
          return Stream.fromArray(initial).pipe(
...
type EncryptedRemoteEntrySql = Schema.Schema.Type<
  typeof EncryptedRemoteEntrySql
>;
...
const decodeEntryRows = Schema.decodeUnknownEffect(
  Schema.Array(EncryptedRemoteEntrySql)
);
const decodeSessionAuthBindingRows = Schema.decodeUnknownEffect(
  Schema.Array(SessionAuthBindingSql)
);

const toEncryptedRemoteEntry = (
  row: EncryptedRemoteEntrySql
...
const decodeEntries = (
  ...
  ReadonlyArray<EventLogEncryption.EncryptedRemoteEntry>,
  Schema.SchemaError
> =>
  decodeEntryRows(rows).pipe(
    Effect.map((entries) => entries.map(toEncryptedRemoteEntry))
  );

const decodeSessionAuthBindings = (
  rows: unknown
): Effect.Effect<ReadonlyArray<SessionAuthBindingSql>, Schema.SchemaError> =>
  decodeSessionAuthBindingRows(rows);
...
export const layerStorageSubtle = (options?: {
  ...
  SqlError.SqlError,
  SqlClient.SqlClient
> => layerStorage(options).pipe(Layer.provide(EventLogEncryption.layerSubtle));

const makeEncryptedScopeKey = (publicKey: string, storeId: string): string =>
  `${publicKey}/${storeId}`;
