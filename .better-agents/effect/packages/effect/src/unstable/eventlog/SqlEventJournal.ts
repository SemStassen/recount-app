...
type WriteFromRemoteOptions = Parameters<
  EventJournal.EventJournal["Service"]["writeFromRemote"]
>[0];
...
> =>
  Effect.gen(function* () {
    ...
    ): ReadonlyArray<EventJournal.Entry> => rows.map(toEntry);

    const insertEntry = SqlSchema.void({
      Request: EntryRow,
      execute: (entry) =>
        sql`INSERT INTO ${entryTableSql} ${sql.insert(entry)} ON CONFLICT DO NOTHING`,
    });
    const insertEntries = SqlSchema.void({
      Request: EntryRowArray,
      execute: (entries) =>
        sql`INSERT INTO ${entryTableSql} ${sql.insert(entries)} ON CONFLICT DO NOTHING`,
    });
    const insertRemotes = SqlSchema.void({
      Request: RemoteRowArray,
      execute: (entries) =>
        sql`INSERT INTO ${remotesTableSql} ${sql.insert(entries)} ON CONFLICT DO NOTHING`,
    });
    ...
    > {
      ...
      if (entries.length > 0) {
        yield* sql<{
          id: Uint8Array;
        }>`SELECT id FROM ${entryTableSql} WHERE ${sql.in(
          "id",
      ...
      const uncommitted = options.entries.filter(
        (entry) => !existingIds.has(entry.entry.idString)
      );
