 ...
 * @since 4.0.0
 */
export const layerAuthMiddleware: Layer.Layer<EventLogAuthentication> =
  Layer.succeed(EventLogAuthentication, (effect, { client, rpc }) => {
    const identity = Context.getOrUndefined(
      client.annotations,
      EventLog.Identity
    );
    if (identity)
      return Effect.provideService(effect, EventLog.Identity, identity);
    return Effect.fail(
  ...
  EventLogRemoteRpcs.toLayer(
    Effect.gen(function* () {
      const clientChallenges = yield* Cache.make({
        lookup: (_clientId: number) =>
          Effect.orDie(EventLogSessionAuth.makeSessionAuthChallenge),
      ...
      const persistedSigningPublicKeys = yield* Cache.make({
        lookup: (key: SessionAuthCacheKey) =>
          options
            .getOrCreateSessionAuthBinding(key.publicKey, key.signingPublicKey)
            .pipe(
              Effect.catchCause((_) =>
                Effect.fail(
                  new EventLogProtocolError({
      ...
      return EventLogRemoteRpcs.of({
        ...
        "EventLog.Authenticate": Effect.fnUntraced(function* (
          request,
          { client }
        ) {
          const challenge = Option.getOrNull(
            yield* Cache.getOption(clientChallenges, client.id)
          );
          if (!challenge) {
            ...
            .pipe(
              Stream.mapArray(
                Arr.flatMap(
                  (
                    data
                  ): NonEmptyReadonlyArray<SingleMessage | ChunkedMessage> => {
