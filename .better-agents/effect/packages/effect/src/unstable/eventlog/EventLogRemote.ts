 ...
 * @since 4.0.0
 */
export class EventLogRemote extends Context.Service<
  EventLogRemote,
  {
    ...
    readonly changes: (options: {
      ...
      Queue.Dequeue<RemoteEntry, EventLogRemoteError>,
      never,
      Scope.Scope
    >;
 ...
 * @since 4.0.0
 */
export class EventLogRemoteError extends Data.TaggedError(
  "EventLogRemoteError"
)<{
...
> {
  ...
  let hello: HelloResponse | null = yield* client["EventLog.Hello"]().pipe(
    Effect.mapError(
      (cause) => new EventLogRemoteError({ method: "hello", cause })
    )
  );
  ...
  const authCache = yield* Cache.make({
    lookup: Effect.fnUntraced(
      ...
      Effect.mapError(
        (cause) => new EventLogRemoteError({ method: "authenticate", cause })
      )
    ),
  ...
  const retryForbidden = <A, E, R>(
    ...
    Effect.retry(effect, {
      while(e) {
        hello = null;
        const isForbidden =
          Predicate.isTagged(e, "EventLogProtocolError") &&
  ...
  const remote = EventLogRemote.of({
    ...
    write: Effect.fnUntraced(
      ...
      retryForbidden,
      Effect.mapError(
        (cause) => new EventLogRemoteError({ method: "write", cause })
      )
    ),
          ...
          for (let i = 0; i < parts.length; i++) {
            ...
            if (part._tag === "Single") {
              yield* Queue.offerAll(
                outgoing,
                yield* decodeChanges(options.identity, part.data)
              );
              continue;
            }
            ...
            yield* Queue.offerAll(
              outgoing,
              yield* decodeChanges(options.identity, data)
            );
          }
        }
      }).pipe(
    ...
    whenAuthenticated: (effect) =>
      IdentityService.use((identity) =>
        Effect.flatMap(ensureAuthenticated(identity), () => effect)
      ),
  });
