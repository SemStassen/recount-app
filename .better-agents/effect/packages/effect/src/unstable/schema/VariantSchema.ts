...
export const isField = (u: unknown): u is Field<any> =>
  Predicate.hasProperty(u, FieldTypeId);
 ...
 * @since 4.0.0
 */
export const fields = <A extends Struct<any>>(self: A): A[typeof TypeId] =>
  self[TypeId];
...
>
  extends
    Schema.Bottom<
      ...
      Schema.decodeTo<
        Schema.declareConstructor<Self, S["Encoded"], readonly [S], S["Iso"]>,
        S
      >,
      S["~type.make.in"],
      S["Iso"],
...
> extends Schema.Union<{
  readonly [K in keyof Members]: [Members[K]] extends [Schema.Top]
    ? Members[K]
    : never;
}> {}
...
>(options: {
  ...
  function FieldOnly<const Keys extends ReadonlyArray<Variants[number]>>(
    keys: Keys
  ) {
  ...
  function FieldExcept<const Keys extends ReadonlyArray<Variants[number]>>(
    keys: Keys
  ) {
    return function <S extends Schema.Top>(schema: S) {
      const obj: Record<string, S> = {};
