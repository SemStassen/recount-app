 ...
 * @since 4.0.0
 */
export interface GeneratedByDb<
 ...
 * @since 4.0.0
 */
export interface optionalOption<S extends Schema.Top> extends Schema.decodeTo<
 ...
 * @since 4.0.0
 */
export const optionalOption = <S extends Schema.Top>(
  schema: S
): optionalOption<S> =>
 ...
 * @since 4.0.0
 */
export const DateTimeWithNow = VariantSchema.Overrideable(
 ...
 * @since 4.0.0
 */
export interface UuidV4BytesInsert<
  B extends string,
> extends VariantSchema.Field<{
...
> =>
  schema.pipe(
    Schema.withConstructorDefault(
      Effect.sync(() => Uuid.v4({}, new globalThis.Uint8Array(16)))
    )
  );
