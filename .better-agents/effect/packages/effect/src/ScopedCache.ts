...
> =>
  effect.contextWith((context: Context.Context<any>) => {
    ...
    self.lookup = (key: Key): Effect.Effect<A, E> =>
      effect.updateContext(options.lookup(key), (input) =>
    ...
    self.timeToLive = options.timeToLive
      ? (exit: Exit.Exit<A, E>, key: Key) =>
          Duration.fromInputUnsafe(options.timeToLive!(exit, key))
      : defaultTimeToLive;
    return effect.as(
      Scope.addFinalizer(
        scope,
...
export const get: {
  <Key, A>(
    key: Key
  ): <E, R>(self: ScopedCache<Key, A, E, R>) => Effect.Effect<A, E, R>;
  <Key, A, E, R>(
        ...
        return checkCapacity(fiber, state.map, self.capacity).pipe(
          ...
          effect.onExit((exit) => {
            ...
            if (Duration.isFinite(ttl)) {
              entry.expiresAt =
                fiber.getRef(effect.ClockRef).currentTimeMillisUnsafe() +
...
const hasExpired = <A, E>(
  ...
  return (
    fiber.getRef(effect.ClockRef).currentTimeMillisUnsafe() >= entry.expiresAt
  );
};
...
const checkCapacity = <K, A, E>(
  ...
  for (const [key, entry] of map) {
    ...
    fibers.push(
      effect.forkUnsafe(
        parent as any,
        Scope.close(entry.scope, effect.exitVoid),
        true
      )
    );
    diff--;
    ...
    effect.uninterruptibleMask((restore) =>
      core.withFiber((fiber) =>
        effect.flatMap(getImpl(self, key, fiber), (entry) =>
          entry
            ? effect.asSome(restore(Deferred.await(entry.deferred)))
            : effect.succeedNone
        )
      )
    )
...
export const getSuccess: {
  <Key, A, R>(
    key: Key
  ): <E>(self: ScopedCache<Key, A, E, R>) => Effect.Effect<Option.Option<A>>;
  <Key, A, E, R>(
    self: ScopedCache<Key, A, E, R>,
    key: Key
...
export const set: {
  <Key, A>(
    key: Key,
    value: A
  ): <E, R>(self: ScopedCache<Key, A, E, R>) => Effect.Effect<void>;
  <Key, A, E, R>(
    self: ScopedCache<Key, A, E, R>,
    key: Key,
    value: A
...
export const has: {
  <Key, A>(
    key: Key
  ): <E, R>(self: ScopedCache<Key, A, E, R>) => Effect.Effect<boolean>;
  <Key, A, E, R>(
    self: ScopedCache<Key, A, E, R>,
    key: Key
  ...
  <Key, A, E>(self: ScopedCache<Key, A, E>, key: Key): Effect.Effect<boolean> =>
    effect.uninterruptible(
      core.withFiber((fiber) =>
        effect.map(getImpl(self, key, fiber, false), Predicate.isNotUndefined)
      )
    )
);
    ...
    effect.uninterruptibleMask(
      effect.fnUntraced(function* (restore) {
        ...
        const exit = yield* effect.exit(
          restore(Scope.provide(self.lookup(key), scope))
        );
        Deferred.doneUnsafe(deferred, exit);
        // @ts-ignore async gap
        if (self.state._tag === "Closed") {
 ...
 * @since 4.0.0
 */
export const invalidateAll = <Key, A, E, R>(
  self: ScopedCache<Key, A, E, R>
): Effect.Effect<void> =>
 ...
 * @since 4.0.0
 */
export const size = <Key, A, E, R>(
  self: ScopedCache<Key, A, E, R>
): Effect.Effect<number> =>
  effect.sync(() =>
    self.state._tag === "Closed" ? 0 : MutableHashMap.size(self.state.map)
  );
...
export const keys = <Key, A, E, R>(
  ...
  core.withFiber((fiber) => {
    ...
    return fibers.length === 0
      ? effect.succeed(keys)
      : effect.as(effect.fiberAwaitAll(fibers), keys);
  });
 ...
 * @since 4.0.0
 */
export const entries = <Key, A, E, R>(
  self: ScopedCache<Key, A, E, R>
