...
describe("Effect Eager Operations", () => {
  describe("fnUntracedEager", () => {
    ...
    it.effect("works with arguments and this context", () =>
      Effect.gen(function* () {
        const obj = {
          value: 10,
          fn: Effect.fnUntracedEager(function* (
            this: { value: number },
            x: number
          ) {
  ...
  describe("mapEager", () => {
    ...
    it.effect("failed effect preserves failure", () =>
      Effect.gen(function* () {
        ...
        const failure = exit.cause.reasons.find(
          (failure: any) => failure._tag === "Fail"
        );
        assert.ok(failure, "Expected to find a Fail cause");
        assert.strictEqual(
          failure._tag,
          "Fail",
          "Expected failure to be a Fail type"
        );
        assert.strictEqual(
          (failure as any).error,
          "error",
          "Expected error to be preserved"
        );
      })
    );
    ...
    it.effect("complex effect falls back to regular map", () =>
      Effect.gen(function* () {
        const effect = Effect.mapEager(
          Effect.delay(Effect.succeed(10), 1),
          (n) => n + 5
        );
    ...
    it.effect("applies transformation eagerly for failure effects", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          ...
          "Expected error to be transformed"
        );
      })
    );
  ...
  describe("catchEager", () => {
    it.effect("preserves success effects without applying catch", () =>
      Effect.gen(function* () {
        ...
        const caught = Effect.catchEager(effect, (err) =>
          Effect.succeed(`recovered: ${err}`)
        );
  ...
  describe("matchEager", () => {
    ...
    it.effect(
      "performance: applies transformations immediately for resolved effects",
      () =>
        Effect.gen(function* () {
  ...
  describe("matchCauseEager", () => {
    ...
    it.effect(
      "falls back to regular matchCause for delayed failure effects",
      () =>
        Effect.gen(function* () {
          let onSuccessCount = 0;
          let onFailureCount = 0;

          const delayedFailure = Effect.delay(
            Effect.fail("delayed error"),
            "5 millis"
          );
          const matcher = {
            onSuccess: (value: any) => {
