...
const TestGroup = RpcGroup.make(Rpc.make("Ping", { success: Schema.String }));
...
const makeProtocolLayer = (
  ...
  RpcClient.layerProtocolHttp({ url: "http://localhost/rpc" }).pipe(
    ...
    Layer.provideMerge(
      Layer.succeed(HttpClient.HttpClient, makeHttpClient(body))
    )
  );
...
const assertEmptyResponseFailsRequest = (
  ...
  Effect.gen(function* () {
    ...
    const cause = yield* client
      .Ping()
      .pipe(Effect.timeout("1 second"), Effect.sandbox, Effect.flip);
    ...
    assert.strictEqual(
      ...
      "Received empty HTTP response from RPC server"
    );
  });

describe("RpcClient", () => {
  it.effect(
    "fails request on empty HTTP response for unframed serialization",
    () => assertEmptyResponseFailsRequest(RpcSerialization.layerJson, "[]")
  );

  it.effect(
    "fails request on empty HTTP response for framed serialization",
