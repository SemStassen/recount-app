...
describe("Semaphore", () => {
        ...
        Effect.all(
          [
            ...
            sem.withPermits(
              "partition-1",
              1
            )(Effect.sync(() => messages.push("p1-task3"))),
          ],
          { concurrency: "unbounded", discard: true }
        )
      );
        ...
        Effect.all(
          [
            ...
            sem.withPermits(
              "partition-2",
              1
            )(Effect.sync(() => messages.push("p2-task2"))),
            sem.withPermits(
              "partition-2",
              1
            )(Effect.sync(() => messages.push("p2-task3"))),
          ],
          { concurrency: "unbounded", discard: true }
        )
      );
  ...
  it.effect(
    "interruption with partial permit acquisition releases all taken permits",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("module-level combinators delegate to the instance api", () =>
    Effect.gen(function* () {
      ...
      const available = yield* Semaphore.withPermitsIfAvailable(
        sem,
        1,
        Effect.succeed("ok")
      );
      ...
      const pipedAvailable = yield* Effect.succeed("pipe").pipe(
        Semaphore.withPermitsIfAvailable(sem, 1)
      );
