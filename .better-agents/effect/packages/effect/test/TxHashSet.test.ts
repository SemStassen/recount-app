...
describe("TxHashSet", () => {
  describe("constructors", () => {
    ...
    it.effect("fromIterable creates TxHashSet from iterable", () =>
      Effect.tx(
        Effect.gen(function* () {
          const txSet = yield* TxHashSet.fromIterable([
            "x",
            "y",
            "z",
            "x",
    ...
    it.effect("size returns correct count", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const fromIterable = yield* TxHashSet.fromIterable([
            "x",
            "y",
            "z",
            "x",
    ...
    it.effect("some tests if any element satisfies predicate", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            yield* TxHashSet.some(numbers, (n) => n > 3),
            true
          );
          assert.strictEqual(
            yield* TxHashSet.some(numbers, (n) => n > 10),
            false
          );
    ...
    it.effect("every tests if all elements satisfy predicate", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            yield* TxHashSet.every(numbers, (n) => n % 2 === 0),
            true
          );
          assert.strictEqual(
            yield* TxHashSet.every(numbers, (n) => n > 5),
            false
          );
    ...
    it.effect("reduce accumulates values", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const concatenated = yield* TxHashSet.reduce(
            strings,
            "",
            (acc, s) => acc + s
          );
    ...
    it.effect("works with custom Equal implementations", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          assert.strictEqual(yield* TxHashSet.size(txSet), 3);
          assert.strictEqual(
            yield* TxHashSet.has(txSet, new TestValue("alice")),
            true
          );
          assert.strictEqual(
            yield* TxHashSet.has(txSet, new TestValue("dave")),
            false
          );
          ...
          assert.strictEqual(yield* TxHashSet.size(txSet), 2);
          assert.strictEqual(
            yield* TxHashSet.has(txSet, new TestValue("bob")),
            false
          );
        })
      )
  ...
  describe("stress testing", () => {
    ...
    it.effect("functional operations work with large sets", () =>
      Effect.tx(
        Effect.gen(function* () {
          const numbers = yield* TxHashSet.fromIterable(
    ...
    it.effect("handles empty set operations correctly", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const emptyIntersection = yield* TxHashSet.intersection(
            empty1,
            nonEmpty
          );
          ...
          assert.strictEqual(
            yield* TxHashSet.isSubset(nonEmpty, empty1),
            false
          );
        })
      )
