...
describe("Result", () => {
  describe("Constructors", () => {
    ...
    it("try", () => {
      ...
      deepStrictEqual(
        Result.try({ try: () => 1, catch: (e) => new Error(String(e)) }),
        Result.succeed(1)
      );
      deepStrictEqual(
        Result.try({
  ...
  describe("Methods", () => {
    ...
    it("toJSON", () => {
      deepStrictEqual(Result.succeed(1).toJSON(), {
        _id: "Result",
        _tag: "Success",
        value: 1,
      });
      deepStrictEqual(Result.fail("e").toJSON(), {
        _id: "Result",
        _tag: "Failure",
        failure: "e",
      });
    ...
    it("inspect", () => {
      deepStrictEqual(
        inspect(Result.succeed(1)),
        inspect({ _id: "Result", _tag: "Success", value: 1 })
      );
      deepStrictEqual(
        inspect(Result.fail("e")),
        inspect({ _id: "Result", _tag: "Failure", failure: "e" })
      );
    });
  ...
  describe("Getters", () => {
    ...
    it("getOrThrow", () => {
      ...
      throws(
        () => pipe(Result.fail(new Error("e")), Result.getOrThrow),
        new Error("e")
      );
    });
  ...
  describe("Mapping", () => {
    ...
    it("mapError", () => {
      ...
      // data-first
      assertSuccess(
        Result.mapError(Result.succeed("a"), (n: number) => n * 2),
        "a"
      );
      assertFailure(
        Result.mapError(Result.fail(1), (n) => n * 2),
        2
      );
    });
  ...
  describe("Pattern Matching", () => {
    it("match handles failure and success cases", () => {
      ...
      // data-first
      strictEqual(
        Result.match(Result.fail("abc"), { onFailure, onSuccess }),
        "failure3"
      );
      strictEqual(
        Result.match(Result.succeed("abc"), { onFailure, onSuccess }),
        "success3"
      );
    });
  });
  ...
  describe("Utils", () => {
    ...
    it("liftPredicate", () => {
      ...
      const isNumberRefinement = (n: string | number): n is number =>
        typeof n === "number";
      const onNumberRefinementError = (n: string | number) =>
        `${n} is not a number`;

      assertSuccess(
        pipe(
          1,
          Result.liftPredicate(isPositivePredicate, onPositivePredicateError)
        ),
        1
      );
      assertFailure(
      ...
      assertSuccess(
        pipe(
          1,
          Result.liftPredicate(isNumberRefinement, onNumberRefinementError)
        ),
        1
      );
      assertFailure(
  ...
  describe("Equivalence", () => {
    it("makeEquivalence compares successes and failures by channel", () => {
      ...
      deepStrictEqual(
        isEquivalent(Result.succeed(1), Result.succeed(2)),
        false
      );
      deepStrictEqual(
        isEquivalent(Result.succeed(1), Result.fail("foo")),
        false
      );
      deepStrictEqual(
        isEquivalent(Result.fail("foo"), Result.fail("foo")),
        true
      );
      deepStrictEqual(
        isEquivalent(Result.fail("foo"), Result.fail("bar")),
        false
      );
      deepStrictEqual(
        isEquivalent(Result.fail("foo"), Result.succeed(1)),
        false
      );
    });
  });
  ...
  describe("Sequencing", () => {
    it("flatMap", () => {
      ...
      // data-first
      assertSuccess(
        Result.flatMap(Result.succeed("abc"), flow(Str.length, Result.succeed)),
        3
      );
      assertFailure(
        Result.flatMap(
          Result.fail("maError"),
          flow(Str.length, Result.succeed)
        ),
        "maError"
      );
    });
    ...
    it("andThen replaces successes with Result or plain values", () => {
      ...
      assertSuccess(
        pipe(Result.succeed(1), Result.andThen(Result.succeed(2))),
        2
      );
    ...
    it("all", () => {
      ...
      assertSuccess(Result.all([Result.succeed(1), Result.succeed(true)]), [
        1,
        true,
      ]);
      ...
      assertSuccess(
        Result.all({ a: Result.succeed(1), b: Result.succeed(true) }),
        { a: 1, b: true }
      );
      assertFailure(
        Result.all({ a: Result.succeed(1), b: Result.fail("e") }),
        "e"
      );
      // iterables
  ...
  describe("Error Handling", () => {
    it("orElse recovers failures and preserves successes", () => {
      ...
      // data-first
      assertSuccess(
        Result.orElse(Result.succeed(1), () => Result.succeed(2)),
        1
      );
      assertSuccess(
        Result.orElse(Result.fail("a"), () => Result.succeed(2)),
        2
      );
      assertFailure(
        Result.orElse(Result.fail("a"), () => Result.fail("b")),
        "b"
      );
    });
  });
  ...
  describe("Do Notation", () => {
    ...
    it("bindTo", () => {
      ...
      assertSuccess(pipe(Result.succeed(1), Result.bindTo("__proto__")), {
        ["__proto__"]: 1,
      });
    });
  ...
  describe("Transposing", () => {
    it("transposeOption", () => {
      assertSuccess(
        Result.transposeOption(Option.some(Result.succeed(1))),
        Option.some(1)
      );
    ...
    it("transposeMapOption", () => {
      assertSuccess(
        Result.transposeMapOption(Option.some(Result.succeed(1)), identity),
        Option.some(1)
      );
      assertSuccess(
        Result.transposeMapOption(Option.none(), identity),
        Option.none()
      );
      assertFailure(
        Result.transposeMapOption(Option.some(Result.fail("e")), identity),
        "e"
      );
      // data-last
      assertSuccess(
        pipe(Option.some(1), Result.transposeMapOption(Result.succeed)),
        Option.some(1)
      );
      assertSuccess(
        pipe(Option.none<number>(), Result.transposeMapOption(Result.succeed)),
        Option.none()
      );
    });
  });
