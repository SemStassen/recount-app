...
describe("AtomRpc", () => {
  it.effect("query creates a serializable atom", () =>
    Effect.gen(function* () {
      ...
      const atomFromEncodedPayload = Client.query(
        "getUser",
        { id: 1 },
        {
          headers: {
      ...
      assert(
        Atom.isSerializable(atomFromEncodedPayload),
        "expected query atom from encoded payload to be serializable"
      );
      assert(atom === atomFromEncodedPayload);
      assert.strictEqual(
        atomFromEncodedPayload[Atom.SerializableTypeId].key,
        key
      );
