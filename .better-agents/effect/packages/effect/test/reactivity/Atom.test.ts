...
describe.sequential("Atom", () => {
  ...
  it("runtime", async () => {
    const count = counterRuntime
      .atom(Counter.use((_) => _.get))
      .pipe(Atom.withLabel("count"));
    const r = AtomRegistry.make();
  ...
  it("runtime replacement", async () => {
    ...
    const r = AtomRegistry.make({
      initialValues: [Atom.initialValue(count, 10)],
    });
    const result = r.get(count);
  ...
  it("runtime multiple", async () => {
    const buildCount = buildCounterRuntime.fn<void>()((_) =>
      BuildCounter.use((_) => _.get)
    );
    const count = counterRuntime.atom(Counter.use((_) => _.get));
  ...
  it("effect initial", async () => {
    const count = Atom.make(Effect.succeed(1).pipe(Effect.delay(100)), {
      initialValue: 0,
    }).pipe(Atom.keepAlive);
    const r = AtomRegistry.make();
  ...
  it("disposed lifetime apis are no-ops", () => {
    ...
    const option = Atom.make<Option.Option<number>>(Option.some(1)).pipe(
      Atom.keepAlive
    );
    const result = Atom.make<AsyncResult.AsyncResult<number, never>>(
    ...
    expect(() => disposed.addFinalizer(() => {})).not.toThrow();
    ...
    expect(() =>
      disposed.setResult(result, AsyncResult.success(2))
    ).not.toThrow();
    ...
    expect(() => disposed.subscribe(state, () => {})).not.toThrow();
  ...
  it("stream initial", async () => {
    const count = Atom.make(
      Stream.range(1, 2).pipe(Stream.tap(() => Effect.sleep(50))),
      { initialValue: 0 }
    );
    const r = AtomRegistry.make();
    const unmount = r.mount(count);
  ...
  it("streamFn", async () => {
    const count = Atom.fn((start: number) =>
      Stream.range(start, start + 1).pipe(Stream.tap(() => Effect.sleep(50)))
    );
    const r = AtomRegistry.make();
    const unmount = r.mount(count);
    let result = r.get(count);
  ...
  it("pull", async () => {
    const count = Atom.pull(
      Stream.range(0, 1, 1).pipe(Stream.tap(() => Effect.sleep(50)))
    );
    const r = AtomRegistry.make();
    const unmount = r.mount(count);
    ...
    assert.deepEqual(
      ...
      Option.some({ done: false, items: [0] })
    );
    ...
    assert.deepEqual(
      AsyncResult.value(result),
      Option.some({ done: true, items: [0, 1] })
    );
  ...
  it("pull runtime", async () => {
    ...
    assert.deepEqual(
      AsyncResult.value(result),
      Option.some({ done: false, items: [1] })
    );
    ...
    assert.deepEqual(
      AsyncResult.value(result),
      Option.some({ done: true, items: [1, 2] })
    );
  ...
  it("pull refreshable", async () => {
    const count = Atom.pull(() =>
      Stream.range(1, 2, 1).pipe(Stream.tap(() => Effect.sleep(50)))
    );
    const r = AtomRegistry.make();
    const unmount = r.mount(count);
  ...
  it("family", async () => {
    ...
    const countKeep = Atom.family((n: number) =>
      Atom.make(n).pipe(Atom.keepAlive)
    );
  ...
  it("label", async () => {
    expect(Atom.make(0).pipe(Atom.withLabel("counter")).label![1]).toMatch(
      /Atom.test.ts:\d+:\d+/
    );
  });
  ...
  it("map with initialValue still rerenders when source changes", () => {
    ...
    const r = AtomRegistry.make({
      initialValues: [Atom.initialValue(mapped, 10)],
    });
    r.mount(mapped);
  ...
  it("get.stream", async () => {
    ...
    const multiplied = Atom.make((get) =>
      get.stream(count).pipe(Stream.map((_) => _ * 2))
    );
    ...
    assert.deepStrictEqual(
      r.get(multiplied),
      AsyncResult.success(0, { waiting: true })
    );
    ...
    assert.deepStrictEqual(
      r.get(multiplied),
      AsyncResult.success(2, { waiting: true })
    );
  ...
  it("get.streamResult", async () => {
    ...
    const plusOne = Atom.make((get) =>
      get.streamResult(multiplied).pipe(Stream.map((_) => _ + 1))
    );
    ...
    assert.deepStrictEqual(
      r.get(plusOne),
      AsyncResult.success(1, { waiting: true })
    );
    ...
    assert.deepStrictEqual(
      r.get(plusOne),
      AsyncResult.success(3, { waiting: true })
    );
  ...
  it("stream failure keeps previousSuccess", async () => {
    const atom = Atom.make(() =>
      Stream.succeed(1).pipe(Stream.concat(Stream.fail("boom")))
    );
    const r = AtomRegistry.make();
  ...
  test(`refreshOnSignal uses registry initial values as source state`, () => {
    ...
    const atom = source.pipe(Atom.makeRefreshOnSignal(signal), Atom.keepAlive);
    const r = AtomRegistry.make({
      initialValues: [Atom.initialValue(atom, 10)],
    });
    r.mount(atom);
  ...
  test(`debounce uses registry initial values as source state`, async () => {
    ...
    const atom = source.pipe(Atom.debounce(100), Atom.keepAlive);
    const r = AtomRegistry.make({
      initialValues: [Atom.initialValue(atom, 10)],
    });
    r.mount(atom);
  ...
  test(`withRefresh uses registry initial values as source state`, async () => {
    ...
    const atom = source.pipe(Atom.withRefresh(100), Atom.keepAlive);
    const r = AtomRegistry.make({
      initialValues: [Atom.initialValue(atom, 10)],
    });
    r.mount(atom);
  ...
  test(`swr keeps previous value while stale revalidation runs`, async () => {
    ...
    let runs = 0;
    const atom = Atom.make(
      Effect.sync(() => ++runs).pipe(Effect.delay(50))
    ).pipe(Atom.swr({ staleTime: 100 }));
    const unmount = r.mount(atom);
  ...
  test(`swr refreshes failure without previousSuccess`, () => {
    ...
    }).pipe(Atom.swr({ staleTime: 1_000, revalidateOnMount: false }));
    const unmount = r.mount(atom);
  ...
  test(`swr refresh is forceful while waiting`, async () => {
    ...
    let runs = 0;
    const atom = Atom.make(
      Effect.sync(() => ++runs).pipe(Effect.tap(() => Effect.sleep(50)))
    ).pipe(Atom.swr({ staleTime: 0 }));
    const unmount = r.mount(atom);
  ...
  test(`swr revalidates on stale remount when enabled`, async () => {
    ...
    const atom = base.pipe(
      Atom.swr({ staleTime: 100, revalidateOnMount: true })
    );
  ...
  test(`swr does not revalidate on fresh remount when enabled`, async () => {
    ...
    const base = Atom.make(Effect.sync(() => ++runs)).pipe(Atom.keepAlive);
    const atom = base.pipe(
      Atom.swr({ staleTime: 10_000, revalidateOnMount: true })
    );
  ...
  test(`swr does not revalidate on stale remount when disabled`, async () => {
    ...
    const base = Atom.make(Effect.sync(() => ++runs)).pipe(Atom.keepAlive);
    const atom = base.pipe(
      Atom.swr({ staleTime: 100, revalidateOnMount: false })
    );
  ...
  test(`swr preserves writable set semantics`, () => {
    ...
    const source = Atom.writable<
      AsyncResult.AsyncResult<number, never>,
      number
    >(
      (get) =>
        Option.getOrElse(
          get.self<AsyncResult.AsyncResult<number, never>>(),
  ...
  test(`swr uses registry initial values as source state`, () => {
    ...
    const r = AtomRegistry.make({
      initialValues: [Atom.initialValue(atom, initial)],
    });
    r.mount(atom);
  ...
  describe("optimistic", () => {
    ...
    it("Result", async () => {
      ...
      // optimistic phase: the optimistic value is set, but the true value is not
      expect(r.get(atom)).toEqual(AsyncResult.success(0));
      expect(r.get(optimisticAtom)).toEqual(
        AsyncResult.success(1, { waiting: true })
      );
    ...
    it("intermediate updates", async () => {
      ...
      assert.deepStrictEqual(
        r.get(optimisticAtom),
        AsyncResult.success(123, { waiting: true })
      );
  ...
  describe("Reactivity", () => {
    ...
    it("rebuilds on mutation with a registry initial value", async () => {
      ...
      const atom = Atom.make(() => {
        rebuilds++;
        return value;
      }).pipe(Atom.withReactivity(["counter"]), Atom.keepAlive);
      const r = AtomRegistry.make({
        initialValues: [Atom.initialValue(atom, 10)],
      });
      const fn = counterRuntime.fn(
  ...
  it("writable derived clears waiting after refresh", async () => {
    ...
    const base = Atom.make(
      Effect.sync(() => ++count).pipe(Effect.delay(100))
    ).pipe(Atom.withLabel("base"));
  ...
  it("get.result suspendOnWaiting", async () => {
    ...
    const outer = Atom.make((get) =>
      get.result(inner, { suspendOnWaiting: true })
    );
