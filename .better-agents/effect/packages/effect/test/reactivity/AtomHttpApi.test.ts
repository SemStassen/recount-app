...
describe("AtomHttpApi", () => {
  it.effect(
    "query creates a serializable atom that encodes the request and decodes the response",
    () =>
      Effect.gen(function* () {
        ...
        if (!Atom.isSerializable(atomFromEncodedInput)) {
          assert.fail(
            "expected query atom from encoded input to be serializable"
          );
        }
        assert.strictEqual(
          atomFromEncodedInput[Atom.SerializableTypeId].key,
          key
        );
