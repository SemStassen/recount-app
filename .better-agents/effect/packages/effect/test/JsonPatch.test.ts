...
describe("JsonPatch", () => {
  describe("get", () => {
    describe("root values", () => {
      ...
      it("emits a root replace for primitive changes", () => {
        ...
        for (const [from, to] of cases) {
          deepStrictEqual(JsonPatch.get(from, to), [
            { op: "replace", path: "", value: to },
          ]);
        }
      });

      it("emits a root replace for type changes", () => {
        deepStrictEqual(JsonPatch.get(1, "string"), [
          { op: "replace", path: "", value: "string" },
        ]);
        deepStrictEqual(JsonPatch.get([1, 2], { a: 1 }), [
          { op: "replace", path: "", value: { a: 1 } },
        ]);
        deepStrictEqual(JsonPatch.get({ a: 1 }, [1, 2]), [
    ...
    describe("arrays", () => {
      it("adds new elements (append semantics)", () => {
        deepStrictEqual(JsonPatch.get([1, 2], [1, 2, 3]), [
      ...
      it("removes elements in descending index order (no index shifting)", () => {
        deepStrictEqual(JsonPatch.get([1, 2, 3], [1, 2]), [
          { op: "remove", path: "/2" },
        ]);
      ...
      it("replaces modified elements", () => {
        deepStrictEqual(JsonPatch.get([1, 2, 3], [1, 4, 3]), [
    ...
    describe("objects", () => {
      ...
      it("adds / removes / replaces properties", () => {
        deepStrictEqual(JsonPatch.get({ a: 1 }, { a: 1, b: 2 }), [
          { op: "add", path: "/b", value: 2 },
        ]);
        deepStrictEqual(JsonPatch.get({ a: 1, b: 2 }, { a: 1 }), [
          { op: "remove", path: "/b" },
        ]);
        deepStrictEqual(JsonPatch.get({ a: 1, b: 2 }, { a: 1, b: 3 }), [
    ...
    describe("edge cases", () => {
      it("handles single-element arrays", () => {
        ...
        deepStrictEqual(JsonPatch.get([], [1]), [
          { op: "add", path: "/0", value: 1 },
        ]);
      });

      it("handles single-property objects", () => {
        deepStrictEqual(JsonPatch.get({ a: 1 }, { a: 2 }), [
          { op: "replace", path: "/a", value: 2 },
        ]);
        deepStrictEqual(JsonPatch.get({ a: 1 }, {}), [
          { op: "remove", path: "/a" },
        ]);
        deepStrictEqual(JsonPatch.get({}, { a: 1 }), [
          { op: "add", path: "/a", value: 1 },
      ...
      it("handles array index 0 operations", () => {
        deepStrictEqual(JsonPatch.get([1, 2, 3], [0, 2, 3]), [
      ...
      it("handles operations at last array index", () => {
        deepStrictEqual(JsonPatch.get([1, 2, 3], [1, 2, 4]), [
  ...
  describe("apply", () => {
    describe("happy paths", () => {
      ...
      it("replace", () => {
        deepStrictEqual(
          JsonPatch.apply([{ op: "replace", path: "", value: 42 }], 1),
          42
        );
        ...
        deepStrictEqual(
          JsonPatch.apply([{ op: "replace", path: "/a/b", value: 2 }], {
            a: { b: 1 },
          }),
          { a: { b: 2 } }
        );
      ...
      it("add", () => {
        ...
        deepStrictEqual(
          JsonPatch.apply(
            [{ op: "add", path: "/users/0/tags/-", value: "admin" }],
            {
              users: [{ id: 1, tags: [] }],
            }
          ),
      ...
      it("remove", () => {
        ...
        deepStrictEqual(
          JsonPatch.apply([{ op: "remove", path: "/a/b" }], {
            a: { b: 1, c: 2 },
          }),
          { a: { c: 2 } }
        );
    ...
    describe("JSON Pointer decoding", () => {
      it("decodes '~1' as '/' and '~0' as '~'", () => {
        deepStrictEqual(
          JsonPatch.apply([{ op: "replace", path: "/a~1b", value: 2 }], {
            "a/b": 1,
          }),
          { "a/b": 2 }
        );
        ...
        deepStrictEqual(
          JsonPatch.apply(
            [{ op: "add", path: "/path~1to~0key", value: "value" }],
            {}
          ),
          { "path/to~key": "value" }
        );
    ...
    describe("errors", () => {
      ...
      it("rejects add/replace when the parent is missing or not a container", () => {
        ...
        expectMessage(
          () =>
            JsonPatch.apply([{ op: "replace", path: "/a/b", value: 1 }], {
              a: 42,
            }),
          "not a container"
        );
        ...
        expectMessage(
          () =>
            JsonPatch.apply([{ op: "add", path: "/a/b/c", value: 1 }], {
              a: { b: "not-object" },
            }),
          "not a container"
        );
