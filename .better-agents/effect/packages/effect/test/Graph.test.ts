...
describe("Graph", () => {
    ...
    describe("undefined edge data", () => {
      it("should allow adding edges with undefined data", () => {
        ...
        expect(Graph.getEdge(graph, 0)).toEqual(
          Option.some(new Graph.Edge({ source: 0, target: 1, data: undefined }))
        );
      });
      ...
      it("should correctly update edges with undefined data", () => {
        ...
        expect(edge0).toEqual(
          Option.some(new Graph.Edge({ source: 0, target: 1, data: 100 }))
        );
        expect(edge1).toEqual(
          Option.some(new Graph.Edge({ source: 1, target: 0, data: undefined }))
        );
      });
  ...
  describe("mapEdges", () => {
    ...
    it("should modify graph in place during construction", () => {
      ...
      const graph = Graph.directed<string, number>((mutable) => {
        ...
        assertSome(
          beforeData,
          new Graph.Edge({ source: 0, target: 1, data: 10 })
        );
  ...
  describe("reverse", () => {
    it("should reverse all edge directions", () => {
      ...
      assertSome(
        edge0,
        new Graph.Edge({ source: nodeB!, target: nodeA!, data: 1 })
      );
      assertSome(
        edge1,
        new Graph.Edge({ source: nodeC!, target: nodeB!, data: 2 })
      );
      assertSome(
        edge2,
        new Graph.Edge({ source: nodeA!, target: nodeC!, data: 3 })
      );
    });
  ...
  describe("filterMapNodes", () => {
    ...
    it("should remove edges connected to filtered nodes", () => {
      ...
      assertSome(
        remainingEdge,
        new Graph.Edge({ source: 0, target: 2, data: 3 })
      );
