...
describe("Effect", () => {
  ...
  describe("tracing", () => {
    it.effect("failCause captures stack frame", () =>
      Effect.gen(function* () {
        const cause = yield* Effect.failCause(
          Cause.die(new Error("boom"))
        ).pipe(Effect.withSpan("test span"), Effect.sandbox, Effect.flip);
  ...
  describe("fromOption", () => {
    ...
    it("from a none", () =>
      Option.none().pipe(
        ...
        Effect.tap((error) =>
          Effect.sync(() =>
            assert.ok(error instanceof Cause.NoSuchElementError)
          )
        ),
        Effect.runPromise
      ));
  ...
  describe("fromResult", () => {
    ...
    it("from a failure", () =>
      Result.fail("error").pipe(
        ...
        Effect.flip,
        Effect.tap((error) =>
          Effect.sync(() => assert.strictEqual(error, "error"))
        ),
        Effect.runPromise
      ));
  });
  ...
  describe("gen", () => {
    it("gen", () =>
      Effect.gen(function* () {
        ...
        .then((_) => assert.deepStrictEqual(_, 1)));
    ...
    it("gen with context", () =>
      Effect.gen({ self: { a: 1, b: 2 } }, function* () {
        ...
        .pipe(Effect.runPromise)
        .then((_) => assert.deepStrictEqual(_, 3)));
  });

  describe("forEach", () => {
    it("sequential", () =>
      Effect.gen(function* () {
        const results = yield* Effect.forEach([1, 2, 3], (_) =>
          Effect.succeed(_)
        );
    ...
    it.effect("inherit unbounded", () =>
      Effect.gen(function* () {
        const handle = yield* Effect.forEach(
          [1, 2, 3],
          (_) => Effect.succeed(_).pipe(Effect.delay(50)),
          {
            concurrency: "inherit",
          }
        ).pipe(Effect.withConcurrency("unbounded"), Effect.forkChild);
  ...
  describe("all", () => {
    ...
    it.effect("iterable", () =>
      Effect.gen(function* () {
        const results = (yield* Effect.all(
          new Set([Effect.succeed(1), Effect.succeed(2), Effect.succeed(3)])
  ...
  describe("validate", () => {
    ...
    it.effect("accumulates all failures", () =>
      Effect.gen(function* () {
        const values = [0, 1, 2, 3, 4, 5];
        const errors = yield* Effect.validate(values, (n) =>
          n % 2 === 0 ? Effect.fail(n) : Effect.succeed(n)
        ).pipe(Effect.flip);
    ...
    it.effect("supports discard option", () =>
      Effect.gen(function* () {
        const result = yield* Effect.validate([1, 2, 3], Effect.succeed, {
          discard: true,
        });
  ...
  describe("findFirstFilter", () => {
    it.effect("returns first successful transformed value", () =>
      Effect.gen(function* () {
        const result = yield* Effect.findFirstFilter([1, 2, 3, 4], (n) =>
          Effect.succeed(
            n % 2 === 0 ? Result.succeed(`n=${n}`) : Result.failVoid
          )
        );
    ...
    it.effect("passes index to filter", () =>
      Effect.gen(function* () {
        const result = yield* Effect.findFirstFilter([10, 11, 12], (n, i) =>
          Effect.succeed(
            i === 2 ? Result.succeed(`${n}:${i}`) : Result.failVoid
          )
        );
    ...
    it.effect("returns none when filter never succeeds", () =>
      Effect.gen(function* () {
        const result = yield* Effect.findFirstFilter([1, 2, 3], () =>
  ...
  describe("filter", () => {
    it.live("odd numbers", () =>
      Effect.gen(function* () {
        const results = yield* Effect.filter([1, 2, 3, 4, 5], (value) =>
    ...
    it.live("iterable", () =>
      Effect.gen(function* () {
        const results = yield* Effect.filter(
          new Set([1, 2, 3, 4, 5]),
          (value) => Effect.succeed(value % 2 === 1)
        );
        assert.deepStrictEqual(results, [1, 3, 5]);
      })
    );
  ...
  describe("acquireDisposable", () => {
    it.effect("releases disposables", ({ expect }) =>
      Effect.gen(function* () {
        const acquire = Effect.sync(
          (): Disposable => ({ [Symbol.dispose]: release })
        );
    ...
    it.effect("releases async disposables", ({ expect }) =>
      Effect.gen(function* () {
        const acquire = Effect.sync(
          (): AsyncDisposable => ({ [Symbol.asyncDispose]: release })
        );
  ...
  it.effect("raceAll", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* Effect.raceAll(
        [500, 300, 200, 0, 100].map((ms) =>
  ...
  it.effect("raceAllFirst", () =>
    Effect.gen(function* () {
      const interrupted: Array<number> = [];
      const fiber = yield* Effect.raceAllFirst(
        [500, 300, 200, 0, 100].map((ms) =>
          (ms === 0 ? Effect.fail("boom") : Effect.succeed(ms)).pipe(
            Effect.delay(ms),
            Effect.onInterrupt(() =>
  ...
  describe("repeat", () => {
    ...
    it.effect(
      "repeat/until - repeats until an effectful condition is true",
      () =>
        Effect.gen(function* () {
          let input = 10;
          let output = 0;
    ...
    it.effect(
      "repeat/while - repeats while an effectful condition is true",
      () =>
        Effect.gen(function* () {
          let input = 10;
          let output = 0;
    ...
    it.effect(
      "repeat/schedule - repeats according to the specified schedule",
      () =>
        Effect.gen(function* () {
          let n = 0;
          const increment = Effect.sync(() => ++n);
  ...
  describe("retry", () => {
    it.live("nothing on success", () =>
      Effect.gen(function* () {
        let count = 0;
        yield* Effect.sync(() => count++).pipe(Effect.retry({ times: 10000 }));
    ...
    it.effect(
      "retry/until - retries until an effectful condition is true",
      () =>
        Effect.gen(function* () {
          let input = 10;
          let output = 0;
    ...
    it.effect("retry/until - always evaluates at least once", () =>
      Effect.gen(function* () {
        ...
        yield* increment.pipe(Effect.retry({ until: constTrue }), Effect.flip);
        assert.strictEqual(n, 1);
      })
    );
    ...
    it.effect(
      "retry/while - retries while an effectful condition is true",
      () =>
        Effect.gen(function* () {
          let input = 10;
          let output = 0;
    ...
    it.effect("retry/while - always evaluates at least once", () =>
      Effect.gen(function* () {
        let n = 0;
        const increment = Effect.failSync(() => n++);
        yield* increment.pipe(Effect.retry({ while: constFalse }), Effect.flip);
        assert.strictEqual(n, 1);
      })
    );

    it.effect(
      "retry/schedule - retries according to the specified schedule",
      () =>
        Effect.gen(function* () {
          let n = 0;
          const increment = Effect.failSync(() => n++);
          yield* increment.pipe(Effect.retry(Schedule.recurs(3)), Effect.flip);
  ...
  describe("timeoutOption", () => {
    ...
    it.effect(
      "timeout in uninterruptible region",
      () =>
        Effect.void.pipe(Effect.timeoutOption(20_000), Effect.uninterruptible),
      { timeout: 1000 }
    );
  });
  ...
  describe("awaitAllChildren", () => {
    ...
    it.effect("does not await children forked outside the wrapped effect", () =>
      Effect.gen(function* () {
        const preexisting = yield* Effect.never.pipe(
          Effect.forkChild({ startImmediately: true })
        );
  ...
  describe("stack safety", () => {
    it.live("recursion", () => {
      ...
      return loop.pipe(Effect.timeoutOption(50));
    });
  });
  ...
  describe("finalization", () => {
    ...
    it.effect(
      "onResult - ensures that a cleanup function runs when an effect fails",
      () =>
        Effect.gen(function* () {
          let ref = false;
  ...
  describe("Effect.ignore", () => {
    type IgnoreOptions = {
      readonly log?: boolean | LogLevel.Severity;
  ...
  describe("Effect.ignoreCause", () => {
    ...
    it.effect("ignores interrupts", () =>
      Effect.gen(function* () {
        const ignored = yield* Effect.interrupt.pipe(
          Effect.ignoreCause,
          Effect.exit
        );
  ...
  describe("error handling", () => {
    ...
    it.effect("catchIf with refinement", () =>
      Effect.gen(function* () {
        ...
        const effect: Effect.Effect<never, ErrorA | ErrorB> = Effect.fail({
    ...
    it.effect("catchNoSuchElement", () =>
      Effect.gen(function* () {
        const some = yield* Effect.fromNullishOr("value").pipe(
          Effect.catchNoSuchElement
        );
        assert.deepStrictEqual(some, Option.some("value"));

        const none = yield* Effect.fromNullishOr(null as string | null).pipe(
          Effect.catchNoSuchElement
        );
    ...
    it.effect("concurrent: true", () => {
      ...
      return Effect.gen(function* () {
        const fiber = yield* Effect.forkChild(
          Effect.zip(task1, task2, { concurrent: true })
        );
        yield* TestClock.adjust(50);
        const result = yield* Fiber.join(fiber);
        assert.deepStrictEqual(result, ["a", 1]);
  ...
  describe("zipWith", () => {
    it.effect("concurrent: false", () => {
      ...
      return Effect.gen(function* () {
        const fiber = yield* Effect.forkChild(
          Effect.zipWith(task1, task2, (a, b) => a + b)
        );
        yield* TestClock.adjust(51);
        const result = yield* Fiber.join(fiber);
  ...
  describe("catchCauseIf with predicate", () => {
    ...
    it.effect("predicate no match preserves error", () =>
      Effect.gen(function* () {
        const result = yield* Effect.exit(
          Effect.catchCauseIf(Effect.fail("e1" as const), constFalse, () =>
            Effect.succeed("recovered")
          )
        );
        assert.deepStrictEqual(result, Exit.fail("e1"));
  ...
  describe("tapCauseIf", () => {
      ...
      Effect.gen(function* () {
        ...
        const result = yield* Effect.exit(
          Effect.tapCauseFilter(
            ...
            (cause) =>
              Effect.sync(() => tapped.push(Cause.squash(cause) as string))
          )
        );
    ...
    it.effect("predicate match taps", () =>
      Effect.gen(function* () {
        const tapped: Array<string> = [];
        const result = yield* Effect.exit(
          Effect.tapCauseIf(Effect.fail("e1"), Cause.hasFails, (cause) =>
            Effect.sync(() => tapped.push(Cause.squash(cause) as string))
          )
        );
        assert.deepStrictEqual(tapped, ["e1"]);
    ...
    it.effect("predicate no match skips tap", () =>
      Effect.gen(function* () {
        const tapped: Array<string> = [];
        const result = yield* Effect.exit(
          Effect.tapCauseIf(Effect.fail("e1"), constFalse, () =>
            Effect.sync(() => tapped.push("tapped"))
          )
        );
        assert.deepStrictEqual(tapped, []);
  ...
  describe("onErrorIf", () => {
    it.effect("predicate match runs finalizer", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.exit(
          Effect.onErrorIf(Effect.fail("e1"), Cause.hasFails, (cause) =>
            Effect.sync(() => finalized.push(Cause.squash(cause) as string))
          )
        );
    ...
    it.effect("predicate no match skips finalizer", () =>
      Effect.gen(function* () {
        const finalized: Array<string> = [];
        const result = yield* Effect.exit(
          Effect.onErrorIf(Effect.fail("e1"), constFalse, () =>
  ...
  describe("onExitIf", () => {
    ...
    it.effect("predicate no match on failure skips finalizer", () =>
      Effect.gen(function* () {
        const finalized: Array<string> = [];
        const result = yield* Effect.exit(
          Effect.onExitIf(Effect.fail("e1"), Exit.isSuccess, () =>
            Effect.sync(() => finalized.push("finalized"))
          )
        );
        assert.deepStrictEqual(finalized, []);
    ...
    it.effect("predicate match on failure runs finalizer", () =>
      Effect.gen(function* () {
        const finalized: Array<string> = [];
        const result = yield* Effect.exit(
          Effect.onExitIf(Effect.fail("e1"), Exit.isFailure, (exit) => {
            if (Exit.isFailure(exit)) {
              return Effect.sync(() =>
                finalized.push(`failure:${Cause.squash(exit.cause)}`)
              );
            }
            return Effect.void;
          })
    ...
    it.effect("filter match receives pass value and exit", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.onExitFilter(
          ...
          (exit) =>
            Exit.isSuccess(exit)
              ? Result.succeed(`value:${exit.value}`)
              : Result.fail(exit),
          (value, exit) =>
            Effect.sync(() =>
              finalized.push(
                `${value}:${Exit.isSuccess(exit) ? `success:${exit.value}` : "failure"}`
              )
            )
        );
  ...
  describe("filter with predicate/refinement", () => {
    ...
    it.effect("Filter.Filter overload maps pass values", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.filterMap([1, 2, 3, 4], filter);
  ...
  describe("catch", () => {
    it.effect("first argument as success", () =>
      Effect.gen(function* () {
        const result = yield* Effect.catch(Effect.succeed(1), () =>
          Effect.fail("e2" as const)
        );
        assert.deepStrictEqual(result, 1);
      })
    ...
    it.effect("first argument as failure", () =>
      Effect.gen(function* () {
        const result = yield* Effect.flip(
          Effect.catch(Effect.fail("e1" as const), () =>
            Effect.fail("e2" as const)
          )
        );
        assert.deepStrictEqual(result, "e2");
  ...
  describe("firstSuccessOf", () => {
    ...
    it.effect("defects on an empty collection", () =>
      Effect.gen(function* () {
        const result = yield* Effect.firstSuccessOf([]).pipe(
          Effect.sandbox,
          Effect.flip
        );
  ...
  describe("catchCause", () => {
    it.effect("first argument as success", () =>
      Effect.gen(function* () {
        const result = yield* Effect.catchCause(Effect.succeed(1), () =>
          Effect.fail("e2" as const)
        );
        assert.deepStrictEqual(result, 1);
      })
    ...
    it.effect("first argument as failure", () =>
      Effect.gen(function* () {
        const result = yield* Effect.flip(
          Effect.catchCause(Effect.fail("e1" as const), () =>
            Effect.fail("e2" as const)
          )
        );
        assert.deepStrictEqual(result, "e2");
  ...
  describe("transaction (composable transactions)", () => {
    describe("basic behavior", () => {
      it.effect(
        "should create transaction boundaries when no transaction is active",
        () =>
          Effect.gen(function* () {
      ...
      it.effect(
        "should allow TxRef.modify outside an existing transaction",
        () =>
          Effect.gen(function* () {
            const ref = TxRef.makeUnsafe(0);

            const result = yield* TxRef.modify(ref, (current) => [
              current + 1,
              current + 10,
            ]);
      ...
      it.effect(
        "should roll back nested changes when the outer transaction fails",
        () =>
          Effect.gen(function* () {
            const ref1 = TxRef.makeUnsafe(0);
            const ref2 = TxRef.makeUnsafe(100);

            const parentError = yield* Effect.tx(
              Effect.gen(function* () {
      ...
      it.effect(
        "should allow catching nested failures in the same transaction",
        () =>
          Effect.gen(function* () {
            const ref1 = TxRef.makeUnsafe(0);
            const ref2 = TxRef.makeUnsafe(100);
            ...
            yield* Effect.tx(
              Effect.gen(function* () {
                ...
                const childResult = yield* Effect.tx(
                  Effect.gen(function* () {
    ...
    describe("transaction nesting", () => {
      it.effect(
        "should handle multiple levels of nested composed transactions",
        () =>
          Effect.gen(function* () {
            const ref1 = TxRef.makeUnsafe(0);
    ...
    describe("transaction composition behavior", () => {
      it.effect(
        "should compose nested tx calls into the same transaction",
        () =>
          Effect.gen(function* () {
            const ref1 = TxRef.makeUnsafe(0);
            const ref2 = TxRef.makeUnsafe(0);
      ...
      it.effect(
        "should rollback the entire composed transaction on failure",
        () =>
          Effect.gen(function* () {
            const ref = TxRef.makeUnsafe(0);

            const txError = yield* Effect.tx(
              Effect.gen(function* () {
      ...
      it.effect(
        "should preserve nested writes when nested failure is caught",
        () =>
          Effect.gen(function* () {
            const ref = TxRef.makeUnsafe(0);
            ...
            yield* Effect.tx(
              Effect.gen(function* () {
                ...
                const childError = yield* Effect.tx(
                  Effect.gen(function* () {
                    yield* TxRef.set(ref, 20);
  ...
  describe("Effect.fn", () => {
    it.effect("should support pipeable arguments", () => {
      const fn = Effect.fn(
        ...
        (effect, ...args) =>
          effect.pipe(Effect.map((result) => [result, ...args]))
      );
      return Effect.gen(function* () {
    ...
    it("should proxy body length", () => {
      ...
      const named = Effect.fn("named")(function* (
        a: string,
        b: number,
        c: boolean
      ) {
      ...
      const untraced = Effect.fnUntraced(
  ...
  describe("catchReason", () => {
    ...
    it.effect("catches matching reason - handler succeeds", () =>
      Effect.gen(function* () {
        const result = yield* Effect.fail(
          ...
          Effect.catchReason("AiError", "RateLimitError", (r) =>
            Effect.succeed(`retry: ${r.retryAfter}`)
          )
        );
    ...
    it.effect("ignores non-matching parent tag", () =>
      Effect.gen(function* () {
        ...
        const exit = yield* (
          Effect.fail(error) as Effect.Effect<never, AiError | OtherError>
        ).pipe(
          Effect.catchReason("AiError", "RateLimitError", () =>
            Effect.succeed("no")
          ),
  ...
  describe("unwrapReason", () => {
    ...
    it.effect("preserves other errors", () =>
      Effect.gen(function* () {
        ...
        const exit = yield* (
          Effect.fail(error) as Effect.Effect<never, AiError | OtherError>
        ).pipe(Effect.unwrapReason("AiError"), Effect.exit);
        assertExitFailure(exit, Cause.fail(error));
      })
    );
  });
