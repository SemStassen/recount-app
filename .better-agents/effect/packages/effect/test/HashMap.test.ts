...
describe("HashMap", () => {
  ...
  describe("basic operations", () => {
    ...
    it("getUnsafe - non-existing key", () => {
      ...
      expect(() => HashMap.getUnsafe(map, "b")).toThrow(
        "HashMap.getUnsafe: key not found"
      );
    });
  });
  ...
  describe("iterators and getters", () => {
    ...
    it("entries", () => {
      const map = HashMap.make(["a", 1], ["b", 2]);
      const entries = Array.from(HashMap.entries(map)).sort(([a], [b]) =>
        a.localeCompare(b)
      );
    ...
    it("toEntries", () => {
      const map = HashMap.make(["a", 1], ["b", 2]);
      const entries = HashMap.toEntries(map).sort(([a], [b]) =>
        a.localeCompare(b)
      );
      expect(entries).toEqual([
        ["a", 1],
  ...
  describe("filtering operations", () => {
    ...
    it("filterMap - key argument", () => {
      const map1 = HashMap.make(["a", 1], ["b", 2], ["c", 3], ["d", 4]);
      const map2 = HashMap.filterMap(map1, (value, key) =>
        key < "c" ? Result.succeed(`${key}:${value}`) : Result.failVoid
      );
  ...
  describe("search operations", () => {
    ...
    it("hasBy", () => {
      const map = HashMap.make(["a", 1], ["b", 2], ["c", 3]);
      expect(
        HashMap.hasBy(map, (value, key) => key === "b" && value === 2)
      ).toBe(true);
      expect(
        HashMap.hasBy(map, (value, key) => key === "b" && value === 5)
      ).toBe(false);
    });
  });
  ...
  describe("modification operations", () => {
    ...
    it("modifyAt - non-existing key", () => {
      const map1 = HashMap.make(["a", 1]);
      const map2 = HashMap.modifyAt(map1, "b", (option) =>
        Option.isSome(option) ? option : Option.some(10)
      );
    ...
    it("keeps all inserted entries addressable across random bit-31-heavy shapes", () => {
      const entriesArbitrary = fc
        ...
        .chain((hashes) => {
          ...
          return fc
            .uniqueArray(fc.uuid(), {
              minLength: allHashes.length,
              maxLength: allHashes.length,
            })
