...
describe("Ref", () => {
  ...
  it.effect(
    "getAndSet returns the previous value and stores the replacement",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "getAndUpdate returns the previous value and stores the computed value",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(current);
  ...
  it.effect(
    "getAndUpdateSome leaves state unchanged when no transition matches",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make<State>(Active);
        const result1 = yield* Ref.getAndUpdateSome(ref, (state) =>
  ...
  it.effect(
    "getAndUpdateSome returns each previous state while applying matching transitions",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make<State>(Active);
        const result1 = yield* Ref.getAndUpdateSome(ref, (state) =>
          isActive(state) ? Option.some(Changed) : Option.none()
        );
  ...
  it.effect(
    "updateSome leaves state unchanged when no transition matches",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make<State>(Active);
  ...
  it.effect(
    "updateSomeAndGet returns the current state when no transition matches",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make<State>(Active);
        const result = yield* Ref.updateSomeAndGet(ref, (state) =>
          isClosed(state) ? Option.some(Changed) : Option.none()
        );
  ...
  it.effect("updateSomeAndGet - twice", () =>
    Effect.gen(function* () {
      const ref = yield* Ref.make<State>(Active);
      const result1 = yield* Ref.updateSomeAndGet(ref, (state) =>
        isActive(state) ? Option.some(Changed) : Option.none()
      );
  ...
  it.effect(
    "modifySome returns a result while applying matching transitions",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make<State>(Active);
