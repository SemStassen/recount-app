...
describe("ReducerAnd", () => {
  ...
  it("initialValue acts as identity element", () => {
    assert.strictEqual(
      Boolean.ReducerAnd.combine(Boolean.ReducerAnd.initialValue, true),
      true
    );
    assert.strictEqual(
      Boolean.ReducerAnd.combine(Boolean.ReducerAnd.initialValue, false),
      false
    );
    assert.strictEqual(
      Boolean.ReducerAnd.combine(true, Boolean.ReducerAnd.initialValue),
      true
    );
    assert.strictEqual(
      Boolean.ReducerAnd.combine(false, Boolean.ReducerAnd.initialValue),
      false
    );
  });
});
...
describe("ReducerOr", () => {
  ...
  it("initialValue acts as identity element", () => {
    assert.strictEqual(
      Boolean.ReducerOr.combine(Boolean.ReducerOr.initialValue, true),
      true
    );
    assert.strictEqual(
      Boolean.ReducerOr.combine(Boolean.ReducerOr.initialValue, false),
      false
    );
    assert.strictEqual(
      Boolean.ReducerOr.combine(true, Boolean.ReducerOr.initialValue),
      true
    );
    assert.strictEqual(
      Boolean.ReducerOr.combine(false, Boolean.ReducerOr.initialValue),
      false
    );
  });
});
