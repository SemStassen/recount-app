...
describe("Chunk", () => {
  ...
  it("toJSON", () => {
    deepStrictEqual(Chunk.make(0, 1, 2).toJSON(), {
      _id: "Chunk",
      values: [0, 1, 2],
    });
  ...
  it("inspect", () => {
    if (typeof window === "undefined") {
      ...
      assertEquals(
        inspect(Chunk.make(0, 1, 2)),
        inspect({ _id: "Chunk", values: [0, 1, 2] })
      );
    }
  });
  ...
  it("replace", () => {
    ...
    assertSome(
      pipe(Chunk.make(1, 2, 3), Chunk.replace(0, 2)),
      Chunk.make(2, 2, 3)
    );
  });
  ...
  it("chunksOf", () => {
    ...
    assertEquals(
      pipe(Chunk.make(1, 2, 3, 4, 5), Chunk.chunksOf(5)),
    ...
    assertEquals(
      pipe(Chunk.make(1, 2, 3, 4, 5), Chunk.chunksOf(10)),
      Chunk.make(Chunk.make(1, 2, 3, 4, 5))
    );
  });
  ...
  describe("fromIterable", () => {
    describe("Given an iterable", () => {
      ...
      it("should process it", () => {
        assertEquals(
          Chunk.fromIterable(myIterable),
          Chunk.fromArrayUnsafe([1, 2, 3, 4, 5])
        );
      });
    });
      ...
      it("should throw", () => {
        fc.assert(
          fc.property(
            ...
            fc.array(fc.anything()),
            (arr1, arr2) => {
              const chunk: Chunk.Chunk<unknown> = Chunk.appendAll(
                Chunk.fromIterable(arr2)
              )(Chunk.fromArrayUnsafe(arr1));
              throws(() =>
                pipe(chunk, Chunk.getUnsafe(arr1.length + arr2.length))
              );
            }
          )
        );
      ...
      it("should return the value", () => {
        fc.assert(
          fc.property(
            ...
            (a, b) => {
              const c = [...a, ...b];
              const d = Chunk.appendAll(Chunk.fromArrayUnsafe(b))(
                Chunk.fromArrayUnsafe(a)
              );
  ...
  it("prependAll", () => {
    assertEquals(
      pipe(Chunk.empty(), Chunk.prependAll(Chunk.make(1))),
      Chunk.make(1)
    );
    assertEquals(
      pipe(Chunk.make(1), Chunk.prependAll(Chunk.empty())),
      Chunk.make(1)
    );

    assertEquals(
      pipe(Chunk.empty(), Chunk.prependAll(Chunk.make(1, 2))),
      Chunk.make(1, 2)
    );
    assertEquals(
      pipe(Chunk.make(1, 2), Chunk.prependAll(Chunk.empty())),
      Chunk.make(1, 2)
    );

    assertEquals(
      pipe(Chunk.make(2, 3), Chunk.prependAll(Chunk.make(1))),
      Chunk.make(1, 2, 3)
    );
    assertEquals(
      pipe(Chunk.make(3), Chunk.prependAll(Chunk.make(1, 2))),
      Chunk.make(1, 2, 3)
    );
  });
  ...
  describe("take", () => {
    describe("Given a Chunk with more elements than the amount taken", () => {
      it("should return the subset", () => {
        assertEquals(
          pipe(Chunk.fromArrayUnsafe([1, 2, 3]), Chunk.take(2)),
          Chunk.fromArrayUnsafe([1, 2])
        );
      });
    });
    ...
    describe("Given a concatenated Chunk and an amount > 1", () => {
      ...
      it("should return the available subset", () => {
        deepStrictEqual(
          pipe(chunk, Chunk.take(amount), Chunk.toReadonlyArray),
          [1, 2]
        );
      });
    });
    ...
    describe("Given a concatenated Chunk and an amount <= self.left", () => {
      it("should return the available subset", () => {
        ...
        deepStrictEqual(
          pipe(chunk, Chunk.take(2), Chunk.toReadonlyArray),
          [2, 3]
        );
        deepStrictEqual(
          pipe(chunk, Chunk.take(3), Chunk.toReadonlyArray),
          [2, 3, 4]
        );
      });
    });
  ...
  describe("drop", () => {
    ...
    it("should drop twice", () => {
      ...
      deepStrictEqual(
        Chunk.toReadonlyArray(Chunk.drop(1)(Chunk.drop(1)(self))),
        [2, 3]
      );
    });
  ...
  describe("dropRight", () => {
    describe("Given a Chunk and an amount to drop below the length", () => {
      ...
      it("should remove the given amount of items", () => {
        assertEquals(
          pipe(chunk, Chunk.dropRight(toDrop)),
          Chunk.fromArrayUnsafe([1, 2])
        );
      });
    });
  ...
  describe("dropWhile", () => {
    describe("Given a Chunk and a criteria that applies to part of the chunk", () => {
      ...
      it("should return the subset that doesn't pass the criteria", () => {
        assertEquals(
          pipe(chunk, Chunk.dropWhile(criteria)),
          Chunk.fromArrayUnsafe([3])
        );
      });
    });
  ...
  describe("concat", () => {
    describe("Given 2 chunks of the same length", () => {
      ...
      it("should concatenate them following order", () => {
        assertEquals(
          pipe(chunk1, Chunk.appendAll(chunk2)),
  ...
  it("zip", () => {
    ...
    deepStrictEqual(Chunk.toArray(Chunk.zip(Chunk.make(1), Chunk.make(2))), [
      [1, 2],
    ]);
  });
  ...
  it("partition", () => {
    assertTuple(
      Chunk.partition(Chunk.empty<number>(), (n) =>
        n > 2 ? Result.succeed(n) : Result.fail(n)
      ),
      [Chunk.empty(), Chunk.empty()]
    );
    assertTuple(
      Chunk.partition(Chunk.make(1, 3), (n) =>
        n > 2 ? Result.succeed(n) : Result.fail(n)
      ),
    ...
    assertTuple(
      Chunk.partition(Chunk.empty<number>(), (n, i) =>
        n + i > 2 ? Result.succeed(n + i) : Result.fail(`negative:${n + i}`)
      ),
      [Chunk.empty(), Chunk.empty()]
    );
    assertTuple(
      Chunk.partition(Chunk.make(1, 2), (n, i) =>
        n + i > 2 ? Result.succeed(n + i) : Result.fail(`negative:${n + i}`)
      ),
  ...
  it("partition (identity)", () => {
    assertTuple(Chunk.partition(Chunk.empty(), identity), [
      Chunk.empty(),
      Chunk.empty(),
    ]);
    assertTuple(
      Chunk.partition(
        Chunk.make(Result.succeed(1), Result.fail("a"), Result.succeed(2)),
        identity
      ),
  ...
  it("split", () => {
    ...
    assertEquals(
      pipe(Chunk.make(1), Chunk.split(2)),
      Chunk.make(Chunk.make(1))
    );
    assertEquals(
      pipe(Chunk.make(1, 2), Chunk.split(2)),
  ...
  it("filter", () => {
    ...
    assertEquals(
      Chunk.filter(
        Chunk.make(Option.some(3), Option.none(), Option.some(1)),
        Option.isSome
      ),
  ...
  it("filterMap", () => {
    assertEquals(
      Chunk.filterMap(Chunk.make(1, 2, 3, 4), (n) =>
        n % 2 === 0 ? Result.succeed(n * 2) : Result.failVoid
      ),
      Chunk.make(4, 8)
    );
    assertEquals(
      Chunk.filterMap(Chunk.make(1, 2, 3, 4), (n, i) =>
        i % 2 === 0 ? Result.succeed(n + i) : Result.failVoid
      ),
      Chunk.make(1, 5)
    );
  });
  ...
  it("filterMapWhile", () => {
    assertEquals(
      Chunk.filterMapWhile(Chunk.make(1, 3, 4, 5), (n) =>
        n % 2 === 1 ? Result.succeed(n) : Result.failVoid
      ),
      Chunk.make(1, 3)
    );
  });
  ...
  it("compact", () => {
    ...
    assertEquals(
      Chunk.compact(Chunk.make(Option.some(1), Option.some(2), Option.some(3))),
      Chunk.make(1, 2, 3)
    );
    assertEquals(
      Chunk.compact(Chunk.make(Option.some(1), Option.none(), Option.some(3))),
      Chunk.make(1, 3)
    );
  });
  ...
  it("dedupeAdjacent", () => {
    ...
    assertEquals(
      Chunk.dedupeAdjacent(Chunk.make(1, 2, 3)),
      Chunk.make(1, 2, 3)
    );
    assertEquals(
      Chunk.dedupeAdjacent(Chunk.make(1, 2, 2, 3, 3)),
      Chunk.make(1, 2, 3)
    );
  });
  ...
  it("union", () => {
    assertEquals(
      Chunk.union(Chunk.make(1, 2, 3), Chunk.empty()),
      Chunk.make(1, 2, 3)
    );
    assertEquals(
      Chunk.union(Chunk.empty(), Chunk.make(1, 2, 3)),
      Chunk.make(1, 2, 3)
    );
    assertEquals(
      Chunk.union(Chunk.make(1, 2, 3), Chunk.make(2, 3, 4)),
      Chunk.make(1, 2, 3, 4)
    );
  });
  ...
  it("intersection", () => {
    assertEquals(
      Chunk.intersection(Chunk.make(1, 2, 3), Chunk.empty()),
      Chunk.empty()
    );
    assertEquals(
      Chunk.intersection(Chunk.empty(), Chunk.make(2, 3, 4)),
      Chunk.empty()
    );
    assertEquals(
      Chunk.intersection(Chunk.make(1, 2, 3), Chunk.make(2, 3, 4)),
      Chunk.make(2, 3)
    );
  });
  ...
  it("lastUnsafe", () => {
    ...
    throws(
      ...
      new Error("Index out of bounds: -1")
    );
  });

  it("splitNonEmptyAt", () => {
    assertTuple(Chunk.splitNonEmptyAt(Chunk.make(1, 2, 3, 4), 2), [
      Chunk.make(1, 2),
      Chunk.make(3, 4),
    ]);
    assertTuple(Chunk.splitNonEmptyAt(Chunk.make(1, 2, 3, 4), 10), [
      Chunk.make(1, 2, 3, 4),
      Chunk.empty(),
    ]);
  });
  ...
  it("dedupe", () => {
    ...
    assertEquals(
      Chunk.dedupe(Chunk.make(1, 2, 3, 2, 1, 3)),
      Chunk.make(1, 2, 3)
    );
  });
  ...
  it("reverse", () => {
    ...
    assertEquals(
      Chunk.reverse(Chunk.take(Chunk.make(1, 2, 3, 4), 3)),
      Chunk.make(3, 2, 1)
    );
  });
  ...
  it("sortWith", () => {
    ...
    const chunk: Chunk.Chunk<X> = Chunk.make(
      { a: "a", b: 2 },
      { a: "b", b: 1 }
    );
    deepStrictEqual(
  ...
  it("makeEquivalence", () => {
    const equivalence = Chunk.makeEquivalence(
  ...
  it("differenceWith", () => {
    ...
    deepStrictEqual(
      differenceWith(chunk, Chunk.make({ id: 1 }, { id: 2 })),
      Chunk.make({ id: 3 })
    );
  ...
  it("difference", () => {
    ...
    assertEquals(
      Chunk.difference(curr, Chunk.make(1, 2, 3, 4, 5)),
      Chunk.make(7, 9)
    );
