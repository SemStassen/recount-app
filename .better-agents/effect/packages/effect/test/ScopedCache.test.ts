...
describe("ScopedCache", () => {
  describe("constructors", () => {
    describe("make", () => {
        ...
        Effect.gen(function* () {
          ...
          const program = Effect.gen(function* () {
            const cache = yield* ScopedCache.make({
              capacity: 10,
              lookup: (_key: string) =>
                Effect.map(TestService, (service) => service.value),
            });
      ...
      it.effect(
        "cache resources are released and future gets interrupt when scope closes",
        () =>
          Effect.gen(function* () {
      ...
      it.effect("creates cache with function-based TTL", () =>
        Effect.gen(function* () {
          ...
          yield* TestClock.adjust(
            Duration.sum(Duration.minutes(1), Duration.seconds(1))
          );
          ...
          const cache = yield* ScopedCache.makeWith({
            ...
            lookup: (key: string) =>
              key === "fail"
                ...
                : Effect.succeed(key.length),
            timeToLive: (exit) =>
              Exit.isSuccess(exit) ? "1 hour" : "1 minute",
          });
  ...
  describe("basic operations", () => {
    describe("get", () => {
      ...
      it.effect(
        "cache hit - same key multiple times doesn't invoke lookup again",
        () =>
          Effect.gen(function* () {
            const { cache, lookupCount, setLookupResult } =
              yield* makeScopedTestCache(10);
      ...
      it.effect(
        "concurrent access - multiple fibers getting same key only invoke lookup once",
        () =>
          Effect.gen(function* () {
            let lookupCount = 0;
            const cache = yield* ScopedCache.make({
              ...
              lookup: () =>
                Effect.delay(Duration.millis(100))(
                  Effect.sync(() => ++lookupCount)
                ),
            });

            const effects = [
              ScopedCache.get(cache, "key"),
              ScopedCache.get(cache, "key"),
              ScopedCache.get(cache, "key"),
            ];
            const resultsFiber = yield* Effect.all(effects, {
      ...
      it.effect("concurrent access - race between get and invalidate", () =>
        Effect.gen(function* () {
          ...
          // First get starts
          const fiber1 = yield* ScopedCache.get(cache, "test").pipe(
            Effect.forkChild
          );
          ...
          const fiber2 = yield* ScopedCache.get(cache, "test").pipe(
            Effect.forkChild
          );
    ...
    describe("getOption", () => {
      ...
      it.effect("waits for value being computed and returns result", () =>
        Effect.gen(function* () {
          ...
          const getFiber = yield* ScopedCache.get(cache, "test").pipe(
            Effect.forkChild
          );
          const optionFiber = yield* ScopedCache.getOption(cache, "test").pipe(
            Effect.forkChild
          );
  ...
  describe("modification operations", () => {
    ...
    describe("invalidateWhen", () => {
      it.effect("invalidates when predicate matches", () =>
        Effect.gen(function* () {
          ...
          yield* ScopedCache.get(cache, "test");
          const result = yield* ScopedCache.invalidateWhen(
            cache,
            "test",
            (value) => value === 4
          );
    ...
    describe("refresh", () => {
      ...
      it.effect(
        "concurrent refresh calls each invoke lookup independently",
        () =>
          Effect.gen(function* () {
            let lookupCount = 0;
            const cache = yield* ScopedCache.make({
    ...
    describe("invalidateAll", () => {
      ...
      it.effect("resource cleanup for all entries", () =>
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(cleanupTracker.cleanedUp.sort(), [
            "test1",
            "test2",
            "test3",
          ]);
  ...
  describe("utility operations", () => {
    ...
    describe("entries", () => {
      ...
      it.effect(
        "removes expired entries as side effect with resource cleanup",
        () =>
          Effect.gen(function* () {
            const { cache, cleanupTracker } = yield* Effect.gen(function* () {
              const cleanupTracker: CleanupTracker = {
    ...
    describe("Fixed TTL", () => {
      it.effect("entry expires after specified duration", () =>
        Effect.gen(function* () {
          ...
          yield* TestClock.adjust(
            Duration.sum(Duration.hours(1), Duration.seconds(1))
          );
          assert.isFalse(yield* ScopedCache.has(cache, "test"));
        })
      );
          ...
          const cache = yield* ScopedCache.makeWith({
            ...
            lookup: (key: string) =>
              key === "fail"
                ...
                : Effect.succeed(key.length),
            timeToLive: (exit) =>
              Exit.isSuccess(exit) ? "2 hours" : "30 minutes",
          });
  ...
  describe("scope integration", () => {
    describe("resource management", () => {
              ...
              const cache = yield* ScopedCache.make({
                ...
                lookup: (key: string) =>
                  Effect.gen(function* () {
                    yield* Effect.acquireRelease(
                      Effect.sync(() =>
                        resourceTracker.push(`acquired-${key}`)
                      ),
                      () =>
                        Effect.sync(() =>
                          resourceTracker.push(`released-${key}`)
                        )
                    );
                    return `value-${key}`;
                  }),
      ...
      it.effect("resource cleanup is atomic and doesn't leak", () =>
        Effect.gen(function* () {
          ...
          const acquired = resourceTracker.filter((r) =>
            r.startsWith("acquired")
          ).length;
          const released = resourceTracker.filter((r) =>
            r.startsWith("released")
          ).length;
    ...
    describe("scope lifecycle", () => {
      ...
      it.effect(
        "resource cleanup happens in correct order during cache closure",
        () =>
          Effect.gen(function* () {
                  ...
                  lookup: (key: string) =>
                    Effect.gen(function* () {
                      yield* Effect.acquireRelease(
                        ...
                        () =>
                          Effect.sync(() =>
                            cleanupOrder.push(`released-${key}`)
                          )
                      );
                      return `value-${key}`;
                    }),
            ...
            const releases = cleanupOrder.filter((item) =>
              item.startsWith("released")
            );
  ...
  describe("service context", () => {
    describe("Service Dependency Injection", () => {
        ...
        Effect.gen(function* () {
          ...
          const cache = yield* ScopedCache.make({
            ...
            Effect.provideService(
              ConfigService,
              ConfigService.of({ multiplier: 10 })
            )
          );
      ...
      it.effect(
        "requireServicesAt: 'lookup' provides services at lookup time",
        () =>
          Effect.gen(function* () {
            ...
            const cache = yield* ScopedCache.make({
              capacity: 10,
              lookup: (_key: string) =>
                Effect.map(CounterService, (service) => service.value),
      ...
      it.effect("complex object keys work correctly", () =>
        Effect.gen(function* () {
          ...
          const cache = yield* ScopedCache.make({
            capacity: 10,
            lookup: (key: CacheKey) =>
              Effect.succeed(`${key.userId}-${key.productId}`),
          });
