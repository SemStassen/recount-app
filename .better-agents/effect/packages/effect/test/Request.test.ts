...
class Counter extends Context.Service<Counter, { count: number }>()(
  "Counter"
) {}
class Requests extends Context.Service<Requests, { count: number }>()(
  "Requests"
) {}
...
const GetRequestService =
  Request.tagged<GetRequestService>("GetRequestService");
...
const makeUserResolver = Effect.gen(function* () {
  ...
  const getNameById = (id: number) =>
    Effect.request(new GetNameById({ id }), resolver);
  const getNameByIdPiped = (id: number) =>
    pipe(new GetNameById({ id }), Effect.request(resolver));
  ...
  const resolver = Resolver.fromEffectTagged<UserRequest>()({
    ...
    GetNameById: Effect.fnUntraced(function* (reqs) {
      ...
      for (let i = 0; i < reqs.length; i++) {
        const req = reqs[i];
        if (!userNames.has(req.request.id))
...
const processRequest = (
  entry: Request.Entry<UserRequest>
): Effect.Effect<void> => {
...
describe.sequential("Request", () => {
        ...
        Effect.fnUntraced(function* (entries) {
          ...
          for (const entry of entries) {
            entry.completeUnsafe(
              Exit.succeed(userNames.get(entry.request.id)!)
            );
          }
        })
      ).pipe(
  ...
  it.effect(
    ...
    Effect.fnUntraced(function* () {
      const resolver = Resolver.make<GetRequestService>(
        Effect.fnUntraced(function* (entries) {
