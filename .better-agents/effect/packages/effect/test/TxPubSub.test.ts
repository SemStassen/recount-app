...
describe("TxPubSub", () => {
  ...
  describe("publish and subscribe", () => {
    ...
    it.effect(
      "subscriber only receives messages published after subscription",
      () =>
        Effect.gen(function* () {
          const hub = yield* Effect.tx(TxPubSub.unbounded<number>());
  ...
  describe("strategies", () => {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            const fiber = yield* Effect.forkChild(
              Effect.tx(TxPubSub.publish(hub, 3))
            );
    ...
    it.effect(
      "dropping strategy drops messages when subscriber queue is full",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "sliding strategy drops oldest messages when subscriber queue is full",
      () =>
        Effect.gen(function* () {
  ...
  describe("shutdown", () => {
    ...
    it.effect("awaitShutdown blocks until shutdown", () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Effect.forkChild(
          Effect.tx(TxPubSub.awaitShutdown(hub))
        );
        yield* Effect.tx(TxPubSub.shutdown(hub));
        yield* Fiber.join(fiber);
      })
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            const f1 = yield* Effect.forkChild(
              Effect.tx(TxPubSub.publishAll(hub, [1, 2, 3]))
            );
            const f2 = yield* Effect.forkChild(
              Effect.tx(TxPubSub.publishAll(hub, [4, 5, 6]))
            );
