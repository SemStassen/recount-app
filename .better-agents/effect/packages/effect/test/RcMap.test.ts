...
describe("RcMap", () => {
  it.effect("deallocation", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* RcMap.get(map, "boom").pipe(
        Effect.scoped,
        Effect.exit
      );
  ...
  it.effect("capacity", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* RcMap.get(map, "baz").pipe(
        Effect.scoped,
        Effect.exit
      );
      assert.deepStrictEqual(
        ...
        Exit.fail(
          new Cause.ExceededCapacityError(
            `RcMap attempted to exceed capacity of 2`
          )
        )
      );
  ...
  it.effect("keys lookup", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(Array.from(yield* RcMap.keys(map)), [
        "foo",
        "bar",
        "baz",
      ]);
