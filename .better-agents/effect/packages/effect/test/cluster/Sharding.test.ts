...
describe.concurrent("Sharding", () => {
  ...
  it.effect("interrupts are sent for volatile messages on shutdown", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        (config as any).runnerAddress = Option.some(
          RunnerAddress.make("localhost", 1234)
        );
        ...
        Effect.provide(
          TestShardingWithoutRunners.pipe(
            Layer.provide(
    ...
    Effect.gen(function* () {
      ...
      assert(
        reply._tag === "WithExit" &&
          ...
          reply.exit.cause[0]._tag === "Die"
      );
    }).pipe(Effect.provide(TestSharding))
  );

  it.effect(
    "fails volatile requests immediately when the mailbox is full",
    () =>
      Effect.gen(function* () {
        const makeClient = yield* TestEntity.client;
        yield* TestClock.adjust(1);
        ...
        yield* client
          .NeverVolatile()
          .pipe(Effect.forkChild, Effect.replicateEffect(10));
        yield* TestClock.adjust(1);
  ...
  it.effect("durable messages are retried when mailbox is full", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const fibers = yield* client
          .NeverFork()
          .pipe(Effect.forkChild, Effect.replicateEffect(11));
        yield* TestClock.adjust(1);
        ...
        Effect.provide(
          TestShardingWithoutStorage.pipe(
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.provide(
          TestShardingWithoutStorage.pipe(
            Layer.provideMerge(
              Layer.effect(MessageStorage.MemoryDriver)(
  ...
  it.effect("durable messages are retried on restart", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* TestClock.adjust(1);
      }).pipe(Effect.provide(EnvLayer), Effect.scoped);
      ...
      Effect.provide(
        MessageStorage.layerMemory.pipe(
          Layer.provide(TestShardingConfig),
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* Effect.forkChild(
          Stream.runDrain(client.StreamWithKey({ key: "abc" }))
        );
        yield* TestClock.adjust(2000);
        // second chunk
  ...
  it.effect(
    "client discard stores durable requests without waiting for replies",
    () =>
      Effect.gen(function* () {
        yield* TestClock.adjust(1);
        const driver = yield* MessageStorage.MemoryDriver;
  ...
  it.effect(
    "client discard returns while the durable request keeps processing",
    () =>
      Effect.gen(function* () {
        yield* TestClock.adjust(1);
        const driver = yield* MessageStorage.MemoryDriver;
  ...
  it.effect("defects when a durable request has no MessageStorage", () =>
    Effect.gen(function* () {
      const makeClient = yield* TestEntity.client;
      const client = makeClient("1");
      const cause = yield* client.Never().pipe(Effect.sandbox, Effect.flip);
      assert(Cause.hasDies(cause));
    }).pipe(
      Effect.provide(
        TestShardingWithoutStorage.pipe(Layer.provide(MessageStorage.layerNoop))
      )
    )
  );
        ...
        Effect.provide(
          TestShardingWithoutStorage.pipe(
            Layer.updateService(MessageStorage.MessageStorage, (storage) => ({
              ...storage,
              saveReply(reply) {
                return MessageStorage.MemoryTransaction.use(
                  (isTransaction_) => {
