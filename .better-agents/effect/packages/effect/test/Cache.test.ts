      ...
      Effect.gen(function* () {
        const program = Effect.gen(function* () {
          const cache = yield* Cache.make({
            capacity: 10,
            lookup: (_key: string) =>
              Effect.currentSpan.pipe(Effect.withSpan("lookup")),
          });
          return yield* Cache.get(cache, "test");
        });

        const result = yield* program.pipe(Effect.withSpan("parent"));
    ...
    describe("getOption", () => {
      ...
      it.effect("waits for value being computed and returns result", () =>
        Effect.gen(function* () {
          ...
          const getFiber = yield* Cache.get(cache, "test").pipe(
            Effect.forkChild
          );
          const optionFiber = yield* Cache.getOption(cache, "test").pipe(
            Effect.forkChild
          );
    ...
    describe("invalidateWhen", () => {
      it.effect("invalidates when predicate matches", () =>
        Effect.gen(function* () {
          ...
          yield* Cache.get(cache, "test");
          const result = yield* Cache.invalidateWhen(
            cache,
            "test",
            (value) => value === 4
          );
    ...
    describe("entries", () => {
      it.effect("returns all key-value pairs", () =>
        Effect.gen(function* () {
          ...
          const entriesArray = Array.from(entries).sort(([a], [b]) =>
            a.localeCompare(b)
          );
