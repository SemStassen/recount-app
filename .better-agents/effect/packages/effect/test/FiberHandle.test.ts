...
describe("FiberHandle", () => {
  ...
  it.effect(
    "join ignores managed replacement interruptions and fails with child errors",
    () =>
      Effect.gen(function* () {
        const handle = yield* FiberHandle.make();
  ...
  it.effect(
    "onlyIfMissing keeps the current fiber and interrupts rejected starts",
    () =>
      Effect.gen(function* () {
        const handle = yield* FiberHandle.make();
        const fiberA = yield* FiberHandle.run(handle, Effect.never);
        const fiberB = yield* FiberHandle.run(handle, Effect.never, {
          onlyIfMissing: true,
        });
        const fiberC = yield* FiberHandle.run(handle, Effect.never, {
          onlyIfMissing: true,
        });
        yield* Effect.yieldNow;
  ...
  it.effect("propagateInterruption: true", () =>
    Effect.gen(function* () {
      ...
      yield* Fiber.interrupt(fiber);
      assertTrue(
        Exit.hasInterrupts(yield* FiberHandle.join(handle).pipe(Effect.exit))
      );
    })
  );
