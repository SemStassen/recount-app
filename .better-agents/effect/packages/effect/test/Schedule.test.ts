...
describe("Schedule", () => {
  describe("collecting", () => {
    ...
    it.effect("collectWhile - should collect while the predicate holds", () =>
      Effect.gen(function* () {
        const schedule = Schedule.collectWhile(Schedule.forever, ({ output }) =>
          Effect.succeed(output < 3)
        );
        const inputs = Array.makeBy(5, constUndefined);
        const outputs = yield* runLast(schedule, inputs);
    ...
    it.effect(
      "collectWhile - should collect while the effectful predicate holds",
      () =>
        Effect.gen(function* () {
          const schedule = Schedule.collectWhile(
            Schedule.forever,
            ({ output }) => Effect.succeed(output < 3)
          );
          const inputs = Array.makeBy(5, constUndefined);
          const outputs = yield* runLast(schedule, inputs);
          expect(outputs).toEqual([0, 1, 2, 3]);
  ...
  describe("sequencing", () => {
    ...
    it.effect(
      "andThenResult - executes schedules sequentially to completion",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "andThenResult - emits right completion when right schedule is finite",
      () =>
        Effect.gen(function* () {
          const left = Schedule.fixed("500 millis").pipe(
  ...
  describe("cron", () => {
    ...
    it.effect(
      "should recur on interval matching cron expression (second granularity)",
      () =>
        Effect.gen(function* () {
  ...
  describe("jittered", () => {
    it.effect("keeps delays within 80%-120% of the original", () =>
      Effect.gen(function* () {
        ...
        const output = yield* runDelays(schedule, inputs).pipe(
          Random.withSeed("jittered-bounds")
        );
        expect(
          output.every((delay) => {
    ...
    it.effect("does not change completion output", () =>
      Effect.gen(function* () {
        const schedule = Schedule.jittered(
          Schedule.duration(Duration.seconds(1))
        );
        const inputs = Array.makeBy(5, constUndefined);
        const output = yield* runDelays(schedule, inputs).pipe(
          Random.withSeed("jittered-completion")
        );
  ...
  describe("fixed", () => {
    ...
    it.effect(
      "delays until the nearest window boundary when action is slow",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "matches effect v3 when action duration exceeds the interval",
      () =>
        Effect.gen(function* () {
          const delays: Array<Duration.Duration> = [];
          const schedule = Schedule.fixed("1 seconds").pipe(
