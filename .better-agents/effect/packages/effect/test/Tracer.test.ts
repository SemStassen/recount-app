...
describe("Tracer", () => {
  describe("Effect.withSpan", () => {
    ...
    it.effect(
      "should still apply minimum trace level with sampled parent spans",
      () =>
        Effect.gen(function* () {
          const span = yield* Effect.currentSpan.pipe(
            Effect.withSpan("A", {
  ...
  describe("Layer.parentSpan", () => {
    it.effect(
      "should set the parent trace span for the layer constructor",
      () =>
        Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.unwrap(
              Effect.map(Effect.makeSpanScoped("parent"), (span) =>
                Layer.parentSpan(span)
              )
            )
          )
        )
  ...
  describe("Layer.span", () => {
    it.effect(
      "should create a new parent trace span for the layer constructor",
      () =>
        Effect.gen(function* () {
          const span = yield* Effect.makeSpan("child");
          const parent = getParent(span) as Tracer.Span;
  ...
  describe("Tracer.DisablePropagation", () => {
    it.effect("should allow disabling span propagation via noop spans", () =>
      Effect.gen(function* () {
        ...
        const spanB = yield* Effect.currentSpan.pipe(Effect.withSpan("B"));
