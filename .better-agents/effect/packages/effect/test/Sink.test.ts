    ...
    it.effect("empty", () =>
      Effect.gen(function* () {
        const result = yield* Stream.empty.pipe(
          Stream.transduce(
            Sink.reduceWhile<number, number>(
              () => 0,
              constTrue,
              (x, y) => x + y
            )
              ...
              stream.pipe(
                Stream.transduce(
                  Sink.reduceWhileEffect(
                    ...
                    constTrue,
                    (_, y: number) =>
                      Effect.as(Ref.update(ref, Array.append(y)), 30)
                  )
                ),
                Stream.runCollect,
                Effect.flatMap((exit) =>
                  Ref.get(ref).pipe(Effect.map((result) => [exit, result]))
                )
              )
            ),
            Effect.exit
  ...
  describe("reduce", () => {
    it.effect("equivalence with Array.reduce", () =>
      Effect.gen(function* () {
        ...
        const result1 = yield* stream.pipe(
          Stream.run(
            Sink.reduce(
              () => "",
              (s, n) => s + `${n}`
  ...
  describe("takeWhile", () => {
    ...
    it.effect("takeWhileFilter", () =>
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4).pipe(
          Stream.run(
            Sink.takeWhileFilter((n) =>
              n < 3 ? Result.succeed(n * 2) : Result.failVoid
            )
          )
        );
    ...
    it.effect("takeWhileFilterEffect", () =>
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4).pipe(
          Stream.run(
            Sink.takeWhileFilterEffect((n) =>
              Effect.succeed(n < 3 ? Result.succeed(n + 1) : Result.failVoid)
            )
          )
        );
    ...
    it.effect("takeWhileFilterEffect consumes the first failing input", () =>
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4).pipe(
          Stream.transduce(
            Sink.takeWhileFilterEffect((n) =>
              Effect.succeed(n < 3 ? Result.succeed(n) : Result.failVoid)
            )
          ),
          Stream.runCollect
        );
