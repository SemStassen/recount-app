...
describe("Record", () => {
  ...
  it("makeReducerUnion", () => {
    ...
    deepStrictEqual(R.combine({ a: 1, b: 2 }, { a: 3, b: 4, c: 5 }), {
      a: 4,
      b: 6,
      c: 5,
    });
...
describe("Record", () => {
  describe("string | symbol APIs", () => {
    ...
    it("fromIterableWith", () => {
      deepStrictEqual(
        Record.fromIterableWith([1, 2, 3, 4], (a) => [
          a === 3 ? "a" : String(a),
          a * 2,
        ]),
        {
    ...
    it("replace", () => {
      ...
      assertSome(pipe(stringRecord, Record.replace("a", 2)), {
        a: 2,
        [symA]: null,
      });
      assertSome(pipe(stringRecord, Record.replace("a", true)), {
        a: true,
        [symA]: null,
      });

      assertNone(pipe(Record.empty<symbol>(), Record.replace(symA, 2)));
      assertSome(pipe(symbolRecord, Record.replace(symA, 2)), {
        [symA]: 2,
        [symB]: 2,
      });
      assertSome(pipe(symbolRecord, Record.replace(symA, true)), {
        [symA]: true,
        [symB]: 2,
      });
    });
    ...
    describe("set", () => {
      it("should replace an existing value", () => {
        deepStrictEqual(Record.set(stringRecord, "a", 2), {
          a: 2,
          [symA]: null,
        });

        deepStrictEqual(Record.set(symbolRecord, symA, 2), {
          [symA]: 2,
          [symB]: 2,
        });
      });
      ...
      it("should add the key / value pair", () => {
        deepStrictEqual(Record.set(stringRecord, "c", 3), {
          a: 1,
          [symA]: null,
          c: 3,
        });

        deepStrictEqual(Record.set(symbolRecord, symC, 3), {
          [symA]: 1,
          [symB]: 2,
          [symC]: 3,
        });
  ...
  describe("string only APIs", () => {
    ...
    it("filterMap", () => {
      ...
      const filtered = Record.filterMap(x, (value, key) =>
        value > 2 ? Result.succeed(key) : Result.failVoid
      );
    ...
    it("partition", () => {
      const f = (n: number) =>
        n > 2 ? Result.succeed(n + 1) : Result.fail(n - 1);
      deepStrictEqual(Record.partition({}, f), [{}, {}]);
      deepStrictEqual(Record.partition({ a: 1, b: 3, [symA]: null }, f), [
        { a: 0 },
        { b: 4 },
      ]);
    });
    ...
    it("isSubrecord", () => {
      ...
      assertFalse(
        Record.isSubrecord({ b: 2 } as Record<string, number>, { a: 1 })
      );
    });
    ...
    it("every", () => {
      assertTrue(
        Record.every((n: number) => n <= 2)({ a: 1, b: 2, [symA]: null })
      );
      assertFalse(
        Record.every((n: number) => n <= 1)({ a: 1, b: 2, [symA]: null })
      );
    });

    it("some", () => {
      assertTrue(
        Record.some((n: number) => n <= 1)({ a: 1, b: 2, [symA]: null })
      );
      assertFalse(
        Record.some((n: number) => n <= 0)({ a: 1, b: 2, [symA]: null })
      );
    });
    ...
    it("makeEquivalence", () => {
      deepStrictEqual(
        Record.makeEquivalence(Equivalence.strictEqual<number>())(
          { a: 1 },
          { a: 1 }
        ),
        true
