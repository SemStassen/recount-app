...
describe("PartitionedSemaphore", () => {
  it.effect("module-level combinators delegate to the instance api", () =>
    Effect.gen(function* () {
      ...
      const value = yield* PartitionedSemaphore.withPermit(
        sem,
        "a",
        Effect.succeed(1)
      );
      ...
      const value2 = yield* PartitionedSemaphore.withPermits(
        sem,
        "b",
        2,
        Effect.succeed(2)
      ...
      const available = yield* PartitionedSemaphore.withPermitsIfAvailable(
        sem,
        1,
        Effect.succeed("ok")
      );
      ...
      const piped = yield* Effect.succeed(3).pipe(
        PartitionedSemaphore.withPermit(sem, "c")
      );
      assert.strictEqual(piped, 3);

      const piped2 = yield* Effect.succeed(4).pipe(
        PartitionedSemaphore.withPermits(sem, "c", 1)
      );
      assert.strictEqual(piped2, 4);

      const pipedAvailable = yield* Effect.succeed("pipe").pipe(
        PartitionedSemaphore.withPermitsIfAvailable(sem, 1)
      );
  ...
  it.effect(
    "withPermitsIfAvailable does not block or run when unavailable",
    () =>
      Effect.gen(function* () {
        const sem = yield* PartitionedSemaphore.make<string>({ permits: 1 });
        let executed = false;
