...
export const make = Effect.gen(function* () {
  ...
  const inputKey: MockTerminal["inputKey"] = (
    ...
    return shouldQuit(input)
      ? Queue.end(queue)
      : Queue.offer(queue, input).pipe(Effect.asVoid);
  };

  const display: MockTerminal["display"] = (input) => Console.log(input);

  const readInput: MockTerminal["readInput"] = Effect.succeed(
    Queue.asDequeue(queue)
  );
...
export const columns: Effect.Effect<number, never, Terminal.Terminal> =
  Effect.flatMap(MockTerminal, (terminal) => terminal.columns);
...
export const readLine: Effect.Effect<
  string,
  Terminal.QuitError,
  Terminal.Terminal
> = Effect.flatMap(MockTerminal, (terminal) => terminal.readLine);
