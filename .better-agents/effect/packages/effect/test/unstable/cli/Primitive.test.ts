    ...
    it.layer(TestLayer)((it) => {
      ...
      it.effect("should fail when a required file path does not exist", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            error,
            "Path does not exist: /non/existent/file.txt"
          );
        }).pipe(
          Effect.provide(
      ...
      it.effect(
        "should fail when a required directory path does not exist",
        () =>
          Effect.gen(function* () {
            ...
            assert.strictEqual(
              error,
              "Path does not exist: /non/existent/directory"
            );
          }).pipe(
            Effect.provide(
              FileSystem.layerNoop({
