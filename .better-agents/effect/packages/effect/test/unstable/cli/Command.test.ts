...
describe("Command", () => {
  describe("annotations", () => {
    it.effect("should expose annotations in help docs", () =>
      Effect.gen(function* () {
        const Team = Context.Service<never, string>(
          "effect/test/unstable/cli/Team"
        );
        const Priority = Context.Service<never, number>(
          "effect/test/unstable/cli/Priority"
        );
    ...
    it.effect("should keep annotations when adding subcommands", () =>
      Effect.gen(function* () {
        const Scope = Context.Service<never, string>(
          "effect/test/unstable/cli/Scope"
        );
        const docs: Array<Parameters<CliOutput.Formatter["formatHelpDoc"]>[0]> =
          [];
        ...
        const child = Command.make("child").pipe(
          Command.annotate(Scope, "child")
        );
  ...
  describe("run", () => {
    ...
    it.effect(
      "should merge repeated key=value flags into a single record",
      () =>
        Effect.gen(function* () {
          ...
          yield* runCommand(["--env", "foo=bar", "--env", "cool=dude"]);
    ...
    it.effect(
      "should merge key=value flags even when interleaved with other options",
      () =>
        Effect.gen(function* () {
    ...
    it.effect("should support mixed action and setting global flags", () =>
      Effect.gen(function* () {
        ...
        ).pipe(Command.withGlobalFlags([VerboseAction, Format]));
    ...
    it.effect(
      "should reject explicit values for canonical negated global flags",
      () =>
        Effect.gen(function* () {
          ...
          yield* runCommand(["--no-verbose", "false"]).pipe(
    ...
    it.effect(
      "should expose setting global flags in Command.provide APIs",
      () =>
        Effect.gen(function* () {
          const Region = GlobalFlag.setting("region")({
            flag: Flag.string("region").pipe(Flag.optional),
          });
          const RegionFromProvide = Context.Service<
            never,
          ...
          assert.deepStrictEqual(capturedFromProvide, [
            Option.some("eu-west-1"),
          ]);
          assert.deepStrictEqual(capturedFromProvideEffect, [
            Option.some("eu-west-1"),
          ]);
        }).pipe(Effect.provide(TestLayer))
    );
    ...
    it.effect(
      "should reject global flags that are out of scope for selected subcommand",
      () =>
        Effect.gen(function* () {
          const Region = GlobalFlag.setting("region")({
    ...
    it.effect(
      "should show only path-active global flags in subcommand help",
      () =>
        Effect.gen(function* () {
          const Region = GlobalFlag.setting("region")({
            flag: Flag.string("region").pipe(Flag.withDefault("us-east-1")),
          ...
          assert.isTrue(
            deployHelp.some((line) => String(line).includes("--region"))
          );
          ...
          assert.isFalse(
            dbSection.some((line) => String(line).includes("--region"))
          );
        }).pipe(Effect.provide(TestLayer))
    );
    ...
    it.effect("should fail for malformed key=value flags", () =>
      Effect.gen(function* () {
        ...
        yield* runCommand(["--env", "invalid"]).pipe(Effect.ignore);

        const stderr = yield* TestConsole.errorLines;
        assert.isTrue(
          stderr.some((line) =>
            String(line).includes("Invalid key=value format")
          ),
    ...
    it.effect("should handle parsing errors from run", () =>
      Effect.gen(function* () {
        ...
        assert.isTrue(
          stdout.some((line) => String(line).includes("DESCRIPTION"))
        );
        assert.isTrue(
          stdout.some((line) => String(line).includes("comprehensive CLI tool"))
        );
        ...
        assert.isTrue(
          stderr.some((line) => String(line).includes("Unknown subcommand"))
        );
        assert.isTrue(
          stderr.some((line) => String(line).includes("invalid-command"))
        );
      }).pipe(Effect.provide(TestLayer))
    );
    ...
    it.effect("should propagate handler errors from run", () =>
      Effect.gen(function* () {
        const result = yield* Effect.flip(
          Cli.run(["test-failing", "--input", "test"])
        );
  ...
  describe("withSubcommands", () => {
    ...
    it.effect("should execute subcommand when provided by alias", () =>
      Effect.gen(function* () {
        ...
        const plan = Command.make("plan", {}, () =>
          Effect.sync(() => executed.push("plan"))
        ).pipe(Command.withAlias("p"));
    ...
    it.effect("should accept grouped and ungrouped subcommands", () =>
      Effect.gen(function* () {
        ...
        const login = Command.make("login", {}, () =>
          Effect.sync(() => executed.push("login"))
        );
        const logout = Command.make("logout", {}, () =>
          Effect.sync(() => executed.push("logout"))
        );
        const status = Command.make("status", {}, () =>
          Effect.sync(() => executed.push("status"))
        );
    ...
    it.effect(
      "should execute parent handler with options when no subcommand provided",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "should accept shared parent flags before or after a subcommand (npm-style)",
      () =>
        Effect.gen(function* () {
          const messages: Array<string> = [];
          ...
          const install = Command.make(
            ...
            (config) =>
              Effect.gen(function* () {
                ...
                messages.push(
                  `install: global=${parentConfig.global}, pkg=${config.pkg}`
                );
              })
          );
      ...
      Effect.gen(function* () {
        ...
        const root = Command.make("tool", {
          ...
          Command.withHandler((config) =>
            Effect.sync(() => {
              messages.push(
                `root: workspace=${config.workspace}, model=${config.model}`
              );
            })
          )
        );
        ...
        assert.deepStrictEqual(messages, [
          "root: workspace=docs, model=gpt-4o",
        ]);
      }).pipe(Effect.provide(TestLayer))
    );
    ...
    it.effect("should allow child flags to reuse parent local flag names", () =>
      Effect.gen(function* () {
        ...
        const chat = Command.make(
          ...
          (config) =>
            Effect.gen(function* () {
              ...
              messages.push(
                `chat: workspace=${parent.workspace}, model=${config.model}`
              );
            })
        );
        ...
        assert.deepStrictEqual(messages, [
          "chat: workspace=docs, model=gpt-4o-mini",
        ]);
      }).pipe(Effect.provide(TestLayer))
    );
    ...
    it.effect("should reject parent local flags on subcommand paths", () =>
      Effect.gen(function* () {
        ...
        yield* runCli([
          ...
          "--topic",
          "bugs",
        ]).pipe(Effect.catchTag("ShowHelp", () => Effect.void));

        const stderr = yield* TestConsole.errorLines;
        const text = stderr.join("\n");
        assert.isTrue(
          text.includes("Unrecognized flag: --model in command tool chat")
        );
      }).pipe(Effect.provide(TestLayer))
    );

    it.effect(
      "should allow direct accessing parent config in subcommands",
      () =>
        Effect.gen(function* () {
          const messages: Array<string> = [];
    ...
    it.effect("should handle nested subcommands with context sharing", () =>
      Effect.gen(function* () {
        ...
        const runCommand = Command.runWith(appWithService, {
          version: "1.0.0",
        });
        yield* runCommand([
          "--env",
      ...
      Effect.gen(function* () {
        ...
        const parent = Command.make("app").pipe(
          ...
          Command.withHandler((config) =>
            Effect.gen(function* () {
              messages.push(
                `parent: verbose=${config.verbose}, config=${config.config}`
              );
            })
          )
        );
        ...
        const combined = parent.pipe(Command.withSubcommands([deploy]));
    ...
    it.effect(
      "should work with Command.provide when shared flags are present",
      () =>
        Effect.gen(function* () {
          const messages: Array<string> = [];

          const DbUrl = Context.Service<never, string>(
            "effect/test/unstable/cli/DbUrl"
          );
          ...
          const cli = root.pipe(
            ...
            Command.provideSync(
              DbUrl,
              (input) => `db://${input.env}.example.com`
            )
          );
          ...
          assert.deepStrictEqual(messages, [
            "env=production, db=db://production.example.com",
          ]);
        }).pipe(Effect.provide(TestLayer))
    );

    it.effect(
      "should reject shared/child flag collisions when withSharedFlags is applied after withSubcommands",
      () =>
        Effect.sync(() => {
          const child = Command.make("run", {
            verbose: Flag.boolean("verbose"),
    ...
    it.effect(
      "should require withSharedFlags before withSubcommands for subcommand access",
      () =>
        Effect.gen(function* () {
          const messages: Array<string> = [];
    ...
    it.effect(
      "should treat tokens after -- as operands (no subcommand or flags)",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "should coerce boolean flags to false when given falsey literals",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "should support optional boolean flags and --no-<flag> negation",
      () =>
        Effect.gen(function* () {
    ...
    it.effect("should prefer canonical negation over conflicting aliases", () =>
      Effect.gen(function* () {
        ...
        const cmd = Command.make(
          ...
          (config) =>
            Effect.sync(() =>
              captured.push([config.prompt, config.force] as const)
            )
        );
    ...
    it.effect("should reject explicit values for canonical negated flags", () =>
      Effect.gen(function* () {
        ...
        yield* runCmd(["--no-open", "false"]).pipe(
          Effect.catchTag("ShowHelp", () => Effect.void)
        );
        yield* runCmd(["--no-open=true"]).pipe(
          Effect.catchTag("ShowHelp", () => Effect.void)
        );
    ...
    it.effect(
      "should not leak a negated flag literal into subcommand parsing",
      () =>
        Effect.gen(function* () {
          ...
          yield* runCmd(["--no-open", "false", "child"]).pipe(
            Effect.catchTag("ShowHelp", () => Effect.void)
          );
          ...
          assert.isTrue(
            output.includes("use --no-open by itself to set --open to false")
          );
    ...
    it.effect(
      "should parse combined short flags including one that expects a value",
      () =>
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(captured, [
            { all: true, verbose: true, pkg: "cowsay" },
          ]);
        }).pipe(Effect.provide(TestLayer))
    );
    ...
    it.effect("should honor -- while still applying parent flags", () =>
      Effect.gen(function* () {
        ...
        const root = Command.make(
          ...
          (config) =>
            Effect.sync(() =>
              captured.push({ global: config.global, rest: config.rest })
            )
        );
        ...
        assert.deepStrictEqual(captured, [
          { global: true, rest: ["child", "--value", "x"] },
        ]);
      }).pipe(Effect.provide(TestLayer))
    );

    it.effect(
      "should report unknown flag even when subcommand is unknown",
      () =>
        Effect.gen(function* () {
          ...
          yield* runCli(["--unknown", "bogus"]).pipe(
            Effect.catchTag("ShowHelp", () => Effect.void)
          );
    ...
    it.effect(
      "should keep variadic argument order when options are interleaved",
      () =>
        Effect.gen(function* () {
          const captured: Array<{
          ...
          const cmd = Command.make(
            ...
            (config) =>
              Effect.sync(() =>
                captured.push({ files: config.files, verbose: config.verbose })
              )
          );
          ...
          assert.deepStrictEqual(captured, [
            { files: ["a.txt", "b.txt", "c.txt"], verbose: true },
          ]);
        }).pipe(Effect.provide(TestLayer))
    );

    it.effect(
      "should support options before, after, or between operands (relaxed POSIX Syntax Guideline No. 9)",
      () =>
        Effect.gen(function* () {
          ...
          for (let i = 0; i < 3; i++) {
            ...
            assert.isTrue(
              String(actions[i].details.source).includes("src.txt")
            );
            assert.isTrue(
              String(actions[i].details.destination).includes("dest.txt")
            );
          }
        }).pipe(Effect.provide(TestLayer))
    );
    ...
    it.effect(
      "should suggest similar subcommands for unknown subcommands",
      () =>
        Effect.gen(function* () {
          yield* Cli.run(["cpy"]).pipe(
            Effect.catchTag("ShowHelp", () => Effect.void)
          );
    ...
    it.effect(
      "should suggest similar subcommands for nested unknown subcommands",
      () =>
        Effect.gen(function* () {
          yield* Cli.run(["admin", "usrs", "list"]).pipe(
            Effect.catchTag("ShowHelp", () => Effect.void)
          );
    ...
    it.effect("should suggest similar options for unrecognized options", () =>
      Effect.gen(function* () {
        yield* Cli.run(["--debugs", "copy", "src.txt", "dest.txt"]).pipe(
          Effect.catchTag("ShowHelp", () => Effect.void)
        );
    ...
    it.effect(
      "should suggest similar short options for unrecognized short options",
      () =>
        Effect.gen(function* () {
          yield* Cli.run(["-u", "copy", "src.txt", "dest.txt"]).pipe(
            Effect.catchTag("ShowHelp", () => Effect.void)
          );
    ...
    it.effect(
      "should print version and exit even with subcommands (global precedence)",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "should print version and exit when --version appears before subcommand",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "should let global flags before subcommands override non-shared parent locals",
      () =>
        Effect.gen(function* () {
          const captured: Array<boolean> = [];
          let childInvoked = false;
    ...
    it.effect(
      "should keep root local flags over globals when no subcommand is selected",
      () =>
        Effect.gen(function* () {
          const captured: Array<string> = [];
    ...
    it.effect(
      "should skip intervening local flag values before selecting subcommands",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "should stop looking for subcommands after the first positional argument",
      () =>
        Effect.gen(function* () {
          const captured: Array<{
            files: ReadonlyArray<string>;
    ...
    it.effect(
      "should let local flags override global flags on the selected command",
      () =>
        Effect.gen(function* () {
          const captured: Array<string> = [];
          ...
          const packageCommand = Command.make("package").pipe(
            Command.withSubcommands([release])
          );
    ...
    it.effect(
      "should let local short aliases override global short aliases on the selected command",
      () =>
        Effect.gen(function* () {
          const Output = GlobalFlag.setting("output")({
            flag: Flag.choice("output", [
              ...
              "json",
              "yaml",
            ] as const).pipe(Flag.withAlias("o"), Flag.withDefault("pretty")),
          });
          const captured: Array<"summary" | "json" | "csv"> = [];
          const report = Command.make(
            "report",
            {
              output: Flag.choice("output", [
                "summary",
                "json",
                "csv",
              ] as const).pipe(
    ...
    it.effect(
      "should let local flags override scoped global flags from another command branch",
      () =>
        Effect.gen(function* () {
          const Region = GlobalFlag.setting("region")({
            flag: Flag.choice("region", ["us", "eu"] as const).pipe(
              Flag.withDefault("us")
            ),
          });
          const captured: Array<string> = [];
          ...
          const command = Command.make("app").pipe(
            Command.withSubcommands([deploy, status])
          );
    ...
    it.effect(
      "should include short description metadata in HelpDoc subcommands",
      () =>
        Effect.gen(function* () {
          const child = Command.make("build").pipe(
            Command.withDescription("Build the project and all artifacts"),
          ...
          assert.strictEqual(
            listed?.description,
            "Build the project and all artifacts"
          );
        })
    );
  });
      ...
      Effect.gen(function* () {
        const command = Command.make("login").pipe(
          Command.withExamples([
            {
              command: "myapp login",
              description: "Log in with browser OAuth",
            },
            {
