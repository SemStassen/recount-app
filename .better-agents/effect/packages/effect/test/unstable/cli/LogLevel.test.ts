...
class MockLogger extends Context.Service<
  MockLogger,
  {
...
describe("LogLevel", () => {
  ...
  const filterLogs = (minLevel: string) => {
    ...
    const minSeverity = severityOrder.indexOf(
      minLevel === "warning"
        ? "Warn"
        : minLevel.charAt(0).toUpperCase() + minLevel.slice(1)
    );
    return allLogs
  ...
  it.effect(
    "uses Info as the default minimum when --log-level is omitted",
    () =>
      Effect.gen(function* () {
        const logs = yield* testLogLevels();
        // Default minimum log level filters out Debug but keeps Info and above
        const expected = allLogs
          .filter((log) => log.message !== "debug")
          .reverse();
    ...
    Effect.gen(function* () {
      ...
      const command = Command.make("test").pipe(
        Command.withHandler(
          Effect.fnUntraced(function* () {
            seen.push(
              Option.map(yield* GlobalFlag.LogLevel, (level) =>
                level.toLowerCase()
              )
            );
          })
  ...
  it.effect("keeps concurrent command log levels scoped to each run", () =>
    Effect.gen(function* () {
      const testCommand = Command.make("test", {}, () =>
        Effect.logInfo("Should not see this")
      );
