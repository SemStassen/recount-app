...
describe("Prompt.text", () => {
  ...
  it.effect(
    "does not insert characters for unsupported ctrl key combinations",
    () =>
      Effect.gen(function* () {
        const prompt = Prompt.text({
          message: "Name",
...
describe("Prompt.multiSelect", () => {
  it.effect("underlines the active label", () =>
    Effect.gen(function* () {
      ...
      const highlightedFrame = [...frames]
        .reverse()
        .find((frame) => frame.includes("Beta"));

      assert.isTrue(highlightedFrame !== undefined);
      assert.isTrue(
        highlightedFrame?.includes(`${escape}[4m${escape}[96mBeta${escape}[0m`)
      );
