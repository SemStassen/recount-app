...
describe("CommandDescriptor", () => {
  describe("fromCommand", () => {
    ...
    it("extracts file, directory, and path flags", () => {
      ...
      assert.deepStrictEqual(inputFlag.type, {
        _tag: "Path",
        pathType: "file",
      });
      assert.deepStrictEqual(outDirFlag.type, {
        _tag: "Path",
        pathType: "directory",
      });
      assert.deepStrictEqual(configFlag.type, {
        _tag: "Path",
        pathType: "either",
      });
    });
    ...
    it("extracts nested subcommands recursively", () => {
      ...
      const mid = Command.make("mid").pipe(Command.withSubcommands([leaf]));
      const root = Command.make("root").pipe(Command.withSubcommands([mid]));
      ...
      assert.strictEqual(
        desc.subcommands[0].subcommands[0].description,
        "A leaf command"
      );
    });
    ...
    it("extracts descriptions from flags and arguments", () => {
      const cmd = Command.make("test", {
        port: Flag.integer("port").pipe(Flag.withDescription("Port number")),
    ...
    it("handles deeply nested command trees", () => {
      ...
      const root = Command.make("root").pipe(Command.withSubcommands([level1]));
      const desc = fromCommand(root);
    ...
    it("extracts file/directory arguments", () => {
      ...
      assert.deepStrictEqual(outputArg.type, {
        _tag: "Path",
        pathType: "directory",
      });
    });
