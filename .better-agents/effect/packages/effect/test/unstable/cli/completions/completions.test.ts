...
const simpleCmd = Command.make("greet", {
  name: Argument.string("name").pipe(Argument.withDescription("Name to greet")),
...
const withSubcommands = (() => {
  const start = Command.make("start", {
    ...
    daemon: Flag.boolean("daemon").pipe(Flag.withDescription("Run as daemon")),
...
const withPaths = Command.make("process", {
  ...
  outDir: Flag.directory("output-dir").pipe(
...
const nested3Levels = (() => {
  ...
  const mid = Command.make("sub").pipe(Command.withSubcommands([leaf]));

  return Command.make("top").pipe(Command.withSubcommands([mid]));
})();
...
describe("Fish completions", () => {
  ...
  it("combines subcommand and boolean dedup conditions", () => {
    ...
    assert.include(
      script,
      "__fish_seen_subcommand_from start; and not __fish_contains_opt daemon no-daemon"
    );
  ...
  it("suppresses default file completion for commands without path arguments", () => {
    ...
    assert.include(
      script,
      "complete -c server -n '__fish_seen_subcommand_from start' -f"
    );
  });
  ...
  it("bare-TAB -a entries use double quotes around glob pattern to avoid nested single-quote errors", () => {
    ...
    for (const line of argEntries) {
      ...
      assert.include(
        line,
        '"-*"',
        `bare-TAB entry should use double-quoted glob pattern: ${line}`
      );
      assert.notInclude(
        line,
        "'-*'",
        `bare-TAB entry must NOT use single-quoted glob pattern: ${line}`
      );
    }
  });
  ...
  it("does not suppress file completion for commands with path arguments", () => {
    ...
    assert.isFalse(
      bareSuppression,
      "Commands with path arguments should not suppress file completion"
    );
  });
});
