...
describe("Command arguments", () => {
  ...
  it("should handle file mustExist validation", () =>
    Effect.gen(function* () {
      ...
      const existingFileCommand = Command.make(
        "test",
        {
      ...
      yield* Command.runWith(existingFileCommand, { version: "1.0.0" })([
        "/file.txt",
      ]);
      ...
      const runCommand = Command.runWith(existingFileCommand, {
        version: "1.0.0",
      });
      ...
      assert.isTrue(
        stderr.some((line) => String(line).includes("does not exist"))
      );

      // Test 3: mustExist: false - should always pass
      const result3Ref = yield* Ref.make<string | null>(null);
      const optionalFileCommand = Command.make(
        "test",
        {
  ...
  it("should handle choiceWithValue", () =>
    Effect.gen(function* () {
      ...
      const testCommand = Command.make(
        "test",
        {
          level: Argument.choiceWithValue("level", [
            ["debug", 0],
            ["info", 1],
            ["error", 2],
          ] as const),
  ...
  it("should handle filter combinator - invalid", () =>
    Effect.gen(function* () {
      ...
      const stderr = yield* TestConsole.errorLines;
      assert.isTrue(
        stderr.some((line) => String(line).includes("out of range"))
      );
    }).pipe(Effect.provide(TestLayer)));
  ...
  it("should handle filterMap combinator - invalid", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        stderr.some((line) =>
          String(line).includes("Expected positive integer")
        )
      );
    }).pipe(Effect.provide(TestLayer)));
      ...
      const testCommand = Command.make(
        "test",
        {
          value: Argument.integer("value").pipe(
            Argument.orElse(() =>
              Argument.string("value").pipe(Argument.map(() => -1))
            )
          ),
        },
        (config) => Ref.set(resultRef, config)
  ...
  it("should handle withMetavar combinator", () =>
    Effect.gen(function* () {
      const testCommand = Command.make(
        "test",
        {
          file: Argument.string("file").pipe(Argument.withMetavar("FILE_PATH")),
        },
        () => Effect.void
      );
