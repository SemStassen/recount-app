...
class LookupService extends Context.Service<
  LookupService,
...
export const suite = (
  storeId: string,
  layer: Layer.Layer<Persistence.Persistence, unknown>
) =>
  describe(`PersistedCache (${storeId})`, { timeout: 30_000 }, () => {
    it.effect("smoke test", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          yield* cache.get(new TTLRequest({ id: 1 })),
        ...
        const results = yield* store.getMany([
          new TTLRequest({ id: 1 }),
          new TTLRequest({ id: 2 }),
        ]);
        ...
        Effect.catchFilter(
          (e) =>
            e instanceof TransientError ? Result.succeed(e) : Result.fail(e),
          () => Effect.void
        ),
        flakyTest
      )
    ...
    it.effect(
      "requireServicesAt: 'lookup' requires lookup services at get-time",
      () =>
        Effect.gen(function* () {
          const cache = yield* PersistedCache.make(
            (req: TTLRequest) =>
              Effect.map(
                LookupService,
                (service) => new User({ id: req.id, name: service.value })
              ),
            {
          ...
          const result1 = yield* cache
            .get(new TTLRequest({ id: 1 }))
            .pipe(
              Effect.provideService(
                LookupService,
                LookupService.of({ value: "first" })
              )
            );
          const result2 = yield* cache
            .get(new TTLRequest({ id: 2 }))
            .pipe(
              Effect.provideService(
                LookupService,
                LookupService.of({ value: "second" })
              )
            );
          const result3 = yield* cache
            .get(new TTLRequest({ id: 1 }))
            .pipe(
              Effect.provideService(
                LookupService,
                LookupService.of({ value: "third" })
              )
            );
