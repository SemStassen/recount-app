...
const RateLimiterTestLayer = RateLimiter.layer.pipe(
...
describe("HttpClient", () => {
  describe("retryTransient", () => {
    it.effect(
      "retries transient responses with retryOn errors-and-responses",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "does not retry transient responses with retryOn errors-only",
      () =>
        Effect.gen(function* () {
          const { attempts, client } = yield* makeStatusClient(503);
          const retryClient = client.pipe(
