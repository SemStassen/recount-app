...
describe("HttpEffect", () => {
  describe("toWebHandler", () => {
    test("json", async () => {
      const handler = HttpEffect.toWebHandler(
        HttpServerResponse.json({ foo: "bar" })
      );
      ...
      const handler = HttpEffect.toWebHandler(
        Effect.succeed(
          HttpServerResponse.jsonUnsafe({ foo: "bar" }).pipe(
            ...
            HttpServerResponse.setCookieUnsafe("test", "123", {
              secure: true,
              httpOnly: true,
              sameSite: "strict",
            })
      ...
      const handler = HttpEffect.toWebHandler(
        Effect.succeed(
          HttpServerResponse.stream(
            Stream.make("foo", "bar").pipe(Stream.encodeText)
          )
        )
      );
      const response = await handler(new Request("http://localhost:3000/"));
  ...
  describe("fromWebHandler", () => {
    test("basic GET request", async () => {
      ...
      strictEqual(
        await response.text(),
        "Hello from http://localhost:3000/hello"
      );
    });
