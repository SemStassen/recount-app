  ...
  describe("logger", () => {
    ...
    it.effect("uses a stable http.span log span name", () =>
      Effect.gen(function* () {
        ...
        const request1 = HttpServerRequest.fromWeb(
          new Request("http://localhost:3000/one")
        );
        const request2 = HttpServerRequest.fromWeb(
          new Request("http://localhost:3000/two")
        );

        yield* loggedApp.pipe(
          Effect.provideService(HttpServerRequest.HttpServerRequest, request1)
        );
        yield* loggedApp.pipe(
          Effect.provideService(HttpServerRequest.HttpServerRequest, request2)
        );
