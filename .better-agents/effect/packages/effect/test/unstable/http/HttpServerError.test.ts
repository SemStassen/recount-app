...
describe("HttpServerError", () => {
  describe("reason messages", () => {
    it("formats request reason messages", () => {
      ...
      assert.strictEqual(
        parseError.message,
        "RequestParseError (POST /widgets): invalid body"
      );
      assert.strictEqual(
        routeNotFound.message,
        "RouteNotFound (POST /widgets)"
      );
    });
    ...
    it("formats response reason messages", () => {
      ...
      assert.strictEqual(
        ...
        "ResponseError (418 GET /tea): short and stout"
      );
    });
  });
      ...
      Effect.gen(function* () {
        ...
        const cases: ReadonlyArray<readonly [Respondable.Respondable, number]> =
          [
            ...
            [
              new HttpServerError({
                reason: new RequestParseError({ request }),
              }),
              400,
            ],
          ];
  ...
  describe("isHttpServerError", () => {
    it("matches wrapper only", () => {
      ...
      assert.isFalse(
        isHttpServerError(new ServeError({ cause: new Error("boom") }))
      );
  ...
  describe("causeResponseStripped", () => {
    ...
    it("returns some cause for non-response failures", () => {
      ...
      const [out, cause] = causeResponseStripped(
        Cause.fail(new RouteNotFound({ request }))
      );
