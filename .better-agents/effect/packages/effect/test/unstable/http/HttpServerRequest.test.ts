...
describe("HttpServerRequest", () => {
    ...
    if (clientRequest.body._tag === "Stream") {
      strictEqual(
        await Effect.runPromise(
          clientRequest.body.stream.pipe(Stream.decodeText(), Stream.mkString)
        ),
        '{"foo":"bar"}'
      );
    }
  ...
  it.effect(
    "fromClientRequest preserves request metadata and shares concurrent stream body accessors",
    () =>
      Effect.gen(function* () {
        const clientRequest = HttpClientRequest.post(
          new URL("http://localhost:3000/items?existing=1#top")
        ).pipe(
        ...
        assert.strictEqual(
          request.originalUrl,
          "http://localhost:3000/items?existing=1&page=1#top"
        );
  ...
  it.effect(
    "fromClientRequest exposes FormData bodies as multipart parts",
    () =>
      Effect.gen(function* () {
        ...
        if (parts[1]?._tag === "File") {
          ...
          assert.strictEqual(
            new TextDecoder().decode(yield* parts[1].contentEffect),
            "hello"
          );
        }
      })
  ...
  it("modify distinguishes missing and explicit none remoteAddress", () => {
    ...
    const withRemoteAddress = request.modify({
    ...
    assertNone(
      withRemoteAddress.modify({ remoteAddress: Option.none() }).remoteAddress
    );
  });
});
