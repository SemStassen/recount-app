...
describe("Cookies", () => {
  it("expireCookie returns a Result with an expired Set-Cookie value", () => {
    assertSuccess(
      Cookies.expireCookie(Cookies.empty, "session", {
        path: "/",
        secure: true,
      }),
  ...
  it("expireCookieUnsafe adds an expired Set-Cookie value", () => {
    deepStrictEqual(
      Cookies.expireCookieUnsafe(Cookies.empty, "session", {
        path: "/",
        secure: true,
      }),
      Cookies.fromReadonlyRecord({
  ...
  describe("CookiesSchema", () => {
    it("serializerIso annotation", () => {
      ...
      assertSuccess(
        ...
        Cookies.makeCookieUnsafe("sessionId", "abc123", {
          path: "/",
          httpOnly: true,
          secure: true,
        })
    ...
    it("toCodecJson", async () => {
      ...
      const asserts = new TestSchema.Asserts(
        Schema.toCodecJson(Schema.toType(schema))
      );
