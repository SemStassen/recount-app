...
describe("Url", () => {
  ...
  it("setHostname", () => {
    expectUrl(
      Url.setHostname(testURL, "newhostname.com"),
      "https://newhostname.com/test"
    );
  });

  it("setHref", () => {
    expectUrl(
      Url.setHref(testURL, "https://newhref.com"),
      "https://newhref.com/"
    );
  });
  ...
  it("setPassword", () => {
    expectUrl(
      Url.setPassword(testURL, "newpassword"),
      "https://:newpassword@example.com/test"
    );
  });

  it("setPassword - Redacted", () => {
    expectUrl(
      Url.setPassword(testURL, Redacted.make("newpassword")),
      "https://:newpassword@example.com/test"
    );
  });
  ...
  it("setPathname", () => {
    expectUrl(
      Url.setPathname(testURL, "/newpath"),
  ...
  it("setSearch", () => {
    expectUrl(
      Url.setSearch(testURL, "?key=value"),
  ...
  it("setUsername", () => {
    expectUrl(
      Url.setUsername(testURL, "newuser"),
      "https://newuser@example.com/test"
    );
  });

  it("modifyUrlParams", () => {
    expectUrl(
      Url.modifyUrlParams(testURL, UrlParams.append("key", "value")),
      "https://example.com/test?key=value"
    );
  });

  it("urlParams", () => {
    const params = Url.urlParams(
  ...
  it("setUrlParams", () => {
    ...
    const newParams = UrlParams.fromInput([
      ["foo", "bar2"],
      ["baz", "qux"],
    ]);
