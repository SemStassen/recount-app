...
const pollUntilComplete = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    for (
      let i = 0;
      i < 10 && (Option.isNone(polled) || polled.value._tag !== "Complete");
      i++
    ) {
...
describe("DurableQueue", () => {
  ...
  const SuccessLayer = Layer.mergeAll(
    SuccessWorkflow.toLayer(({ id, value }) =>
      DurableQueue.process(SuccessQueue, { id, value })
    ),
    DurableQueue.worker(SuccessQueue, ({ value }) => Effect.succeed(value + 1))
  ).pipe(
  ...
  it.effect("processes queued items and resumes the workflow", () =>
    Effect.gen(function* () {
      const executionId = yield* SuccessWorkflow.execute(
        { id: "success", value: 41 },
        { discard: true }
      );
      const polled = yield* pollUntilComplete(
        SuccessWorkflow.poll(executionId)
      );
  ...
  const FailureLayer = Layer.mergeAll(
    FailureWorkflow.toLayer(({ id }) =>
      DurableQueue.process(FailureQueue, { id })
    ),
    DurableQueue.worker(FailureQueue, () => Effect.fail("boom"))
  ).pipe(
    Layer.provideMerge(WorkflowEngine.layerMemory),
    Layer.provideMerge(PersistedQueueLayer)
  );
  ...
  it.effect("propagates worker failures to the workflow", () =>
    Effect.gen(function* () {
      const executionId = yield* FailureWorkflow.execute(
        { id: "failure" },
        { discard: true }
      );
      const polled = yield* pollUntilComplete(
        FailureWorkflow.poll(executionId)
      );
