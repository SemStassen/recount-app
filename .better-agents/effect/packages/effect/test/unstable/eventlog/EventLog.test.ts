...
const handlerLayer = (handled: Ref.Ref<ReadonlyArray<string>>) =>
  EventLog.group(UserGroup, (handlers) =>
    handlers.handle("UserCreated", ({ payload }) =>
      Ref.update(handled, (values) => [...values, payload.id])
    )
...
describe("EventLog", () => {
  it.effect(
    "writes a typed event, commits the entry, and runs its handler",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "publishes local journal changes through a scoped subscription",
    () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          second.duplicateEntries.map((_) => _.idString),
          [entry.idString]
        );
