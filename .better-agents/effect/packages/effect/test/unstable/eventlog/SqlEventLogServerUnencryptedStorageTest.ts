...
const makeStoreId = (prefix: string) =>
  `${uniqueNamespace(prefix)}_store` as StoreId;
...
}) => SqlEventLogServerUnencrypted.makeStorage(options).pipe(Effect.orDie);

export const suite = (
  name: string,
  layer: Layer.Layer<SqlClient.SqlClient, unknown>
) =>
  it.layer(Layer.mergeAll(Reactivity.layer, layer), { timeout: "30 seconds" })(
    `SqlEventLogServerUnencrypted (${name})`,
    (it) => {
      ...
      it.effect(
        "replays backlog and then streams live changes without startup duplication",
        () =>
          Effect.gen(function* () {
            const storage = yield* makeStorage(
              makeOptions("changes_backlog_then_live")
            );
            ...
            assert.deepStrictEqual(
              replayed.map((entry) => entry.remoteSequence),
              [1, 2]
            );
            assert.deepStrictEqual(
              replayed.map((entry) => entry.entry.idString),
              [entryA.idString, entryB.idString]
            );
      ...
      it.effect(
        "handles the changes startup race without losing or duplicating rows",
        () =>
          Effect.gen(function* () {
            const storage = yield* makeStorage(
              makeOptions("changes_startup_race")
            );
            ...
            for (let iteration = 0; iteration < 5; iteration++) {
              ...
              const changesFiber = yield* storage
                .changes({
                  storeId,
                  startSequence: 0,
                  compactors: new Map(),
              ...
              yield* Effect.yieldNow;
              assert.strictEqual(
                Option.isNone(yield* Queue.poll(changes)),
                true
              );
            }
          })
      ...
      it.effect("isolates reads and streams between stores", () =>
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(
            backlogA.map((entry) => entry.entry.idString),
            [entryA1.idString]
          );
