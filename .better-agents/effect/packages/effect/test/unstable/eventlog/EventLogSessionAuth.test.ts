...
describe("EventLogSessionAuth", () => {
  it.effect(
    "encodes and decodes canonical auth payload deterministically",
    () =>
      Effect.gen(function* () {
        ...
        const encodedA =
          yield* EventLogSessionAuth.encodeSessionAuthPayload(payload);
        const encodedB =
          yield* EventLogSessionAuth.encodeSessionAuthPayload(payload);
        ...
        const decoded =
          yield* EventLogSessionAuth.decodeSessionAuthPayload(encodedA);
    ...
    Effect.gen(function* () {
      ...
      const error = yield* EventLogSessionAuth.verifySessionAuthPayload({
        ...
        signature: new Uint8Array(
          EventLogSessionAuth.Ed25519SignatureLength - 1
        ),
      }).pipe(Effect.flip);
  ...
  it.effect("rejects malformed signing public key length", () =>
    Effect.gen(function* () {
      const error = yield* EventLogSessionAuth.encodeSessionAuthPayload({
        ...
        publicKey: "client-public-key",
        signingPublicKey: new Uint8Array(
          EventLogSessionAuth.Ed25519PublicKeyLength - 1
        ),
      }).pipe(Effect.flip);
      assert.strictEqual(error._tag, "EventLogSessionAuthError");
