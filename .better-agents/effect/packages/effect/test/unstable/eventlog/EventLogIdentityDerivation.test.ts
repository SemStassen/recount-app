...
const makeRootSecret = () =>
  Uint8Array.from(Array.from({ length: 32 }, (_, index) => (index * 17) % 256));
...
describe("EventLog root secret derivation", () => {
  it.effect(
    "derives the same material for identities with the same root secret",
    () =>
      Effect.gen(function* () {
        const rootSecret = makeRootSecret();
        const derive = makeGetIdentityRootSecretMaterial(globalThis.crypto);

        const first = yield* derive(
          makeIdentity({
            publicKey: "user-a",
            rootSecret,
          })
        );
        const second = yield* derive(
          makeIdentity({
            publicKey: "user-b",
            rootSecret,
          })
        ...
        assert.deepStrictEqual(
          first.encryptionKeyMaterial,
          second.encryptionKeyMaterial
        );
        assert.deepStrictEqual(first.signingPublicKey, second.signingPublicKey);
        assert.deepStrictEqual(
          Redacted.value(first.signingPrivateKey),
          Redacted.value(second.signingPrivateKey)
        );
      })
  );

  it.effect(
    "uses versioned, domain-separated labels for encryption and signing",
    () =>
      Effect.gen(function* () {
        const derive = makeGetIdentityRootSecretMaterial(globalThis.crypto);
        ...
        const signingSeed = extractSigningSeedFromPkcs8(
          Redacted.value(derived.signingPrivateKey)
        );

        assert.strictEqual(
          equalBytes(derived.encryptionKeyMaterial, signingSeed),
          false
        );
