...
describe("EventLogRemote", () => {
  it.effect(
    "makeUnencrypted authenticates before writing plaintext entries",
    () =>
      Effect.gen(function* () {
        ...
        const writeFiber = yield* harness.remote
          .write({
            identity,
        ...
        const payload = request.request as Rpc.Payload<
          typeof EventLogMessage.WriteSingleRpc
        >;
        const decoded = yield* EventLogMessage.WriteEntriesUnencrypted.decode(
          payload.data
        );
  ...
  it.effect(
    "makeEncrypted authenticates before writing encrypted entries",
    () =>
      Effect.gen(function* () {
        ...
        const payload = request.request as Rpc.Payload<
          typeof EventLogMessage.WriteSingleRpc
        >;
        const decoded = yield* EventLogMessage.WriteEntries.decode(
          payload.data
        );
...
class RemoteHarness extends Context.Service<
  RemoteHarness,
  {
    readonly remote: EventLogRemote.EventLogRemote["Service"];
    readonly take: Effect.Effect<{
      readonly _tag: Rpc.Tag<
        RpcGroup.Rpcs<typeof EventLogMessage.EventLogRemoteRpcs>
      >;
      readonly request: Rpc.Payload<
        RpcGroup.Rpcs<typeof EventLogMessage.EventLogRemoteRpcs>
      >;
...
> {
  ...
  const client = yield* RpcTest.makeClient(
    ...
    Effect.provide(
      EventLogMessage.EventLogRemoteRpcs.toLayer({
    ...
    Effect.provideService(EventLogMessage.EventLogAuthentication, (effect) =>
      Effect.provideService(effect, EventLog.Identity, {} as any)
    )
  );
...
const authenticate = Effect.fnUntraced(function* (options: {
  ...
  const payload = auth.request as Rpc.Payload<
    typeof EventLogMessage.AuthenticateRpc
  >;
  ...
  assert.deepStrictEqual(
    ...
    rootSecretMaterial.signingPublicKey
  );
