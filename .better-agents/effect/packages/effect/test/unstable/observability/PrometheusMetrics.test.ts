    ...
    describe("Counter", () => {
      ...
      it.effect("formats a counter with attributes", () =>
        Effect.gen(function* () {
          ...
          assert.include(
            output,
            'labeled_counter{method="GET",endpoint="/api/users"} 10'
          );
        })
      );
    ...
    describe("Histogram", () => {
      ...
      it.effect("formats a histogram with attributes", () =>
        Effect.gen(function* () {
          ...
          assert.include(
            output,
            'labeled_histogram_bucket{service="api",le="100"} 0'
          );
          assert.include(
            output,
            'labeled_histogram_bucket{service="api",le="500"} 1'
          );
          assert.include(
            output,
            'labeled_histogram_bucket{service="api",le="+Inf"} 1'
          );
    ...
    describe("Frequency", () => {
      ...
      it.effect("formats a frequency metric with attributes", () =>
        Effect.gen(function* () {
          ...
          assert.include(
            output,
            'labeled_frequency{service="api",key="200"} 1'
          );
          assert.include(
            output,
            'labeled_frequency{service="api",key="404"} 1'
          );
        })
      );
    });
