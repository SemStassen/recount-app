...
describe("LanguageModel tracker lifecycle integration", () => {
  it.effect(
    "tracks generateText across turns and recovers after session drop",
    () =>
      Effect.gen(function* () {
        ...
        const provideModel = Effect.provideService(
        ...
        const expectedPostDropFullPrompt = Prompt.fromMessages([
          ...secondSentPrompt.content,
          assistant2,
          user3,
        ]);
        ...
        assert.deepStrictEqual(
          providerOptions[1]!.incrementalPrompt,
          Prompt.fromMessages([user2])
        );
        ...
        assert.deepStrictEqual(
          providerOptions[2]!.prompt,
          expectedPostDropFullPrompt
        );
        assert.strictEqual(providerOptions[3]!.previousResponseId, "resp_3");
        assert.deepStrictEqual(
          providerOptions[3]!.incrementalPrompt,
          Prompt.fromMessages([user4])
        );
      })
  );

  it.effect(
    "falls back to full prompt after stale previous response id and keeps tracking",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const providerOptions: Array<LanguageModel.ProviderOptions> = [];
        ...
        assert.strictEqual(providerOptions[0]!.incrementalPrompt, undefined);
        assert.strictEqual(
          providerOptions[1]!.previousResponseId,
          "resp_object_1"
        );
        assert.deepStrictEqual(
          providerOptions[1]!.incrementalPrompt,
  ...
  it.effect("tracks streamText with metadata parts across turns", () =>
    Effect.gen(function* () {
      ...
      yield* LanguageModel.streamText({
        prompt: Prompt.fromMessages([system, user1]),
      }).pipe(Stream.runDrain, provideModel);
  ...
  it.effect(
    "emits finish parts in streamText when toolkit is provided and no tool calls are returned",
    () =>
      Effect.gen(function* () {
        const languageModel = yield* LanguageModel.make({
          generateText: () => Effect.succeed([]),
  ...
  it.effect(
    "keeps incremental provider fields undefined when tracker is omitted",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "prepares after approval pre-resolution using stripped prompt",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const providerOptions: Array<LanguageModel.ProviderOptions> = [];
        ...
        const languageModel = yield* LanguageModel.make({
          generateText: (options) =>
            Effect.sync(() => {
              ...
              responseIndex += 1;
              providerOptions.push(options);
              return [responseMetadataPart(responseId), finishPart];
            }),
          streamText: () => Stream.empty,
        });
        ...
        yield* LanguageModel.generateText({
          ...
          toolkit: ApprovalToolkit,
        }).pipe(provideModel, Effect.provide(ApprovalToolkitLayer));
