...
describe("ResponseIdTracker", () => {
  ...
  it.effect(
    "prepares only the new user message after a tracked first turn",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const msg1 = userMessage("msg1");
        ...
        const prepared = tracker.prepareUnsafe(
          Prompt.fromMessages([msg1, asst, msg2])
        );
  ...
  it.effect("shares tracker state through its Context service layer", () =>
    Effect.gen(function* () {
      ...
      const prepared = tracker.prepareUnsafe(
        Prompt.fromMessages([msg1New, asst, msg2])
      );
  ...
  it.effect("remains usable after concurrent markParts and clear", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.all([markPartsRace, clearRace], {
  ...
  it.effect("returns incremental user follow-up after assistant", () =>
    Effect.gen(function* () {
      ...
      tracker.markParts([sys, user1], "resp_1");
      const prepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sys, user1, asst1, user2])
      );
  ...
  it.effect("returns incremental tool results after assistant tool call", () =>
    Effect.gen(function* () {
      ...
      tracker.markParts([sys, user1], "resp_1");
      const prepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sys, user1, asst1, tool1])
      );
  ...
  it.effect(
    "returns incremental tool results plus user message in multi-step flow",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const sys = systemMessage("sys");
        ...
        tracker.markParts([sys, user1], "resp_1");
        const prepared = tracker.prepareUnsafe(
          Prompt.fromMessages([sys, user1, asst1, tool1, user2])
        );

        assertPreparedSome(
          prepared,
          "resp_1",
          Prompt.fromMessages([tool1, user2])
        );
      })
  );
  ...
  it.effect("returns None when no new messages follow assistant", () =>
    Effect.gen(function* () {
      ...
      tracker.markParts([sys, user1], "resp_1");
      const prepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sys, user1, asst1])
      );
  ...
  it.effect(
    "returns None in multi-turn prompt with no messages after last assistant",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const sys = systemMessage("sys");
        ...
        const prepared = tracker.prepareUnsafe(
          Prompt.fromMessages([sys, user1, asst1, user2, asst2])
        );
  ...
  it.effect("returns None when system prompt object changes", () =>
    Effect.gen(function* () {
      ...
      tracker.markParts([sys, user1], "resp_1");
      const prepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sysNew, user1, asst1, user2])
      );
  ...
  it.effect("returns None when user message object changes", () =>
    Effect.gen(function* () {
      ...
      tracker.markParts([sys, user1], "resp_1");
      const prepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sys, user1Edited, asst1, user2])
      );
  ...
  it.effect("returns None when multiple prefix messages diverge", () =>
    Effect.gen(function* () {
      ...
      tracker.markParts([sys, user1], "resp_1");
      const prepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sysNew, user1Edited, asst1, user2])
      );
  ...
  it.effect(
    "clears stale state after divergence and recovers when the new prefix is re-marked",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const sys = systemMessage("sys");
        ...
        tracker.markParts([sys, user1], "resp_1");
        const diverged = tracker.prepareUnsafe(
          Prompt.fromMessages([sysNew, user1, asst1, user2])
        );
        assert.isTrue(Option.isNone(diverged));

        tracker.markParts([sysNew, user1], "resp_2");
        const recovered = tracker.prepareUnsafe(
          Prompt.fromMessages([sysNew, user1, asst1, user2])
        );
  ...
  it.effect("supports full lifecycle across two turns", () =>
    Effect.gen(function* () {
      ...
      tracker.markParts([sys, user1], "resp_1");
      const firstPrepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sys, user1, asst1, user2])
      );
      assertPreparedSome(firstPrepared, "resp_1", Prompt.fromMessages([user2]));

      tracker.markParts([sys, user1, asst1, user2], "resp_2");
      const secondPrepared = tracker.prepareUnsafe(
        Prompt.fromMessages([sys, user1, asst1, user2, asst2, user3])
      );
      assertPreparedSome(
        secondPrepared,
        "resp_2",
        Prompt.fromMessages([user3])
      );
    })
  );

  it.effect(
    "returns None for structurally-equal but identity-different prefix message",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const msg1 = userMessage("msg1");
        ...
        tracker.markParts([msg1], "resp_1");
        const prepared = tracker.prepareUnsafe(
          Prompt.fromMessages([msg1Copy, asst1, msg2])
        );
  ...
  it.effect("isolates two conversations on the same tracker", () =>
    Effect.gen(function* () {
      ...
      const preparedA = tracker.prepareUnsafe(
        Prompt.fromMessages([sysA, userA1, asstA1, userA2])
      );
      assertPreparedSome(preparedA, "resp_A1", Prompt.fromMessages([userA2]));

      const preparedB = tracker.prepareUnsafe(
        Prompt.fromMessages([sysB, userB1, asstB1, userB2])
      );
  ...
  it.effect(
    "does not leak tracked state across conversations without assistant turns",
    () =>
      Effect.gen(function* () {
        const tracker = yield* ResponseIdTracker.make;
        const sysA = systemMessage("sysA");
        ...
        const preparedB = tracker.prepareUnsafe(
          Prompt.fromMessages([sysB, userB1])
        );
