...
describe("Prompt", () => {
  describe("fromResponseParts", () => {
    ...
    it("places tool calls in assistant messages and tool results in tool messages", () => {
      ...
      assert.strictEqual(
        typeof assistantContent[0] === "object" && assistantContent[0].type,
        "tool-call"
      );
      ...
      assert.strictEqual(
        typeof toolContent[0] === "object" && toolContent[0].type,
        "tool-result"
      );
    });
    ...
    it("should handle out-of-order tool results (result before call in stream)", () => {
      ...
      assert.strictEqual(
        typeof assistantContent[1] === "object" && assistantContent[1].type,
        "tool-call"
      );
      ...
      assert.strictEqual(
        typeof toolContent[1] === "object" && toolContent[1].type,
        "tool-result"
      );
  ...
  describe("merge", () => {
    it("should sequentially combine the content of two Prompts", () => {
      ...
      assert.deepStrictEqual(
        merged,
        Prompt.fromMessages([...leftMessages, ...rightMessages])
      );
    });
  ...
  describe("appendSystem", () => {
    it("appends text to the leading system message", () => {
      ...
      const result = Prompt.appendSystem(
        prompt,
      ...
      assert.deepStrictEqual(
        ...
        Prompt.makeMessage("system", {
          content:
            "You are an expert in programming. You are a helpful assistant.",
        })
      );
    });
  ...
  describe("prependSystem", () => {
    it("prepends text to the leading system message", () => {
      ...
      const result = Prompt.prependSystem(
        prompt,
        "You are a helpful assistant. "
      );
      ...
      assert.deepStrictEqual(
        ...
        Prompt.makeMessage("system", {
          content:
            "You are a helpful assistant. You are an expert in programming.",
        })
      );
    });
