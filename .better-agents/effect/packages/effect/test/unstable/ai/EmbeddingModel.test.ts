...
describe("EmbeddingModel", () => {
  ...
  it.effect(
    "concurrent embed calls are batched into one provider embedMany call",
    () => {
      let calls = 0;
  ...
  it.effect("provider AiError propagates through embed", () => {
    ...
    }).pipe(Effect.provide(makeLayer(() => Effect.fail(error))));
  });

  it.effect(
    "embed fails with InvalidOutputError when provider misses results",
    () =>
      Effect.gen(function* () {
        const model = yield* EmbeddingModel.EmbeddingModel;
  ...
  it.effect(
    "embedMany fails with InvalidOutputError when provider misses results",
    () =>
      Effect.gen(function* () {
        const model = yield* EmbeddingModel.EmbeddingModel;
        const error = yield* model
          .embedMany(["hello", "world"])
          .pipe(
            Effect.match({
              onFailure: (error) => error,
              onSuccess: () => undefined,
