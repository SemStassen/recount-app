...
describe("McpServer", () => {
  it.effect(
    "replays MCP session and negotiated protocol headers after initialize",
    () =>
      Effect.gen(function* () {
        ...
        strictEqual(
          responses[0].headers.get("Mcp-Protocol-Version"),
          "2025-06-18"
        );
      })
  );
  ...
  it.effect(
    "returns 404 when a non-initialize request omits the MCP session id",
    () =>
      Effect.gen(function* () {
