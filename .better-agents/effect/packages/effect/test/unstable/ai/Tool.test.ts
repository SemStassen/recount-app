...
describe("Tool", () => {
  describe("User Defined", () => {
    ...
    it.effect(
      "should return tool call handler failures when failure mode is return",
      () =>
        Effect.gen(function* () {
          const toolkit = Toolkit.make(FailureModeReturn);
    ...
    it.effect(
      "should return tool call handler failures with failure mode return using OpenAI transformer",
      () =>
        Effect.gen(function* () {
          const toolkit = Toolkit.make(FailureModeReturn);
    ...
    it.effect(
      "should raise an error on tool call handler failures when failure mode is error",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "should raise an error when tool call parameters are invalid",
      () =>
        Effect.gen(function* () {
          const toolkit = Toolkit.make(FailureModeReturn);
    ...
    it.effect(
      "should return AiError when user returns an AiErrorReason when failure mode is return",
      () =>
        Effect.gen(function* () {
          const toolkit = Toolkit.make(FailureModeReturn);
    ...
    it.effect(
      "should raise an AiError when user returns an AiErrorReason when failure mode is error",
      () =>
        Effect.gen(function* () {
          const toolkit = Toolkit.make(FailureModeError);
    ...
    describe("Preliminary Results", () => {
      it.effect(
        "should not have preliminary results when generateText is used",
        () =>
          Effect.gen(function* () {
      ...
      it.effect("should handle multiple preliminary results in sequence", () =>
        Effect.gen(function* () {
          ...
          deepStrictEqual(toolResults[0].result, {
            status: "loading",
            progress: 0,
          });

          strictEqual(toolResults[1].preliminary, true);
          deepStrictEqual(toolResults[1].result, {
            status: "processing",
            progress: 50,
          });

          strictEqual(toolResults[2].preliminary, true);
          deepStrictEqual(toolResults[2].result, {
            status: "finalizing",
            progress: 90,
          });
      ...
      it.effect(
        "should handle preliminary results for long-running tools",
        () =>
          Effect.gen(function* () {
            const toolkit = Toolkit.make(IncrementalTool);
      ...
      it.effect(
        "should emit preliminary results before failure when handler fails",
        () =>
          Effect.gen(function* () {
      ...
      it.effect(
        "should interleave results from concurrent tools with preliminary results",
        () =>
          Effect.gen(function* () {
            const toolkit = Toolkit.make(ConcurrentIncrementalTool);

            const handlers = toolkit.toLayer({
              ConcurrentIncrementalTool: Effect.fnUntraced(function* (
                { name, delay },
                ctx
              ) {
                yield* ctx.preliminary({
                  status: `loading`,
                  job: name,
                  progress: 50,
                });
            ...
            const toolResults: Array<
              Response.ToolResultParts<Toolkit.Tools<typeof toolkit>>
            > = [];
            ...
            strictEqual(toolResults.length, 4);
            strictEqual(toolResults[3].preliminary, false);
            deepStrictEqual(toolResults[3].result, {
              status: "complete",
              job: "slow",
            });
          })
...
const IncrementalTool = Tool.make("IncrementalTool", {
  ...
  success: Schema.Union([
    ...
    Schema.Struct({
      status: Schema.Literal("finalizing"),
      progress: Schema.Number,
    }),
...
describe("Dynamic", () => {
  ...
  describe("getJsonSchema", () => {
    it.effect(
      "Tool.make EmptyParams produces same output as omitting parameters",
      () =>
        Effect.gen(function* () {
          ...
          deepStrictEqual(
            withEmptyJsonSchema,
            Tool.getJsonSchema(withoutParams)
          );
      ...
      Effect.gen(function* () {
        ...
        assertTrue(
          "properties" in jsonSchema &&
            "query" in (jsonSchema.properties as object)
        );
        assertTrue(
          "properties" in jsonSchema &&
            "limit" in (jsonSchema.properties as object)
        );
      })
    );
    ...
    it.effect(
      "preserves complex JSON Schema features like $ref and oneOf",
      () =>
        Effect.gen(function* () {
          const inputSchema = {
            type: "object",
  ...
  describe("handler execution", () => {
    it.effect(
      "decodes parameters with Effect Schema before calling handler",
      () =>
        Effect.gen(function* () {
          ...
          const layer = toolkit.toLayer({
            SearchTool: ({ query, limit }) =>
              Effect.succeed(
                Array.from({ length: limit }, (_, i) => `${query}-${i}`)
              ),
          });
          ...
          deepStrictEqual(response.toolResults[0].result, [
            "test-0",
            "test-1",
            "test-2",
          ]);
    ...
    it.effect("encodes success result using success schema", () =>
      Effect.gen(function* () {
        ...
        const layer = toolkit.toLayer({
          TestTool: () =>
            Effect.succeed({
              timestamp: DateTime.fromDateUnsafe(new Date(1000)),
            }),
        });
