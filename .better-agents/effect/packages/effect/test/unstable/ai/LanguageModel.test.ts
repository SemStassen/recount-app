...
describe("LanguageModel", () => {
  ...
  describe("streamText", () => {
    it("should emit tool calls before executing tool handlers", () =>
      Effect.gen(function* () {
        const parts: Array<
          Response.StreamPart<Toolkit.Tools<typeof MyToolkit>>
        > = [];
  ...
  describe("provider options", () => {
      ...
      Effect.gen(function* () {
        const fullPrompt = Prompt.make([
          ...
          Prompt.assistantMessage({
            content: [Prompt.textPart({ text: "assistant" })],
          }),
              ...
              generateText: (options) => {
                ...
                if (calls.length === 1) {
                  (options as any).prompt =
                    options.incrementalPrompt ?? options.prompt;
                  return Effect.fail(
                    AiError.make({
    ...
    it("uses tracker prepareUnsafe and markParts in generateText without toolkit", () =>
      Effect.gen(function* () {
        ...
        const incrementalPrompt = Prompt.make([
          Prompt.userMessage({
            content: [Prompt.textPart({ text: "incremental" })],
          }),
        ]);
      ...
      Effect.gen(function* () {
        ...
        for (const msg of preparedPrompt.content) {
          if (msg.role === "assistant") {
            strictEqual(
              msg.content.filter((p) => p.type === "tool-approval-request")
                .length,
              0
            );
          }
          if (msg.role === "tool") {
            strictEqual(
              msg.content.filter((p) => p.type === "tool-approval-response")
                .length,
              0
            );
          }
  ...
  describe("tool approval", () => {
    it("emits tool-approval-request when tool has needsApproval: true", () =>
      Effect.gen(function* () {
        const parts: Array<
          Response.StreamPart<Toolkit.Tools<typeof ApprovalToolkit>>
        > = [];
    ...
    it("pre-resolves approved tool calls before calling LLM", () =>
      Effect.gen(function* () {
        ...
        const toolResults = (
          lastMessage.content as Array<Prompt.ToolMessagePart>
        ).filter((p): p is Prompt.ToolResultPart => p.type === "tool-result");
    ...
    it("strips approved approval artifacts from prompt sent to provider (streamText)", () =>
      Effect.gen(function* () {
        ...
        if (assistantMsg.role === "assistant") {
          ...
          strictEqual(
            assistantMsg.content.filter((p) => p.type === "tool-call").length,
            1
          );
        }
      ...
      Effect.gen(function* () {
        ...
        for (const msg of messages) {
          if (msg.role === "assistant") {
            ...
            strictEqual(
              msg.content.filter((p) => p.type === "tool-call").length,
              1
            );
          }
          if (msg.role === "tool") {
    ...
    it("strips previous-round approval artifacts even when no new pending approvals (streamText)", () =>
      Effect.gen(function* () {
        ...
        if (toolMsg.role === "tool") {
          strictEqual(
            toolMsg.content.filter((p) => p.type === "tool-result").length,
            1
          );
        }
      }));
    ...
    it("streamText emits pre-resolved tool results as stream parts", () =>
      Effect.gen(function* () {
        ...
        if (toolResultParts[0].type === "tool-result") {
          ...
          deepStrictEqual(toolResultParts[0].result, {
