  ...
  describe("reason types", () => {
    ...
    describe("AuthenticationError", () => {
      ...
      it("should format message based on kind", () => {
        const invalidKey = new AiError.AuthenticationError({
          kind: "InvalidKey",
        });
        assert.match(invalidKey.message, /InvalidKey/);

        const expiredKey = new AiError.AuthenticationError({
          kind: "ExpiredKey",
        });
        assert.match(expiredKey.message, /ExpiredKey/);

        const missingKey = new AiError.AuthenticationError({
          kind: "MissingKey",
        });
        assert.match(missingKey.message, /MissingKey/);

        const insufficientPermissions = new AiError.AuthenticationError({
        ...
        Effect.gen(function* () {
          ...
          if (result._tag === "Failure") {
            ...
            if ("error" in cause && Schema.isSchemaError(cause.error)) {
              const parseError = AiError.InvalidOutputError.fromSchemaError(
                cause.error
              );
    ...
    describe("InvalidToolResultError", () => {
      ...
      it("should format message with tool name", () => {
        ...
        assert.match(
          error.message,
          /Tool 'GetWeather' returned invalid result/
        );
      });
    ...
    describe("ToolResultEncodingError", () => {
      ...
      it("should format message with tool name", () => {
        ...
        assert.match(
          error.message,
          /Failed to encode result for tool 'GetWeather'/
        );
      });
    ...
    describe("ToolConfigurationError", () => {
      ...
      it("should format message with tool name", () => {
        ...
        assert.match(
          error.message,
          /Invalid configuration for tool 'OpenAiCodeInterpreter'/
        );
      });
      ...
      it("should format message with description", () => {
        ...
        assert.match(
          error.message,
          /search_context_size must be between 1 and 10/
        );
      });
      ...
      it.effect("should create from SchemaError", () =>
        Effect.gen(function* () {
          ...
          if (result._tag === "Failure") {
            const cause = result.cause;
            if ("error" in cause && Schema.isSchemaError(cause.error)) {
              const parseError = AiError.StructuredOutputError.fromSchemaError(
                cause.error,
                '{"name":123}'
              );
    ...
    describe("UnsupportedSchemaError", () => {
      ...
      it("should format message with description", () => {
        const error = new AiError.UnsupportedSchemaError({
          description:
            "Unions are not supported in Anthropic structured output",
        });
    ...
    describe("ProviderMetadata", () => {
      it.effect("should encode and decode roundtrip", () =>
        Effect.gen(function* () {
          ...
          const encoded = yield* Schema.encodeEffect(AiError.ProviderMetadata)(
            metadata
          );
          const decoded = yield* Schema.decodeEffect(AiError.ProviderMetadata)(
            encoded
          );
    ...
    describe("UsageInfo", () => {
      it.effect("should encode and decode roundtrip", () =>
        Effect.gen(function* () {
          ...
          const decoded = yield* Schema.decodeEffect(AiError.UsageInfo)(
            encoded
          );
    ...
    describe("HttpContext", () => {
      it.effect("should encode and decode roundtrip", () =>
        Effect.gen(function* () {
          ...
          const encoded = yield* Schema.encodeEffect(AiError.HttpContext)(
            context
          );
          const decoded = yield* Schema.decodeEffect(AiError.HttpContext)(
            encoded
          );
  ...
  describe("schema roundtrip", () => {
    it.effect("RateLimitError roundtrip", () =>
      Effect.gen(function* () {
        ...
        const encoded = yield* Schema.encodeEffect(AiError.RateLimitError)(
          error
        );
        const decoded = yield* Schema.decodeEffect(AiError.RateLimitError)(
          encoded
        );
    ...
    it.effect("AiErrorReason union roundtrip", () =>
      Effect.gen(function* () {
        const rateLimitError: AiError.AiErrorReason =
          new AiError.RateLimitError({});
        const encoded = yield* Schema.encodeEffect(AiError.AiErrorReason)(
          rateLimitError
        );
        const decoded = yield* Schema.decodeEffect(AiError.AiErrorReason)(
          encoded
        );
        assert.strictEqual(decoded._tag, "RateLimitError");

        const authError: AiError.AiErrorReason =
          new AiError.AuthenticationError({
            kind: "ExpiredKey",
          });
        const authEncoded = yield* Schema.encodeEffect(AiError.AiErrorReason)(
          authError
        );
        const authDecoded = yield* Schema.decodeEffect(AiError.AiErrorReason)(
          authEncoded
        );
    ...
    it.effect("ToolNotFoundError roundtrip", () =>
      Effect.gen(function* () {
        ...
        const encoded = yield* Schema.encodeEffect(AiError.ToolNotFoundError)(
          error
        );
        const decoded = yield* Schema.decodeEffect(AiError.ToolNotFoundError)(
          encoded
        );
        assert.strictEqual(decoded._tag, "ToolNotFoundError");
        assert.strictEqual(decoded.toolName, "UnknownTool");
        assert.deepStrictEqual(decoded.availableTools, [
          "GetWeather",
          "GetTime",
        ]);
      })
    ...
    it.effect("AiErrorReason union with tool errors roundtrip", () =>
      Effect.gen(function* () {
        const toolNotFound: AiError.AiErrorReason =
        ...
        const paramError: AiError.AiErrorReason =
          new AiError.ToolParameterValidationError({
            toolName: "Test",
            toolParams: {},
            description: "Error",
          });
        const paramEncoded = yield* Schema.encodeEffect(AiError.AiErrorReason)(
          paramError
        );
        const paramDecoded = yield* Schema.decodeEffect(AiError.AiErrorReason)(
          paramEncoded
        );
