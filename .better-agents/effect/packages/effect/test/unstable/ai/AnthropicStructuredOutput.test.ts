...
describe("toCodecAnthropic", () => {
  describe("Unsupported", () => {
    ...
    describe("Objects", () => {
      it("non-string property signature name", () => {
        assertError(
          Schema.Struct({
            [Symbol.for("effect/Schema/test/a")]: Schema.String,
          }),
  ...
  describe("String", () => {
    ...
    it("String + isMinLength + isMaxLength", () => {
      assertJsonSchema(
        Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(2)),
        {
          type: "string",
          description:
            "a value with a length of at least 1 and a value with a length of at most 2",
        }
      );
    });
    ...
    it("String + startsWith + endsWith", () => {
      assertJsonSchema(
        Schema.String.check(Schema.isStartsWith("a"), Schema.isEndsWith("b")),
        {
          type: "string",
    ...
    it("String + pattern + description", () => {
      assertJsonSchema(
        Schema.String.check(
          Schema.isPattern(/^a/, { description: "description" })
        ),
        {
          type: "string",
          description: "description",
  ...
  describe("Number", () => {
    ...
    describe("Finite", () => {
      it("Finite", () => {
        ...
        assertJsonSchema(
          Schema.Finite.annotate({ description: "description" }),
          {
            type: "number",
            description: "description",
          }
      ...
      it("Finite + isGreaterThan + isLessThan", () => {
        assertJsonSchema(
          Schema.Finite.check(Schema.isGreaterThan(1), Schema.isLessThan(2)),
          {
            type: "number",
  ...
  describe("Union", () => {
    it("oneOf", () => {
      assertJsonSchema(
        Schema.Union([Schema.String, Schema.Finite], { mode: "oneOf" }),
        {
  ...
  describe("Tuple", () => {
    ...
    it("Tuple([String, Finite]) + description", () => {
      const schema = Schema.Tuple([Schema.String, Schema.Finite]).annotate({
        description: "description",
      });
      assertJsonSchema(schema, {
        type: "object",
  ...
  describe("TupleWithRest", () => {
    it("TupleWithRest([String, Finite], Boolean)", async () => {
      ...
      await encoding.succeed(["a", 1, true, false], {
        "0": "a",
        "1": 1,
        __rest__: [true, false],
      });
      ...
      await decoding.succeed({ "0": "a", "1": 1, __rest__: [true] }, [
        "a",
        1,
        true,
      ]);
      await decoding.succeed({ "0": "a", "1": 1, __rest__: [true, false] }, [
        "a",
        1,
        true,
        false,
  ...
  describe("Record", () => {
    ...
    it("Record(String, Finite)", async () => {
      ...
      const encoding = asserts.encoding();
      await encoding.succeed({ a: 1, b: 2 }, [
        { 0: "a", 1: 1 },
        { 0: "b", 1: 2 },
      ]);
    ...
    it("Record(String, Finite) + description", () => {
      const schema = Schema.Record(Schema.String, Schema.Finite).annotate({
        description: "description",
      });
      assertJsonSchema(schema, {
        type: "array",
