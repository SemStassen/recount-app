...
const assertUniqueViolation = (
  reason: SqlError.SqlErrorReason,
  constraint: string
) => {
...
const reasonCases = [
  {
    tag: "ConnectionError",
    isRetryable: true,
    make: (args) => new SqlError.ConnectionError(args),
  },
  {
    tag: "AuthenticationError",
    isRetryable: false,
    make: (args) => new SqlError.AuthenticationError(args),
  },
  {
    tag: "AuthorizationError",
    isRetryable: false,
    make: (args) => new SqlError.AuthorizationError(args),
  },
  {
    tag: "SqlSyntaxError",
    isRetryable: false,
    make: (args) => new SqlError.SqlSyntaxError(args),
  },
  {
    tag: "UniqueViolation",
    isRetryable: false,
    make: (args) =>
      new SqlError.UniqueViolation({
        ...args,
  ...
  },
  {
    tag: "ConstraintError",
    isRetryable: false,
    make: (args) => new SqlError.ConstraintError(args),
  },
  {
    tag: "DeadlockError",
    isRetryable: true,
    make: (args) => new SqlError.DeadlockError(args),
  },
  {
    tag: "SerializationError",
    isRetryable: true,
    make: (args) => new SqlError.SerializationError(args),
  },
  {
    tag: "LockTimeoutError",
    isRetryable: true,
    make: (args) => new SqlError.LockTimeoutError(args),
  },
  {
    tag: "StatementTimeoutError",
    isRetryable: true,
    make: (args) => new SqlError.StatementTimeoutError(args),
  },
  {
    tag: "UnknownError",
    isRetryable: false,
    make: (args) => new SqlError.UnknownError(args),
  },
...
describe("SqlError", () => {
  ...
  it("delegates message, cause and retryability for every reason type", () => {
    for (const reasonCase of reasonCases) {
      ...
      const explicitMessageError = new SqlError.SqlError({
        reason: withMessage,
      });
      const emptyMessageError = new SqlError.SqlError({
        reason: withEmptyMessage,
      });
      ...
      assert.strictEqual(
        explicitMessageError.message,
        `${reasonCase.tag} custom`
      );
      assert.strictEqual(explicitMessageError.cause, withMessage);
      assert.strictEqual(
        explicitMessageError.isRetryable,
        reasonCase.isRetryable
      );
  ...
  it("isSqlError only matches the SqlError wrapper", () => {
    ...
    const mismatch = new SqlError.ResultLengthMismatch({
      expected: 1,
      actual: 0,
    });
  ...
  it("classifySqliteError maps sqlite unique constraint codes to UniqueViolation", () => {
    ...
    const byNumericCode = SqlError.classifySqliteError({
      code: 2067,
      constraint: uniqueViolationConstraint,
    });
    const byErrno = SqlError.classifySqliteError({
      errno: 2067,
      constraint: uniqueViolationConstraint,
    });
  ...
  it("classifySqliteError falls back to unknown for blank or missing sqlite unique identifiers", () => {
    ...
    const missing = SqlError.classifySqliteError({
      code: "SQLITE_CONSTRAINT_UNIQUE",
    });
    const nonString = SqlError.classifySqliteError({
      code: "SQLITE_CONSTRAINT_UNIQUE",
      constraint: 123,
    });
  ...
  it("classifySqliteError keeps generic sqlite constraints as ConstraintError", () => {
    ...
    const primaryKeyString = SqlError.classifySqliteError({
