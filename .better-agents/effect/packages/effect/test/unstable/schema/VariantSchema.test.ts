...
describe("VariantSchema", () => {
  it("FieldOnly and FieldExcept select fields from key arrays", () => {
    ...
    assert.deepStrictEqual(Object.keys(Test.extract(struct, "a").fields), [
      "common",
      "exceptC",
    ]);
    assert.deepStrictEqual(Object.keys(Test.extract(struct, "b").fields), [
      "common",
      "onlyB",
      "exceptC",
    ]);
    assert.deepStrictEqual(Object.keys(Test.extract(struct, "c").fields), [
      "common",
    ]);
  });
});
...
describe("Model", () => {
  it("FieldOnly includes fields only in listed variants", () => {
    ...
    assert.deepStrictEqual(
      Object.keys(Model.extract(InsertOnly, "insert").fields),
      ["value"]
    );
    assert.deepStrictEqual(
      Object.keys(Model.extract(InsertOnly, "select").fields),
      []
    );
  });
  ...
  it.effect(
    "Overrideable defaults are constructor-only and accept explicit overrides",
    () =>
      Effect.gen(function* () {
        const User = Model.Struct({
