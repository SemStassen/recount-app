...
const MockExecutorLayer = Layer.succeed(
  ...
  ChildProcessSpawner.make(
    Effect.fnUntraced(function* (command) {
      ...
      const executable =
        cmd._tag === "StandardCommand" ? cmd.command : "templated";
      ...
      return ChildProcessSpawner.makeHandle({
        ...
        stdin: Sink.forEach<Uint8Array, void, never, never>(
          (_chunk) => Effect.void
        ),
...
describe("ChildProcess", () => {
  ...
  describe("spawn", () => {
    ...
    it.effect(
      "should allow restoring the reference within acquireRelease",
      () =>
        Effect.gen(function* () {
          const handle = yield* ChildProcess.make`echo test`;
          // @effect-diagnostics-next-line floatingEffect:off
          yield* Effect.acquireRelease(handle.unref, (reref) =>
            Effect.ignore(reref)
          );
