...
describe("Stream", () => {
  ...
  describe("constructors", () => {
    ...
    it.effect("serviceOption - some", () =>
      Effect.gen(function* () {
        const result = yield* Stream.serviceOption(Greeter).pipe(
          Stream.map((option) =>
            Option.map(option, (greeter) => greeter.greet("World"))
          ),
          Stream.provideService(Greeter, {
            greet: (name) => `Hello, ${name}!`,
          }),
    ...
    it.effect("fromIterable - supports chunkSize option", () =>
      Effect.gen(function* () {
        const result = yield* Stream.fromIterable([1, 2, 3, 4, 5], {
          chunkSize: 2,
    ...
    it.effect("scoped - provides scope to fromEffect pull effects", () =>
      Effect.gen(function* () {
        ...
        ).pipe(Stream.scoped, Stream.runCollect);
    ...
    it.effect(
      "scoped - provides scope to sequential mapEffect pull effects",
      () =>
        Effect.gen(function* () {
          const releases = yield* Ref.make(0);
          const result = yield* Stream.fromIterable([1, 2]).pipe(
            Stream.mapEffect((n) =>
              Effect.acquireRelease(Effect.succeed(n * 2), () =>
  ...
  describe("encoding", () => {
    it.effect(
      "decodeText handles multi-byte characters split across chunks",
      () =>
        Effect.gen(function* () {
          const bytes = new TextEncoder().encode("🌍");
          const result = yield* Stream.make(
            bytes.slice(0, 2),
            bytes.slice(2)
    ...
    it.effect(
      "decodeText handles mixed ASCII and multi-byte characters across chunks",
      () =>
        Effect.gen(function* () {
    ...
    describe("splitLines", () => {
      const splitLines = (chunks: ReadonlyArray<string>) =>
        Stream.fromIterable(chunks).pipe(Stream.splitLines, Stream.runCollect);
      ...
      it.effect(
        "emits the final line when the stream does not end with a newline",
        () =>
          Effect.gen(function* () {
            const result = yield* splitLines(["a\nb\nc\n", "d\ne"]);
      ...
      it.effect(
        "does not pull upstream again after flushing the final line",
        () =>
          Effect.gen(function* () {
            let pulls = 0;
      ...
      it.effect(
        "handles standalone carriage returns across chunk boundaries",
        () =>
          Effect.gen(function* () {
      ...
      it.effect.prop(
        ...
        Effect.fnUntraced(function* ({ chunks }) {
          ...
          const expected = globalThis.Array.from(
            Str.linesIterator(chunks.join(""))
          );
      ...
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4).pipe(
          Stream.filterMapEffect((n) =>
            Effect.succeed(
              n % 2 === 0 ? Result.succeed(n + 1) : Result.failVoid
            )
          ),
          Stream.runCollect
        );
  ...
  describe("taking", () => {
    ...
    it.effect("takeWhileFilter - takes elements while a filter succeeds", () =>
      Effect.gen(function* () {
        const result = yield* Stream.range(1, 5).pipe(
          Stream.takeWhileFilter((n) =>
            n < 3 ? Result.succeed(n + 10) : Result.failVoid
          ),
          Stream.runCollect
        );
    ...
    it.effect(
      "takeUntilEffect - takes elements until an effectful predicate is satisfied",
      () =>
        Effect.gen(function* () {
          const result = yield* Stream.range(1, 5).pipe(
    ...
    it.effect(
      "takeWhileEffect - takes elements while an effectful predicate is satisfied",
      () =>
        Effect.gen(function* () {
          const result = yield* Stream.range(1, 5).pipe(
      ...
      Effect.gen(function* () {
        ...
        const { result1, result2 } = yield* Effect.all({
          ...
          result2: pipe(
            ...
            Effect.map(Array.takeRight(take))
          ),
        });
  ...
  describe("pagination", () => {
    it.effect("paginate", () =>
      Effect.gen(function* () {
        const s: readonly [ReadonlyArray<number>, Array<number>] = [
          [0],
          [1, 2, 3, 4, 5],
        ];
        const pageSize = 2;
        const result = yield* Stream.paginate(s, ([chunk, nums]) =>
          nums.length === 0
            ? Effect.succeed([
                chunk,
                Option.none<readonly [ReadonlyArray<number>, Array<number>]>(),
              ] as const)
            : Effect.succeed([
                chunk,
                Option.some([
  ...
  describe("error handling", () => {
    ...
    it.effect("catchIf with refinement", () =>
      Effect.gen(function* () {
        ...
        const stream: Stream.Stream<never, ErrorA | ErrorB> = Stream.fail({
          _tag: "ErrorB" as const,
        });
    ...
    it.effect("catchIf with refinement orElse", () =>
      Effect.gen(function* () {
        ...
        const result = yield* (
          Stream.fail({ _tag: "ErrorB" as const }) as Stream.Stream<
            never,
            ErrorA | ErrorB
          >
        ).pipe(
    ...
    it.effect("catchIf with filter input", () =>
      Effect.gen(function* () {
        const matched = yield* Stream.catchFilter(
          ...
          (e: string | number) =>
            typeof e === "string"
              ? Result.succeed(e.length)
              : Result.fail(`fallback: ${e}`),
          ...
          Stream.catchCauseFilter(
            (cause) =>
              Cause.hasFails(cause)
                ? Result.succeed(Cause.squash(cause))
                : Result.fail(cause),
    ...
    it.effect("catchTag orElse", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Stream.catchTag(
          Stream.fail(new ValidationError({ field: "email" })) as Stream.Stream<
            never,
        ...
        Effect.gen(function* () {
          ...
          const program =
            options === undefined
              ...
              : Stream.fail("boom").pipe(
                  Stream.ignore(options),
                  Stream.runDrain
                );
          yield* program.pipe(
    ...
    describe("catchReason", () => {
      ...
      it.effect("catches matching reason - handler succeeds", () =>
        Effect.gen(function* () {
          const result = yield* Stream.fail(
            ...
            Stream.catchReason("AiError", "RateLimitError", (r) =>
              Stream.succeed(`retry: ${r.retryAfter}`)
            ),
            Stream.runCollect
          );
      ...
      it.effect("catches matching reason - handler fails", () =>
        Effect.gen(function* () {
          ...
          const exit = yield* Stream.fail(new AiError({ reason })).pipe(
            Stream.catchReason("AiError", "RateLimitError", () =>
              Stream.fail(error)
            ),
            Stream.runCollect,
            Effect.exit
      ...
      it.effect("ignores non-matching parent tag", () =>
        Effect.gen(function* () {
          ...
          const exit = yield* (
            Stream.fail(error) as Stream.Stream<never, AiError | OtherError>
          ).pipe(
            Stream.catchReason("AiError", "RateLimitError", () =>
              Stream.succeed("no")
            ),
            ...
            Stream.catchReasons(
              "AiError",
              {
                QuotaExceededError: (reason) =>
                  Stream.succeed(`quota: ${reason.limit}`),
              },
      ...
      const runIgnoreCause = (
        ...
        Effect.gen(function* () {
          const stream: Stream.Stream<never, string, never> =
            Stream.fail("boom");
          const program: Effect.Effect<Array<never>, never, never> = options ===
          undefined
      ...
      Effect.gen(function* () {
        ...
        const { result1, result2 } = yield* Effect.all({
          ...
          result2: Stream.runCollect(stream).pipe(
            Effect.map((chunk) =>
              Array.scan(chunk, 0, (acc, curr) => acc + curr)
            )
          ),
        });
    ...
    it.effect("scanEffect", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          result,
          Array.scan([1, 2, 3, 4, 5], 0, (acc, curr) => acc + curr)
        );
      })
    );
  });
  ...
  describe("flattening", () => {
    it.effect("flatten supports dropping parens in pipe", () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.make(Stream.make(1, 2), Stream.make(3, 4)),
          Stream.flatten,
          Stream.runCollect
        );
        assert.deepStrictEqual(result, [1, 2, 3, 4]);
    ...
    it.effect("flattenEffect supports dropping parens in pipe", () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.make(Effect.succeed(1), Effect.succeed(2), Effect.succeed(3)),
  ...
  it.effect.prop(
    ...
    Effect.fnUntraced(function* ({ chunks, size }) {
      const actual = yield* Stream.fromArray(chunks).pipe(
        Stream.filter((chunk): chunk is NonEmptyArray<number> =>
          isReadonlyArrayNonEmpty(chunk)
        ),
        Stream.flattenArray,
        Stream.rechunk(size),
        Stream.chunks,
            ...
            Effect.gen(function* () {
              ...
              yield* pipe(
                ...
                Effect.flatMap((chunk) =>
                  Ref.update(ref, Array.appendAll(chunk))
                )
              );
  ...
  describe("bufferArray - sliding", () => {
    ...
    it.effect("fast producer progress independently", () =>
      Effect.gen(function* () {
        ...
        const expected2 = [9, 10, 11, 12, 13, 14, 15, 16];
        const expected3 = [
          9, 10, 11, 12, 13, 14, 15, 16, 18, 19, 20, 21, 22, 23, 24, -1,
        ];
        deepStrictEqual(result1, expected1);
        deepStrictEqual(result2, expected2);
        deepStrictEqual(result3, expected3);
  ...
  describe("aggregateWithin", () => {
    it.effect(
      "groupedWithin does not emit empty arrays when upstream is idle",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "groupedWithin flushes final partial batch on upstream end without waiting for the schedule",
      () =>
        Effect.gen(function* () {
          const result = yield* Stream.make(1, 2, 3, 4, 5).pipe(
    ...
    it.effect(
      "groupedWithin flushes partial batch when the schedule elapses while upstream is idle",
      () =>
        Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Stream.aggregateWithin(
            pipe(
              Sink.fold(
                () =>
                  [[] as Array<number>, true] as readonly [
                    Array<number>,
                    boolean,
                  ],
                (tuple) => tuple[1],
    ...
    it.effect("interruption propagation #2", () =>
      Effect.gen(function* () {
        ...
        const sink = Sink.fromEffect(
          Effect.never.pipe(Effect.onInterrupt(() => Ref.set(ref, true)))
        );
  ...
  describe("debounce", () => {
    ...
    it.effect("should work properly with parallelization", () =>
      Effect.gen(function* () {
        const coordination = yield* chunkCoordination([[1], [2], [3]]);
        const stream = pipe(
          coordination.stream,
          Stream.chunks,
          Stream.debounce(Duration.seconds(1))
        ...
        const fiber = yield* pipe(stream, Stream.runCollect, Effect.forkScoped);
        yield* Effect.all(
          [coordination.offer, coordination.offer, coordination.offer],
  ...
  describe("zipping", () => {
      ...
      it.effect("basic zipping with equal length streams", () =>
        Effect.gen(function* () {
          ...
          const result = yield* Stream.zipWithArray(
            ...
            (left, right) => {
              const minLength = Math.min(left.length, right.length);
              const output = Array.makeBy(
                minLength,
                (i: number) => [left[i], right[i]] as const
              );
        ...
        Effect.gen(function* () {
          ...
          const result = yield* Stream.zipWithArray(
            ...
            (leftArr, rightArr) => {
              ...
              const output = Array.makeBy(
                minLength,
                (i: number) => [leftArr[i], rightArr[i]] as const
              );
              return [
                output,
    ...
    describe("zip", () => {
      it.effect("zips two streams into tuples", () =>
        Effect.gen(function* () {
          const stream1 = Stream.make(1, 2, 3);
          const stream2 = Stream.make("a", "b", "c");
          const result = yield* Stream.zip(stream1, stream2).pipe(
            Stream.runCollect
          );
      ...
      it.effect("propagates errors from left stream", () =>
        Effect.gen(function* () {
          const stream1 = Stream.make(1, 2).pipe(
            Stream.concat(Stream.fail("boom"))
          );
          const stream2 = Stream.make("a", "b", "c");
          const result = yield* Stream.zip(stream1, stream2).pipe(
      ...
      it.effect("propagates errors from right stream", () =>
        Effect.gen(function* () {
          const stream1 = Stream.make(1, 2, 3);
          const stream2 = Stream.make("a", "b").pipe(
      ...
      it.effect("works with pipe syntax", () =>
        Effect.gen(function* () {
          const stream1 = Stream.make(1, 2, 3);
          const stream2 = Stream.make("a", "b", "c");
          const result = yield* stream1.pipe(
            Stream.zip(stream2),
            Stream.runCollect
          );
    ...
    describe("zipLeft", () => {
      it.effect("keeps only left values", () =>
        Effect.gen(function* () {
          const stream1 = Stream.make(1, 2, 3);
          const stream2 = Stream.make("a", "b", "c");
          const result = yield* Stream.zipLeft(stream1, stream2).pipe(
            Stream.runCollect
          );
    ...
    describe("zipRight", () => {
      it.effect("keeps only right values", () =>
        Effect.gen(function* () {
          const stream1 = Stream.make(1, 2, 3);
          const stream2 = Stream.make("a", "b", "c");
          const result = yield* Stream.zipRight(stream1, stream2).pipe(
            Stream.runCollect
          );
    ...
    describe("zipFlatten", () => {
      it.effect("flattens tuples when zipping", () =>
        Effect.gen(function* () {
          ...
          const result = yield* Stream.zipFlatten(stream1, stream2).pipe(
            Stream.runCollect
          );
      ...
      it.effect("propagates errors from right stream", () =>
        Effect.gen(function* () {
          ...
          const stream2 = Stream.make("x", "y").pipe(
            Stream.concat(Stream.fail("ouch"))
          );
          const result = yield* Stream.zipFlatten(stream1, stream2).pipe(
            Stream.runCollect,
    ...
    describe("zipWithIndex", () => {
      it.effect("zips stream with indices starting at 0", () =>
        Effect.gen(function* () {
          ...
          const result = yield* Stream.zipWithIndex(stream).pipe(
            Stream.runCollect
          );
    ...
    describe("zipLatest", () => {
      it.effect("combines streams with latest values", () =>
        Effect.gen(function* () {
          ...
          assert.isTrue(result.length > 0);
          assert.isTrue(
            result.every((item) => Array.isArray(item) && item.length === 2)
          );
        })
      );
    ...
    describe("zipLatestWith", () => {
      ...
      it.effect("works with pipe syntax", () =>
        Effect.gen(function* () {
          const result = yield* Stream.make(1, 2, 3).pipe(
            Stream.zipLatestWith(Stream.make(10, 20, 30), (n, m) => n + m),
            Stream.runCollect
          );
  ...
  describe("dropping", () => {
    ...
    it.effect("dropWhileFilter", () =>
      Effect.gen(function* () {
        const result = yield* Stream.make(1, 2, 3, 4, 5).pipe(
          Stream.dropWhileFilter((n) =>
            n < 3 ? Result.succeed(n) : Result.failVoid
          ),
          Stream.runCollect
        );
  ...
  describe("intersperse", () => {
    ...
    it.effect(
      "intersperse - several from repeat effect chunk single element (ZIO #3729)",
      () =>
        Effect.gen(function* () {
          const result = yield* pipe(
  ...
  describe("partition", () => {
    ...
    it.effect("partitionEffect - values", () =>
      Effect.gen(function* () {
        const { result1, result2 } = yield* pipe(
          ...
          Stream.partitionEffect((n) =>
            Effect.succeed(
              n % 2 === 0 ? Result.succeed(n + 10) : Result.fail(`odd:${n}`)
            )
          ),
    ...
    it.effect("partitionQueue - values", () =>
      Effect.gen(function* () {
        const { result1, result2 } = yield* pipe(
          Stream.range(0, 5),
          Stream.partitionQueue((n) =>
            n % 2 === 0 ? Result.succeed(n + 1) : Result.fail(`odd:${n}`)
          ),
          Effect.flatMap(([passes, fails]) =>
            Effect.all({
        ...
        const result = yield* pipe(
          Stream.fromEffect(
            pipe(
              ...
              Effect.flatMap((n) =>
                n <= 2 ? Effect.succeed(n) : Effect.fail("boom")
              )
            )
          ),
  ...
  describe("retry", () => {
    ...
    it.effect("retry - retries a failing stream according to a schedule", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          Ref.get(ref),
          Effect.map(Array.map((n) => new Date(n).getSeconds()))
        );
    ...
    it.effect("retry - Schedule.CurrentMetadata", () =>
      Effect.gen(function* () {
        const metadata = yield* Ref.make(
          Array.empty<undefined | Schedule.Metadata>()
        );
        const fiber = yield* pipe(
          Stream.fail(1),
          Stream.tapError(
  ...
  describe("sliding", () => {
    ...
    it.effect(
      "sliding - returns all elements if chunkSize is greater than the size of the stream",
      () =>
        Effect.gen(function* () {
          const result = yield* pipe(
            Stream.range(1, 5),
    ...
    it.effect(
      "sliding - is mostly equivalent to ZStream#grouped when stepSize and chunkSize are equal",
      () =>
        Effect.gen(function* () {
          const stream = Stream.range(1, 5);
          const { result1, result2 } = yield* Effect.all({
    ...
    it.effect(
      "sliding - should return an empty chunk when the stream is empty",
      () =>
        Effect.gen(function* () {
          const result = yield* pipe(
            Stream.empty,
    ...
    it.effect("sliding - emits elements properly when a failure occurs", () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(
          Array.empty<Array.NonEmptyReadonlyArray<number>>()
        );
  ...
  describe("split", () => {
    it.effect("split - should split properly", () =>
      Effect.gen(function* () {
        ...
        deepStrictEqual(result1, [[1, 2, 3], [5, 6, 7], [9]]);
        deepStrictEqual(result2, [[1, 2], [4, 5], [7, 8], [10]]);
      })
    );

    it.effect(
      "split - is equivalent to identity when the predicate is not satisfied",
      () =>
        Effect.gen(function* () {
          ...
          const { result1, result2 } = yield* Effect.all({
            ...
            result2: pipe(
              Stream.runCollect(stream),
              Effect.map((chunk) =>
                pipe(Array.of(chunk), Array.filter(Array.isArrayNonEmpty))
              )
            ),
          });
  ...
  describe("when", () => {
    ...
    it.effect(
      "when - returns an empty stream if the condition is not satisfied",
      () =>
        Effect.gen(function* () {
          const result = yield* pipe(
            Stream.make(1, 2, 3, 4, 5),
    ...
    it.effect(
      "when - returns the stream if the effectful condition is satisfied",
      () =>
        Effect.gen(function* () {
          const result = yield* pipe(
            Stream.make(1, 2, 3, 4, 5),
    ...
    it.effect(
      "when - returns an empty stream if the effectful condition is not satisfied",
      () =>
        Effect.gen(function* () {
          const result = yield* pipe(
            Stream.make(1, 2, 3, 4, 5),
  ...
  describe("zipWithNext", () => {
    ...
    it.effect(
      "zipWithPrevious - should zip with previous element for a single chunk",
      () =>
        Effect.gen(function* () {
          const result = yield* pipe(
            Stream.make(1, 2, 3),
      ...
      fc.assert(
        fc.asyncProperty(fc.array(fc.array(fc.integer())), async (chunks) => {
          ...
          const { result1, result2 } = await Effect.runPromise(
            Effect.all({
              result1: pipe(stream, Stream.zipWithPrevious, Stream.runCollect),
              result2: pipe(
