...
describe("String", () => {
  ...
  describe("noCase", () => {
    ...
    it("data-last with options", () => {
      strictEqual(
        pipe("helloWorld", S.noCase({ delimiter: "-" })),
        "hello-world"
      );
    });
  ...
  describe("ReducerConcat", () => {
    ...
    it("identity with initialValue", () => {
      strictEqual(
        S.ReducerConcat.combine("a", S.ReducerConcat.initialValue),
        "a"
      );
      strictEqual(
        S.ReducerConcat.combine(S.ReducerConcat.initialValue, "a"),
        "a"
      );
    });
  });
