...
describe("Channel", () => {
  describe("constructors", () => {
    it.effect("empty", () =>
      Effect.gen(function* () {
        const result = yield* Channel.empty.pipe(Channel.runCollect);
    ...
    it.effect("sync", () =>
      Effect.gen(function* () {
        const result = yield* Channel.sync(() => 1).pipe(Channel.runCollect);
        assert.deepStrictEqual(result, [1]);
      })
    );
    ...
    it.effect("fromIterableArray", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Channel.runCollect(
          Channel.fromIterableArray(numbers)
        );
        const resultChunked = yield* Channel.runCollect(
          Channel.fromIterableArray(numbers, 4)
        );
    ...
    it.effect("acquireRelease", () =>
      Effect.gen(function* () {
        ...
        yield* Channel.acquireRelease(Ref.set(acquired, true), () =>
  ...
  describe("mapping", () => {
    ...
    it.effect(
      "mapEffect interrupts the running effect when the channel is interrupted",
      () =>
        Effect.gen(function* () {
        ...
        const result = yield* Channel.fromArray([1, 2, 3]).pipe(
          Channel.mapEffect(
            (n) => {
              ...
              return Effect.fail("boom").pipe(latch1.whenOpen, latch2.whenOpen);
            },
            { concurrency: 3 }
          ),
          Channel.runDrain,
  ...
  describe("filtering", () => {
    it.effect("filterMap with Filter", () =>
      Effect.gen(function* () {
        const filter = Filter.make((n: number) =>
          n % 2 === 0 ? Result.succeed(n * 2) : Result.fail(n)
        );
  ...
  describe("encoding", () => {
    it.effect(
      "decodeText handles multi-byte characters split across Uint8Array boundaries",
      () =>
        Effect.gen(function* () {
          const bytes = new TextEncoder().encode("a🌍b");
          const chunks: ReadonlyArray<
            readonly [Uint8Array, ...ReadonlyArray<Uint8Array>]
          > = [[bytes.slice(0, 2)], [bytes.slice(2, 4), bytes.slice(4)]];
  ...
  describe("merging", () => {
    it.effect(
      "merge - interrupts left side if halt strategy is set to 'right'",
      () =>
        Effect.gen(function* () {
          const latch = yield* Latch.make(false);
    ...
    it.effect(
      "merge - interrupts right side if halt strategy is set to 'left'",
      () =>
        Effect.gen(function* () {
          const latch = yield* Latch.make(false);
          const leftQueue = yield* Queue.make<number, Cause.Done>();
    ...
    it.effect(
      "merge - interrupts losing side if halt strategy is set to 'either'",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "merge - waits for both sides if halt strategy is set to 'both'",
      () =>
        Effect.gen(function* () {
  ...
  describe("conditional catch", () => {
    ...
    it.effect("catchIf with refinement", () =>
      Effect.gen(function* () {
        const exit = yield* (
          ...
          ) as Channel.Channel<never, HttpError | ValidationError, never>
        ).pipe(
          Channel.catchIf(
    ...
    it.effect("catchFilter with Filter", () =>
      Effect.gen(function* () {
        const filter = Filter.make((error: string) =>
          error === "boom" ? Result.succeed(error) : Result.fail(error)
        );
        const result = yield* Channel.fail("boom").pipe(
          Channel.catchFilter(filter, (error) =>
            Channel.succeed(`recovered: ${error}`)
          ),
          Channel.runCollect
        );
        assert.deepStrictEqual(result, ["recovered: boom"]);
    ...
    it.effect("catchCauseFilter with Filter", () =>
      Effect.gen(function* () {
        const result = yield* Channel.fail("boom").pipe(
          Channel.catchCauseFilter(Cause.findError as any, (error) =>
            Channel.succeed(`recovered: ${error}`)
          ),
          Channel.runCollect
        );
    ...
    it.effect("catchReason ignores non-matching parent tag", () =>
      Effect.gen(function* () {
        ...
        const exit = yield* (
          Channel.fail(error) as Channel.Channel<
            never,
            AiError | OtherError,
            never
          >
          ...
          Channel.catchReasons(
            "AiError",
            {
              QuotaExceededError: (reason) =>
                Channel.succeed(`quota: ${reason.limit}`),
            },
      ...
      Effect.gen(function* () {
        ...
        const exit = yield* (
          Channel.fail(error) as Channel.Channel<
            ...
            never
          >
        ).pipe(Channel.unwrapReason("AiError"), Channel.runDrain, Effect.exit);
        assertExitFailure(exit, Cause.fail(error));
      })
    );
  });
