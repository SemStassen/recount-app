...
describe("Cron", () => {
  ...
  it("CronParseError constructor", () => {
    const error = new Cron.CronParseError({
      message: "boom",
      input: "0 0 * * *",
    });
  ...
  it("parse", () => {
    ...
    const withNamedTimeZone = Cron.parseUnsafe(
      "23 0-20/2 * * *",
      "Europe/Berlin"
    );
    assertTrue(Option.isSome(withNamedTimeZone.tz));
    deepStrictEqual(
      DateTime.zoneToString(withNamedTimeZone.tz.value),
      "Europe/Berlin"
    );
    ...
    deepStrictEqual(
      DateTime.zoneToString(withOffsetTimeZone.tz.value),
      "+02:00"
    );
  ...
  it("parseUnsafe", () => {
    throws(
      ...
      (error) => {
        ...
        deepStrictEqual(
          error.message,
          "Invalid number of segments in cron expression"
        );
  ...
  it("next", () => {
    ...
    deepStrictEqual(
      next(Cron.parseUnsafe("5 0 8 2 *", london), after),
  ...
  it("prev", () => {
    ...
    deepStrictEqual(
      prev(Cron.parseUnsafe("5 0 8 2 *", utc), before),
      new Date("2023-02-08T00:05:00.000Z")
    );
    deepStrictEqual(
      prev(Cron.parseUnsafe("15 14 1 * *", utc), before),
      new Date("2024-01-01T14:15:00.000Z")
    );
    deepStrictEqual(
      prev(Cron.parseUnsafe("23 0-20/2 * * * 0", utc), before),
      new Date("2023-12-31T23:20:23.000Z")
    );
    deepStrictEqual(
      prev(Cron.parseUnsafe("5 4 * * SUN", utc), before),
      new Date("2023-12-31T04:05:00.000Z")
    );
    deepStrictEqual(
      prev(Cron.parseUnsafe("5 4 * DEC SUN", utc), before),
      new Date("2023-12-31T04:05:00.000Z")
    );
    deepStrictEqual(
      prev(Cron.parseUnsafe("30 5 0 8 2 *", utc), before),
      new Date("2023-02-08T00:05:30.000Z")
    );

    const wednesday = new Date("2025-10-22T01:00:00.000Z");
    deepStrictEqual(
      prev(Cron.parseUnsafe("0 1 * * MON", utc), wednesday),
      new Date("2025-10-20T01:00:00.000Z")
    );
    deepStrictEqual(
      next(Cron.parseUnsafe("0 1 * * MON", utc), wednesday),
      new Date("2025-10-27T01:00:00.000Z")
    );
    deepStrictEqual(
      prev(Cron.parseUnsafe("0 1 * * TUE", utc), wednesday),
      new Date("2025-10-21T01:00:00.000Z")
    );
    deepStrictEqual(
      next(Cron.parseUnsafe("0 1 * * TUE", utc), wednesday),
  ...
  it("forward and reverse sequences stay aligned", () => {
    ...
    for (const [expr, lowerStr, upperStr] of cases) {
      ...
      deepStrictEqual(
        gatherForward(cron, lower, upper),
        gatherReverse(cron, lower, upper)
      );
    }
  });
  ...
  it("prev chooses the preferred occurrence in DST fall-back", () => {
    const make = (s: string): DateTime.Zoned =>
      Option.getOrThrow(DateTime.makeZonedFromString(s));
    const cron = Cron.parseUnsafe("0 30 2 * * *", "Europe/Berlin");
    const before = make("2024-10-27T03:30:00.000+01:00[Europe/Berlin]");
    const result = DateTime.makeZonedUnsafe(prev(cron, before), {
  ...
  it("handles transition into daylight savings time", () => {
    const make = (date: string): DateTime.Zoned =>
      Option.getOrThrow(DateTime.makeZonedFromString(date));
    ...
    const next = (): DateTime.Zoned =>
      DateTime.makeZonedUnsafe(sequence.next().value, {
        timeZone: "Europe/Berlin",
      });
  ...
  it("handles utc timezone", () => {
    ...
    const make = (date: string): DateTime.Zoned =>
      Option.getOrThrow(DateTime.makeZonedFromString(date));
    const sequence = Cron.sequence(
      Cron.parseUnsafe("30 * * * *", utc),
      make("2024-10-27T00:00:00.000+00:00[UTC]")
    );
    const next = (): DateTime.Zoned =>
      DateTime.makeZonedUnsafe(sequence.next().value, { timeZone: utc });
