...
describe("Pool", () => {
  ...
  it.effect("reports failures via get", () =>
    Effect.gen(function* () {
      ...
      const values = yield* Effect.all(
        Effect.replicate(9)(Effect.flip(Pool.get(pool)))
      );
  ...
  it.effect(
    "invalidate all items in pool and check that pool.get doesn't hang forever",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("compositional retry", () =>
    Effect.gen(function* () {
      const cond = (i: number) =>
        i <= 10 ? Effect.fail(i) : Effect.succeed(i);
      const count = yield* Ref.make(0);
      const get = Effect.acquireRelease(
        Ref.updateAndGet(count, (n) => n + 1).pipe(Effect.flatMap(cond)),
        () => Ref.update(count, (n) => n - 1)
      );
      const pool = yield* Pool.make({ acquire: get, size: 10 });
  ...
  it.effect("max pool size", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Effect.scoped(Effect.andThen(Pool.get(pool), Deferred.await(deferred))),
        Effect.forkChild,
  ...
  it.effect("shutdown robustness", () =>
    Effect.gen(function* () {
      ...
      yield* Scope.close(scope, Exit.succeed(void 0));
      const result = yield* Effect.repeat(Ref.get(count), {
        until: (n) => n === 0,
      });
      strictEqual(result, 0);
    })
  ...
  it.effect("finalizer is called for failed allocations", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(Pool.get(pool)).pipe(Effect.ignore);
