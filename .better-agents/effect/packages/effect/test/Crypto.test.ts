...
const testCrypto = Crypto.make({
  randomBytes: (size) =>
    size === 7
      ? Uint8Array.of(0x18, 0, 0, 0, 0, 0, 0)
      : Uint8Array.from({ length: size }, (_, i) => i),
  digest: (algorithm, data) =>
    Effect.succeed(Uint8Array.of(data.length, algorithm.length)),
});
...
describe("Crypto", () => {
  ...
  it.effect("random generators delegate to the service", () =>
    Effect.gen(function* () {
      ...
      const randomBetweenDecimalBounds = yield* crypto.randomBetween(
        10.5,
        20.5
      );
  ...
  it.effect(
    "randomIntBetween excludes the upper bound in half-open ranges",
    () =>
      Effect.gen(function* () {
        const crypto = yield* Crypto.Crypto;
