...
describe("Tuple", () => {
  ...
  describe("renameIndices", () => {
    it("partial index mapping", () => {
      deepStrictEqual(pipe(tuple, Tuple.renameIndices(["1", "0"])), [
        2,
        "a",
        true,
      ]);
    ...
    it("full index mapping", () => {
      deepStrictEqual(pipe(tuple, Tuple.renameIndices(["2", "1", "0"])), [
        true,
        2,
        "a",
      ]);
      deepStrictEqual(Tuple.renameIndices(tuple, ["2", "1", "0"]), [
        true,
        2,
        "a",
      ]);
  ...
  it("map transforms every schema element with a lambda", () => {
    ...
    TestSchema.Asserts.ast.elements.equals(
      pipe(tuple, Tuple.map(Schema.NullOr)),
      [
  ...
  it("mapPick transforms only the selected schema elements", () => {
    const tuple = [Schema.String, Schema.Number, Schema.Boolean] as const;
    TestSchema.Asserts.ast.elements.equals(
      pipe(tuple, Tuple.mapPick([0, 2], Schema.NullOr)),
      [
        Schema.NullOr(Schema.String),
        Schema.Number,
        Schema.NullOr(Schema.Boolean),
    ...
    TestSchema.Asserts.ast.elements.equals(
      Tuple.mapPick(tuple, [0, 2], Schema.NullOr),
      [
        Schema.NullOr(Schema.String),
        Schema.Number,
        Schema.NullOr(Schema.Boolean),
  ...
  it("mapOmit transforms every schema element except the omitted indices", () => {
    const tuple = [Schema.String, Schema.Number, Schema.Boolean] as const;
    TestSchema.Asserts.ast.elements.equals(
      pipe(tuple, Tuple.mapOmit([1], Schema.NullOr)),
      [
        Schema.NullOr(Schema.String),
        Schema.Number,
        Schema.NullOr(Schema.Boolean),
    ...
    TestSchema.Asserts.ast.elements.equals(
      Tuple.mapOmit(tuple, [1], Schema.NullOr),
      [
        Schema.NullOr(Schema.String),
        Schema.Number,
        Schema.NullOr(Schema.Boolean),
