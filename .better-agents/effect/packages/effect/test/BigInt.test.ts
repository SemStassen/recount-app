...
describe("BigInt", () => {
  ...
  it("fromNumber returns none for unsafe or non-integral numbers", () => {
    ...
    strictEqual(
      BigInt.ReducerSum.combine(BigInt.ReducerSum.initialValue, 2n),
      2n
    );
    strictEqual(
      BigInt.ReducerSum.combine(2n, BigInt.ReducerSum.initialValue),
      2n
    );
  });
  ...
  it("ReducerMultiply combines values with one as the identity", () => {
    ...
    strictEqual(
      BigInt.ReducerMultiply.combine(BigInt.ReducerMultiply.initialValue, 2n),
      2n
    );
    strictEqual(
      BigInt.ReducerMultiply.combine(2n, BigInt.ReducerMultiply.initialValue),
      2n
    );
  });
