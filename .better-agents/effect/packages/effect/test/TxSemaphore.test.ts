...
describe("TxSemaphore", () => {
    ...
    it.effect("withPermit automatically manages permit lifecycle", () =>
      Effect.gen(function* () {
        ...
        const result = yield* TxSemaphore.withPermit(
          semaphore,
          Effect.gen(function* () {
            const available = yield* Effect.tx(
              TxSemaphore.available(semaphore)
            );
        ...
        const finalAvailable = yield* Effect.tx(
          TxSemaphore.available(semaphore)
        );
      ...
      Effect.tx(
        Effect.gen(function* () {
          ...
          const result = yield* Effect.exit(
            TxSemaphore.acquireN(semaphore, -1)
          );
          assert.isTrue(Exit.hasDies(result));
        })
      )
    ...
    it.effect("releaseN with negative permits causes defect", () =>
      Effect.tx(
        Effect.gen(function* () {
          const semaphore = yield* TxSemaphore.make(5);
          const result = yield* Effect.exit(
            TxSemaphore.releaseN(semaphore, -1)
          );
          assert.isTrue(Exit.hasDies(result));
        })
      )
  ...
  describe("concurrency", () => {
    it.effect(
      "withPermit bounds concurrent work and releases for the next waiter",
      () =>
        Effect.gen(function* () {
          const semaphore = yield* Effect.tx(TxSemaphore.make(3));
