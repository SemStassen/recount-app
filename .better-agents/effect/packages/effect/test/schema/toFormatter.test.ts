...
describe("toFormatter", () => {
  ...
  describe("Tuple", () => {
    ...
    it("elements", () => {
      const format = Schema.toFormatter(
        Schema.Tuple([Schema.Option(Schema.String)])
      );
  ...
  it("Array", () => {
    const format = Schema.toFormatter(
      Schema.Array(Schema.Option(Schema.String))
    );
    strictEqual(format([Option.some("a")]), `[some("a")]`);
    strictEqual(format([Option.none()]), `[none()]`);
  });
  ...
  it("TupleWithRest", () => {
    ...
    strictEqual(
      format([Option.some(true), Option.some(1), Option.some("a")]),
      `[some(true), some(1), some("a")]`
    );
    strictEqual(
      format([Option.none(), Option.none(), Option.some("a")]),
      `[none(), none(), some("a")]`
    );
  });
  ...
  describe("Struct", () => {
    ...
    it("required field with undefined", () => {
      ...
      strictEqual(format({ a: Option.some("a") }), `{ "a": some("a") }`);
      strictEqual(
        format({ a: Option.some(undefined) }),
        `{ "a": some(undefined) }`
      );
      strictEqual(format({ a: Option.none() }), `{ "a": none() }`);
    });
  ...
  describe("Record", () => {
    it("Record(String, Option(Number))", () => {
      const format = Schema.toFormatter(
        Schema.Record(Schema.String, Schema.Option(Schema.Number))
      );
    ...
    it("Record(Symbol, Option(Number))", () => {
      const format = Schema.toFormatter(
        Schema.Record(Schema.Symbol, Schema.Option(Schema.Number))
      );
      strictEqual(
        format({ [Symbol.for("a")]: Option.some(1) }),
        `{ Symbol(a): some(1) }`
      );
      strictEqual(
        format({ [Symbol.for("a")]: Option.none() }),
        `{ Symbol(a): none() }`
      );
    });
  });
  ...
  describe("suspend", () => {
    ...
    it("ReadonlySet", () => {
      ...
      strictEqual(
        format(new Set([new Set([new Set()])])),
        `ReadonlySet(1) { ReadonlySet(1) { ReadonlySet(0) {} } }`
      );
    });
  ...
  it("URL", () => {
    ...
    strictEqual(
      format(new URL("https://www.example.com")),
  ...
  it("ReadonlyMap(String, Option(Number))", () => {
    const format = Schema.toFormatter(
      Schema.ReadonlyMap(Schema.String, Schema.Option(Schema.Number))
    );
    strictEqual(
      format(new Map([["a", Option.some(1)]])),
      `ReadonlyMap(1) { "a" => some(1) }`
    );
    strictEqual(
      format(new Map([["a", Option.none()]])),
      `ReadonlyMap(1) { "a" => none() }`
    );
  });

  it("HashMap(String, Option(Number))", () => {
    const format = Schema.toFormatter(
      Schema.HashMap(Schema.String, Schema.Option(Schema.Number))
    );
    strictEqual(
      format(HashMap.make(["a", Option.some(1)])),
      `HashMap(1) { "a" => some(1) }`
    );
    strictEqual(
      format(HashMap.make(["a", Option.none()])),
      `HashMap(1) { "a" => none() }`
    );
  });
  ...
  describe("Redacted", () => {
    ...
    it("with label", () => {
      ...
      strictEqual(
        format(Redacted.make("a", { label: "password" })),
        `<redacted:password>`
      );
    });
  });
  ...
  it("DateTimeUtc", () => {
    ...
    strictEqual(
      format(DateTime.makeUnsafe("2021-01-01T00:00:00.000Z")),
      "DateTime.Utc(2021-01-01T00:00:00.000Z)"
    );
  });
  ...
  it("TimeZoneNamed", () => {
    ...
    strictEqual(
      format(DateTime.zoneMakeNamedUnsafe("Europe/London")),
      "Europe/London"
    );
  });
  ...
  it("DateTimeZoned", () => {
    ...
    const zoned = DateTime.makeZonedUnsafe("2024-01-01T00:00:00.000Z", {
