...
class Value extends Schema.Class<Value, { readonly brand: unique symbol }>(
  "Value"
)({
  a: Schema.DateValid,
}) {}
...
describe("Optic generation", () => {
  it("overrideToCodecIso", () => {
    ...
    deepStrictEqual(
      modify(new URL("https://example.com")),
  ...
  describe("toIso", () => {
    describe("Class", () => {
      it("Class", () => {
        ...
        deepStrictEqual(
          modify(
            B.make({ a: A.make({ value: Value.make({ a: new Date(0) }) }) })
          ),
    ...
    describe("brand", () => {
      it("Number & isPositive", () => {
        const schema = Schema.Number.check(Schema.isGreaterThan(0)).pipe(
          Schema.brand("isPositive")
        );
        const optic = Schema.toIso(schema);
    ...
    it("Tuple", () => {
      ...
      deepStrictEqual(modify([Value.make({ a: new Date(0) })]), [
        Value.make({ a: new Date(1) }),
      ]);
    });
    ...
    it("NonEmptyArray", () => {
      ...
      const modify = optic.modify(([a, ...rest]) => [
        item.modify(addOne)(a),
        ...rest.map(item.modify(addTwo)),
      ]);
    ...
    it("Record", () => {
      ...
      const optic = Schema.toIso(schema);
      const item = Schema.toIsoFocus(Value).key("a");
      const modify = optic.modify((rec) =>
        Record.map(rec, item.modify(addOne))
      );
    ...
    it("StructWithRest", () => {
      const schema = Schema.StructWithRest(Schema.Struct({ a: Value }), [
    ...
    it("Union", () => {
      ...
      const optic = Schema.toIso(schema);
      const item = Schema.toIsoFocus(Value).key("a");
      const modify = optic.modify((x) =>
        Predicate.isString(x) ? x : item.modify(addOne)(x)
      );
    ...
    it("suspend", () => {
      ...
      deepStrictEqual(
        modify({
          a: Value.make({ a: new Date(0) }),
          as: [{ a: Value.make({ a: new Date(1) }), as: [] }],
        }),
        {
          a: Value.make({ a: new Date(1) }),
    ...
    it("Cause", () => {
      ...
      const optic = Schema.toIso(schema);
      const failure = Schema.toIsoFocus(Schema.CauseReason(Value, Value))
        .tag("Fail")
        .key("error")
        .key("a");
      const modify = optic.modify((failures) =>
        failures.map(failure.modify(addOne))
      );
    ...
    it("ReadonlyMap", () => {
      ...
      const entry = Schema.toIsoFocus(Schema.Tuple([Schema.String, Value]))
        .key("1")
        .key("a");
      const modify = optic.modify((entries) =>
        entries.map(([key, value]) => entry.modify(addOne)([key, value]))
      );
    ...
    it("HashMap", () => {
      ...
      deepStrictEqual(
        HashMap.toEntries(
          modify(HashMap.make(["a", Value.make({ a: new Date(0) })]))
        ),
