...
const OuterCategory = Schema.Struct({
  ...
  children: Schema.Array(
    Schema.suspend((): Schema.Codec<Category> => OuterCategory)
  ),
...
const InnerCategory = Schema.Struct({
  ...
  children: Schema.Array(
    Schema.suspend(
      (): Schema.Codec<Category> =>
        InnerCategory.annotate({ identifier: "Category" })
    )
  ),
});
...
describe("toCodeDocument", () => {
  ...
  function assertSchema(
    input: {
      ...
      readonly reviver?:
        | SchemaRepresentation.Reviver<SchemaRepresentation.Code>
        | undefined;
    },
    expected: Expected
  ) {
  ...
  describe("String", () => {
      ...
      assertSchema(
        {
          schema: Schema.String.check(
            Schema.isMinLength(1, { description: "a" })
          ),
        },
        {
          codes: makeCode(
    ...
    it("String & check & annotations", () => {
      assertSchema(
        {
          schema: Schema.String.check(Schema.isMinLength(1)).annotate({
            description: "a",
          }),
        },
        {
    ...
    describe("checks", () => {
      it("isStartsWith", () => {
        assertSchema(
          ...
          {
            codes: makeCode(
              `Schema.String.check(Schema.isStartsWith("a"))`,
              "string"
            ),
          }
        );
      ...
      it("isEndsWith", () => {
        assertSchema(
          ...
          {
            codes: makeCode(
              `Schema.String.check(Schema.isEndsWith("a"))`,
              "string"
            ),
          }
        );
      ...
      it("isIncludes", () => {
        assertSchema(
          ...
          {
            codes: makeCode(
              `Schema.String.check(Schema.isIncludes("a"))`,
              "string"
            ),
          }
        );
  ...
  describe("UniqueSymbol", () => {
    ...
    it("should create a global Symbol artifact", () => {
      ...
      assertSchema(
        {
          schema: Schema.UniqueSymbol(Symbol.for("a")).annotate({
            description: "a",
          }),
        },
        {
  ...
  describe("Tuple", () => {
    ...
    it("optional element", () => {
      ...
      assertSchema(
        {
          schema: Schema.Tuple([Schema.optionalKey(Schema.String)]).annotate({
            description: "a",
          }),
        },
        {
    ...
    it("annotateKey", () => {
      assertSchema(
        {
          schema: Schema.Tuple([
            Schema.String.annotateKey({ description: "a" }),
          ]),
        },
        {
          codes: makeCode(
  ...
  it("Record(String, Number)", () => {
    ...
    assertSchema(
      {
        schema: Schema.Record(Schema.String, Schema.Number).annotate({
          description: "a",
        }),
      },
      {
  ...
  describe("Union", () => {
    it("union with anyOf mode (default)", () => {
      ...
      assertSchema(
        {
          schema: Schema.Union([Schema.String, Schema.Number]).annotate({
            description: "z",
          }),
        },
        {
  ...
  describe("brand", () => {
    ...
    it("check & brand", () => {
      assertSchema(
        {
          schema: Schema.String.check(Schema.isMinLength(1)).pipe(
            Schema.brand("b")
          ),
        },
        {
    ...
    describe("checks", () => {
      ...
      it("isLessThanOrEqualToDate", () => {
        assertSchema(
          {
            schema: Schema.Date.check(
              Schema.isLessThanOrEqualToDate(new Date(0))
            ),
          },
          {
            codes: makeCode(
  ...
  describe("ReadonlySet", () => {
    ...
    describe("checks", () => {
      it("isMinSize", () => {
        assertSchema(
          {
            schema: Schema.ReadonlySet(Schema.String).check(
              Schema.isMinSize(2)
            ),
          },
          {
...
describe("sanitizeJavaScriptIdentifier", () => {
  const sanitizeJavaScriptIdentifier =
    SchemaRepresentation.sanitizeJavaScriptIdentifier;
...
describe("topologicalSort", () => {
  function assertTopologicalSort(
    ...
    deepStrictEqual(
      SchemaRepresentation.topologicalSort(definitions),
      expected
    );
  }
