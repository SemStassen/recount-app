...
describe("toSchema", () => {
  function assertToSchemaRoundtrip(
    ...
    const codeDocument = SchemaRepresentation.toCodeDocument(
      SchemaRepresentation.toMultiDocument(roundtrip)
    );
  ...
  describe("toSchemaDefaultReviver", () => {
    function assertToSchemaWithReviver(schema: Schema.Top, runtime: string) {
      assertToSchemaRoundtrip(
        { schema, reviver: SchemaRepresentation.toSchemaDefaultReviver },
        runtime
      );
    }
    ...
    it("Json", () => {
      assertToSchemaWithReviver(Schema.Json, `Schema.Json`);
    });

    it("MutableJson", () => {
      assertToSchemaWithReviver(Schema.MutableJson, `Schema.MutableJson`);
    });
    ...
    it("ErrorWithStack", () => {
      assertToSchemaWithReviver(Schema.ErrorWithStack, `Schema.ErrorWithStack`);
    });
    ...
    it("BigDecimal", () => {
      assertToSchemaWithReviver(Schema.BigDecimal, `Schema.BigDecimal`);
    });

    it("TimeZoneOffset", () => {
      assertToSchemaWithReviver(Schema.TimeZoneOffset, `Schema.TimeZoneOffset`);
    });

    it("TimeZoneNamed", () => {
      assertToSchemaWithReviver(Schema.TimeZoneNamed, `Schema.TimeZoneNamed`);
    });

    it("TimeZone", () => {
      assertToSchemaWithReviver(Schema.TimeZone, `Schema.TimeZone`);
    });

    it("DateTimeZoned", () => {
      assertToSchemaWithReviver(Schema.DateTimeZoned, `Schema.DateTimeZoned`);
    });
