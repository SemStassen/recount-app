...
describe("Schema", () => {
  ...
  it("toString", () => {
    ...
    strictEqual(
      String(result.cause.reasons[0]),
      "Fail(SchemaError(Expected string, got null))"
    );
  });
...
Expected an integer, got -1.2`
  ...
  at [Symbol(sym)]`
        ...
        const decodingAll = asserts.decoding({
...
Unexpected key with value "c"
  at ["c"]`
      ...
      it("preserve", async () => {
        ...
        const sym = Symbol("sym");
        await decoding.succeed({ a: "a", b: "b", c: "c", [sym]: "sym" });
      });
    });
...
Missing key
  ...
  at ["b"]`
      ...
      it("Struct & check & unsafePreserveChecks: true", async () => {
        ...
        const schema = from.mapFields(Struct.assign({ c: Schema.String }), {
...
Expected a string including "c", got "ab"`
    ...
    it("refine", async () => {
      ...
      await decoding.fail(Option.some(""), `Expected length > 0, got some("")`);
      await decoding.fail(Option.none(), `Expected isSome, got none()`);
    });
    ...
    describe("String checks", () => {
      ...
      it("isEndsWith", async () => {
        ...
        const decoding = asserts.decoding();
        await decoding.succeed("a");
        await decoding.fail("b", `Expected a string ending with "a", got "b"`);

        const encoding = asserts.encoding();
        await encoding.succeed("a");
        await encoding.fail("b", `Expected a string ending with "a", got "b"`);
      });
    ...
    describe("Number checks", () => {
      it("isGreaterThan", async () => {
        ...
        await decoding.fail(1, `Expected a value greater than 1, got 1`);

        const encoding = asserts.encoding();
        await encoding.succeed(2);
        await encoding.fail(1, `Expected a value greater than 1, got 1`);
      });
      ...
      it("isLessThan", async () => {
        ...
        await decoding.fail(1, `Expected a value less than 1, got 1`);
      });
      ...
      describe("isBetween", () => {
        it("included & included", async () => {
          ...
          await decoding.fail(0, `Expected a value between 1 and 3, got 0`);
          await decoding.fail(4, `Expected a value between 1 and 3, got 4`);

          const encoding = asserts.encoding();
          await encoding.succeed(1);
          await encoding.succeed(3);
          await encoding.fail(0, `Expected a value between 1 and 3, got 0`);
        });

        it("included & excluded", async () => {
          const schema = Schema.Int.check(
            Schema.isBetween({ minimum: 1, maximum: 3, exclusiveMaximum: true })
          );
          const asserts = new TestSchema.Asserts(schema);
          ...
          await decoding.fail(
            3,
            `Expected a value between 1 and 3 (excluded), got 3`
          );
          await decoding.fail(
            0,
        ...
        it("excluded & included", async () => {
          ...
          await decoding.fail(
            1,
            `Expected a value between 1 (excluded) and 3, got 1`
          );
          await decoding.succeed(3);
          await decoding.fail(
            0,
        ...
        it("excluded & excluded", async () => {
          ...
          await decoding.fail(
            1,
            `Expected a value between 1 (excluded) and 3 (excluded), got 1`
          );
          await decoding.fail(
            3,
            `Expected a value between 1 (excluded) and 3 (excluded), got 3`
          );
          await decoding.fail(
            0,
      ...
      it("isInt", async () => {
        ...
        const decoding = asserts.decoding();
        await decoding.succeed(1);
        await decoding.fail(1.1, `Expected an integer, got 1.1`);

        const encoding = asserts.encoding();
        await encoding.succeed(1);
        await encoding.fail(1.1, `Expected an integer, got 1.1`);
...
Expected a value between -2147483648 and 2147483647, got 9007199254740992`
    ...
    describe("BigInt Checks", () => {
      ...
      it("isBetween", async () => {
        const schema = Schema.BigInt.check(
          isBetween({ minimum: 5n, maximum: 10n })
        );
        const asserts = new TestSchema.Asserts(schema);
        ...
        await decoding.fail(4n, `Expected a value between 5n and 10n, got 4n`);
      });
      ...
      it("isGreaterThan", async () => {
        ...
        await decoding.fail(5n, `Expected a value greater than 5n, got 5n`);
      });
      ...
      it("isLessThan", async () => {
        ...
        await decoding.fail(5n, `Expected a value less than 5n, got 5n`);
      });
    ...
    describe("Record checks", () => {
      ...
      it("isMinProperties with symbol keys", async () => {
        ...
        const schema = Schema.Record(
          Schema.Union([Schema.String, Schema.Symbol]),
          Schema.Finite
  ...
  at ["ab"]`
      ...
      it("isPropertyNames with Never", async () => {
        const schema = Schema.Record(Schema.String, Schema.Finite).check(
          Schema.isPropertyNames(Schema.Never)
        );
        const asserts = new TestSchema.Asserts(schema);
...
Expected a value with at most 2 entries, got {"a":1,"b":NaN,"c":3}`
      ...
      it("ReadonlyMap + isMaxSize", async () => {
        const schema = Schema.ReadonlyMap(Schema.String, Schema.Finite).check(
          Schema.isMaxSize(2)
        );
        const asserts = new TestSchema.Asserts(schema);
...
Expected a value with a size of at most 2, got Map([["a",1],["b",NaN],["c",3]])`
    ...
    describe("Array checks", () => {
      it("UniqueArray", async () => {
        const schema = Schema.UniqueArray(
          Schema.Struct({
            a: Schema.String,
            b: Schema.String,
          })
  ...
  describe("Transformations", () => {
    ...
    it("FiniteFromString", async () => {
      ...
      await decoding.fail("Infintiy", `Expected a finite number, got NaN`);
      await decoding.fail("+Infintiy", `Expected a finite number, got NaN`);
      await decoding.fail("-Infintiy", `Expected a finite number, got NaN`);
    ...
    it("BigDecimalFromString", async () => {
      ...
      await decoding.succeed(
        "0",
        Option.getOrThrow(BigDecimal.fromString("0"))
      );
      await decoding.succeed(
        "123.456",
        Option.getOrThrow(BigDecimal.fromString("123.456"))
      );

      const encoding = asserts.encoding();
      await encoding.succeed(BigDecimal.make(0n, 0), "0");
      await encoding.succeed(BigDecimal.make(123456n, 3), "123.456");
      await encoding.fail("a", `Expected BigDecimal, got "a"`);
    });
    ...
    it("TimeZoneNamedFromString", async () => {
      ...
      await decoding.succeed(
        "Europe/London",
        DateTime.zoneMakeNamedUnsafe("Europe/London")
      );
      ...
      await encoding.fail("a", `Expected DateTime.TimeZone.Named, got "a"`);
    });
    ...
    it("TimeZoneFromString", async () => {
      ...
      await decoding.succeed(
        "+03:00",
        DateTime.zoneMakeOffset(3 * 60 * 60 * 1000)
      );
      ...
      await encoding.fail("a", `Expected DateTime.TimeZone, got "a"`);
    });
    ...
    it("DateTimeZonedFromString", async () => {
      ...
      const zoned = DateTime.makeZonedUnsafe("2021-01-01T00:00:00.000Z", {
      ...
      await encoding.fail("a", `Expected DateTime.Zoned, got "a"`);
    });
    ...
    it("FiniteFromString & isGreaterThan", async () => {
      ...
      await decoding.fail("1", `Expected a value greater than 2, got 1`);

      const encoding = asserts.encoding();
      await encoding.succeed(3, "3");
      await encoding.fail(1, `Expected a value greater than 2, got 1`);
    });
  });
  ...
  describe("decodeTo", () => {
    ...
    it("required to required", async () => {
      const schema = Schema.Struct({
        a: Schema.String.pipe(
          Schema.decodeTo(Schema.String, SchemaTransformation.passthrough())
        ),
      });
      const asserts = new TestSchema.Asserts(schema);
  ...
  at ["a"]`
    ...
    it("double transformation", async () => {
      ...
      await decoding.fail(" a2 ", `Expected a finite number, got NaN`);
  ...
  describe("encodeTo", () => {
    it("required to required", async () => {
      const schema = Schema.Struct({
        a: Schema.String.pipe(
          Schema.encodeTo(Schema.String, SchemaTransformation.passthrough())
        ),
      });
      const asserts = new TestSchema.Asserts(schema);
  ...
  at ["a"]`
    ...
    it("double transformation", async () => {
      const schema = Schema.FiniteFromString.pipe(
        Schema.encodeTo(Schema.Trim, SchemaTransformation.passthrough())
      );
      const asserts = new TestSchema.Asserts(schema);
      ...
      const schema = Schema.String.pipe(
        Schema.encode(
          SchemaTransformation.trim()
            .compose(SchemaTransformation.toLowerCase())
            .flip()
        )
      );
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("flip", () => {
    ...
    it("string & isMinLength(3) <-> number & isGreaterThan(2)", async () => {
      ...
      const decoding = asserts.decoding();
      await decoding.fail(2, `Expected a value greater than 2, got 2`);
      await decoding.fail(
        3,
  ...
  at ["value"]`
    ...
    it("with label", async () => {
      const schema = Schema.Redacted(Schema.NonEmptyString, {
        label: "password",
      });
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("RedactedFromValue", () => {
    ...
    it("should decode a value", async () => {
      const schema = Schema.RedactedFromValue(
        Schema.FiniteFromString.check(Schema.isInt())
      );
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("CauseReason", () => {
    ...
    it("CauseReason(FiniteFromString, FiniteFromString)", async () => {
      const schema = Schema.CauseReason(
        Schema.FiniteFromString,
        Schema.FiniteFromString
      );
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("Exit", () => {
    ...
    it("Exit(FiniteFromString, String, Unknown)", async () => {
      ...
      await decoding.fail(null, `Expected Exit, got null`);
      await decoding.fail(
        Exit.succeed(123),
    ...
    it("should work", async () => {
      ...
      const schema = Schema.Struct({
        ...
        categories: Schema.Array(
          Schema.suspend(
            (): Schema.Codec<CategoryType, CategoryEncoded> => schema
          )
        ),
      });
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("makeOption", () => {
    ...
    it("Class", () => {
      class A extends Schema.Class<A>("A")(
        Schema.Struct({ a: Schema.Number.check(Schema.isGreaterThan(0)) })
      ) {}
    ...
    describe("Struct", () => {
      it("should not apply defaults when decoding / encoding", async () => {
        const schema = Schema.Struct({
          a: Schema.String.pipe(
            Schema.optionalKey,
            Schema.withConstructorDefault(Effect.succeed("a"))
          ),
        });
        const asserts = new TestSchema.Asserts(schema);
      ...
      describe("nested defaults", () => {
        it("Struct", async () => {
          const schema = Schema.Struct({
            a: Schema.Struct({
              b: Schema.FiniteFromString.pipe(
                Schema.withConstructorDefault(Effect.succeed(-1))
              ),
  ...
  at ["a"]`
      ...
      it("Effect failing via another schema's makeEffect propagates as parse failure", async () => {
        const Inner = Schema.Struct({
          n: Schema.FiniteFromString.pipe(
  ...
  at ["a"]`
    ...
    it("Record(Number, String)", async () => {
      ...
      const decoding = asserts.decoding();
      await decoding.succeed(
        { 1: "1", 2.2: "2", Infinity: "3", NaN: "4", "-Infinity": "5" },
        {
          "1": 1,
          "2.2": 2,
          Infinity: 3,
  ...
  at ["-Infinity"]`
    ...
    it(`Record(Union(Number, "a"), FiniteFromString)`, async () => {
      const schema = Schema.Record(
        Schema.Union([Schema.Number, Schema.Literal("a")]),
        Schema.FiniteFromString
      );
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("Union", () => {
    ...
    it(`String | Number`, async () => {
      ...
      await decoding.succeed("a");
      await decoding.succeed(1);
      await decoding.fail(null, `Expected string | number, got null`);
    });
    ...
    it("{} & Literal", async () => {
      const schema = Schema.Union([Schema.Struct({}), Schema.Literal("a")]);
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("TupleWithRest", () => {
    it("A required element cannot follow an optional element", () => {
      ...
      throws(
        () =>
          Schema.TupleWithRest(Schema.Tuple([Schema.optional(Schema.String)]), [
            Schema.Boolean,
            Schema.String,
          ]),
        new Error(
  ...
  at ["ab"]`
    ...
    it("should preserve both checks", async () => {
      const schema = Schema.StructWithRest(
        ...
        [
          Schema.Record(Schema.String, Schema.Number).check(
            Schema.makeFilter((s) => s.b === undefined || s.b > 1, {
              expected: "bgt(1)",
            })
          ),
        ]
      ...
      await decoding.succeed({ a: 1 });
      await decoding.succeed({ a: 1, b: 2 });
      await decoding.fail({ a: 0 }, `Expected agt(0), got {"a":0}`);
      await decoding.fail({ a: 1, b: 1 }, `Expected bgt(1), got {"a":1,"b":1}`);
    });
  });
  ...
  it("DateTimeZoned", async () => {
    ...
    const decoding = asserts.decoding();
    await decoding.succeed(
      DateTime.makeZonedUnsafe("2021-01-01T00:00:00.000Z", {
        timeZone: "Europe/London",
      })
    );
  ...
  describe("instanceOf", () => {
    it("arg: message: string", async () => {
      ...
      const schema = Schema.instanceOf(MyError, { expected: "MyError" });
      const asserts = new TestSchema.Asserts(schema);
  ...
  it("DurationFromNanos", async () => {
    ...
    await encoding.fail(
      Duration.infinity,
      "Unable to encode Infinity into a bigint"
    );
  });
  ...
  it("DurationFromMillis", async () => {
    ...
    await decoding.fail(
      NaN,
      "Expected a value greater than or equal to 0, got NaN"
    );
  ...
  describe("BigDecimal checks", () => {
    it("isGreaterThanBigDecimal", async () => {
      const schema = Schema.BigDecimal.check(
        Schema.isGreaterThanBigDecimal(BigDecimal.fromStringUnsafe("1"))
      );
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("isGreaterThanOrEqualToBigDecimal", async () => {
      const schema = Schema.BigDecimal.check(
        Schema.isGreaterThanOrEqualToBigDecimal(
          BigDecimal.fromStringUnsafe("1")
        )
      );
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("isLessThanBigDecimal", async () => {
      const schema = Schema.BigDecimal.check(
        Schema.isLessThanBigDecimal(BigDecimal.fromStringUnsafe("1"))
      );
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("isLessThanOrEqualToBigDecimal", async () => {
      const schema = Schema.BigDecimal.check(
        Schema.isLessThanOrEqualToBigDecimal(BigDecimal.fromStringUnsafe("1"))
      );
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("decoding: default & encoding: omit & constructor: optional", async () => {
      const schema = Schema.Struct({
        _tag: Schema.tag("a").pipe(
          Schema.encodeTo(Schema.optionalKey(Schema.Literal("a")), {
  ...
  it("URLFromString", async () => {
    ...
    await decoding.fail("123", `Invalid URL string: 123`);
  ...
  it("transformOrFail", async () => {
    ...
    await decoding.fail("a", `input should not be "a"`);

    const encoding = asserts.encoding();
    await encoding.succeed("a");
    await encoding.fail("b", `input should not be "b"`);
  });
  ...
  describe("TemplateLiteral", () => {
    ...
    it("getTemplateLiteralRegExp", () => {
      const assertSource = (
        ...
        strictEqual(
          SchemaAST.getTemplateLiteralRegExp(Schema.TemplateLiteral(parts).ast)
            .source,
          source
        );
      };
      ...
      assertSource(
        [
          "c",
          Schema.Union([
            Schema.TemplateLiteral(["a", Schema.String, "b"]),
            Schema.Literal("e"),
          ]),
          "d",
        ],
    ...
    it(`"a"`, async () => {
      ...
      await decoding.succeed("a");
      await decoding.fail(null, "Expected string, got null");
      await decoding.fail("ab", `Expected a value matching ^(a)$, got "ab"`);
      await decoding.fail("", `Expected a value matching ^(a)$, got ""`);
    });
    ...
    it("https://www.typescriptlang.org/docs/handbook/2/template-literal-types.html", async () => {
      const EmailLocaleIDs = Schema.Literals([
        "welcome_email",
        "email_heading",
      ]);
      const FooterLocaleIDs = Schema.Literals([
        "footer_title",
        "footer_sendoff",
      ]);
      const schema = Schema.TemplateLiteral([
    ...
    it("< + h + (1|2n) + >", async () => {
      const schema = Schema.TemplateLiteral([
        "<",
        Schema.TemplateLiteral(["h", Schema.Literals([1, 2n])]),
        ">",
      ]);
      const asserts = new TestSchema.Asserts(schema);
  ...
  at [2]`
    ...
    it(`"c" + (\`a\${string}b\`|"e") + "d"`, async () => {
      const schema = Schema.TemplateLiteralParser([
        "c",
        Schema.Union([
          Schema.TemplateLiteralParser(["a", Schema.NonEmptyString, "b"]),
          Schema.Literal("e"),
        ]),
        "d",
      ]);
  ...
  describe("Class", () => {
    ...
    describe("should be compatible with `immer`", () => {
      it("`[immerable]`", () => {
        class A extends Schema.Class<A>("A")({
          a: Schema.Struct({ b: Schema.FiniteFromString }).pipe(
            Schema.optional
          ),
    ...
    it("Struct with nested Class", async () => {
      ...
      const schema = Schema.Struct({
        a: A.pipe(
          Schema.withConstructorDefault(Effect.succeed(new A({ a: "default" })))
        ),
      });
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("Class with nested Class", async () => {
      ...
      class B extends Schema.Class<B, { readonly brand: unique symbol }>("B")(
        Schema.Struct({
          a: A.pipe(
            Schema.withConstructorDefault(
              Effect.succeed(new A({ a: "default" }))
            )
          ),
        })
      ) {}
  ...
  at ["a"]`
      ...
      await encoding.fail(null, "Expected A, got null");
      await encoding.fail({ a: "a" }, `Expected A, got {"a":"a"}`);
    });
    ...
    it("Struct argument", async () => {
      class A extends Schema.TaggedClass<A>()(
        ...
        Schema.Struct({
          ...
          Schema.makeFilter(({ a }) => a.length > 0, {
            expected: `"a" being longer than 0`,
          })
        )
      ) {}
      ...
      await make.fail(
        { a: "" },
        `Expected "a" being longer than 0, got {"_tag":"A","a":""}`
      );
  ...
  describe("Enum", () => {
    ...
    it("Numeric enum", async () => {
      ...
      await decoding.fail(3, `Expected 0 | 1, got 3`);
  ...
  describe("catchDecoding", () => {
    it("sync fallback", async () => {
      ...
      const encoding = asserts.encoding();
      await encoding.succeed("a");
      await encoding.fail(null, "Expected string, got null");
    });
  ...
  describe("middlewareDecoding", () => {
    it("providing a service", async () => {
      ...
      const schema = Schema.FiniteFromString.pipe(
        ...
        Schema.middlewareDecoding(
          Effect.provideService(Service, { fallback: Effect.succeed(0) })
        )
      );
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("forced failure", async () => {
      const schema = Schema.String.pipe(
        Schema.middlewareDecoding(() =>
          Effect.fail(
            new SchemaIssue.Forbidden(Option.none(), { message: "my message" })
          )
        )
      );
      const asserts = new TestSchema.Asserts(schema);
      ...
      const decoding = asserts.decoding();
      await decoding.fail("1", "my message");
    });
  });
  ...
  it("catchEncodingWithContext", async () => {
    ...
    const encoding = asserts
      .encoding()
      .provide(Service, { fallback: Effect.succeed(0) });
    await encoding.succeed(1);
    await encoding.succeed(null, 0);
    await encoding.fail(1.2, `Expected an integer, got 1.2`);
  });
  ...
  describe("decodeUnknownPromise / encodeUnknownPromise", () => {
    it("FiniteFromString", async () => {
      ...
      const decodeUnknownPromiseIssue =
        SchemaParser.decodeUnknownPromise(schema);
      const encodeUnknownPromiseIssue =
        SchemaParser.encodeUnknownPromise(schema);

      const r1 = await decodeUnknownPromise("1").then(
        Result.succeed,
        Result.fail
      );
      deepStrictEqual(r1, Result.succeed(1));

      const r2 = await decodeUnknownPromise(null).then(
        Result.succeed,
        Result.fail
      );
      ...
      const r3 = await encodeUnknownPromise(1).then(
        Result.succeed,
        Result.fail
      );
      deepStrictEqual(r3, Result.succeed("1"));

      const r4 = await encodeUnknownPromise(null).then(
        Result.succeed,
        Result.fail
      );
      ...
      const r5 = await decodeUnknownPromiseIssue(null).then(
        Result.succeed,
        Result.fail
      );
      ...
      const r6 = await encodeUnknownPromiseIssue(null).then(
        Result.succeed,
        Result.fail
      );
  ...
  describe("decodeUnknownSync / encodeUnknownSync", () => {
    it("FiniteFromString", () => {
      ...
      throws(
        () => Schema.encodeUnknownSync(schema)(null),
        (e) => {
          assertTrue(Schema.isSchemaError(e));
      ...
      throws(
        () => SchemaParser.decodeUnknownSync(schema)(null),
        (e) => {
          ok(e instanceof Error);
      ...
      throws(
        () => SchemaParser.encodeUnknownSync(schema)(null),
        (e) => {
          ok(e instanceof Error);
          assertTrue(SchemaIssue.isIssue(e.cause));
  ...
  describe("decodeUnknownResult", () => {
    ...
    it("should throw on missing dependency", () => {
      class MagicNumber extends Context.Service<MagicNumber, number>()(
        "MagicNumber"
      ) {}
  ...
  describe("annotateKey", () => {
    describe("the messageMissingKey annotation should be used as a error message", () => {
      it("Struct", async () => {
        const schema = Schema.Struct({
          a: Schema.String.pipe(
            Schema.annotateKey({ messageMissingKey: "this field is required" })
          ),
        });
        const asserts = new TestSchema.Asserts(schema);
  ...
  describe("annotateEncoded", () => {
    it("non-transforming schema", () => {
      ...
      strictEqual(
        SchemaAST.toEncoded(schema.ast).annotations?.title,
        "encoded title"
      );
    });
  ...
  describe("Tuple.mapElements", () => {
    it("appendElement", () => {
      const schema = Schema.Tuple([Schema.String]).mapElements(
        Tuple.appendElement(Schema.Number)
      );
    ...
    it("appendElements", () => {
      const schema = Schema.Tuple([Schema.String]).mapElements(
        Tuple.appendElements([Schema.Number, Schema.Boolean])
      );
    ...
    describe("evolve", () => {
      it("readonly [string] -> readonly [string?]", () => {
        const schema = Schema.Tuple([Schema.String]).mapElements(
          Tuple.evolve([(v) => Schema.optionalKey(v)])
        );
    ...
    describe("renameIndices", () => {
      it("partial index mapping", () => {
        const schema = Schema.Tuple([
          ...
          Schema.Number,
          Schema.Boolean,
        ]).mapElements(Tuple.renameIndices(["1", "0"]));
        TestSchema.Asserts.ast.elements.equals(schema.elements, [
          Schema.Number,
          Schema.String,
          Schema.Boolean,
      ...
      it("full index mapping", () => {
        const schema = Schema.Tuple([
          ...
          Schema.Number,
          Schema.Boolean,
        ]).mapElements(Tuple.renameIndices(["2", "1", "0"]));
        TestSchema.Asserts.ast.elements.equals(schema.elements, [
          Schema.Boolean,
          Schema.Number,
          Schema.String,
  ...
  describe("Union.mapMembers", () => {
    it("appendElement", () => {
      const schema = Schema.Union([Schema.String, Schema.Number]).mapMembers(
  ...
  describe("Schema.makeFilter", () => {
    ...
    it("returns false", async () => {
      ...
      const asserts = new TestSchema.Asserts(schema);
      const decoding = asserts.decoding();
      await decoding.fail("a", `Expected <filter>, got "a"`);
    });

    it("returns string", async () => {
      const schema = Schema.String.check(
        Schema.makeFilter(() => "error message")
      );
      const asserts = new TestSchema.Asserts(schema);
      const decoding = asserts.decoding();
      await decoding.fail("a", `error message`);
    });
    ...
    describe("returns issue", () => {
      it("abort: false", async () => {
        const schema = Schema.String.check(
          ...
          Schema.makeFilter(() => false, {
            title: "filter title 2",
...
error message 2`
      ...
      it("abort: true", async () => {
        ...
        const decoding = asserts.decoding({ parseOptions: { errors: "all" } });
        await decoding.fail("a", `error message 1`);
      });
    });
      ...
      it("issue: Issue", async () => {
        const schema = Schema.String.check(
          Schema.makeFilter(
            (s) => ({
              path: ["a"],
              issue: new SchemaIssue.InvalidValue(Option.some(s), {
  ...
  at ["a"]`
    ...
    describe("returns array", () => {
      ...
      it("single-element array collapses to the element", async () => {
        const schema = Schema.String.check(
          Schema.makeFilter(() => [{ path: ["a"], issue: "error message 1" }], {
            title: "filter title",
          })
        );
        const asserts = new TestSchema.Asserts(schema);
...
pointed message
  ...
  describe("extendTo", () => {
    ...
    it("Union", async () => {
      ...
      const DiscriminatedShape = Schema.Union([
        Circle.pipe(
          Schema.extendTo(
            { kind: Schema.tag("circle") },
            { kind: () => Option.some("circle" as const) }
          )
        ),
        Square.pipe(
          Schema.extendTo(
            { kind: Schema.tag("square") },
    ...
    describe("toTaggedUnion", () => {
      ...
      it("should support multiple tags", () => {
        const schema = Schema.Union([
          Schema.Struct({
            _tag: Schema.tag("A"),
            type: Schema.tag("TypeA"),
            a: Schema.String,
          }),
          Schema.Struct({
            _tag: Schema.tag("B"),
            type: Schema.tag("TypeB"),
            b: Schema.FiniteFromString,
          }),
        ...
        a: Schema.FiniteFromString.pipe(
          Schema.withDecodingDefaultKey(
            Effect.fail(
              new Schema.SchemaError(
                new SchemaIssue.InvalidValue(Option.none(), {
                  message: "decoding default failed",
                })
              )
            )
          )
  ...
  at ["a"]`
    ...
    it("default Effect can require a service", async () => {
      ...
      const decoding = asserts
        .decoding()
        .provide(Service, { fallback: Effect.succeed("3") });
      await decoding.succeed({ a: "2" }, { a: 2 });
  ...
  describe("withDecodingDefault", () => {
    ...
    it("should return a decoding default value if the schema is used as standalone and the input is undefined", async () => {
      const schema = Schema.String.pipe(
        Schema.withDecodingDefault(Effect.succeed("a"))
      );
      const asserts = new TestSchema.Asserts(schema);
  ...
  describe("withDecodingDefaultType", () => {
    ...
    it("should return a decoding default value if the schema is used as standalone and the input is undefined", async () => {
      const schema = Schema.String.pipe(
        Schema.withDecodingDefaultType(Effect.succeed("a"))
      );
      const asserts = new TestSchema.Asserts(schema);
  ...
  it("Char", async () => {
    ...
    const decoding = asserts.decoding();
    await decoding.succeed("a");
    await decoding.fail("ab", `Expected a value with a length of 1, got "ab"`);

    const encoding = asserts.encoding();
    await encoding.succeed("a");
    await encoding.fail("ab", `Expected a value with a length of 1, got "ab"`);
  });
...
describe("Check", () => {
  ...
  describe("fromBrand", () => {
    it("nominal", async () => {
      const schema = Schema.String.pipe(
        Schema.fromBrand("a", Brand.nominal<string & Brand.Brand<"a">>())
      );
      const asserts = new TestSchema.Asserts(schema);
    ...
    it("multiple brands", async () => {
      ...
      deepStrictEqual(schema.ast.checks?.at(-1)?.annotations?.brands, [
        "PositiveInt",
      ]);
    });
  });
...
describe("resolveAnnotations", () => {
  ...
  it("returns annotations from the last check", () => {
    const schema = Schema.String.annotate({ title: "base" }).check(
      Schema.isNonEmpty().annotate({ title: "check" })
    );
...
describe("resolveAnnotationsKey", () => {
  ...
  it("returns key annotations", () => {
    ...
    deepStrictEqual(Schema.resolveAnnotationsKey(schema), {
      messageMissingKey: "required",
    });
  });
});
