...
function verifyGeneration<
  S extends Schema.Codec<unknown, unknown, never, unknown>,
>(schema: S) {
...
describe("Arbitrary generation", () => {
  ...
  it("TupleWithRest", () => {
    verifyGeneration(
      Schema.TupleWithRest(Schema.Tuple([Schema.Boolean]), [
        Schema.Number,
        Schema.String,
      ])
    );
    ...
    verifyGeneration(
      Schema.TupleWithRest(Schema.Tuple([Schema.optionalKey(Schema.Boolean)]), [
        Schema.Number,
  ...
  it("StructWithRest", () => {
    verifyGeneration(
      Schema.StructWithRest(Schema.Struct({ a: Schema.Number }), [
  ...
  describe("checks", () => {
    it("isMinLength(2)", () => {
      ...
      verifyGeneration(
        Schema.Array(Schema.String).pipe(Schema.check(Schema.isMinLength(2)))
      );
    });
    ...
    it("isMaxLength(2)", () => {
      ...
      verifyGeneration(
        Schema.Array(Schema.String).pipe(Schema.check(Schema.isMaxLength(2)))
      );
    });
    ...
    it("isMinLength(2) & isMaxLength(4)", () => {
      verifyGeneration(
        Schema.String.pipe(
          Schema.check(Schema.isMinLength(2), Schema.isMaxLength(4))
        )
      );
      verifyGeneration(
    ...
    it("isLength(2)", () => {
      verifyGeneration(
        Schema.String.pipe(Schema.check(Schema.isLengthBetween(2, 2)))
      );
      verifyGeneration(
        Schema.Array(Schema.String).pipe(
    ...
    it("isLessThanOrEqualToDate", () => {
      verifyGeneration(
        Schema.Date.check(Schema.isLessThanOrEqualToDate(new Date(10)))
      );
    });
    ...
    it("isBetweenDate", () => {
      verifyGeneration(
        Schema.Date.check(
          Schema.isBetweenDate({ minimum: new Date(0), maximum: new Date(10) })
        )
      );
    });
    ...
    it("isGreaterThanBigInt", () => {
      verifyGeneration(
        Schema.BigInt.check(Schema.isGreaterThanBigInt(BigInt(0)))
      );
    });

    it("isLessThanOrEqualToBigInt", () => {
      verifyGeneration(
        Schema.BigInt.check(Schema.isLessThanOrEqualToBigInt(BigInt(10)))
      );
    });

    it("isLessThanBigInt", () => {
      verifyGeneration(
        Schema.BigInt.check(Schema.isLessThanBigInt(BigInt(10)))
      );
    });
    ...
    it("isBetweenBigInt", () => {
      verifyGeneration(
        Schema.BigInt.check(
          Schema.isBetweenBigInt({ minimum: BigInt(0), maximum: BigInt(10) })
        )
      );
    });
  ...
  describe("ReadonlyMap", () => {
    ...
    it("isMinSize(2)", () => {
      verifyGeneration(
        Schema.ReadonlyMap(Schema.String, Schema.Number).check(
          Schema.isMinSize(2)
        )
      );
    });
  ...
  describe("context constraints", () => {
    ...
    it("String & isNonEmpty & isMinLength(2)", () => {
      assertContext(
        Schema.String.check(Schema.isNonEmpty()).check(Schema.isMinLength(2)),
        {
          constraints: {
            array: {
              minLength: 2,
    ...
    it("String & isMinLength(2) & isNonEmpty", () => {
      assertContext(
        Schema.String.check(Schema.isMinLength(2)).check(Schema.isNonEmpty()),
        {
          constraints: {
            array: {
              minLength: 2,
    ...
    it("String & isNonEmpty & isMaxLength(2)", () => {
      assertContext(
        Schema.String.check(Schema.isNonEmpty()).check(Schema.isMaxLength(2)),
        {
          constraints: {
            array: {
              minLength: 1,
    ...
    it("isInt & isGreaterThan", () => {
      assertIntegerConstraints(Schema.Int.check(Schema.isGreaterThan(1)), {
        min: 2,
      });
    });

    it("isInt & isGreaterThan fractional", () => {
      assertIntegerConstraints(Schema.Int.check(Schema.isGreaterThan(1.2)), {
        min: 2,
      });
    });

    it("isInt & isLessThan", () => {
      assertIntegerConstraints(Schema.Int.check(Schema.isLessThan(10)), {
        max: 9,
      });
    });

    it("isInt & isLessThan fractional", () => {
      assertIntegerConstraints(Schema.Int.check(Schema.isLessThan(10.8)), {
        max: 10,
      });
    });
    ...
    it("isInt & isBetween with fractional bounds", () => {
      assertIntegerConstraints(
        Schema.Int.check(Schema.isBetween({ minimum: 1.2, maximum: 10.8 })),
        {
          min: 2,
          max: 10,
        }
    ...
    it("isValidDate & isGreaterThanOrEqualToDate", () => {
      assertContext(
        Schema.Date.check(
          Schema.isDateValid(),
          Schema.isGreaterThanOrEqualToDate(new Date(0))
        ),
        {
          constraints: {
            date: {
    ...
    it("UniqueArray", () => {
      ...
      assertContext(
        Schema.UniqueArray(Schema.String).check(Schema.isMaxLength(2)),
        {
          constraints: {
            array: {
              maxLength: 2,
