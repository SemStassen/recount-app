...
const expectFailure = <A>(
  ...
  issues:
    ...
    | ((issues: ReadonlyArray<StandardSchemaV1.Issue>) => void)
) => {
...
describe("toStandardSchemaV1", () => {
  ...
  it("should support both standards", () => {
    ...
    const both = Schema.toStandardJSONSchemaV1(
  ...
  describe("missing dependencies", () => {
    ...
    it("sync decoding should throw", () => {
      ...
      expectSyncFailure(standardSchema, 1, (issues) => {
        ...
        assertTrue(
          issues[0].message.includes("Service not found: MagicNumber")
        );
      });
    });
  ...
  describe("should respect the `message` annotation", () => {
    describe("String", () => {
      ...
      it("String & isNonEmpty & annotation", () => {
        const schema = Schema.String.check(Schema.isNonEmpty()).annotate({
          message: "Custom message",
        });
        const standardSchema = Schema.toStandardSchemaV1(schema);
        expectSyncFailure(standardSchema, null, [
      ...
      it("String & isNonEmpty(annotation)", () => {
        const schema = Schema.String.check(
          Schema.isNonEmpty({ message: "Custom message" })
        );
        const standardSchema = Schema.toStandardSchemaV1(schema);
        expectSyncFailure(standardSchema, null, [
          {
    ...
    describe("Tuple", () => {
      ...
      it("messageUnexpectedKey", () => {
        const schema = Schema.Tuple([Schema.String]).annotate({
          messageUnexpectedKey: "Custom message",
        });
        const standardSchema = Schema.toStandardSchemaV1(schema, {
          parseOptions: { onExcessProperty: "error" },
        });
    ...
    describe("Union", () => {
      it("Literals", () => {
        const schema = Schema.Literals(["a", "b"]).annotate({
          message: "Custom message",
        });
        const standardSchema = Schema.toStandardSchemaV1(schema);
        expectSyncFailure(standardSchema, null, [
