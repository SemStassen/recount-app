...
describe("SchemaAST", () => {
  ...
  describe("getCandidates", () => {
    ...
    it("should exclude by literals", () => {
      ...
      const ast = schema.ast;
      deepStrictEqual(SchemaAST.getCandidates(Symbol.for("a"), ast.types), [
        ast.types[0],
      ]);
    ...
    it("should handle tagged structs", () => {
      ...
      deepStrictEqual(SchemaAST.getCandidates({ _tag: "a" }, ast.types), [
        ast.types[0],
      ]);
      deepStrictEqual(SchemaAST.getCandidates({ _tag: "b" }, ast.types), [
        ast.types[1],
      ]);
  ...
  describe("getIndexSignatureKeys", () => {
    it("String", () => {
      ...
      const input: { readonly [x: PropertyKey]: number } = {
        a: 1,
        b: 2,
        [sym]: 3,
      };
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.String.ast),
        ["a", "b"]
      );
    });
    ...
    it("TemplateLiteral", () => {
      ...
      deepStrictEqual(SchemaAST.getIndexSignatureKeys(input, schema.ast), [
        "a",
      ]);
    });
    ...
    it("Symbol", () => {
      ...
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.Symbol.ast),
        [a, b]
      );
    });
    ...
    it("Number", () => {
      ...
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.Number.ast),
        ["1", "1.5", "-2", "NaN"]
      );
    });
    ...
    it("default", () => {
      ...
      deepStrictEqual(
        SchemaAST.getIndexSignatureKeys(input, Schema.ObjectKeyword.ast),
        []
      );
    });
  });
