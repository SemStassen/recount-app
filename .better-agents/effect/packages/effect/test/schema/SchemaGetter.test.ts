...
describe("SchemaGetter", () => {
  ...
  it("dateTimeUtcFromInput", async () => {
    ...
    await decoding(
      "2024-01-01 01:00:00",
      DateTime.makeUnsafe("2024-01-01T01:00:00.000Z")
    );
    await decoding(
      "2020-02-01T11:17:00+1100",
      DateTime.makeUnsafe("2020-02-01T00:17:00.000Z")
    );
    // should support strings with explicit GMT zone
    await decoding(
      "Tue, 27 Jan 2026 17:14:06 GMT",
      DateTime.makeUnsafe("2026-01-27T17:14:06.000Z")
    );
  });
  ...
  describe("decodeURLSearchParams / encodeURLSearchParams", () => {
    ...
    it("decodes mixed dot + bracket notation", async () => {
      const urlSearchParams = new URLSearchParams(
        "user.address[city]=Milan&user.address[zip]=20100"
      );
      const object = {
        user: {
          address: {
    ...
    it("decodes nested objects inside appended array elements", async () => {
      const urlSearchParams = new URLSearchParams(
        "users[][name]=John&users[][name]=Alice"
      );
      const object = {
        users: [{ name: "John" }, { name: "Alice" }],
      };
