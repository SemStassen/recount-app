...
const Modulo2 = Schema.Number.annotate({
  toEquivalence: (): Equivalence.Equivalence<number> =>
    Equivalence.make((a, b) => a % 2 === b % 2),
});

const Modulo3 = Schema.Number.annotate({
  toEquivalence: (): Equivalence.Equivalence<number> =>
    Equivalence.make((a, b) => a % 3 === b % 3),
});
...
describe("toEquivalence", () => {
  ...
  describe("Struct", () => {
    ...
    it("optional fields", () => {
      ...
      assertTrue(equivalence({ a: "a" }, { a: "a" }));
      assertTrue(
        equivalence({ a: "a", b: undefined }, { a: "a", b: undefined })
      );
      assertFalse(equivalence({ a: "a", b: 1 }, { a: "b", b: 1 }));
      assertFalse(equivalence({ a: "a", b: 1 }, { a: "a", b: 2 }));
  ...
  describe("suspend", () => {
    it("recursive schema", () => {
      ...
      assertFalse(
        equivalence({ a: "a", as: [{ a: "a", as: [] }] }, { a: "a", as: [] })
      );
      assertFalse(
        equivalence({ a: "a", as: [] }, { a: "a", as: [{ a: "a", as: [] }] })
      );
  ...
  it("HashMap(Modulo2, Modulo3)", () => {
    ...
    assertTrue(
      equivalence(HashMap.make([0, 1], [1, 2]), HashMap.make([0, 1], [1, 2]))
    );
    assertTrue(
      equivalence(HashMap.make([0, 1], [1, 2]), HashMap.make([1, 2], [0, 1]))
    );

    assertFalse(equivalence(HashMap.make([0, 1]), HashMap.make([1, 1])));
    assertFalse(equivalence(HashMap.make([0, 1]), HashMap.make([0, 2])));
    assertFalse(
      equivalence(HashMap.make([0, 1], [1, 2]), HashMap.make([0, 1], [1, 3]))
    );
    assertFalse(
      equivalence(HashMap.make([0, 1], [1, 2]), HashMap.make([0, 1], [2, 2]))
    );
  });
  ...
  it("Duration", () => {
    ...
    assertTrue(
      equivalence(Duration.negativeInfinity, Duration.negativeInfinity)
    );
  ...
  it("BigDecimal", () => {
    ...
    assertTrue(
      equivalence(
        ...
        BigDecimal.fromStringUnsafe("1.50")
      )
    );
    assertFalse(
      equivalence(
  ...
  it("DateTimeUtc", () => {
    ...
    assertFalse(
      equivalence(
        DateTime.makeUnsafe("2021-01-01T00:00:00.000Z"),
        DateTime.makeUnsafe("2021-01-01T00:00:00.001Z")
      )
    );
  });
  ...
  it("TimeZoneOffset", () => {
    ...
    assertFalse(
      equivalence(
        DateTime.zoneMakeOffset(3 * 60 * 60 * 1000),
        DateTime.zoneMakeOffset(4 * 60 * 60 * 1000)
      )
    );
  });
  ...
  it("DateTimeZoned", () => {
    ...
    const z1 = DateTime.makeZonedUnsafe("2024-01-01T00:00:00.000Z", {
      timeZone: "Europe/London",
    });
    const z2 = DateTime.makeZonedUnsafe("2024-01-02T00:00:00.000Z", {
      timeZone: "Europe/London",
    });
      ...
      it("String", () => {
        const schema = Schema.String.pipe(
          Schema.overrideToEquivalence(() =>
            Equivalence.make((a, b) => a.substring(0, 1) === b.substring(0, 1))
          )
        );
        const equivalence = Schema.toEquivalence(schema);
