...
describe("ConfigProvider", () => {
  ...
  it("constantCase", async () => {
    ...
    await assertSuccess(
      provider,
      ["constant.case"],
      ConfigProvider.makeValue("value1")
    );
  });

  describe("mapInput", () => {
    it("two mappings", async () => {
      const appendA = ConfigProvider.mapInput((path) =>
        path.map((sn) => (typeof sn === "string" ? sn + "_A" : sn))
      );
      const appendB = ConfigProvider.mapInput((path) =>
        path.map((sn) => (typeof sn === "string" ? sn + "_B" : sn))
      );
  ...
  describe("nested", () => {
    ...
    it("nested + constantCase", async () => {
      const provider = ConfigProvider.fromEnv({
        ...
        ConfigProvider.nested("prefix.with.dots"),
  ...
  describe("fromEnv", () => {
    ...
    it("single underscore creates nesting", async () => {
      ...
      await assertSuccess(
        provider,
        ["NODE"],
        ConfigProvider.makeRecord(new Set(["ENV"]))
      );
      await assertSuccess(
        provider,
        ["NODE", "ENV"],
    ...
    it("prefix can be both a value and a container", async () => {
      ...
      await assertSuccess(
        provider,
        ["NODE"],
        ConfigProvider.makeRecord(new Set(["ENV"]), "value1")
      );
      await assertSuccess(
        provider,
        ["NODE", "ENV"],
        ConfigProvider.makeValue("value2")
      );
      await assertSuccess(
        provider,
        ["NODE_ENV"],
    ...
    it("multiple nested children under the same prefix", async () => {
      ...
      await assertSuccess(
        provider,
        ["NODE"],
        ConfigProvider.makeRecord(new Set(["ENV", "PATH"]), "value1")
      );
      await assertSuccess(
        provider,
        ["NODE", "ENV"],
      ...
      await assertSuccess(
        provider,
        ["NODE", "PATH"],
        ConfigProvider.makeValue("value3")
      );
      await assertSuccess(
        provider,
        ["NODE_PATH"],
    ...
    it("nested depth > 2 works", async () => {
      ...
      await assertSuccess(
        provider,
        ["A"],
        ConfigProvider.makeRecord(new Set(["B"]))
      );
      await assertSuccess(
        provider,
        ["A", "B"],
        ConfigProvider.makeRecord(new Set(["C"]))
      );
      await assertSuccess(
        provider,
        ["A", "B", "C"],
    ...
    it("mixed children (numeric + non-numeric) yields Record", async () => {
      ...
      await assertSuccess(
        provider,
        ["A"],
        ConfigProvider.makeRecord(new Set(["0", "B"]))
      );
      await assertSuccess(
        provider,
        ["A", 0],
    ...
    it("empty segment node can be both a value and a container", async () => {
      ...
      await assertSuccess(
        provider,
        ["A", ""],
        ConfigProvider.makeRecord(new Set(["B"]), "value1")
      );
      await assertSuccess(
        provider,
        ["A", "", "B"],
    ...
    it("leading underscore creates an empty first segment", async () => {
      ...
      await assertSuccess(
        provider,
        [""],
        ConfigProvider.makeRecord(new Set(["A"]))
      );
      await assertSuccess(
        provider,
        ["", "A"],
    ...
    it("prefix value is preserved when there are multiple nested children", async () => {
      ...
      await assertSuccess(
        provider,
        ["A"],
        ConfigProvider.makeRecord(new Set(["B", "C"]), "root")
      );
      await assertSuccess(
        provider,
        ["A", "B"],
    ...
    it("intermediate node can be both a value and a container (A_B and A_B_C)", async () => {
      ...
      await assertSuccess(
        provider,
        ["A", "B"],
        ConfigProvider.makeRecord(new Set(["C"]), "value1")
      );
      await assertSuccess(
        provider,
        ["A", "B", "C"],
    ...
    it("array: leading-zero indices are not numeric, so it yields a Record", async () => {
      ...
      await assertSuccess(
        provider,
        ["A"],
        ConfigProvider.makeRecord(new Set(["01", "1"]))
      );
      await assertSuccess(
        provider,
        ["A", "01"],
    ...
    it("root path exposes top-level keys", async () => {
      ...
      await assertSuccess(
        provider,
        [],
        ConfigProvider.makeRecord(new Set(["A", "B"]))
      );
      await assertSuccess(provider, ["A"], ConfigProvider.makeValue("value1"));
      await assertSuccess(
        provider,
    ...
    it("array: non-integer indices do not yield an Array node", async () => {
      ...
      await assertSuccess(
        provider,
        ["A"],
        ConfigProvider.makeRecord(new Set(["-1", "1.5"]))
      );
    });
  });
  ...
  describe("fromUnknown", () => {
    ...
    it("Exact leaf resolution", async () => {
      ...
      await assertSuccess(
        provider,
        ["array", 0],
        ConfigProvider.makeValue("value4")
      );
      await assertSuccess(
        provider,
        ["array", 1, "key4"],
        ConfigProvider.makeValue("value5")
      );
      await assertSuccess(
        provider,
        ["array", 2, 0],
        ConfigProvider.makeValue("value6")
      );
    });
    ...
    it("Object detection", async () => {
      await assertSuccess(
        provider,
        ["record"],
        ConfigProvider.makeRecord(new Set(["key1", "key2"]))
      );
      await assertSuccess(
        provider,
        ["record", "key2"],
        ConfigProvider.makeRecord(new Set(["key3"]))
      );
    });
    ...
    it("unknown values", async () => {
      await assertSuccess(
        provider,
        ["unknown"],
        ConfigProvider.makeValue("Symbol(unknown)")
      );
    });
  });
...
`);
      await assertSuccess(
        provider,
        [],
        ConfigProvider.makeRecord(new Set(["API"]))
      );
      await assertSuccess(
        provider,
        ["API_URL"],
        ConfigProvider.makeValue("https://api.example.com")
      );
    });
...
`);
      await assertSuccess(
        provider,
        [],
        ConfigProvider.makeRecord(new Set(["NODE"]))
      );
      await assertSuccess(
        provider,
        ["NODE_ENV"],
        ConfigProvider.makeValue("production")
      );
    });
...
`);
      await assertSuccess(
        provider,
        [],
        ConfigProvider.makeRecord(new Set(["OBJECT"]))
      );
      await assertSuccess(
        provider,
        ["OBJECT"],
...
`);
      ...
      await assertSuccess(
        provider,
        ["OBJECT"],
        ConfigProvider.makeRecord(new Set(["key1", "key2"]), "value1")
      );
      await assertSuccess(
        provider,
        ["OBJECT", "key1"],
...
`);
      await assertSuccess(
        provider,
        [],
        ConfigProvider.makeRecord(new Set(["ARRAY"]))
      );
      await assertSuccess(
        provider,
        ["ARRAY"],
        ConfigProvider.makeArray(2, "value1")
      );
      await assertSuccess(
        provider,
        ["ARRAY", 0],
...
`);
      await assertSuccess(
        provider,
        [],
        ConfigProvider.makeRecord(new Set(["PASSWORD", "DB"]))
      );
      await assertSuccess(
        provider,
        ["PASSWORD"],
      ...
      await assertSuccess(
        provider,
        ["DB_PASS"],
        ConfigProvider.makeValue("$PASSWORD")
      );
    });
...
A=1`),
      ...
      await assertSuccess(
        provider,
        ["CUSTOM_PATH"],
        ConfigProvider.makeValue("custom.env")
      );
      await assertSuccess(provider, ["A"], ConfigProvider.makeValue("1"));
    });
  });
