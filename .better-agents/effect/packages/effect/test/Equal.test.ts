  ...
  describe("plain objects", () => {
    ...
    describe("symbol properties", () => {
      ...
      it("should consider non-enumerable symbol properties", () => {
        ...
        Object.defineProperty(obj2, sym2, {
          value: "hidden",
          enumerable: false,
        });
  ...
  describe("custom Equal implementations", () => {
    class CustomPoint implements Equal.Equal {
      ...
      [Equal.symbol](that: Equal.Equal): boolean {
        return (
          that instanceof CustomPoint && this.x === that.x && this.y === that.y
        );
      }
  ...
  describe("special values", () => {
    describe("NaN", () => {
      ...
      it("should handle NaN in nested structures", () => {
        ...
        expect(
          Equal.equals({ nested: { deep: NaN } }, { nested: { deep: NaN } })
        ).toBe(true);
    ...
    describe("Infinity", () => {
      ...
      it("should handle Infinity in objects", () => {
        ...
        expect(Equal.equals({ a: Infinity, b: 1 }, { a: Infinity, b: 1 })).toBe(
          true
        );
      ...
      it("should handle Infinity in nested structures", () => {
        expect(Equal.equals({ arr: [Infinity] }, { arr: [Infinity] })).toBe(
          true
        );
        expect(Equal.equals([{ val: Infinity }], [{ val: Infinity }])).toBe(
          true
        );
        expect(
          Equal.equals(
            { nested: { deep: Infinity } },
            { nested: { deep: Infinity } }
          )
        ).toBe(true);
        expect(Equal.equals({ arr: [Infinity] }, { arr: [-Infinity] })).toBe(
    ...
    describe("-Infinity", () => {
      ...
      it("should handle -Infinity in objects", () => {
        expect(Equal.equals({ a: -Infinity }, { a: -Infinity })).toBe(true);
        expect(
          Equal.equals({ a: -Infinity, b: 1 }, { a: -Infinity, b: 1 })
        ).toBe(true);
        expect(Equal.equals({ a: -Infinity }, { a: Infinity })).toBe(false);
        expect(Equal.equals({ a: -Infinity }, { a: -42 })).toBe(false);
      });
    ...
    describe("mixed special values", () => {
      it("should handle arrays with multiple special values", () => {
        expect(
          Equal.equals([NaN, Infinity, -Infinity], [NaN, Infinity, -Infinity])
        ).toBe(true);
    ...
    it("should handle circular references in Equal implementations without infinite recursion", () => {
      class CircularEqualTest implements Equal.Equal {
        ...
        [Equal.symbol](that: Equal.Equal): boolean {
          ...
          return (
            this.value === that.value && Equal.equals(this.child, that.child)
          );
        }
