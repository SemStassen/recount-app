  ...
  it("replace should throw an error if the object has a non-Object constructor or null prototype", () => {
    {
      ...
      throws(
        () => optic.replace(2, new A(1)),
        "Cannot clone object with non-Object constructor or null prototype"
      );
    }
...
Expected a value greater than 0, got -1.1`
  ...
  it("refine", () => {
    ...
    assertFailure(
      optic.getResult({ _tag: "a", a: "value" }),
      `Expected "b" tag, got {"_tag":"a","a":"value"}`
    );
    deepStrictEqual(optic.modify(addOne)({ _tag: "a", a: "value" }), {
      _tag: "a",
      a: "value",
    });
    deepStrictEqual(optic.modify(addOne)({ _tag: "b", b: 1 }), {
      _tag: "b",
      b: 2,
    });
  });
  ...
  describe("pick", () => {
    it("Struct", () => {
      ...
      deepStrictEqual(
        optic.replace({ a: "a2", c: false }, { a: "a", b: 1, c: true }),
        { a: "a2", b: 1, c: false }
      );
    });
  });
  ...
  describe("forEach", () => {
    ...
    it("Record", () => {
      const optic = Optic.entries<number>().forEach((entry) =>
        entry.key(1).check(Schema.isGreaterThan(0))
      );

      deepStrictEqual(
        optic.modify((entries) => entries.map((value) => value + 1))({
          a: 0,
          b: 1,
          c: 0,
        }),
  ...
  it("notUndefined", () => {
    ...
    assertSuccess(optic.getResult(1), 1);
    assertFailure(
      optic.getResult(undefined),
      "Expected a value other than `undefined`, got undefined"
    );
  ...
  it("replace copies only objects and arrays along the focused path", () => {
    ...
    assertTrue(
      s2.projects !== s1.projects,
      "parent array on the path should be new"
    );
    ...
    assertTrue(
      s2.projects[1].tasks !== s1.projects[1].tasks,
      "parent array of changed element should be new"
    );
    assertTrue(
      s2.projects[1].tasks[0] !== s1.projects[1].tasks[0],
      "changed task replaced"
    );
    assertTrue(s2.projects[1].tasks[0].done === true, "value updated");
  });

  it("fromChecks", () => {
    const optic = Optic.id<number>().compose(
      Optic.fromChecks(Schema.isGreaterThan(0), Schema.isInt())
    );
    assertSuccess(optic.getResult(1), 1);
  ...
  describe("Option", () => {
    it("some", () => {
      ...
      assertFailure(
        optic.getResult(Option.none()),
        `Expected a Some value, got none()`
      );
    });
    ...
    it("none", () => {
      ...
      assertFailure(
        optic.getResult(Option.some(1)),
        `Expected a None value, got some(1)`
      );
    });
  });
  ...
  describe("Result", () => {
    it("success", () => {
      const optic = Optic.id<Result.Result<number, string>>().compose(
        Optic.success()
      );
      assertSuccess(optic.getResult(Result.succeed(1)), 1);
      assertFailure(
        optic.getResult(Result.fail("error")),
        `Expected a Result.Success value, got failure("error")`
      );
    });
    ...
    it("failure", () => {
      ...
      assertFailure(
        optic.getResult(Result.succeed(1)),
        `Expected a Result.Failure value, got success(1)`
      );
    });
  });
});
