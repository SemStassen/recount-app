...
describe("FiberMap", () => {
  ...
  it.effect("onlyIfMissing", () =>
    Effect.gen(function* () {
      ...
      const fiberB = yield* FiberMap.run(handle, "a", Effect.never, {
        onlyIfMissing: true,
      });
      const fiberC = yield* FiberMap.run(handle, "a", Effect.never, {
        onlyIfMissing: true,
      });
      yield* Effect.yieldNow;
  ...
  it.effect("propagateInterruption true", () =>
    Effect.gen(function* () {
      ...
      yield* Fiber.interrupt(fiber);
      assertTrue(
        Exit.hasInterrupts(yield* FiberMap.join(map).pipe(Effect.exit))
      );
    })
  );
