...
describe("Trie", () => {
  it("toString renders entries in iteration order", () => {
    ...
    strictEqual(String(trie), `Trie([["a",0],["b",1]])`);
  });
  ...
  it("insert returns new tries and orders entries by key", () => {
    ...
    deepStrictEqual(Array.from(trie2), [
      ["call", 0],
      ["me", 1],
    ]);
    deepStrictEqual(Array.from(trie3), [
      ["call", 0],
      ["me", 1],
      ["mind", 2],
    ]);
    deepStrictEqual(Array.from(trie4), [
      ["call", 0],
      ["me", 1],
      ["mid", 3],
      ["mind", 2],
  ...
  it("remove deletes a key without mutating the original trie", () => {
    ...
    deepStrictEqual(Array.from(trie1), [
      ["me", 1],
      ["mid", 3],
      ["mind", 2],
    ]);
  ...
  it("filterMap keeps Result successes and can use keys", () => {
    ...
    strictEqual(
      Equal.equals(
        Trie.filterMap(trie, (v, k) =>
          k.length > 3 ? Result.succeed(v) : Result.failVoid
        ),
        trieMapK
      ),
      true
  ...
  it("modify updates an existing key and ignores a missing key", () => {
    ...
    strictEqual(
      Equal.equals(trie.pipe(Trie.modify("me", (v) => v)), trie),
      true
    );
  });
  ...
  it("insertMany inserts all entries", () => {
    ...
    strictEqual(Equal.equals(trie, trieInsert), true);
  });
  ...
  it("reduce visits entries in key order", () => {
    ...
    strictEqual(trie.pipe(Trie.reduce(0, (acc, n) => acc + n)), 3);
    strictEqual(trie.pipe(Trie.reduce(10, (acc, n) => acc + n)), 13);
    strictEqual(
      trie.pipe(Trie.reduce("", (acc, _, key) => acc + key)),
      "sellssheshells"
    );
  });
  ...
  it("Equal compares tries by entries", () => {
    strictEqual(Equal.equals(Trie.empty<number>(), Trie.empty<number>()), true);
    strictEqual(
      Equal.equals(
