...
describe("Option", () => {
  ...
  it("toString", () => {
    strictEqual(String(Option.none()), `none()`);
    strictEqual(String(Option.some(1)), `some(1)`);
    strictEqual(
  ...
  it("toJSON", () => {
    ...
    deepStrictEqual(Option.some(1).toJSON(), {
      _id: "Option",
      _tag: "Some",
      value: 1,
    });
  ...
  it("inspect", () => {
    ...
    deepStrictEqual(
      inspect(Option.none()),
      inspect({ _id: "Option", _tag: "None" })
    );
    deepStrictEqual(
      inspect(Option.some(1)),
      inspect({ _id: "Option", _tag: "Some", value: 1 })
    );
  });
  ...
  it("toRefinement", () => {
    const f = (s: string | number): Option.Option<string> =>
      typeof s === "string" ? Option.some(s) : Option.none();
    ...
    type C = A | B;
    const isA = Option.toRefinement<C, A>((c) =>
      c.type === "A" ? Option.some(c) : Option.none()
    );
  ...
  it("orElseResult", () => {
    assertSome(
      pipe(
        Option.some(1),
        Option.orElseResult(() => Option.some(2))
      ),
      Result.fail(1)
    );
    assertSome(
      pipe(
        Option.some(1),
        Option.orElseResult(() => Option.none())
      ),
      Result.fail(1)
    );
    assertSome(
  ...
  it("getOrThrow", () => {
    ...
    throws(
      () => pipe(Option.none(), Option.getOrThrow),
  ...
  it("getOrThrowWith", () => {
    strictEqual(
      pipe(
        Option.some(1),
        Option.getOrThrowWith(() => new Error("Unexpected None"))
      ),
      1
    );
    throws(
  ...
  it("andThen", () => {
    ...
    assertSome(
      Option.andThen(Option.some(1), () => Option.some(2)),
      2
    );
    assertSome(Option.andThen(Option.some(1), Option.some(2)), 2);
    assertSome(Option.andThen(Option.some(1), 2), 2);
    assertSome(
      Option.andThen(Option.some(1), () => 2),
      2
    );
    assertSome(
      Option.andThen(Option.some(1), (a) => a),
      1
    );
  });
  ...
  it("partitionMap", () => {
    const f = (n: number) =>
      gt2(n) ? Result.succeed(n + 1) : Result.fail(n - 1);
    deepStrictEqual(pipe(Option.none(), Option.partitionMap(f)), [
      Option.none(),
      Option.none(),
    ]);
    deepStrictEqual(pipe(Option.some(1), Option.partitionMap(f)), [
      Option.some(0),
      Option.none(),
    ]);
    deepStrictEqual(pipe(Option.some(3), Option.partitionMap(f)), [
      Option.none(),
      Option.some(4),
    ]);
  });
  ...
  it("containsWith", () => {
    const containsWith = Option.containsWith<number>(
      (self, that) => self % 2 === that % 2
    );
  ...
  it("tap", () => {
    ...
    assertSome(
      Option.tap(Option.some(1), (n) => Option.some(n * 2)),
      1
    );
  });
  ...
  it("zipWith", () => {
    assertNone(
      pipe(
        Option.none(),
        Option.zipWith(Option.some(2), (a, b) => a + b)
      )
    );
    assertNone(
      pipe(
        Option.some(1),
        Option.zipWith(Option.none(), (a, b) => a + b)
      )
    );
    assertSome(
      pipe(
  ...
  it("makeEquivalence", () => {
    const isEquivalent = Option.makeEquivalence(
  ...
  describe("all", () => {
    it("tuple", () => {
      ...
      assertSome(Option.all([Option.some(1), Option.some("hello")]), [
        1,
        "hello",
      ]);
    ...
    it("struct", () => {
      assertSome(Option.all({ a: Option.some(1), b: Option.some("hello") }), {
        a: 1,
        b: "hello",
      });
