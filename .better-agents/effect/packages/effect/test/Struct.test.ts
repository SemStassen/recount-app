...
describe("Struct", () => {
  ...
  describe("evolve", () => {
    it("partial required fields", () => {
      const s = { a: "a", b: 1 };
      deepStrictEqual(pipe(s, Struct.evolve({ a: (s) => s.length })), {
        a: 1,
        b: 1,
      });
    ...
    it("all required fields", () => {
      const s = { a: "a", b: 1 };
      deepStrictEqual(
        pipe(s, Struct.evolve({ a: (s) => s.length, b: (b) => b > 0 })),
        { a: 1, b: true }
      );
      deepStrictEqual(
        Struct.evolve(s, { a: (s) => s.length, b: (b) => b > 0 }),
        { a: 1, b: true }
      );
    });
  });
  ...
  it("evolveKeys", () => {
    deepStrictEqual(
      pipe(
        { a: "a", b: 2 },
        Struct.evolveKeys({ a: (k) => Str.toUpperCase(k) })
      ),
      { A: "a", b: 2 }
    );
    deepStrictEqual(
      Struct.evolveKeys({ a: "a", b: 2 }, { a: (k) => Str.toUpperCase(k) }),
      { A: "a", b: 2 }
    );
  });
  ...
  it("evolveEntries transforms a selected key and value together", () => {
    deepStrictEqual(
      pipe(
        { a: "a", b: 2 },
        Struct.evolveEntries({ a: (k, v) => [Str.toUpperCase(k), v.length] })
      ),
      { A: 1, b: 2 }
    );
    deepStrictEqual(
  ...
  describe("makeCombiner", () => {
    it("default omitKeyWhen (never omit)", () => {
      ...
      deepStrictEqual(C.combine({ n: 1, s: "a" }, { n: 2, s: "b" }), {
        n: 3,
        s: "ab",
      });
    });
    ...
    it("custom omitKeyWhen", () => {
      ...
      deepStrictEqual(
        C.combine({ n: undefined, s: "a" }, { n: undefined, s: "b" }),
        { s: "ab" }
      );
  ...
  describe("Record", () => {
    it("string keys", () => {
      deepStrictEqual(Struct.Record(["a", "b"], "value"), {
        a: "value",
        b: "value",
      });
    });
    ...
    it("mixed keys", () => {
      ...
      deepStrictEqual(Struct.Record(["a", sym], true), {
        a: true,
        [sym]: true,
      });
    });
