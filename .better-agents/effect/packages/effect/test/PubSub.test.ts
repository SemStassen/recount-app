...
describe("PubSub", () => {
  ...
  it.effect(
    "sequential publishers and subscribers with one publisher and one subscriber",
    () =>
      Effect.gen(function* () {
        ...
        const subscriber = yield* PubSub.subscribe(pubsub).pipe(
          Effect.flatMap((subscription) =>
            latch.await.pipe(
              Effect.andThen(
                Effect.forEach(values, () => PubSub.take(subscription))
              )
            )
          ),
          Effect.scoped,
  ...
  it.effect(
    "sequential publishers and subscribers with one publisher and two subscribers",
    () =>
      Effect.gen(function* () {
        const values = Array.range(0, 9);
        const latch = yield* Latch.make();
  ...
  it.effect(
    "backpressured concurrent publishers and subscribers - one to one",
    () =>
      Effect.gen(function* () {
        ...
        const subscriber = yield* PubSub.subscribe(pubsub).pipe(
          Effect.flatMap((subscription) =>
            Effect.forEach(values, (_) => PubSub.take(subscription))
          ),
          Effect.scoped,
          Effect.forkChild({ startImmediately: true })
        );
  ...
  it.effect(
    "backpressured concurrent publishers and subscribers - one to many",
    () =>
      Effect.gen(function* () {
        const values = Array.range(0, 64);
        const pubsub = yield* PubSub.bounded<number>(64);
  ...
  it.effect(
    "backpressured concurrent publishers and subscribers - many to many",
    () =>
      Effect.gen(function* () {
        ...
        const subscriber1 = yield* PubSub.subscribe(pubsub).pipe(
          Effect.flatMap((subscription) =>
            Effect.forEach(values.concat(values), (_) =>
  ...
  it.effect("dropping concurrent publishers and subscribers - one to one", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.forkChild(
        Effect.forEach(values, (n) => PubSub.publish(pubsub, n))
      );
      const result = yield* Fiber.join(subscriber);
      assert.deepStrictEqual(result, values);
    })
  ...
  it.effect(
    "dropping concurrent publishers and subscribers - one to many",
    () =>
      Effect.gen(function* () {
        const values = Array.range(0, 64);
        const pubsub = yield* PubSub.dropping<number>(64);
  ...
  it.effect(
    "dropping concurrent publishers and subscribers - many to many",
    () =>
      Effect.gen(function* () {
        const values = Array.range(1, 64);
  ...
  it.effect(
    "sliding concurrent publishers and subscribers - many to many",
    () =>
      Effect.gen(function* () {
        const values = Array.range(1, 64);
  ...
  it.effect(
    "unbounded concurrent publishers and subscribers - one to one",
    () =>
      Effect.gen(function* () {
        const values = Array.range(0, 64);
  ...
  it.effect(
    "unbounded concurrent publishers and subscribers - one to many",
    () =>
      Effect.gen(function* () {
        const values = Array.range(0, 64);
        const pubsub = yield* PubSub.unbounded<number>();
  ...
  it.effect(
    "unbounded concurrent publishers and subscribers - many to many",
    () =>
      Effect.gen(function* () {
        const values = Array.range(1, 64);
        const pubsub = yield* PubSub.unbounded<number>();
  ...
  describe("replay", () => {
    ...
    it.effect("dropping", () =>
      Effect.gen(function* () {
        const messages = [1, 2, 3, 4, 5];
        const pubsub = yield* PubSub.dropping<number>({
          capacity: 2,
          replay: 3,
        });
    ...
    it.effect("sliding", () =>
      Effect.gen(function* () {
        const messages = [1, 2, 3, 4, 5];
        const pubsub = yield* PubSub.sliding<number>({
          capacity: 4,
          replay: 3,
        });
