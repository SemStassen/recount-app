...
describe("Metric", () => {
  ...
  it.effect(
    "should record fiber runtime metrics once for yielded fibers",
    () => {
      let starts = 0;
      let ends = 0;
  ...
  describe("Gauge", () => {
    it.effect("custom set with value", () =>
      Effect.gen(function* () {
        const id = nextId();
        const gauge = Metric.gauge(id).pipe(Metric.withAttributes(attributes));
  ...
  describe("Histogram", () => {
    it.effect("custom observe with value", () =>
      Effect.gen(function* () {
        const id = nextId();
        const boundaries = Metric.linearBoundaries({
          start: 0,
          width: 1,
          count: 10,
        });
  ...
  describe("Summary", () => {
    ...
    it.effect("custom observe with constant", () =>
      Effect.gen(function* () {
        ...
        const summary = Metric.summary(id, {
          ...
          maxSize: 10,
          quantiles,
        }).pipe(Metric.withAttributes(attributes), Metric.withConstantInput(1));
        yield* Effect.trackSuccesses(Effect.succeed(1), summary);
        yield* Effect.trackSuccesses(Effect.succeed(3), summary);
        const result = yield* Metric.value(summary);
        assert.deepStrictEqual(result, {
    ...
    it.effect(
      "should return the correct quantile when the first chunk overshoots",
      () =>
        Effect.gen(function* () {
          const id = nextId();
  ...
  describe("track", () => {
    it.effect("updates on success", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.succeed(1).pipe(Effect.track(counter));
        const result = yield* Metric.value(counter);
    ...
    it.effect("updates on failure", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.fail(1).pipe(Effect.track(counter), Effect.exit);
        const result = yield* Metric.value(counter);
        assert.deepStrictEqual(result, { count: 1, incremental: false });
      })
    );
    ...
    it.effect("updates on defect", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.die(1).pipe(Effect.track(counter), Effect.exit);
        const result = yield* Metric.value(counter);
        assert.deepStrictEqual(result, { count: 1, incremental: false });
      })
    );
  ...
  describe("track", () => {
    it.effect(
      "maps exits while preserving the instrumented effect result",
      () =>
        Effect.gen(function* () {
          const id = nextId();
          const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
  ...
  describe("trackErrors", () => {
    it.effect("does not update on success", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.succeed(1).pipe(Effect.trackErrors(counter));
        const result = yield* Metric.value(counter);
    ...
    it.effect("updates on failure", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.fail(1).pipe(Effect.trackErrors(counter), Effect.exit);
        const result = yield* Metric.value(counter);
        assert.deepStrictEqual(result, { count: 1, incremental: false });
      })
    );
    ...
    it.effect("does not update on defect", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.die(1).pipe(Effect.trackErrors(counter), Effect.exit);
        const result = yield* Metric.value(counter);
        assert.deepStrictEqual(result, { count: 0, incremental: false });
      })
    );
  ...
  describe("trackDefects", () => {
    it.effect("does not update on success", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.succeed(1).pipe(Effect.trackDefects(counter));
        const result = yield* Metric.value(counter);
        assert.deepStrictEqual(result, { count: 0, incremental: false });
      })
    );
    ...
    it.effect("does not update on failure", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.fail(1).pipe(Effect.trackDefects(counter), Effect.exit);
        const result = yield* Metric.value(counter);
        assert.deepStrictEqual(result, { count: 0, incremental: false });
      })
    );
    ...
    it.effect("updates on defect", () =>
      Effect.gen(function* () {
        const id = nextId();
        const counter = Metric.counter(id).pipe(Metric.withConstantInput(1));
        yield* Effect.die(1).pipe(Effect.trackDefects(counter), Effect.exit);
        const result = yield* Metric.value(counter);
        assert.deepStrictEqual(result, { count: 1, incremental: false });
      })
    );
  ...
  describe("trackDurationWith", () => {
    it.effect("tracks execution duration", () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Effect.sleep("1 hour").pipe(
          Effect.trackDuration(gauge, (duration) =>
