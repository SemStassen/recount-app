...
describe("Layer", () => {
    ...
    Effect.gen(function* () {
      ...
      const layer2 = Layer.effectContext(
        Effect.acquireRelease(
          latch.open.pipe(Effect.map((bool) => Context.make(BoolTag, bool))),
          () => Effect.void
        )
      );
  ...
  it.effect(
    "catchCause recovers with a fallback layer and exposes the original cause",
    () =>
      Effect.gen(function* () {
        ...
        const context = yield* Layer.effect(Service1Tag)(
          Effect.fail("failed!")
        ).pipe(
  ...
  it.effect(
    "tap - executes effect with success services and preserves output",
    () =>
      Effect.gen(function* () {
        const arr: Array<string> = [];
        const env = makeLayer1(arr).pipe(
          Layer.tap((context) =>
            Effect.sync(() => {
              arr.push(
                `tap:${Context.get(context, Service1Tag).constructor.name}`
              );
            })
          ),
          Layer.build
  ...
  it.effect(
    "launch keeps the layer open until interrupted and then finalizes",
    () =>
      Effect.gen(function* () {
  ...
  describe("mock", () => {
    it.effect("allows passing partial service", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          yield* service
            .two()
            .pipe(Effect.catchDefect(Effect.fail), Effect.flip);
    ...
    it.effect(
      "missing mock members fail as unimplemented effects and channels",
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            const service = yield* Service1;
            const effectDefect = yield* service
              .effect()
              .pipe(Effect.catchDefect(Effect.fail), Effect.flip);
            assert.strictEqual(
              (effectDefect as Error).name,
              "UnimplementedError"
            );
            ...
            assert.strictEqual(
              (transformedDefect as Error).name,
              "UnimplementedError"
            );
          }).pipe(Effect.provide(Layer.mock(Service1)({})));
        })
    );
  });
  ...
  describe("MemoMap", () => {
    ...
    it.effect(
      "forked memo maps reuse parent layers but isolate child layers",
      () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            (child as any)["~effect/Layer/MemoMap"],
            "~effect/Layer/MemoMap"
          );
  ...
  describe("tracing", () => {
    it.effect(
      "withSpan supports data-first usage and runs the onEnd finalizer",
      () =>
        Effect.gen(function* () {
          ...
          const layer = Layer.effect(SpanName)(
            Effect.map(Tracer.ParentSpan, (span) =>
              span._tag === "Span" ? span.name : span.spanId
            )
          );
    ...
    it.effect(
      "withParentSpan supports data-last usage with external spans",
      () =>
        Effect.gen(function* () {
          ...
          ).pipe(Layer.withParentSpan(parent), Layer.build, Effect.scoped);
