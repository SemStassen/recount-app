...
describe("FiberSet", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* Effect.onInterrupt(Effect.never, () =>
            Ref.update(ref, (n) => n + 1)
          ).pipe(FiberSet.run(set), Effect.repeat({ times: 9 }));
  ...
  it.effect(
    "runs fibers concurrently and awaitEmpty waits for completion",
    () =>
      Effect.gen(function* () {
        const set = yield* FiberSet.make();
  ...
  it.effect(
    "propagateInterruption true fails join on external interruption",
    () =>
      Effect.gen(function* () {
        const set = yield* FiberSet.make();
        const fiber = yield* FiberSet.run(set, Effect.never, {
        ...
        yield* Fiber.interrupt(fiber);
        assertTrue(
          Exit.hasInterrupts(yield* FiberSet.join(set).pipe(Effect.exit))
        );
      })
  );
