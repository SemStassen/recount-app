...
describe("ReducerMultiply", () => {
  it("combines values and uses one as the identity", () => {
    ...
    assert.strictEqual(
      N.ReducerMultiply.combine(N.ReducerMultiply.initialValue, 2),
      2
    );
    assert.strictEqual(
      N.ReducerMultiply.combine(2, N.ReducerMultiply.initialValue),
      2
    );
  });
  ...
  it("reduces iterables and short-circuits on zero", () => {
    ...
    assert.strictEqual(
      N.ReducerMultiply.combineAll(iterableThatFailsAfterZero()),
      0
    );
  });
});
