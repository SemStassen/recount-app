...
describe("TxQueue", () => {
  ...
  describe("basic operations", () => {
    ...
    it.effect(
      "takeAll() waits when empty then returns items when available",
      () =>
        Effect.tx(
          Effect.gen(function* () {
            const queue = yield* TxQueue.bounded<number>(5);
    ...
    it.effect(
      "takeN() signature validation - returns Array<A> not [Array<A>, boolean]",
      () =>
        Effect.tx(
          Effect.gen(function* () {
            const queue = yield* TxQueue.bounded<number>(5);
  ...
  describe("E-channel and exit signaling", () => {
    ...
    describe("state transitions", () => {
      ...
      it.effect(
        "interrupt() transitions Open → Closing → Done when not empty",
        () =>
          Effect.tx(
            Effect.gen(function* () {
              const queue = yield* TxQueue.bounded<number>(5);
    ...
    describe("error channel behavior", () => {
      ...
      it.effect(
        "take() with explicit interrupt cause propagates interruption",
        () =>
          Effect.tx(
            Effect.gen(function* () {
              const queue = yield* TxQueue.bounded<number, string>(5);
      ...
      it.effect("error propagation with different error types", () =>
        Effect.tx(
          Effect.gen(function* () {
            ...
            const queue2 = yield* TxQueue.bounded<
              number,
