...
describe("TxHashMap", () => {
    ...
    it.effect("size", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const large = yield* TxHashMap.make(
            ["a", 1],
            ["b", 2],
            ["c", 3],
            ["d", 4]
    ...
    it.effect("modify - existing key", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const oldValue = yield* TxHashMap.modify(
            txMap,
            "counter",
            (n) => n * 2
          );
      ...
      Effect.tx(
        Effect.gen(function* () {
          ...
          yield* Effect.flatMap(
            TxHashMap.set(txMap, "a", 1),
            () => Effect.void
          );
          yield* Effect.flatMap(
            TxHashMap.set(txMap, "b", 2),
            () => Effect.void
          );
      ...
      Effect.tx(
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(
            user1,
            Option.some({ name: "Alice", role: "admin" })
          );
          ...
          const notFound = yield* TxHashMap.getHash(
            txMap,
            "user999",
            Hash.string("user999")
          );
          ...
          const wrongHash = yield* TxHashMap.getHash(
            txMap,
            userId,
            Hash.string("different")
          );
    ...
    it.effect("hasHash should check existence using custom hash", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          // Test non-existent key
          const hasGuest = yield* TxHashMap.hasHash(
            txMap,
            "guest",
            Hash.string("guest")
          );
    ...
    it.effect("getHash and hasHash should work together", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(
            session,
            Option.some({ userId: "user1", active: true })
          );
          ...
          const invalidExists = yield* TxHashMap.hasHash(
            txMap,
            invalidSessionId,
            invalidHash
          );
    ...
    it.effect("map should transform values while preserving keys", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const ageMap = yield* txMap.pipe(
            TxHashMap.map((user, key) => ({ ...user, age: user.age + 1, key }))
          );

          const updatedAlice = yield* TxHashMap.get(ageMap, "user1");
          assert.deepStrictEqual(
            updatedAlice,
            Option.some({ name: "Alice", age: 26, key: "user1" })
          );
        })
      )
    );
    ...
    it.effect("filter should keep only matching entries", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const fruitMap = yield* TxHashMap.filter(
            txMap,
          ...
          const expensiveMap = yield* txMap.pipe(
            TxHashMap.filter((item) => item.price > 1.5)
          );
    ...
    it.effect("reduce should fold over all entries", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const sum = yield* TxHashMap.reduce(
            txMap,
            0,
            (acc, value) => acc + value
          );
          ...
          const emptySum = yield* TxHashMap.reduce(
            emptyMap,
            0,
            (acc, value) => acc + value
          );
      ...
      Effect.tx(
        Effect.gen(function* () {
          ...
          const emailMap = yield* userMap.pipe(
            TxHashMap.filterMap((user, key) =>
              "email" in user
                ? Result.succeed(`${key}:${user.email}`)
                : Result.failVoid
            )
          );
          ...
          assert.deepStrictEqual(
            user1Email,
            Option.some("user1:alice@example.com")
          );
          assert.deepStrictEqual(user2Email, Option.none());
          assert.deepStrictEqual(
            user3Email,
            Option.some("user3:charlie@example.com")
          );
        })
      )
    );
    ...
    it.effect("hasBy should check if any entry matches predicate", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const hasAdmin = yield* TxHashMap.hasBy(
            txMap,
            (user) => user.role === "admin"
          );
          assert.strictEqual(hasAdmin, true);

          const hasGuest = yield* TxHashMap.hasBy(
            txMap,
          ...
          const hasInactive = yield* txMap.pipe(
            TxHashMap.hasBy((user) => !user.active)
          );
    ...
    it.effect("findFirst should return first matching entry", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const user1Session = yield* TxHashMap.findFirst(
            txMap,
          ...
          const otherSession = yield* txMap.pipe(
            TxHashMap.findFirst(
              (session) =>
                session.userId !== "user1" && session.userId !== "user2"
            )
          );
    ...
    it.effect("some should check if any entry matches predicate", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const hasCompleted = yield* TxHashMap.some(
            txMap,
            (order) => order.status === "completed"
          );
          assert.strictEqual(hasCompleted, true);

          const hasCancelled = yield* TxHashMap.some(
            txMap,
          ...
          const hasLargeOrder = yield* txMap.pipe(
            TxHashMap.some((order) => order.amount > 200)
          );
          assert.strictEqual(hasLargeOrder, true);

          const hasVeryLargeOrder = yield* txMap.pipe(
            TxHashMap.some((order) => order.amount > 1000)
          );
    ...
    it.effect("every should check if all entries match predicate", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const allCompleted = yield* TxHashMap.every(
            txMap,
            (task) => task.completed
          );
          assert.strictEqual(allCompleted, true);

          const allHighPriority = yield* TxHashMap.every(
            txMap,
          ...
          yield* TxHashMap.set(txMap, "task4", {
            completed: false,
            priority: "high",
          });

          // Test data-last: now not all tasks are completed
          const stillAllCompleted = yield* txMap.pipe(
            TxHashMap.every((task) => task.completed)
          );
          assert.strictEqual(stillAllCompleted, false);

          // Test that all tasks have a priority
          const allHavePriority = yield* txMap.pipe(
            TxHashMap.every((task) => Boolean(task.priority))
          );
          assert.strictEqual(allHavePriority, true);

          // Test with empty map (vacuous truth)
          const emptyMap = yield* TxHashMap.empty<
            string,
            { completed: boolean }
          >();
    ...
    it.effect("functional operations should work correctly", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const expensiveItems = yield* TxHashMap.filter(
            discountedMap,
            (item) => item.price > 15
          );
          ...
          const allDiscounted = yield* TxHashMap.every(
            expensiveItems,
  ...
  describe("Phase 3: Specialized Operations", () => {
    it.effect("forEach should execute side effects for each entry", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            processed.filter((p) => p.status === "active").length,
            2
          );
      ...
      Effect.tx(
        Effect.gen(function* () {
          ...
          const employees = yield* TxHashMap.flatMap(
            ...
            (members, dept) =>
              Effect.gen(function* () {
                const empMap = yield* TxHashMap.empty<
                  string,
                ...
                for (let i = 0; i < members.length; i++) {
                  ...
                  yield* TxHashMap.set(empMap, member, {
                    department: dept,
                    role,
                  });
                }
          ...
          assert.deepStrictEqual(
            alice,
            Option.some({ department: "eng", role: "lead" })
          );

          const bob = yield* TxHashMap.get(employees, "bob");
          assert.deepStrictEqual(
            bob,
            Option.some({ department: "eng", role: "member" })
          );

          const charlie = yield* TxHashMap.get(employees, "charlie");
          assert.deepStrictEqual(
            charlie,
            Option.some({ department: "sales", role: "lead" })
          );
        })
      )
    );
    ...
    it.effect("compact should remove None values and unwrap Some values", () =>
      Effect.tx(
        Effect.gen(function* () {
          const optionalData = yield* TxHashMap.make<
            string,
    ...
    it.effect("toValues should return all values as array", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(sortedValues, [
            { price: 100 },
            { price: 200 },
            { price: 300 },
          ]);
