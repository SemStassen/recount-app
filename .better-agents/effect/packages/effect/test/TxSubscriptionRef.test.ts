...
describe("TxSubscriptionRef", () => {
    ...
    it.effect("modify returns a value and updates", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const result = yield* TxSubscriptionRef.modify(ref, (n) => [
            `was ${n}`,
            n + 1,
          ]);
    ...
    it.effect("updateAndGet applies function and returns new value", () =>
      Effect.tx(
        Effect.gen(function* () {
          ...
          const result = yield* TxSubscriptionRef.updateAndGet(
            ref,
            (n) => n * 3
          );
  ...
  describe("subscriptions", () => {
    ...
    it.effect(
      "changesStream starts from the current value for each subscriber",
      () =>
        Effect.gen(function* () {
          const ref = yield* Effect.tx(TxSubscriptionRef.make(0));
