  ...
  return Effect.withFiber((parent) => {
    ...
    return Effect.succeed(function handler(
      ...
      const fiber = Fiber.runIn(
        Effect.runForkWith(Context.makeUnsafe<any>(map))(handled),
        options.scope
      );
...
> => {
  ...
  return Effect.withFiber((parent) => {
    ...
    return Effect.succeed(function handler(
      ...
      const fiber = Fiber.runIn(
        Effect.runForkWith(Context.makeUnsafe<any>(map))(handledApp),
        options.scope
      );
      socket.on("close", () => {
...
class ServerRequestImpl
  extends NodeHttpIncomingMessage<HttpServerError>
  implements HttpServerRequest
{
  ...
  private upgradeEffect?:
    | Effect.Effect<Socket.Socket, HttpServerError>
    | undefined;
  readonly url: string;
  ...
  > {
    ...
    this.multipartEffect = Effect.runSync(
      Effect.cached(NodeMultipart.persisted(this.source, this.source.headers))
    );
    return this.multipartEffect;
  }
...
) => Layer.Layer<HttpServer.HttpServer, ServeError> = flow(
  make,
...
> =>
  Layer.mergeAll(
    Layer.effect(HttpServer.HttpServer)(
      Effect.flatMap(Config.unwrap(options), (options) =>
        make(evaluate, options)
      )
    ),
    layerHttpServices
  );
    ...
    case "Raw": {
      ...
      if (
        ...
        return Effect.tryPromise({
          try: (signal) =>
            pipeline(body.body as any, nodeResponse, { signal, end: true }),
          catch: (cause) =>
            new HttpServerError({
