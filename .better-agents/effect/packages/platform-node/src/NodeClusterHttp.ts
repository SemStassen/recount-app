...
>(options: {
  ...
  const layer: Layer.Layer<any, any, any> = options.clientOnly
    ...
    : // with server
      options.transport === "http"
      ? Layer.provide(HttpRunner.layerHttp, [
          layerHttpServer,
          NodeHttpClient.layerUndici,
        ])
      : Layer.provide(HttpRunner.layerWebsocket, [
          layerHttpServer,
          NodeSocket.layerWebSocketConstructor,
        ]);
  ...
  const runnerHealth: Layer.Layer<any, any, any> = options?.clientOnly
    ...
    : options?.runnerHealth === "k8s"
      ...
      : RunnerHealth.layerPing.pipe(
          ...
          Layer.provide(
            options.transport === "http"
              ? HttpRunner.layerClientProtocolHttpDefault.pipe(
                  Layer.provide(NodeHttpClient.layerUndici)
                )
              : HttpRunner.layerClientProtocolWebsocketDefault.pipe(
...
> = Effect.gen(function* () {
  ...
  const listenAddress = Option.orElse(
    config.runnerListenAddress,
    () => config.runnerAddress
  );
  if (Option.isNone(listenAddress)) {
    return yield* Effect.die(
      "NodeClusterHttp.layerHttpServer: ShardingConfig.runnerAddress is None"
    );
  }
