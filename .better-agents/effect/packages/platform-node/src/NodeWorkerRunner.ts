    ...
    start<O = unknown, I = unknown>() {
      return Effect.gen(function* () {
        ...
        const sendUnsafe = WorkerThreads.parentPort
          ? (_portId: number, message: any, transfers?: any) =>
              WorkerThreads.parentPort!.postMessage(message, transfers)
          : (_portId: number, message: any, _transfers?: any) =>
        ...
        const send = (
          ...
          Effect.sync(() =>
            sendUnsafe(_portId, [1, message], transfers as any)
          );
            ...
            Effect.fnUntraced(function* (scope) {
              ...
              const onExit = (exit: Exit.Exit<any, E>) => {
                if (
                  ...
                  runFork(
                    Effect.logError("unhandled error in worker", exit.cause)
                  );
                }
              };
              (WorkerThreads.parentPort ?? process).on(
                "message",
