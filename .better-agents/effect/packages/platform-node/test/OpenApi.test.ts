...
describe("OpenAPI spec", () => {
  describe("api", () => {
    ...
    it("annotateMerge", () => {
      const Api = HttpApi.make("api").annotateMerge(
        OpenApi.annotations({
          title: "title",
          version: "version",
    ...
    it("AdditionalSchemas", () => {
      const Api = HttpApi.make("api").annotate(HttpApi.AdditionalSchemas, [
    ...
    it("Override", () => {
      const Api = HttpApi.make("api").annotate(OpenApi.Override, {
        tags: [{ name: "A" }],
      });
      const spec = OpenApi.fromApi(Api);
      ...
      describe("POST", () => {
        ...
        it("NoContent + payload", () => {
          const Api = HttpApi.make("api").add(
            HttpApiGroup.make("group").add(
              HttpApiEndpoint.post("a", "/a", {
                payload: [HttpApiSchema.NoContent, Schema.String],
              })
            )
          );
          const spec = OpenApi.fromApi(Api);
        ...
        describe("encodings", () => {
          it("asMultipart", () => {
            ...
            assert.deepStrictEqual(
              spec.paths["/a"].post?.requestBody?.content,
              {
                "multipart/form-data": {
                  schema: {
                    type: "string",
          ...
          it("asMultipart with FilesSchema", () => {
            const Api = HttpApi.make("api").add(
              HttpApiGroup.make("group").add(
                HttpApiEndpoint.post("a", "/a", {
                  payload: Multipart.FilesSchema.pipe(
          ...
          it("asMultipart with SingleFileSchema", () => {
            const Api = HttpApi.make("api").add(
              HttpApiGroup.make("group").add(
                HttpApiEndpoint.post("a", "/a", {
                  payload: Multipart.SingleFileSchema.pipe(
                    HttpApiSchema.asMultipart()
                  ),
                })
              )
    ...
    describe("success option", () => {
      ...
      describe("encodings", () => {
        it("asJson + contentType", () => {
          ...
          const spec = OpenApi.fromApi(Api);
          assert.deepStrictEqual(
            spec.paths["/a"].get?.responses["200"].content,
            {
              "application/problem+json": {
                schema: {
                  type: "string",
