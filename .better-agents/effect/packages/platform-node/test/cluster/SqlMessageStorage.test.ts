    ...
    it.layer(StorageLive.pipe(Layer.provideMerge(layer)), {
        ...
        Effect.gen(function* () {
          ...
          for (let i = 2; i <= 5; i++) {
            yield* storage.saveRequest(
              yield* makeRequest({ payload: { id: i } })
            );
          }

          yield* storage.saveReply(yield* makeReply(request));

          let messages = yield* storage.unprocessedMessages([
            request.envelope.address.shardId,
          ]);
          expect(messages).toHaveLength(4);
          expect(messages.map((m: any) => m.envelope.payload.id)).toEqual([
            2, 3, 4, 5,
          ]);
          ...
          messages = yield* storage.unprocessedMessages([
            request.envelope.address.shardId,
          ]);
        ...
        Effect.gen(function* () {
          ...
          assert(
            result._tag === "Duplicate" &&
              Option.isSome(result.lastReceivedReply)
          );
          ...
          const fiber = yield* storage
            .saveReply(yield* makeReply(request))
      ...
      it.effect("unprocessedMessages excludes complete requests", () =>
        Effect.gen(function* () {
          ...
          yield* storage.saveRequest(request);
          yield* storage.saveReply(yield* makeReply(request));
          const messages = yield* storage.unprocessedMessages([
            request.envelope.address.shardId,
          ]);
          expect(messages).toHaveLength(0);
        })
      ...
      it.effect("unprocessedMessagesById", () =>
        Effect.gen(function* () {
          ...
          const request = yield* makeRequest();
          yield* storage.saveRequest(request);
          let messages = yield* storage.unprocessedMessagesById([
            request.envelope.requestId,
          ]);
          expect(messages).toHaveLength(1);
          yield* storage.saveReply(yield* makeReply(request));
          messages = yield* storage.unprocessedMessagesById([
            request.envelope.requestId,
          ]);
          expect(messages).toHaveLength(0);
        })
