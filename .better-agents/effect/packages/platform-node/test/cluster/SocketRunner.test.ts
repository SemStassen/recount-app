  ...
  it.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Layer.launch(makeRunnerLayer(RUNNER_PORT)).pipe(
          Effect.forkScoped
        );
        ...
        yield* Effect.gen(function* () {
          ...
          yield* client.Process(TestPayload.make({ id: "req-1", amount }), {
            discard: true,
          });
        }).pipe(Effect.provide(makeClientLayer(RUNNER_PORT)), Effect.scoped);
