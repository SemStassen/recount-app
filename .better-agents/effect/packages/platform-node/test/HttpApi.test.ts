...
function* assertClientText<E, R>(
  res: Effect.Effect<string, E, R>,
  text: string
) {
  assert.strictEqual(yield* res, text);
}

function* assertClientJson<E, R>(
  res: Effect.Effect<unknown, E, R>,
  json: unknown
) {
...
describe("HttpApi", () => {
  it.effect("catch all path", () => {
    ...
    const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
      handlers.handle("catchAll", (ctx) => Effect.succeed(ctx.request.url))
    );
  ...
  describe("middleware", () => {
    it.effect("error", () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("a", () => Effect.succeed(1))
      );
      const MLive = Layer.succeed(M, () => Effect.fail("error"));
    ...
    it.effect("error array", () => {
      ...
      const MLive = Layer.succeed(M, (_, { endpoint }) =>
      ...
      return Effect.gen(function* () {
        yield* assertServerText(
          yield* HttpClient.get("/unauthorized"),
          401,
          ""
        );
        ...
        const client = yield* HttpApiClient.make(Api);
        yield* assertClientError(
          client.group.unauthorized(),
          new HttpApiError.Unauthorized({})
        );
        yield* assertClientError(
          client.group.forbidden(),
    ...
    it.effect(
      "client middleware modifies request and receives metadata",
      () => {
        ...
        const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
          handlers.handle("a", (ctx) =>
            Effect.succeed(ctx.request.headers["x-client"] ?? "missing")
          )
        );
        ...
        return Effect.gen(function* () {
          const metadata = yield* Ref.make<
            { readonly group: string; readonly endpoint: string } | undefined
          >(undefined);
          ...
          const MClient = HttpApiMiddleware.layerClient(
            ...
            Effect.fnUntraced(function* ({ endpoint, group, next, request }) {
              ...
              return yield* next(
                HttpClientRequest.setHeader(request, "x-client", "from-client")
              );
            })
          );
    ...
    it.effect("client middleware chain order is LIFO", () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("a", (ctx) =>
          Effect.succeed(
            `${ctx.request.headers["x-m1"] ?? "0"}${ctx.request.headers["x-m2"] ?? "0"}`
          )
        )
      );
      ...
      return Effect.gen(function* () {
        ...
        const M1Client = HttpApiMiddleware.layerClient(
          ...
          Effect.fnUntraced(function* ({ next, request }) {
            ...
            const response = yield* next(
              HttpClientRequest.setHeader(request, "x-m1", "1")
            );
            order.push("m1-after");
            return response;
          })
        ...
        const M2Client = HttpApiMiddleware.layerClient(
          ...
          Effect.fnUntraced(function* ({ next, request }) {
            ...
            const response = yield* next(
              HttpClientRequest.setHeader(request, "x-m2", "1")
            );
            order.push("m2-after");
            return response;
          })
    ...
    it.effect(
      "required client middleware can fail with typed clientError",
      () => {
        class ClientFailure extends Schema.ErrorClass<ClientFailure>(
          "ClientFailure"
        )({
          _tag: Schema.tag("ClientFailure"),
        }) {}

        class M extends HttpApiMiddleware.Service<
          M,
          {
        ...
        return Effect.gen(function* () {
          const MClient = HttpApiMiddleware.layerClient(M, () =>
    ...
    it.effect("optional client middleware is skipped when not provided", () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("a", (ctx) =>
          Effect.succeed(ctx.request.headers["x-optional"] ?? "missing")
        )
      );
      const MLive = Layer.succeed(M, (effect) => effect);
    ...
    it.effect("layerClient supports effectful construction", () => {
      class HeaderValue extends Context.Service<HeaderValue, string>()(
        "HeaderValue"
      ) {}
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("a", (ctx) =>
          Effect.succeed(ctx.request.headers["x-effect"] ?? "missing")
        )
      );
      const MLive = Layer.succeed(M, (effect) => effect);
      ...
      return Effect.gen(function* () {
        const MClient = HttpApiMiddleware.layerClient(
          ...
          Effect.gen(function* () {
            ...
            return ({ next, request }) =>
              next(HttpClientRequest.setHeader(request, "x-effect", header));
          })
        );
    ...
    it.effect("security middleware can be implemented with layerClient", () => {
      class CurrentToken extends Context.Service<CurrentToken, string>()(
        "CurrentToken"
      ) {}
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("a", () => CurrentToken)
      );
      const MLive = Layer.succeed(M)({
        apiKey: (effect, options) =>
          Effect.provideService(
            effect,
            CurrentToken,
            Redacted.value(options.credential)
          ),
      });
      ...
      return Effect.gen(function* () {
        ...
        const MClient = HttpApiMiddleware.layerClient(M, ({ next, request }) =>
          next(HttpClientRequest.setHeader(request, "authorization", "token"))
        );
    ...
    it("security middleware cache does not reuse the first service impl", async () => {
      class CurrentMarker extends Context.Service<CurrentMarker, string>()(
        "CurrentMarker"
      ) {}
      ...
      const makeWebHandler = (marker: string) => {
        const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
          handlers.handle("a", () =>
            Effect.map(CurrentMarker, (marker) => ({ marker }))
          )
        );
        const MLive = Layer.succeed(M)({
          apiKey: (effect) =>
            Effect.provideService(effect, CurrentMarker, marker),
        });
    ...
    it.effect("addHttpApi + middleware works across merged groups", () => {
      ...
      const UsersLive = HttpApiBuilder.group(Api, "users", (handlers) =>
        handlers.handle("list", () => Effect.succeed("ok"))
      );
      const M1Live = Layer.succeed(M1, (effect, { endpoint, group }) =>
        Effect.sync(() =>
          calls.push(`m1:${group.identifier}.${endpoint.name}`)
        ).pipe(Effect.flatMap(() => effect))
      );
      const M2Live = Layer.succeed(M2, (effect, { endpoint, group }) =>
        Effect.sync(() =>
          calls.push(`m2:${group.identifier}.${endpoint.name}`)
        ).pipe(Effect.flatMap(() => effect))
      );
    ...
    it.effect(
      "missing middleware layer fails with service not found error",
      () => {
        class M extends HttpApiMiddleware.Service<M>()(
          "Server/MissingMiddleware"
        ) {}
        ...
        const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
          handlers.handle("a", () => Effect.succeed("ok"))
        );
        const ApiLive = HttpRouter.serve(
          HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive)),
          { disableListenLog: true, disableLogger: true }
        ...
        return HttpClient.get("/a").pipe(
          ...
          Effect.flatMap((cause) =>
            Effect.sync(() => {
              ...
              assert.include(
                defect.message,
                "Service not found: Server/MissingMiddleware"
              );
  ...
  it.effect("missing addHttpApi group layer has actionable error", () => {
    ...
    const Api = HttpApi.make("api").add(HealthApi).addHttpApi(V0);
    ...
    return HttpClient.get("/users").pipe(
      ...
      Effect.flatMap((cause) =>
        Effect.sync(() => {
          ...
          assert.include(
            defect,
            "Available groups: effect/httpapi/HttpApiGroup/users"
          );
        })
      )
    ) as Effect.Effect<void, HttpClientResponse.HttpClientResponse>;
  ...
  describe("payload option", () => {
    describe("GET", () => {
      it.effect("query parameters", () => {
        ...
        const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
          handlers.handle("a", (ctx) => Effect.succeed(ctx.payload))
        );
        const ApiLive = HttpRouter.serve(
          HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLive)),
          { disableListenLog: true, disableLogger: true }
        ...
        return Effect.gen(function* () {
          ...
          yield* assertServerJson(yield* HttpClient.get("/a?required=1"), 200, {
            required: 1,
          });
          yield* assertServerJson(
            yield* HttpClient.get("/a?required=1&optionalKey=1"),
            200,
            {
              required: 1,
              optionalKey: 1,
          ...
          yield* assertServerJson(
            yield* HttpClient.get("/a?required=1&optional=1"),
            200,
          ...
          // TODO: fix this test
          yield* assertServerJson(
            yield* HttpClient.get("/a?required=1&optional="),
            200,
          ...
          // client side
          const client = yield* HttpApiClient.make(Api);
          yield* assertClientJson(
            client.group.a({ payload: { required: 1 } }),
            { required: 1 }
          );
          yield* assertClientJson(
            client.group.a({ payload: { required: 1, optionalKey: 1 } }),
            {
              required: 1,
              optionalKey: 1,
            }
          ...
          yield* assertClientJson(
            client.group.a({ payload: { required: 1, optional: 1 } }),
            {
              required: 1,
              optional: 1,
            }
          ...
          yield* assertClientJson(
            client.group.a({ payload: { required: 1, optional: undefined } }),
            { required: 1 }
          );
        }).pipe(Effect.provide(ApiLive));
      });
      ...
      return Effect.gen(function* () {
        ...
        yield* assertServerJson(
          yield* HttpClient.post("/a", {
            body: HttpBody.jsonUnsafe({ a: "text" }),
          }),
          200,
          {
            a: "text",
        ...
        yield* assertServerJson(
          yield* HttpClient.post("/a", { body: HttpBody.text("text") }),
          200,
          "text"
        );
        yield* assertServerJson(
        ...
        // client side
        const client = yield* HttpApiClient.make(Api);
        yield* assertClientJson(client.group.a({ payload: { a: "text" } }), {
          a: "text",
        });
        yield* assertClientJson(client.group.a({ payload: "text" }), "text");
        yield* assertClientJson(
          client.group.a({ payload: new Uint8Array([1, 2, 3]) }),
          { 0: 1, 1: 2, 2: 3 }
        );
      }).pipe(Effect.provide(ApiLive));
    });
  ...
  describe("params option", () => {
    it.effect("should accept a record of schemas", () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("get", (ctx) => Effect.succeed(`User ${ctx.params.id}`))
      );
  ...
  describe("query option", () => {
    it.effect("should accept a record of schemas", () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("get", (ctx) => Effect.succeed(`User ${ctx.query.id}`))
      );
  ...
  describe("success option", () => {
    ...
    it("no content via toWebHandler", async () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handle("remove", () => Effect.void)
      );
      ...
      try {
        const response = await handler(
          new Request("http://localhost/items/123", { method: "DELETE" })
        );
    ...
    describe("encodings", () => {
      describe("asJson", () => {
        it.effect("custom contentType", () => {
          ...
          const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
            handlers.handle("a", () => Effect.succeed("a"))
          );
  ...
  describe("original tests", () => {
    describe("payload", () => {
      ...
      it.live("multipart", () =>
        Effect.gen(function* () {
          ...
          const result = yield* client.users.upload({
            payload: data,
            params: {},
          });
    ...
    describe("headers", () => {
      it.effect("should accept a record of schemas", () => {
        ...
        const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
          handlers.handle("get", (ctx) =>
            Effect.succeed(`User ${ctx.headers.id}`)
          )
        );
    ...
    describe("errors", () => {
      ...
      it.effect("empty errors decode", () =>
        Effect.gen(function* () {
          const client = yield* HttpApiClient.make(Api);
          const error = yield* client.groups
      ...
      it.effect("BadRequest", () =>
        Effect.gen(function* () {
          const client = yield* HttpApiClient.make(Api);
          const error = yield* client.users
            .upload({ params: {}, payload: new FormData() })
            .pipe(Effect.flip);
          assert(
    ...
    describe("security", () => {
      it.effect("security middleware sets current user", () =>
        Effect.gen(function* () {
          const ref = yield* Ref.make(
            Cookies.empty.pipe(Cookies.setUnsafe("token", "foo"))
          );
          const client = yield* HttpApiClient.makeWith(Api, {
            httpClient: HttpClient.withCookiesRef(
      ...
      it.effect("apiKey query security", () =>
        Effect.gen(function* () {
          const redacted = yield* HttpApiBuilder.securityDecode(
            securityQuery
          ).pipe(
            Effect.provideService(
              HttpServerRequest.HttpServerRequest,
    ...
    it.effect("client responseMode response-only skips decoding", () => {
      ...
      const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
        handlers.handleRaw("bad", () =>
          Effect.succeed(HttpServerResponse.text("not-json"))
        )
      );
    ...
    it.effect("error from plain text", () => {
      class RateLimitError extends Schema.ErrorClass<RateLimitError>(
        "RateLimitError"
      )({
        ...
        Effect.provide([
          ...
          HttpApiBuilder.group(Api, "groups", (handlers) =>
            handlers
              ...
              .handle("handle", () => Effect.die("unimplemented"))
...
const HttpUsersLive = HttpApiBuilder.group(
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers
      .handle("findById", (_) =>
        _.params.id === -1 ? CurrentUser : repo.findById(_.params.id)
      )
      .handle("create", (_) =>
        _.payload.name === "boom"
...
).pipe(Layer.provide([UserRepo.Live, AuthorizationLive]));
...
const TopLevelLive = HttpApiBuilder.group(Api, "root", (handlers) =>
  handlers.handle("healthz", (_) => Effect.void)
);
