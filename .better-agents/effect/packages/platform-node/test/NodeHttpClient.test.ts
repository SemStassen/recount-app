...
const JsonPlaceholder = Context.Service<JsonPlaceholder>(
  ...
  describe(`NodeHttpClient - ${name}`, () => {
    ...
    it.effect("jsonplaceholder", () =>
      Effect.gen(function* () {
        ...
        Effect.provide(JsonPlaceholderLive.pipe(Layer.provide(layer))),
        flaky
      )
    );
          ...
          .pipe(
            Effect.flatMap(
              HttpClientResponse.schemaJson(
                Schema.Struct({ status: Schema.Literal(200) })
              )
            )
          );
