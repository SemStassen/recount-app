...
describe("HttpServer", () => {
  ...
  it.effect("formData", () =>
    Effect.gen(function* () {
      ...
      const result = yield* client
        .post("/upload", { body: HttpBody.formData(formData) })
  ...
  it.effect("schemaBodyForm", () =>
    Effect.gen(function* () {
      yield* HttpRouter.add(
        ...
        "/upload",
        Effect.gen(function* () {
          const files = yield* HttpServerRequest.schemaBodyForm(
            Schema.Struct({
  ...
  it.effect("mountApp", () =>
    Effect.gen(function* () {
      const child = Effect.map(HttpServerRequest.HttpServerRequest, (_) =>
        HttpServerResponse.text(_.url)
      );
      yield* HttpRouter.use((router) =>
        router.prefixed("/child").add("*", "*", child)
      ).pipe(HttpRouter.serve, Layer.build);
      const client = yield* HttpClient.HttpClient;
      const todo = yield* client
        .get("/child/1")
        .pipe(Effect.flatMap((_) => _.text));
      expect(todo).toEqual("/1");
      const root = yield* client
        .get("/child")
        .pipe(Effect.flatMap((_) => _.text));
      expect(root).toEqual("/");
      const rootSearch = yield* client
        .get("/child?foo=bar")
        .pipe(Effect.flatMap((_) => _.text));
      expect(rootSearch).toEqual("?foo=bar");
      const rootSlash = yield* client
        .get("/child/")
        .pipe(Effect.flatMap((_) => _.text));
  ...
  it.effect("file", () =>
    Effect.gen(function* () {
      yield* (yield* HttpServerResponse.file(
        `${__dirname}/fixtures/text.txt`
      ).pipe(
        Effect.updateService(HttpPlatform.HttpPlatform, (_) => ({
          ..._,
          fileResponse: (path, options) =>
            Effect.map(_.fileResponse(path, options), (res) => {
      ...
      yield* HttpServerResponse.fileWeb(file).pipe(
        Effect.updateService(HttpPlatform.HttpPlatform, (_) => ({
          ...
          fileWebResponse: (path, options) =>
            Effect.map(_.fileWebResponse(path, options), (res) => ({
              ...res,
              headers: { ...res.headers, etag: 'W/"etag"' },
            })),
        })),
      ...
      yield* HttpRouter.add(
        ...
        Effect.flatMap(
          ...
          Effect.catchTag("SchemaError", (error) =>
            Effect.succeed(
              HttpServerResponse.jsonUnsafe({ error }, { status: 400 })
            )
          )
        )
      ).pipe(HttpRouter.serve, Layer.build);
  ...
  it.effect("schemaBodyFormJson file", () =>
    Effect.gen(function* () {
      ...
      formData.append(
        "json",
        new Blob([JSON.stringify({ test: "content" })], {
  ...
  it.effect("tracing", () =>
    Effect.gen(function* () {
      yield* HttpRouter.add(
        "GET",
        "/",
        Effect.flatMap(Effect.currentSpan, (_) =>
          HttpServerResponse.json({ spanId: _.spanId, parent: _.parent })
        )
      ).pipe(HttpRouter.serve, Layer.build);
      const client = yield* HttpClient.HttpClient;
      ...
      Effect.gen(function* () {
        class CustomError extends Schema.ErrorClass<CustomError>("CustomError")(
          ...
          [HttpServerRespondable.symbol]() {
            return HttpServerResponse.schemaJson(CustomError)(this, {
              status: 599,
            });
          }
        }
  ...
  it.effect("HttpRouter prefixed", () =>
    Effect.gen(function* () {
      const handler = HttpRouter.serve(
        HttpRouter.use(
          Effect.fnUntraced(function* (router_) {
            ...
            yield* router.addAll([
              HttpRouter.route(
                "GET",
                "/",
                Effect.succeed(HttpServerResponse.text("root"))
              ),
            ]);
          })
        )
