...
const fixturesRoot = fileURLToPath(
  new URL("./fixtures/http-static-server", import.meta.url)
);
const fixturesOutsideFile = fileURLToPath(
  new URL("./fixtures/http-static-server-outside.txt", import.meta.url)
);
...
const withStaticFiles = async (
  ...
  const temporaryRoot = await mkdtemp(
    NodePath.join(tmpdir(), "effect-http-static-server-")
  );
  const root = NodePath.join(temporaryRoot, "root");
  const outsideFile = NodePath.join(
    temporaryRoot,
    NodePath.basename(fixturesOutsideFile)
  );
...
describe("HttpStaticServer", () => {
  ...
  it("returns 304 for If-None-Match exact, weak, and wildcard", async () => {
    await withStaticFiles(
      async ({ handler }) => {
        ...
        const weakComparable = etag.startsWith("W/")
          ? etag.slice(2)
          : `W/${etag}`;
        ...
        const weak = await handler(
          new Request("http://localhost/conditional.txt", {
            headers: { "If-None-Match": weakComparable },
          })
        );
        ...
        assert.deepStrictEqual(
          [exact.status, weak.status, any.status],
          [304, 304, 304]
        );
  ...
  it("handles range requests for valid, invalid, and malformed headers", async () => {
    await withStaticFiles(async ({ handler }) => {
      ...
      assert.strictEqual(
        ...
        `bytes ${fileSize - 10}-${fileSize - 1}/${fileSize}`
      );
  ...
  it("handles SPA fallback for html accept and missing routes", async () => {
    await withStaticFiles(
      async ({ handler }) => {
        ...
        assert.strictEqual(
          (await htmlFallback.text()).trimEnd(),
  ...
  it("rejects directory traversal attempts", async () => {
    await withStaticFiles(async ({ handler, outsideFile }) => {
      const plainTraversal = await handler(
        new Request("http://localhost/../../../etc/passwd")
      );
      const encodedTraversal = await handler(
        new Request("http://localhost/..%2F..%2Fetc%2Fpasswd")
      );
      const encodedExistingParentFileTraversal = await handler(
        new Request(
          `http://localhost/..%2F${encodeURIComponent(NodePath.basename(outsideFile))}`
        )
      );
  ...
  it("rejects null bytes and malformed uri encoding", async () => {
    await withStaticFiles(async ({ handler }) => {
      const nullByte = await handler(
        new Request("http://localhost/null%00byte.txt")
      );
    ...
    await withStaticFiles(
      async ({ handler }) => {
        const response = await handler(
          new Request("http://localhost/hello.txt")
        );
        assert.strictEqual(response.status, 200);
        assert.strictEqual(
          response.headers.get("content-type"),
  ...
  it("uses application/octet-stream for unknown extension and 404 for missing file", async () => {
    await withStaticFiles(async ({ handler }) => {
      ...
      const missing = await handler(
        new Request("http://localhost/does-not-exist.txt")
      );
