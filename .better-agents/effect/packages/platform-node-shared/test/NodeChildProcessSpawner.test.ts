...
describe("NodeChildProcessSpawner", () => {
  it.layer(NodeServices)((it) => {
    describe("spawn", () => {
      ...
      describe("shell option", () => {
        ...
        it.effect("should not expand variables without shell", () =>
          Effect.gen(function* () {
            const handle = yield* ChildProcess.make("echo", ["$HOME"], {
              shell: false,
            });
            const output = yield* decodeByteStream(handle.stdout);
            const exitCode = yield* handle.exitCode;
      ...
      describe("stderr streaming", () => {
        ...
        it.effect("should handle more stdout than stderr", () =>
          Effect.gen(function* () {
            ...
            const handle = yield* ChildProcess.make("sh", [
              "-c",
              "echo line1; echo line2; echo line3; echo line4; echo line5; echo error >&2",
            ]);
        ...
        it.effect("should handle more stderr than stdout", () =>
          Effect.gen(function* () {
            ...
            const handle = yield* ChildProcess.make("sh", [
              "-c",
              "echo output; echo err1 >&2; echo err2 >&2; echo err3 >&2; echo err4 >&2; echo err5 >&2",
            ]);
            const stdout = yield* decodeByteStream(handle.stdout);
            const stderr = yield* decodeByteStream(handle.stderr);
            const exitCode = yield* handle.exitCode;
      ...
      describe("combined output (all)", () => {
        it.effect("should read interspersed stdout and stderr via .all", () =>
          Effect.gen(function* () {
            ...
            assert.strictEqual(
              all,
              ["stdout1", "stderr1", "stdout2", "stderr2"].join("\n")
            );
          }).pipe(Effect.scoped)
        );
        ...
        it.effect("should capture only stdout via .all when no stderr", () =>
          Effect.gen(function* () {
            const handle = yield* ChildProcess.make("echo", [
              "hello from stdout",
            ]);
            const all = yield* decodeByteStream(handle.all);
            const exitCode = yield* handle.exitCode;
        ...
        it.effect(
          "should handle many lines of interspersed output via .all",
          () =>
            Effect.gen(function* () {
              // Use sleep to force buffer flushes, ensuring interleaved arrival
              const handle = yield* ChildProcess.make("sh", [
                "-c",
                "for i in 1 2 3 4 5; do echo stdout$i; sleep 0.01; echo stderr$i >&2; sleep 0.01; done",
              ]);
              const all = yield* decodeByteStream(handle.all);
              const exitCode = yield* handle.exitCode;
        ...
        it.effect("should allow reading .all independently", () =>
          Effect.gen(function* () {
            const handle = yield* ChildProcess.make("sh", [
              "-c",
              "echo out; sleep 0.01; echo err >&2",
            ]);
            const all = yield* decodeByteStream(handle.all);
            const exitCode = yield* handle.exitCode;
    ...
    describe("pipeline spawning", () => {
      ...
      it.effect("should pipe grep output", () =>
        Effect.gen(function* () {
          const handle = yield* ChildProcess.make("echo", [
            "line1\nline2\nline3",
          ]).pipe(ChildProcess.pipeTo(ChildProcess.make`grep line2`));
          const output = yield* decodeByteStream(handle.stdout);
          const exitCode = yield* handle.exitCode;
    ...
    describe("pipeline pipe options", () => {
      ...
      it.effect("should pipe combined output with { from: 'all' }", () =>
        Effect.gen(function* () {
          ...
          ]).pipe(ChildProcess.pipeTo(ChildProcess.make`cat`, { from: "all" }));
          const output = yield* decodeByteStream(handle.stdout);
          const exitCode = yield* handle.exitCode;
      ...
      it.effect("should default to stdout when no options provided", () =>
        Effect.gen(function* () {
          ...
          const handle = yield* ChildProcess.make("sh", [
            "-c",
            "echo stdout; echo stderr >&2",
          ]).pipe(ChildProcess.pipeTo(ChildProcess.make`cat`));
          const output = yield* decodeByteStream(handle.stdout);
          const exitCode = yield* handle.exitCode;
      ...
      it.effect("should support chained pipes with different options", () =>
        Effect.gen(function* () {
          ...
          const handle = yield* ChildProcess.make`echo hello`.pipe(
            ChildProcess.pipeTo(
              ChildProcess.make("sh", ["-c", "cat; echo error >&2"])
            ),
            ChildProcess.pipeTo(ChildProcess.make`cat`, { from: "stderr" })
          );
          const output = yield* decodeByteStream(handle.stdout);
    ...
    describe("additionalFds", () => {
      ...
      it.effect(
        "should handle bidirectional communication via separate fds",
        () =>
          Effect.gen(function* () {
    ...
    describe("process supervision", () => {
      ...
      it.effect("should kill all child processes in process group", () =>
        Effect.gen(function* () {
          ...
          const handle = yield* ChildProcess.make("./spawn-children.sh", {
            cwd,
          });
          ...
          const beforeKill = yield* decodeByteStream(
            beforeKillHandle.stdout
          ).pipe(
          ...
          const afterKill = yield* decodeByteStream(
            afterKillHandle.stdout
          ).pipe(
            Effect.map((s) => Number.parseInt(s.trim())),
            Effect.orElseSucceed(() => 0)
          );
      ...
      it.effect(
        ...
        () =>
          Effect.gen(function* () {
            ...
            const beforeRun = yield* decodeByteStream(
              beforeRunHandle.stdout
            ).pipe(
              Effect.map((s) => Number.parseInt(s.trim())),
              Effect.orElseSucceed(() => 0)
            );
            ...
            const afterExit = yield* decodeByteStream(
              afterExitHandle.stdout
            ).pipe(
              Effect.map((s) => Number.parseInt(s.trim())),
              Effect.orElseSucceed(() => 0)
            );
      ...
      it.effect("should not kill an unrefed process when scope closes", () =>
        Effect.gen(function* () {
          ...
          yield* Scope.provide(scope)(handle.unref).pipe(
      ...
      it.effect("should kill a restored process when scope closes", () =>
        Effect.gen(function* () {
          ...
          const reref = yield* Scope.provide(scope)(handle.unref).pipe(
            Effect.provide(NodeServices)
          );
          yield* reref;
          yield* Scope.close(scope, Exit.void);
      ...
      it.effect(
        "should resolve exitCode after closing the original scope of an unrefed process",
        () =>
          Effect.gen(function* () {
            const scope = yield* Scope.make();
            const handle = yield* Scope.provide(scope)(
      ...
      it.effect(
        "should cleanup descendants after an unrefed parent exits non-zero",
        () =>
          Effect.gen(function* () {
            const path = yield* Path.Path;
            const cwd = path.join(...TEST_BASH_SCRIPTS_PATH);
    ...
    it.effect(
      "should not deadlock on large stdout output",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "ChildProcess.string should not deadlock on large output",
      () =>
        Effect.gen(function* () {
          const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
          const output = yield* spawner.string(
            ChildProcess.make("sh", ["-c", "seq 1 100000"])
          );
          const lines = output.trim().split("\n");
          assert.strictEqual(lines.length, 100000);
          assert.strictEqual(lines[0], "1");
