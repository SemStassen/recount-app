...
type ExitCodeWithSignal = readonly [
  code: number | null,
...
const make = Effect.gen(function* () {
  ...
  const resolveWorkingDirectory = Effect.fnUntraced(function* (
  ...
  const inputToStdioOption = (
    input: ChildProcess.CommandInput | undefined
  ): NodeChildProcess.IOType | undefined =>
    Stream.isStream(input) ? "pipe" : input;

  const outputToStdioOption = (
    input: ChildProcess.CommandOutput | undefined
  ): NodeChildProcess.IOType | undefined =>
    Sink.isSink(input) ? "pipe" : input;

  const resolveStdinOption = (
    options: ChildProcess.CommandOptions
  ): ChildProcess.StdinConfig => {
    const defaultConfig: ChildProcess.StdinConfig = {
      stream: "pipe",
      encoding: "utf-8",
      endOnDone: true,
    };
    ...
    Effect.suspend(() => {
      ...
      if (Predicate.isNotNull(childProcess.stdin)) {
        sink = NodeSink.fromWritable({
          ...
          onError: (error) =>
            toPlatformError("fromWritable(stdin)", toError(error), command),
      ...
      if (Stream.isStream(config.stream)) {
        return Effect.as(
          Effect.forkScoped(Stream.run(config.stream, sink)),
          sink
        );
      }
  ...
  const setupChildOutputStreams = (
    ...
    let stdout = childProcess.stdout
      ? NodeStream.fromReadable({
          ...
          onError: (error) =>
            toPlatformError("fromReadable(stdout)", toError(error), command),
        })
      : Stream.empty;
    let stderr = childProcess.stderr
      ? NodeStream.fromReadable({
          ...
          onError: (error) =>
            toPlatformError("fromReadable(stderr)", toError(error), command),
        })
      : Stream.empty;
      ...
      return Effect.callback<void, PlatformError.PlatformError>((resume) => {
        NodeChildProcess.exec(
          ...
          (error) => {
            if (error) {
              resume(
                Effect.fail(toPlatformError("kill", toError(error), command))
              );
            } else {
              resume(Effect.void);
            }
  ...
  > = Effect.fnUntraced(function* (cmd) {
    switch (cmd._tag) {
      case "StandardCommand": {
        ...
        const stdio = buildStdioArray(
          stdinConfig,
          stdoutConfig,
          stderrConfig,
          resolvedAdditionalFds
        );
        ...
        childProcess.on("exit", (code) => {
          if (cleanupOnNonZeroExit && code !== 0 && Predicate.isNotNull(code)) {
            killProcessGroupOnExit(
              childProcess,
              cmd.options.killSignal ?? "SIGTERM"
            );
          }
        });
        ...
        const { getInputFd, getOutputFd } = yield* setupAdditionalFds(
          cmd,
          childProcess,
          resolvedAdditionalFds
        );
        const isRunning = Effect.map(
          Deferred.isDone(exitSignal),
          (done) => !done
        );
        ...
        const kill = (options?: ChildProcess.KillOptions | undefined) => {
          ...
          ).pipe(Effect.andThen(Deferred.await(exitSignal)), Effect.asVoid);
        };
      ...
      case "PipedCommand": {
        ...
        for (let i = 0; i < pipeline.length; i++) {
          ...
          const sourceStream = Stream.unwrap(
            Effect.succeed(
              getSourceStream(handles[handles.length - 1], options.from)
            )
          );
...
export const flattenCommand = (
  ...
  const nonEmptyCommands: Arr.NonEmptyReadonlyArray<ChildProcess.StandardCommand> =
    [first, ...rest];
