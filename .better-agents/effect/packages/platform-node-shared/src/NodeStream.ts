...
export const fromReadable = <A = Uint8Array, E = Cause.UnknownError>(options: {
  ...
  Stream.fromChannel(fromReadableChannel<A, E>(options));
 ...
 * @since 4.0.0
 */
export const fromReadableChannel = <
  A = Uint8Array,
...
>(options: {
  ...
  Arr.NonEmptyReadonlyArray<O>,
  IE | E,
  void,
  Arr.NonEmptyReadonlyArray<I>,
  IE
> =>
  Channel.fromTransform((upstream, scope) => {
    const duplex = options.evaluate();
    const exit = MutableRef.make<
      Exit.Exit<never, IE | E | Cause.Done> | undefined
    >(undefined);
    ...
    return pullIntoWritable({
      ...
      Effect.catchCause((cause) => {
        ...
        exit.current = Exit.failCause(
          cause as Cause.Cause<IE | E | Cause.Done>
        );
        return Effect.void;
      }),
      Effect.forkIn(scope),
...
export const pipeThroughDuplex: {
  ...
  <R, E, A, B = Uint8Array, E2 = Cause.UnknownError>(
    ...
    Stream.pipeThroughChannelOrFail(self, fromDuplex(options))
);
...
export const pipeThroughSimple: {
  ...
  ) => Stream.Stream<Uint8Array, E | Cause.UnknownError, R>;
  <R, E>(
    self: Stream.Stream<string | Uint8Array, E, R>,
    duplex: LazyArg<Duplex>
  ...
  <R, E>(
    ...
    pipeThroughDuplex(self, { evaluate: duplex })
);
...
export const toReadableNever = <E>(
  stream: Stream.Stream<string | Uint8Array, E, never>
): Readable => new StreamAdapter(Context.empty(), stream);
...
export const toString = <E = Cause.UnknownError>(
  ...
  const maxBytesNumber = options?.maxBytes
...
export const toUint8Array = <E = Cause.UnknownError>(
  ...
  Effect.map(
    toArrayBuffer(readable, options),
...
const readableToPullUnsafe = <A, E>(options: {
  ...
  readonly exit?:
    | MutableRef.MutableRef<Exit.Exit<never, E | Cause.Done> | undefined>
    | undefined;
  ...
  const pull = Effect.suspend(function loop(): Pull.Pull<
  ...
  return Effect.as(
    Scope.addFinalizer(
      ...
      Effect.sync(() => {
        ...
        if (
          closeOnDone &&
          "closed" in options.readable &&
          !options.readable.closed
        ) {
...
class StreamAdapter<E, R> extends Readable {
  ...
  override _destroy(
    ...
    Effect.runFork(Fiber.interrupt(this.fiber)).addObserver((exit) => {
      callback(
        exit._tag === "Failure" ? (Cause.squash(exit.cause) as any) : error
      );
    });
  }
}

const defaultOnError = (error: unknown): Cause.UnknownError =>
  new Cause.UnknownError(error);
