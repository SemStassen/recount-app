...
export const make = Effect.fnUntraced(function* (
  ...
  server.on("error", (err) =>
    Deferred.doneUnsafe(errorDeferred, Exit.fail(err))
  );
      ...
      pipe(
        NodeSocket.fromDuplex(
          Effect.acquireRelease(
            Effect.suspend(
              (): Effect.Effect<Net.Socket, Socket.SocketError> => {
                if (error) {
                  return Effect.fail(
...
> = Effect.fnUntraced(function* (options: NodeWS.ServerOptions) {
  ...
  const pendingConnections = new Set<
    readonly [globalThis.WebSocket, Http.IncomingMessage]
  >();
  function defaultHandler(
    conn: globalThis.WebSocket,
  ...
  Effect.withFiber<void>((fiber) => {
    ...
    if (unhandledLogLevel) {
      return Effect.logWithLevel(unhandledLogLevel)(
        cause,
        "Unhandled error in SocketServer"
      );
    }
    return Effect.void;
  });
