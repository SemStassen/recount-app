...
export const fromWritable = <E, A = Uint8Array | string>(options: {
  ...
  Sink.fromChannel(
    Channel.mapDone(fromWritableChannel<never, E, A>(options), (_) => [_])
  );
...
export const fromWritableChannel = <IE, E, A = Uint8Array | string>(options: {
  ...
  Channel.fromTransform(
    (pull: Pull.Pull<NonEmptyReadonlyArray<A>, IE, unknown>) => {
  ...
  options.pull.pipe(
    Effect.flatMap((chunk) => {
      ...
      return Effect.callback<void, E>(function loop(resume) {
        for (; i < chunk.length; ) {
          const success = options.writable.write(
            chunk[i++],
            options.encoding as any
          );
          if (!success) {
    ...
    Effect.raceFirst(
      Effect.callback<never, E>((resume) => {
