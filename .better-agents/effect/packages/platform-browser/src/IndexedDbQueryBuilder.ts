...
>
  ...
  /** @internal */
  readonly fromCache: Map<
    string,
    IndexedDbQuery.From<IndexedDbVersion.TableWithName<Source, any>>
  >;
  ...
  ) => Effect.Effect<A, E, Exclude<R, IndexedDbTransaction>>;
}
...
export declare namespace IndexedDbQuery {
  ...
  > = KeyPath extends keyof Type
    ? Type[KeyPath]
    : {
        [I in keyof KeyPath]: KeyPath[I] extends keyof Type
          ? Type[KeyPath[I]] | []
          : never;
      };
    ...
    : KeyPath extends readonly [infer K, ...infer Rest]
      ? K extends keyof Type
        ? [
            Type[K],
            ...{
              [P in keyof Rest]?: Rest[P] extends keyof Type
                ? Type[Rest[P]] | []
                : never;
            },
          ]
   ...
   * @since 4.0.0
   */
  export type ModifyWithKey<Table extends IndexedDbTable.AnyWithProps> =
    ModifyType<Table>;
  ...
  > {
    ...
    readonly reverse: () => SelectWithout<
      Table,
      Index,
      ExcludedKeys | "reverse" | "first"
    >;
 ...
 * @since 4.0.0
 */
export class IndexedDbTransaction extends Context.Service<
...
type IndexedDbValidKeys<TableSchema extends IndexedDbTable.AnySchemaStruct> =
  keyof TableSchema["Encoded"] extends infer K
    ? K extends keyof TableSchema["Encoded"]
      ? TableSchema["Encoded"][K] extends Readonly<IDBValidKey>
        ? K
        : never
      : never
    : never;
...
> = keyof TableSchema["Encoded"] extends infer K
  ? K extends keyof TableSchema["Encoded"]
    ? [TableSchema["Encoded"][K]] extends [number | undefined]
      ? K
      : never
    : never
  : never;
...
const applySelect = Effect.fnUntraced(function* (
  ...
  const tableSchema = (query.from.table as IndexedDbTable.AnyWithProps)
    .arraySchema;
...
const applyFirst = Effect.fnUntraced(function* (
  ...
  const data = yield* Effect.callback<
    any,
    IndexedDbQueryError | Cause.NoSuchElementError
  >((resume) => {
          ...
          resume(
            Effect.fail(
              new Cause.NoSuchElementError(
                `No such element in table ${query.select.from.table.tableName}`
              )
            )
          );
        } else {
...
const applyModifyAll = Effect.fnUntraced(
  function* ({
    ...
    const makeValueAutoincrement = SchemaParser.makeEffect(
      query.from.table.autoincrementSchema
    );
    const encodeValueAutoincrement = SchemaParser.encodeUnknownEffect(
      query.from.table.autoincrementSchema
    );
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (autoIncrement && value[keyPath] === undefined) {
        encodedValues[i] = yield* encodeValueAutoincrement(
          yield* makeValueAutoincrement(value)
        );
      } else {
    ...
    >((resume) => {
      ...
      const results: Array<globalThis.IDBValidKey> = new Array(
...
const makeFrom = <const Table extends IndexedDbTable.AnyWithProps>(options: {
  readonly table: Table;
  readonly database: MutableRef.MutableRef<globalThis.IDBDatabase>;
  readonly IDBKeyRange: typeof globalThis.IDBKeyRange;
  readonly reactivity: Reactivity.Reactivity["Service"];
...
> = {
  ...
  equals(
    ...
    value: IndexedDbQuery.ExtractIndexType<any, never>
  ) {
    return makeDelete({
      delete: this as any,
      only: value,
...
> = {
  ...
  filter(
    ...
    filter: (value: IndexedDbTable.Encoded<any>) => boolean
  ) {
...
>(options: {
  ...
  readonly predicate?:
    | ((item: IndexedDbTable.Encoded<Table>) => boolean)
    | undefined;
  ...
  stream(
    ...
    return Stream.suspend(() => {
      ...
      return Stream.paginate(initial, (select) =>
        Effect.map(applySelect(select as any), (data) => {
          total += data.length;
          ...
          return [
            data,
            isPartial || reachedLimit ? Option.none() : Option.some(select),
          ] as const;
        })
      );
    });
...
const FirstProto: Omit<IndexedDbQuery.First<any, never>, "select"> = {
  ...CommonProto,
...
> = {
  ...
  invalidate(
    ...
    keys ??=
      typeof keyPath === "string" && this.value[keyPath] !== undefined
...
const makeModifyAll = <Table extends IndexedDbTable.AnyWithProps>(options: {
  readonly from: IndexedDbQuery.From<Table>;
...
> = {
  ...
  use(
    ...
    f: (database: globalThis.IDBDatabase) => any
  ) {
    return Effect.try({
  ...
  withTransaction(
    ...
    return (effect) =>
      Effect.suspend(() => {
        const transaction = this.database.current.transaction(
          options.tables,
          options.mode,
          options
        );
...
const getOrCreateTransaction = (
  ...
  return (
    Context.getOrUndefined(fiber.context, IndexedDbTransaction) ??
    database.transaction(tables, mode, options)
  );
};
