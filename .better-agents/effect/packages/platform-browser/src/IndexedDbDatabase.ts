...
const layer = <DatabaseName extends string>(
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const open = Effect.callback<void, IndexedDbDatabaseError>((resume) => {
        ...
        request.onupgradeneeded = (event) => {
          ...
          const effect = Effect.forEach(
            ...
            Effect.provideService(
              IndexedDbQueryBuilder.IndexedDbTransaction,
              transaction
            )
          );
      ...
      const rebuild = Effect.callback<void, IndexedDbDatabaseError>(
        (resume) => {
          database.current?.close();
