 ...
 * @since 4.0.0
 */
export const CurrentXHRResponseType: Context.Reference<XHRResponseType> =
  Context.Reference(
...
export const withXHRArrayBuffer = <A, E, R>(
  ...
  Effect.provideService(self, CurrentXHRResponseType, "arraybuffer");
...
const makeXmlHttpRequest = HttpClient.make((request, url, signal, fiber) =>
  Effect.suspend(() => {
    ...
    return Effect.andThen(
      ...
      Effect.callback<ClientResponseImpl, HttpClientError.HttpClientError>(
        (resume) => {
          let sent = false;
          const onChange = () => {
...
abstract class IncomingMessageImpl<E>
  extends Inspectable.Class
  implements HttpIncomingMessage.HttpIncomingMessage<E>
{
  ...
  get headers() {
    ...
    const parsed =
      result._tag === "Headers"
        ? Headers.fromInput(result.headers)
        : Headers.empty;
  ...
  get cookies() {
    ...
    return (this.cachedCookies = Cookies.fromSetCookie(
      this._rawHeaders["set-cookie"]
    ));
  }
    ...
    return Stream.callback<Uint8Array, E>((queue) => {
      ...
      const onReadyStateChange = () => {
        if (this.source.readyState === 3) {
          const encoded = encoder.encode(
            this.source.responseText.slice(offset)
          );
      ...
      const onError = () => {
        Queue.failCauseUnsafe(
          queue,
          Cause.fail(this.onError(this.source.statusText))
        );
      };
      this.source.addEventListener("readystatechange", onReadyStateChange);
      this.source.addEventListener("error", onError);
  ...
  get arrayBuffer(): Effect.Effect<ArrayBuffer, E> {
    ...
    return (this._arrayBufferEffect = Effect.callback<ArrayBuffer, E>(
      (resume) => {
        if (this.source.readyState === 4) {
...
class ClientResponseImpl
  extends IncomingMessageImpl<HttpClientError.HttpClientError>
...
export const layerXMLHttpRequest: Layer.Layer<HttpClient.HttpClient> =
  HttpClient.layerMergedContext(Effect.succeed(makeXmlHttpRequest));
