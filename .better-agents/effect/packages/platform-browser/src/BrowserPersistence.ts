...
export const layerBackingIndexedDb = (options?: {
  ...
  Layer.effect(Persistence.BackingPersistence)(
    Effect.gen(function* () {
...
export const layerIndexedDb = (options?: {
  ...
  Persistence.layer.pipe(Layer.provide(layerBackingIndexedDb(options)));

const openDatabase = (
  database: string
): Effect.Effect<IDBDatabase, Persistence.PersistenceError> =>
  Effect.gen(function* () {
...
const isExpired = (row: EntryRow, now: number): boolean =>
  row.expires !== null && row.expires <= now;
...
const getMany = (
  ...
  Arr.NonEmptyArray<object | undefined>,
