...
describe.sequential("BrowserPersistence", () => {
  it.effect("set + get", () =>
    ...
    }).pipe(Effect.provide(BrowserPersistence.layerBackingIndexedDb()))
  );

  it.effect(
    "setMany + getMany preserves order, missing keys, and duplicates",
    () =>
      Effect.gen(function* () {
        const backing = yield* Persistence.BackingPersistence;
        const store = yield* backing.make("users");
  ...
  it.effect("TTL expiry performs lazy deletion", () =>
    Effect.gen(function* () {
      ...
      const rowBefore = yield* getRawEntry(
        defaultDatabase,
        "users",
        "ttl-key"
      ).pipe(Effect.orDie);
      ...
      const rowAfter = yield* getRawEntry(
        defaultDatabase,
        "users",
        "ttl-key"
      ).pipe(Effect.orDie);
  ...
  it.effect("custom database option", () =>
    Effect.gen(function* () {
      yield* withStore("users", undefined, (store) =>
        store.set("shared", { database: "default" }, undefined)
      );
      ...
      const fromCustom = yield* withStore(
        "users",
        { database: customDatabase },
        (store) => store.get("shared")
      );
...
const withStore = <A>(
  ...
  options:
    ...
    | undefined,
  f: (
    store: Persistence.BackingPersistenceStore
...
const getRawEntry = (
  ...
  storeId: string,
  id: string
): Effect.Effect<EntryRow | undefined, unknown> =>
