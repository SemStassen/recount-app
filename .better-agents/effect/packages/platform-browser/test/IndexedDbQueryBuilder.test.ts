...
describe.sequential("IndexedDbQueryBuilder", () => {
  ...
  describe("modify", () => {
    ...
    it.effect("insert schema with context", () => {
      ...
      return Effect.gen(function* () {
        ...
        provideDb(Db),
        Effect.provideService(VerifyContext, VerifyContext.of({ maxLength: 4 }))
      );
    });
    ...
    it.effect("insert with manual key optional", () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.equal(data.length, 1);
        assert.deepStrictEqual(data, [
          new ProductSchema({ key: 10, name: "insert1", price: 10 }),
        ]);
      }).pipe(provideDb(Db));
    });
  ...
  describe("modify all", () => {
    ...
    it.effect("upsertAll withTransaction", () => {
      ...
      return Effect.gen(function* () {
        const api = yield* Db.getQueryBuilder;
        const addedKeys = yield* api.withTransaction({
          tables: ["todo"],
          mode: "readwrite",
        })(
          Effect.gen(function* () {
  ...
  it.effect("reactive", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* api
        .from("todo")
        .insert({ id: 4, title: "test4", count: 4, completed: false })
        .invalidate();
