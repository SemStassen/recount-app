...
describe.sequential("IndexedDbDatabase", () => {
  it.effect("insert and read todos", () => {
    ...
    return Effect.gen(function* () {
      ...
      const indexNames = yield* api.use(
        (database) =>
          database.transaction("todo").objectStore("todo").indexNames
      );
      const index = yield* api.use((database) =>
  ...
  it.effect("delete object store migration", () => {
    ...
    class Migration extends IndexedDbDatabase.make(V1, (api) =>
      api.createObjectStore("todo")
    ).add(
      V2,
