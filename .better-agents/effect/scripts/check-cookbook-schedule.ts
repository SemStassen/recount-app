...
if (files.length === 0) {
  console.error(
    "Usage: node scripts/check-cookbook-schedule.ts <markdown-file...>"
  );
  process.exit(1);
}
...
function extractExamples(file: string): Array<Example> {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (open === undefined) {
      ...
      if (
        isTypeScript(info.language) &&
        info.noCheck &&
        !hasNoCheckComment(lines, index)
      ) {
        throw new Error(
...
function expectedOutput(example: Example): string {
  ...
  if (outputLine === -1) {
    throw new Error(
      `${example.file}:${example.startLine} is deterministic but has no // Output: comment`
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (status) => {
      ...
      if (example.deterministic) {
        ...
        hasVariableOutputMarker(example) &&
...
async function runExamples(
  examples: ReadonlyArray<GeneratedExample>
): Promise<Array<RunResult>> {
  const concurrency = Math.max(
    1,
    Math.min(Os.availableParallelism(), examples.length)
  );
  ...
  if (examples.length > 0) {
    console.error(
      `Running ${examples.length} runnable example(s) with concurrency ${concurrency}`
    );
  }
  ...
  function reportProgress(): void {
    if (completed === examples.length || completed % 10 === 0) {
      console.error(
        `Ran ${completed}/${examples.length} runnable example(s) in ${elapsed()}`
      );
    }
  }
  ...
  async function worker(workerId: number): Promise<void> {
    while (next < examples.length) {
      ...
      console.error(
        `Worker ${workerId}: ${example.file}:${example.startLine} started at ${elapsed()}`
      );
      results[index] = await runExample(example);
      completed++;
      console.error(
        `Worker ${workerId}: ${example.file}:${example.startLine} finished in ${formatDuration(
          performance.now() - exampleStartedAt
        )}`
      );
      reportProgress();
    }
...
try {
  ...
  const generatedFiles: Array<GeneratedExample> = checkedExamples.map(
    (example, index) => {
  ...
  const deterministicExamples = runnableExamples.filter(
    (example) => example.deterministic
  );
  for (const example of runnableExamples) {
    ...
    if (!example.deterministic && variableOutputSample(example).length === 0) {
      console.error(
        `${example.file}:${example.startLine} has // Output may vary: but no sample output lines`
      );
      process.exit(1);
    }
  }
