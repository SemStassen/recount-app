...
export class AiWriter extends Context.Service<
  AiWriter,
  {
    ...
    streamReleaseHighlights(
      version: string
    ): Stream.Stream<string, AiWriterError>;
  }
>()("docs/AiWriter") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const launchPlanModel =
        yield* OpenAiLanguageModel.model("gpt-4.1").captureRequirements;
      ...
      const draftAnnouncement = Effect.fn("AiWriter.draftAnnouncement")(
        function* (product: string) {
          ...
          const response = yield* model.generateText({
            prompt:
              `Write a short launch announcement for ${product}. ` +
...
export const program: Effect.Effect<void, AiWriterError, AiWriter> = Effect.gen(
  function* () {
