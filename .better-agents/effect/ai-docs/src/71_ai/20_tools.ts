...
const ProductToolkitLayer = ProductToolkit.toLayer(
  Effect.gen(function* () {
    ...
    return ProductToolkit.of({
      SearchProducts: Effect.fn("ProductToolkit.SearchProducts")(function* ({
        query,
        maxResults,
      }) {
        return [
          ...
          new Product({
            id: ProductId.make("p-2"),
            name: `${query} gadget`,
            price: 29.99,
          }),
        ].slice(0, maxResults);
      }),
      GetInventory: Effect.fn("ProductToolkit.GetInventory")(function* ({
        productId,
      }) {
...
>()("docs/ProductAssistant") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const model =
        yield* OpenAiLanguageModel.model("gpt-5.2").captureRequirements;
...
export const AssistantToolkitLayer = AssistantToolkit.toLayer(
  Effect.gen(function* () {
    ...
    return AssistantToolkit.of({
      ...
      GetInventory: Effect.fn("AssistantToolkit.GetInventory")(function* ({
        productId,
      }) {
        return { productId, available: 42 };
      }),
